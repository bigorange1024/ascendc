...
**最后刷新**：2026-07-17（Decaps device 全链 PASS；下一 Cloud **T2** 单库/单 session）
...
- **KEM Alg.21 Decaps device**：[**`fix-f203-alg21-kem-decaps-device-k4`**](ascendc-tests/fix-f203-alg21-kem-decaps-device-k4/) **全链 + E3 PASS**（2026-07-17）；CPU 单库；SIM **双库 + 2-session**；`scripts/` Decaps 默认已指 device  
...
- **下一刀（交 Cloud）**：**T2** SIM 单库合库 / 单 session（见 [`AGENT_HANDOFF.md`](AGENT_HANDOFF.md)）
