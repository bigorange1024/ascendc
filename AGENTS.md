...
**最后刷新**：2026-07-17（**T2 PASS**：Decaps SIM 单库+1-session；下一 pass-fix 更名 / `#交付#`）
...
- **KEM Alg.21 Decaps device**：[**`fix-f203-alg21-kem-decaps-device-k4`**](ascendc-tests/fix-f203-alg21-kem-decaps-device-k4/) **全链 + E3 + T2 PASS**（2026-07-17）；CPU/SIM **单库**；默认 **`decaps_1session`**（D**286803**+E**745925**）；`scripts/` Decaps 默认已指 device  
- **Host 随机（PKE/KEM 已正确性）**：默认 [`library/shared/fips203_host_rng`](library/shared/fips203_host_rng/)；`SEED_D=` 定点可覆盖；勿再默认写死 `20260619`  
- 行为基线探针：`pass-fix-f203-alg19-kem-keygen-device-k4` · `pass-fix-f203-alg20-kem-encaps-device-k4` · Decaps device（`pass-fix` 更名待）（勿当 CMake 依赖）  
- **下一刀**：Decaps `pass-fix` 更名 / KAT 扩量 / `#交付#`（见 [`AGENT_HANDOFF.md`](AGENT_HANDOFF.md)）
