...
│   └── ml-kem/
│       ├── ml-kem-512/       # ML-KEM-512 活跃探针（P2 W0–W3 全绿；W4+glue 见 incubating）
│       ├── ml-kem-1024/      # ML-KEM-1024 活跃探针（frozen 仍在 frozen/）
│       └── ml-kem-768/       # ML-KEM-768 探针（W0–W3 全绿）
├── examples/
│   ├── incubating/ml-kem/ml-kem-1024/exp-*/   # 研究中（按参数组）
│   ├── incubating/ml-kem/ml-kem-512/           # ML-KEM-512 incubating（W4+glue 已绿；禁 stable）
│   ├── incubating/ml-kem/ml-kem-768/exp-*/    # 768 incubating（W4+glue 已绿）
│   └── stable/ml-kem/ml-kem-1024/stable-*/    # 定型（按参数组；无 768/512）
...
| KEM Encaps | **定型** [`stable-fips203-mlkem-kem-encaps-k4`](examples/stable/ml-kem/ml-kem-1024/stable-fips203-mlkem-kem-encaps-k4/)（2026-07-15 `#验收#`；SIM **721119**）；预研副本 [`exp-…`](examples/incubating/ml-kem/ml-kem-1024/exp-fips203-mlkem-kem-encaps-k4/)；行为基线 [`pass-fix-…-encaps-device-k4`](ascendc-tests/ml-kem/ml-kem-1024/pass-fix-f203-alg20-kem-encaps-device-k4/)（**721010**）；**默认 `m=urandom`**（禁默认可全 0；`M_DEFAULT_HEX=00…0` → exit 1） |
...
| **ML-KEM-512（k=2）** | **有条件完成至 incubating**：P0–P3（W0–W4+glue）全绿；Encrypt **`r←η1=3` / `e←η2=2`**（glue-c）；D14 SIM **365995** / E14 **366129** / E20 **427927**；`USE_LIBOQS=1` KeyGen/Encaps c+K/Decaps accept+reject **max=0**；参数卡 [`fips203-mlkem512-parameter-card.md`](docs/specs/fips203-mlkem512-parameter-card.md)；**禁** stable-512；纪要 [`2026-07-27`](qa/2026-07/2026-07-27-768收尾复盘与文档刷新.md) |
| **1024 实机（910B4）口径** | 7 探针 + 7 stable 统一 `${REPO_ROOT}/scripts/env.sh`；npu **按树分卡**（stable=**1** / examples=**2** / tests=**3**，[`npu_device_map.sh`](scripts/npu_device_map.sh)）；SIM 强制 0；套件 [`npu_kem_real_machine_suite.sh`](scripts/npu_kem_real_machine_suite.sh)。见 [`NPU真机环境说明.md`](docs/engineering/NPU真机环境说明.md) §4.1–4.5 |
| **GitCode CANNLab 远程驱动** | Cursor Agent 经 Tailscale 驱动真机 **910B3**（单卡强制 `ASCEND_DEVICE_ID=0`）：[`CANNLab接入与远程驱动.md`](docs/engineering/CANNLab接入与远程驱动.md) · [`scripts/cannlab/`](scripts/cannlab/) |
| Host 随机 | 已正确性 PKE/KEM：**默认** [`fips203_host_rng`](library/shared/fips203_host_rng/) SHA3/SHAKE 派生；`SEED_D=` 定点可覆盖 |
| KEM golden 后端 | [`library/shared/f203_kem_ref/`](library/shared/f203_kem_ref/)：**liboqs 优先**，缺失则回落仓内已验证 PKE golden + SHA3（日志标 `via liboqs` / `via python`）；Decaps 密钥对 stash 缺失按 `SEED_D` 自举。`KEM_GOLDEN_CROSS=1` 实测两路逐字节一致 |
