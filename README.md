...
| **1024 实机（910B4）口径** | 7 探针 + 7 stable 统一 `${REPO_ROOT}/scripts/env.sh`、`ASCEND_DEVICE_ID`（**npu 缺省 0** / SIM 强制 0；device1 曾致 l18_l19 复跑死锁）、[`scripts/msprof_run.sh`](scripts/msprof_run.sh)（默认**不**采集）；**无 `thirdparty/` 也能跑**（KEM golden 见下行）。见 [`NPU真机环境说明.md`](docs/engineering/NPU真机环境说明.md) §4.1–4.4 |
