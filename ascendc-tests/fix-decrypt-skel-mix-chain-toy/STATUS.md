...
> Decrypt fused **握手**骨架 toy（TASK-009 建树；TASK-010 `SKEL_OMIT_SLOT0`；TASK-011 `SKEL_OMIT_SET4_R2`）。不对 ML-KEM 正确性；SIM-only 门禁。

## 目标

| 档 | 配置 | 预期 |
|----|------|------|
| **A** | 默认 `SKEL_OMIT_SET4_R2=0` `SKEL_OMIT_SET4=0` `SKEL_OMIT_SLOT0=0` | SoftSync + 两轮 GATE 4/8 + stub NTT/INTT Cube 跑完；magic `SKELDEC1` / `out[8]=0x04` |
...
`SKEL_OMIT_SET4` / `SKEL_OMIT_SLOT0` / `SKEL_OMIT_SET4_R2` **三者互斥**（`run.sh` 同时开多个报错）。
