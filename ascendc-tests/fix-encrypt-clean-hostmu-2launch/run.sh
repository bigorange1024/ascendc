#!/bin/bash
# fix-encrypt-clean-hostmu-2launch：干净 Encrypt P0（Host 2-launch + 默认 Host μ + skipNtt 无 PrefixEmbed）
#
# Usage（默认 = 生产全量结构；无 Host-μ 开关）：
...
echo "[clean-enc] Host 2-launch + HostFoldMuAlways (by construction; no SKEL_HOST_MU switch)"
...
echo "[SUCCESS] fix-encrypt-clean-hostmu-2launch (${RUN_MODE})"
