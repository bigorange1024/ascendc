/**
 * @file aic_func.hpp
 * @brief 骨架 toy 的 AIC MMAD：C[m,n] = A[m,k] @ B[k,n]（int8→int32）。
 *
 * 工程壳参考 pass-toy-mix-s123-byteencode-k2 的 AicMmad 四段流水
 * （CopyIn Nd2Nz → SplitA/B → Mmad → Fixpipe）。
 * 维度由调用方传入：SKEL_HEAVY=0 → 16×32×32；SKEL_HEAVY=1 → 16×64×64。
 * 本文件**不**实现真 NTT LUT；B 由 host 预填单位阵，仅验证 Cube 握手路径。
 *
 * AscendC API：DataCopy(+Nd2NzParams)、LoadData、LoadDataWithTranspose、Mmad、Fixpipe
 * ——均已在查阅索引有记录，本任务无新增 API。
 */
#ifndef ENCRYPT_SKEL_AIC_FUNC_HPP
#define ENCRYPT_SKEL_AIC_FUNC_HPP
...
__aicore__ inline static constexpr T skel_max(T a, U b)
{
    return (a > (T)b) ? a : (T)b;
}
...
static constexpr __aicore__ inline uint16_t skel_ceil_div(uint16_t a, uint16_t mod)
{
    return (a + mod - 1) / mod;
}
...
/**
 * @class AicMmad
 * @brief Cube 侧轻量 int8 矩阵乘：C[m,n] int32 = A[m,k] int8 @ B[k,n] int8。
 * @param m/k/n 典型 16/32/32 或 16/64/64；k、n 须为 32 倍数。
 */
class AicMmad {
public:
    __aicore__ inline AicMmad(uint16_t m, uint16_t k, uint16_t n) : m(m), k(k), n(n)
    {
        aSize = skel_max(16, m) * k;
        bSize = k * n;
        cSize = skel_max(16, m) * n;
    }
...
private:
    __aicore__ inline void CopyIn()
    {
        ...
        nd2nzA1Params.dstNzC0Stride = skel_ceil_div(m, 16) * 16;
        ...
        nd2nzB1Params.dstNzC0Stride = skel_ceil_div(k, 16) * 16;
    ...
    __aicore__ inline void SplitA()
    {
        ...
        uint32_t dstOffset = skel_ceil_div(k, 32) * CUBE_BLOCK_SIZE;
        uint32_t srcOffset = CUBE_BLOCK_SIZE;

        AscendC::LoadData2dParams loadDataParams;
        loadDataParams.repeatTimes = skel_ceil_div(k, 32);
        loadDataParams.srcStride = skel_ceil_div(m, 16);
        loadDataParams.dstGap = 0;
        loadDataParams.ifTranspose = false;
        for (int32_t i = 0; i < skel_ceil_div(m, 16); i++) {
    ...
    __aicore__ inline void SplitB()
    {
        ...
        uint32_t dstOffset = skel_ceil_div(n, 32) * (2 * CUBE_BLOCK_SIZE);
        uint32_t srcOffset = 2 * CUBE_BLOCK_SIZE;

        AscendC::LoadData2dTransposeParams loadDataParams;
        loadDataParams.repeatTimes = skel_ceil_div(n, 32);
        loadDataParams.srcStride = skel_ceil_div(k, 32);
        loadDataParams.dstGap = 1;
        loadDataParams.dstFracGap = 0;

        for (int i = 0; i < skel_ceil_div(k, 32); i++) {
    ...
    __aicore__ inline void Compute()
    {
        ...
        mmadParams.m = skel_ceil_div(m, 16) * 16;
        mmadParams.k = k;
        mmadParams.n = n;
    ...
    __aicore__ inline void CopyOut()
    {
        ...
        fixpipeParams.srcStride = skel_ceil_div(m, 16) * 16;
