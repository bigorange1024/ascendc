# INTEGRATION_PLAN — fix-f203-alg14-lines2-18-19-21-encrypt-compute-k4
...
| 符号 | 含义 | 本探针 GM / 文件 |
|------|------|------------------|
| **r** | randomness（32B） | 非本探针范围（由 [`fix-f203-alg14-lines3-15-encrypt-prep-k4`](../fix-f203-alg14-lines3-15-encrypt-prep-k4/) 的 `coins.bin` 生成 **y,e₁,e₂**） |
...
**前置探针**：[`fix-f203-alg14-lines3-15-encrypt-prep-k4`](../fix-f203-alg14-lines3-15-encrypt-prep-k4/)（行 3–15：Â、y、e₁、e₂）。
...
**状态**：**行 18–19（û/u）可行性完成** — 默认 3 launch CPU+SIM；`F203_FEAS_FUSED=1` 单 launch **仅 SIM** max=0。详见 [`STATUS.md`](STATUS.md)。

### 8.1 已实现 Launch 拓扑

| 模式 | Kernel 序列 | CPU | SIM |
|------|-------------|-----|-----|
| **默认（3 launch）** | `f203_encrypt_ntt_y` → `f203_encrypt_at_jp` → `f203_encrypt_intt_e1` | ✓ | ✓ |
| **融合试验（1 launch）** | `f203_encrypt_l18_l19`（`F203_FEAS_FUSED=1`） | ✗ 死锁 | ✓ |

**本日范围**：仅 **4 列 û + u**；**未实现** kP=5（`tr̂`）、行 21 `v`、行 2 `decode_t_hat`、单 launch NoiseTail。
...
| `main.cpp` | `RunPhased*` / `F203_FEAS_FUSED` / fused-trace |
...
4. **与 prep 拼接**：`fix-f203-alg14-lines3-15-encrypt-prep-k4` + 本探针 → 2 launch Encrypt 核心。
...
F203_FEAS_FUSED=1 SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4   # 单 launch，预算 180s
```
...
- prep：[INTEGRATION_PLAN.md](../fix-f203-alg14-lines3-15-encrypt-prep-k4/INTEGRATION_PLAN.md)
