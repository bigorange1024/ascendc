# INTEGRATION_PLAN — fix-f203-alg20-kem-encaps-correctness-k4
...
| **封装公钥 `ek`** | **读 KEM KeyGen 产出**：[`fix-f203-alg19-kem-keygen-correctness-k4`](../fix-f203-alg19-kem-keygen-correctness-k4/) 的 `output/ek_kem.bin`（1568B，与 `ek_PKE` 同布局）；**非**单独再跑 PKE KeyGen、**非** Host 调 liboqs 写 `ek` |
...
# 标准验收序（仓库级或手工）
SEED_D=20260619
cd ascendc-tests/fix-f203-alg19-kem-keygen-correctness-k4 && bash run.sh -r cpu -v Ascend910B4
cd ../fix-f203-alg20-kem-encaps-correctness-k4
# gen_data 复制 ../fix-f203-alg19-kem-keygen-correctness-k4/output/ek_kem.bin → input/
...
| 变量 | 默认 | 说明 |
|------|------|------|
| `EK_KEM_SRC` | `../fix-f203-alg19-kem-keygen-correctness-k4/output/ek_kem.bin` | `gen_data` 复制公钥源 |
...
| 探针 | 算法 | 关系 |
|------|------|------|
| [`fix-f203-alg19-kem-keygen-correctness-k4`](../fix-f203-alg19-kem-keygen-correctness-k4/) | Alg.19 KeyGen | **供给 `ek_kem.bin`**（本探针 input） |
| **本目录** | Alg.20 Encaps | 消费 `ek_kem`；产出 `c`/`K` |
| `fix-f203-alg21-kem-decaps-correctness-k4`（待建） | Alg.21 Decaps | 消费 `dk_kem`+`c`；vendor alg15 |
...
| 文档 | 链接 |
|------|------|
| Alg.19 KeyGen 探针 | [`fix-f203-alg19-kem-keygen-correctness-k4/INTEGRATION_PLAN.md`](../fix-f203-alg19-kem-keygen-correctness-k4/INTEGRATION_PLAN.md) |
