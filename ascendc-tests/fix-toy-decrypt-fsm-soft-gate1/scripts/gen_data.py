#!/usr/bin/env python3
"""
gen_data.py — fix-toy-decrypt-fsm-soft-gate1 输入生成。

仅提供合法输入：tiling.bin + lut.bin（I₃₂）。不对算法正确性；无 golden 对拍。
softSyncGm 由 Host 运行时清零；S0 由 Host 预填常数。
"""
import os
import struct
import numpy as np
...
def main() -> None:
    ...
    print(f"[gen_data] lut=I_{K_DIM} bytes={lut.size} (no golden; SoftSync+GATE toy)")
