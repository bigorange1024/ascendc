 ...
 *
 * 流水线位置：本 SoftSync 玩具中 AIC **不做 WaitFlag**，独立跑完极轻 MMAD 后返回；
 * 形状取 int8 Cube 最小粒度，避免 SIM 过慢。禁止 SyncAll。
 * API：DataCopy+Nd2Nz / LoadData / LoadDataWithTranspose / Mmad / Fixpipe
 * （查阅索引已有记录，本玩具复用）。
 */
#ifndef FIX_TOY_DECRYPT_FSM_SOFTSYNC1_AIC_FUNC_HPP
#define FIX_TOY_DECRYPT_FSM_SOFTSYNC1_AIC_FUNC_HPP
