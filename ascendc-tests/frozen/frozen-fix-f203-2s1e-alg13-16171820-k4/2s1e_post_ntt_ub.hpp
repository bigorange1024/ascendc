 ...
 *   2. multiply_ntts_half_scalar（标量 basemul；post-NTT，不受 NTT Gather 禁令）
...
private:
    ...
    __aicore__ inline void stageEncodeOut(LocalTensor<int32_t> &ub_ntt, LocalTensor<int32_t> &ub_that)
    {
        ...
        LocalTensor<uint8_t> ek_local = scratch_.GetWithOffset<uint8_t>(byte_encode12::kAivShardBytes,
        ...
        for (uint16_t p = pBegin_; p < pEnd_; ++p) {
            ...
            byte_encode12::poly_byte_encode12_local(ek_poly, t_poly, coeffN_);
            KYBER_PIPE_ALL();
            ...
            byte_encode12::poly_byte_encode12_local(sk_poly, s_poly, coeffN_);
            KYBER_PIPE_ALL();
