...
#include "hat_basemul_variants.hpp"
 ...
 *   2. multiply_ntts_half_dispatch（basemul；变体见 basemul_config.hpp）
...
public:
                                ...
                                GM_ADDR t_dump)
    {
        ...
        pipe_.InitBuffer(scratch_, (ubNttLength_ + thatTileLength_ + sBatchPlaneLength_ + eBatchPlaneLength_ +
                                    limbTileLength_ + halfLen_ + coeffN_ + aHatAivTileLength_ + 10U * pc + 4U * hl +
                                    kHatBasemulExtraInt32Slots * pc) *
                                       i32 +
...
private:
    ...
    __aicore__ inline void stageHatInto(LocalTensor<int32_t> &ub_ntt, LocalTensor<int32_t> &ub_that)
    {
        ...
        LocalTensor<int32_t> f = bufI32(hatBase + 2U * halfLen_ + coeffN_ + aHatAivTileLength_ + 10U * pc, halfLen_);
        LocalTensor<int32_t> g = bufI32(hatBase + 2U * halfLen_ + coeffN_ + aHatAivTileLength_ + 10U * pc + halfLen_,
                                      halfLen_);
        LocalTensor<int32_t> t1m =
            bufI32(hatBase + 2U * halfLen_ + coeffN_ + aHatAivTileLength_ + 10U * pc + 2U * halfLen_, halfLen_);
        LocalTensor<int32_t> t2m =
            bufI32(hatBase + 2U * halfLen_ + coeffN_ + aHatAivTileLength_ + 10U * pc + 3U * halfLen_, halfLen_);
#if HAT_BASEMUL_VARIANT >= 1
        LocalTensor<int32_t> basemul_ws =
            bufI32(hatBase + 2U * halfLen_ + coeffN_ + aHatAivTileLength_ + 10U * pc + 4U * halfLen_,
                   kHatBasemulExtraInt32Slots * pc);
#endif
#if F203_MOD_VARIANT == 2
        const uint32_t fOff = hatBase + 2U * halfLen_ + coeffN_ + aHatAivTileLength_ + 10U * pc + 4U * halfLen_ +
                              kHatBasemulExtraInt32Slots * pc;
        const uint32_t fStride = halfLen_ * static_cast<uint32_t>(sizeof(float));
        ...
        for (uint16_t p = pBegin_; p < pEnd_; ++p) {
            ...
            for (uint32_t subOff = 0; subOff < coeffN_; subOff += halfLen_) {
                ...
                for (uint16_t j = 0; j < static_cast<uint16_t>(tiling::kHatK); ++j) {
                    ...
                    KYBER_PIPE_ALL();
#if HAT_BASEMUL_VARIANT >= 1
                    multiply_ntts_half_dispatch(row, f, g, basemul_ws, static_cast<int32_t>(pairCount_), gammaOff);
#else
                    multiply_ntts_half_dispatch(row, f, g, static_cast<int32_t>(pairCount_), gammaOff);
#endif
    ...
    __aicore__ inline void stageEncodeOut(LocalTensor<int32_t> &ub_ntt, LocalTensor<int32_t> &ub_that)
    {
        const uint32_t encBase = ubNttLength_ + thatTileLength_ + sBatchPlaneLength_ + eBatchPlaneLength_ +
                                 ...
                                 10U * pairCount_ + 4U * halfLen_ + kHatBasemulExtraInt32Slots * pairCount_;
