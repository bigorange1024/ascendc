 ...
 *   单趟 mixPass=0：S3→ws 发布→末路 AIV 行18+encode（SIM 串行安全，无 AIV↔AIV 空转）。
 */
                                                  ...
                                                  GM_ADDR src, GM_ADDR a_hat, GM_ADDR ws, TilingData tiling)
{
    ...
    const bool peerShatHandoff = runHat && runS3 && !loadNttPreset;
    const bool hatMaster =
        peerShatHandoff && (subBlockID == static_cast<int32_t>(kAivBatches) - 1);
    ...
    if (AIC) {
        ...
        if (ubFused && subBlockID < static_cast<int32_t>(kAivBatches)) {
            const bool runS3Stage = runS3 && !loadNttPreset;
            AivAlg13UbPipeline pipe(subBlockID, coeffN, hatMaster);
            pipe.Init(ws + MAT_C_PLANAR, ws + SHAT_PEER, a_hat, ek_out, sk_out, dst, t_hat);
            if (peerShatHandoff) {
                pipe.ProcessPublishOnly(runS3Stage);
                if (hatMaster) {
                    pipe.ProcessHatEncodeMaster(runEncode, runS3Stage);
                }
            } else {
                pipe.Process(runS3Stage, runHat, runEncode, loadNttPreset, loadThatPreset);
            }
            KYBER_PIPE_ALL();
        } else if (runS3) {
            STATE = AIV_MERGE;
