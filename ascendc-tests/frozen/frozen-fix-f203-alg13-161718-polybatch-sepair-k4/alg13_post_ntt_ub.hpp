...
public:
    __aicore__ inline AivAlg13UbPipeline(int32_t subCoreIdx, uint32_t coeffN)
        : subCoreIdx(subCoreIdx), coeffN(coeffN), halfLen(coeffN / 2), pairCount(halfLen / 2),
          ...
          thatTileLength(static_cast<uint32_t>(sepair::p_end(subCoreIdx) - sepair::p_begin(subCoreIdx)) * coeffN),
          pBegin(sepair::p_begin(subCoreIdx)), pEnd(sepair::p_end(subCoreIdx))
    {
    }

    __aicore__ inline void Init(GM_ADDR matPlanar, GM_ADDR peerShat, GM_ADDR ubSync, GM_ADDR a_hat,
                                GM_ADDR ek_out, GM_ADDR sk_out, GM_ADDR dst_dump, GM_ADDR t_dump)
    {
        gm_planar.SetGlobalBuffer((__gm__ int32_t *)matPlanar);
        gm_peer.SetGlobalBuffer((__gm__ int32_t *)peerShat);
        syncPtr = reinterpret_cast<__gm__ volatile int32_t *>(ubSync);
    ...
    __aicore__ inline void Process(bool runS3Stage, bool runHat, bool runEncode, bool loadNttPreset,
                                   bool loadThatPreset)
    {
        LocalTensor<int32_t> ub_ntt = que_ub_ntt.AllocTensor<int32_t>();
        LocalTensor<int32_t> ub_that = que_ub_that.AllocTensor<int32_t>();
        ...
        if (loadThatPreset) {
            ...
            if (runS3Stage) {
                publishPeerShat(ub_ntt);
                waitPeerReady();
            }
            stageHatInto(ub_ntt, ub_that, loadNttPreset);
        }
...
private:
    ...
    __aicore__ inline void publishPeerShat(LocalTensor<int32_t> &ub_ntt)
    {
        const uint32_t slotBase = static_cast<uint32_t>(subCoreIdx) * 2U * coeffN;
        const uint16_t j0 = static_cast<uint16_t>(subCoreIdx) * 2U;
        const uint16_t j1 = j0 + 1U;
        DataCopy(gm_peer[slotBase], ub_ntt[localLpForSRow(j0) * coeffN], coeffN);
        DataCopy(gm_peer[slotBase + coeffN], ub_ntt[localLpForSRow(j1) * coeffN], coeffN);
        KYBER_PIPE_ALL();
        syncPtr[subCoreIdx] = 1;
        AscendC::PipeBarrier<PIPE_ALL>();
    }

    __aicore__ inline void waitPeerReady()
    {
#if defined(ASCENDC_CPU_DEBUG)
        /* tikicpu 串行 subBlock：后续 AIV 尚未 publish，禁止 GM 空转。 */
        (void)syncPtr;
        return;
#else
        /* SIM 串行调度同理会死锁；真机并行时 kAivBatches 路 AIV 各自 publish 后汇合。 */
        const int32_t nAiv = static_cast<int32_t>(tiling::kAivBatches);
        for (;;) {
            int32_t ready = 0;
            for (int32_t i = 0; i < nAiv; ++i) {
                ready += syncPtr[i];
            }
            if (ready >= nAiv) {
                break;
        ...
        for (uint16_t p = pBegin; p < pEnd; ++p) {
            ...
            for (uint32_t subOff = 0; subOff < coeffN; subOff += halfLen) {
                ...
                for (uint16_t j = 0; j < tiling::kHatK; ++j) {
                    ...
                    const uint32_t aOff = aPolyOff + static_cast<uint32_t>(j) * coeffN + subOff;
                    DataCopy(f, a_tile[aOff], halfLen);
                    if (peerFromDstPreset) {
                        const uint32_t sOff = sepair::s_row(j) * coeffN + subOff;
                        DataCopy(g, gm_dst_dump[sOff], halfLen);
                    } else {
                        const int32_t peerCore = 1 - subCoreIdx;
                        const uint32_t peerSlot = static_cast<uint32_t>(peerCore) * 2U * coeffN;
                        const uint32_t peerRow =
                            (static_cast<uint32_t>(j) - static_cast<uint32_t>(peerCore) * 2U) * coeffN;
    ...
    __aicore__ inline void stageEncodeOut(LocalTensor<int32_t> &ub_ntt, LocalTensor<int32_t> &ub_that)
    {
        ...
        LocalTensor<uint8_t> ek_local = scratch.GetWithOffset<uint8_t>(byte_encode12::kAivShardBytes,
                                                                       encBase * sizeof(int32_t));
        LocalTensor<uint8_t> sk_local = scratch.GetWithOffset<uint8_t>(
            byte_encode12::kAivShardBytes, (encBase * sizeof(int32_t)) + byte_encode12::kAivShardBytes);
        ...
        for (uint16_t p = pBegin; p < pEnd; ++p) {
            ...
            LocalTensor<int32_t> s_poly = ub_ntt[localLpForSRow(p) * coeffN];
    ...
    __gm__ volatile int32_t *syncPtr;
};
