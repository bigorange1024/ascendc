...
/** 行 18 Phase B：每 AIV 发布 2 行 ŝ（内积握手，非算法 dump）。 */
constexpr size_t SHAT_PEER_BYTES = kAivBatches * 2 * n * sizeof(int32_t);
constexpr size_t SHAT_PEER = wssize;
constexpr size_t ALG13_UB_SYNC = SHAT_PEER + SHAT_PEER_BYTES;
constexpr size_t wssize_alloc = ALG13_UB_SYNC + 8;
