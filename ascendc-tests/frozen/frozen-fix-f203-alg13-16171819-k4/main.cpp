/**
 * Alg.13 行 16–17–18–19：se NTT → ŝ‖ê；内积 → t_hat；ByteEncode₁₂ → ek/sk polyvec 字节。
 */
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    if (tiling->mixPass == 3 || tiling->mixPass == 4 || tiling->mixPass == 5) {
    ...
    std::memset(ws + tiling::LINE18_AIV_SYNC, 0, 8);
