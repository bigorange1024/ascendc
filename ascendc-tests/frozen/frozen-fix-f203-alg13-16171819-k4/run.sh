#!/bin/bash
# Alg.13 行 16–17–18–19（k=4）：Tag5T NTT + AIV 内积 + AIV ByteEncode₁₂
# Usage:
#   bash run.sh -r cpu -v Ascend910B4
#   ALG13_MIX_PASS=5 bash run.sh -r cpu   # 仅行 19（需 dst_preset + t_hat_preset）
CURRENT_DIR=$(
...
if [ "${ALG13_MIX_PASS}" = "4" ]; then
    if [ -f "./output/golden_shat_ehat.bin" ]; then
        cp ./output/golden_shat_ehat.bin ./input/dst_preset.bin
    fi
fi
if [ "${ALG13_MIX_PASS}" = "5" ]; then
    if [ -f "./output/golden_shat_ehat.bin" ]; then
        cp ./output/golden_shat_ehat.bin ./input/dst_preset.bin
    fi
...
echo "[SUCCESS] fix-f203-alg13-16171819-k4 (${RUN_MODE})"
