...
| # | 原因 | 说明 |
|---|------|------|
| 1 | **办公室未复验** | 家里 agent 在本地环境声称全链 PASS；**办公室未跑完**对该探针的 CPU+SIM 独立验收（含默认 15s kernel 预算、stray dump 检查等仓库门禁） |
...
| 5 | **验收口径不一致** | 本树 STATUS 要求 `ENCRYPT_KERNEL_BUDGET_SEC=1000` 等手动 export；仓库默认门禁为 15s + 裸 `bash run.sh` |
