...
extern "C" __global__ __aicore__ void f203_encrypt_decode_t_hat(GM_ADDR ekGm, GM_ADDR tHatGm, GM_ADDR aCol0Gm)
{
    ...
    // 本核在全链中于 NTT(MIX) 之后 launch；CAModel 下 MIX binary 加载后 device_aiv.o
    // 注册失效（107000），AIV-only launch 返回 507000。SIM/NPU 声明 MIX_AIC_1_2 并入
    // 与 NTT/INTT 同一 device.o；本核 kUseCores=1（单核），仅 AIV subcore 0 执行，语义不变。
    // CPU debug 保持原 AIV_ONLY 单核路径（不分 binary，且 CPU MIX subblock 语义不同）。
    ...
    if (GetSubBlockIdx() != 0) {
        return; // 仅 AIV subcore 0 执行 ByteDecode₁₂
    }
#endif
    KernelDecodeTHat op;
