...
constexpr uint32_t kShardPolys = kShakeBatch / 2U;
#else
constexpr uint32_t kShardPolys = kShakeBatch;
#endif
...
constexpr uint32_t kShakeActiveBatch = kShardPolys;
...
/** 填 batch=16（或 2 AIV 时 batch=8）全量消息；等价于 range(0, kShakeActiveBatch)。 */
...
/**
 * SEED_D → ρ → 本分片 poly（1 AIV×16 或 2 AIV×8）SHAKE + 向量 d12/rej → GM a_hat 对应块。
 *
 * 2 AIV：blockIdx 0 → polyIdx 0–7；blockIdx 1 → 8–15；GM 偏移互不重叠。
 */
/**
 * 单分片：blockIdx 0→poly 0–7，1→8–15（BLOCK_DIM=2）；BLOCK_DIM=1 时仅 blockIdx=0 有效。
...
{
#if F203_AHAT16_BLOCK_DIM == 2
    const uint32_t polyBegin = blockIdx * kShardPolys;
    const uint32_t polyEnd = polyBegin + kShardPolys;
#else
    (void)blockIdx;
...
__aicore__ inline void BuildAHat16ShardFromSeedD(uint32_t seed_d, __gm__ int32_t *a_hat_gm, uint32_t blockIdx)
{
    ...
    const uint32_t polyBegin = blockIdx * kShardPolys;
    const uint32_t polyEnd = polyBegin + kShardPolys;
    const uint32_t polyCount = kShardPolys;
#else
    (void)blockIdx;
    const uint32_t polyBegin = 0U;
    const uint32_t polyEnd = kShakeBatch;
