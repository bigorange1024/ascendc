 ...
 * @brief G3 su_dot 向量/标量段（g4_full 内联）。
 */
...
/** w_hat 写入 k=4 polyvec 缓冲前缀，余下清零，供 INTT 读 src。 */
__aicore__ inline void pad_w_hat_for_intt(GM_ADDR wPaddedGm, GM_ADDR wHatGm)
{
    auto *dst = reinterpret_cast<__gm__ int32_t *>(wPaddedGm);
    const auto *src = reinterpret_cast<const __gm__ int32_t *>(wHatGm);
    const uint32_t totalInts =
        static_cast<uint32_t>(::tiling::dstFileBytes / sizeof(int32_t));
    for (uint32_t i = 0; i < totalInts; ++i) {
        dst[i] = 0;
    }
    for (int32_t i = 0; i < kSuDotN; ++i) {
        dst[i] = src[i];
    }
}
