#!/bin/bash
# Tag5T poly-batch Stage1+2（平面 mat_c）：S0 → 偶/奇 LUT 分乘 → mat_c_planar [64,128]
# Usage:
#   bash run.sh -r cpu -v Ascend910B4
#   PLANAR_MIX_PASS=1 bash run.sh -r cpu   # 仅 Stage1
#   PLANAR_MIX_PASS=2 bash run.sh -r cpu   # 仅 Stage2（需 s0 preset）
CURRENT_DIR=$(
...
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} PLANAR_MIX_PASS=${PLANAR_MIX_PASS:-0}"
...
export PLANAR_MIX_PASS="${PLANAR_MIX_PASS:-${TAG5T_MIX_PASS:-0}}"
export TAG5T_MIX_PASS="${PLANAR_MIX_PASS}"
python3 "${CURRENT_DIR}/scripts/gen_data.py"

if [ "${PLANAR_MIX_PASS}" = "2" ]; then
...
echo "[SUCCESS] fix-f203-tag5t-ntt256-limb6-poly8-planar-s12 (${RUN_MODE})"
