#!/bin/bash
# Tag5T poly-batch Stage1+2+3：S0 poly-batch → mat_c → dst [8,256] NTT
# Usage:
...
if [ "${TAG5T_SRC_LAYOUT:-}" = "se" ] || [ "${TAG5T_SRC_LAYOUT:-}" = "hat_se" ]; then
    python3 "${CURRENT_DIR}/scripts/verify_se_aiv_split.py" || exit 1
fi
echo "[SUCCESS] fix-f203-tag5t-ntt256-limb6-poly8-polybatch-s123 (${RUN_MODE})"
