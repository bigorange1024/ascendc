#!/bin/bash
# Phase D′ poly2（fix）：Stage1 batch Split + Stage2 Mmad（无 Merge）
# Usage:
...
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} SIM_DIRECT=${SIM_DIRECT} (fix poly2 s12: single TPipe batch Split)"
...
KERNEL_COMPUTE_BUDGET_SEC="${KERNEL_COMPUTE_BUDGET_SEC:-15}"
...
echo "[SUCCESS] fix-merged-kyber-ntt256-limb6-poly2-s12 (${RUN_MODE})"
