#!/usr/bin/python3
# coding=utf-8
"""poly2 Stage1+2：2×同 poly；golden 为 A[4,256] @ M0（int32，无 Merge）。"""
import os
import sys

import numpy as np

_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_SHARED = os.path.normpath(os.path.join(_SCRIPT_DIR, "../../../../library/shared"))
_MERGED_SCRIPTS = os.path.normpath(os.path.join(_SCRIPT_DIR, "../../../../thirdparty/merged_kyber/scripts"))
sys.path.insert(0, _SHARED)
sys.path.insert(0, _MERGED_SCRIPTS)

import merged_kyber_fixed_poly  # noqa: E402
import ntt_sim_kyber  # noqa: E402

N = 256
K_POLYS = 2
LIMB_BITS = 6
LIMB_MASK = (1 << LIMB_BITS) - 1


def split_poly(f: np.ndarray) -> np.ndarray:
    hi = (f & LIMB_MASK).astype(np.int8)
    lo = ((f >> LIMB_BITS) & LIMB_MASK).astype(np.int8)
    return np.stack([hi, lo], axis=0)


def gen_tiling():
    os.makedirs("input", exist_ok=True)
    os.makedirs("output", exist_ok=True)
    np.array([N, K_POLYS], dtype=np.int32).tofile("./input/tiling.bin")


if __name__ == "__main__":
    gen_tiling()
    input_x = merged_kyber_fixed_poly.FIXED_POLY.copy()
    ntt_sim_kyber.ntt_forward(input_x, N, 3329, 17)
    src_batch = np.tile(input_x.reshape(1, -1), (K_POLYS, 1))

    rows = [split_poly(src_batch[p]) for p in range(K_POLYS)]
    mat_a = np.concatenate(rows, axis=0).astype(np.int32)

    m = ntt_sim_kyber.M.astype(np.int32)
    m0 = ((m >> 0) & LIMB_MASK).astype(np.int8).reshape(N, N)
    golden = (mat_a @ m0).astype(np.int32)

    src_batch.tofile("./input/src.bin")
    golden.tofile("./output/golden.bin")

    m0_flat = ((m >> 0) & LIMB_MASK).astype(np.int8).reshape(-1)
    m1 = ((m >> LIMB_BITS) & LIMB_MASK).astype(np.int8).reshape(-1)
    m2 = ((m >> (2 * LIMB_BITS)) & LIMB_MASK).astype(np.int8).reshape(-1)
    m3 = ((m >> (3 * LIMB_BITS)) & LIMB_MASK).astype(np.int8).reshape(-1)
    m_out = np.concatenate((m0_flat, m1, m2, m3), dtype=np.int8)
    m_out.tofile("./input/M4.bin")

    print(f"[gen_data] fixed poly seed={merged_kyber_fixed_poly._SEED} md5={__import__('hashlib').md5(input_x.tobytes()).hexdigest()}")
    print(f"[gen_data] src [{K_POLYS},{N}] int32")
    print(f"[gen_data] golden [{2 * K_POLYS},{N}] int32 (A@M0)")
    print(f"[gen_data] M4.bin bytes={m_out.nbytes} (same layout as limb6 D)")
