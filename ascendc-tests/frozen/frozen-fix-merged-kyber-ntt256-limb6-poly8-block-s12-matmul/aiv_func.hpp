...
/** Stage1 batch Split：单 TPipe，kPolys 条 poly 一趟 split_vec；S0 写紧凑 [HI_k | LO_k]。 */
class AivSplit {
public:
 ...
 * 从紧凑 A0/A1 [2*kPolys,256] 按 poly 取行 p / k+p（块布局 Gather，等价 F203 idx_hi/idx_lo）。
 */
class AivMerge {
public:
    ...
    __aicore__ inline void CopyIn()
    {
        ...
        for (uint16_t p = 0; p < kPolys; p++) {
            ...
            const uint32_t rowHi = static_cast<uint32_t>(p) * coeffN + static_cast<uint32_t>(subCoreIdx) * halfLen;
            const uint32_t rowLo =
                static_cast<uint32_t>(kPolys + p) * coeffN + static_cast<uint32_t>(subCoreIdx) * halfLen;
