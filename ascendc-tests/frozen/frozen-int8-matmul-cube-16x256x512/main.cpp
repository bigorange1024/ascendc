/**
 * Kyber Stage2 原生 Matmul：C[16,512]=A[16,256]xB[256,512]（无垫片）。
 * Launch 见 launch_profile.h（默认 aicore=1：blockDim=1, SetDim(2)）。
 */
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    WriteFile("./output/mat_c.bin", c, kCFileSize);
    AscendC::GmFree(a);
    AscendC::GmFree(b);
    AscendC::GmFree(c);
    ...
    CHECK_ACL(aclrtMemcpy(bDevice, kBFileSize, bHost, bFileSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ...
    WriteFile("./output/mat_c.bin", cHost, kCFileSize);
