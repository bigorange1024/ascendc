# IMPLEMENTATION_PLAN — pass-f203-alg6-bytedecode-d-vec-k4

**状态**：**pass**（d=**4/5/10/11** CPU+SIM PASS）  
**目录更名**（2026-07-08）：`pass-f203-alg6-bytedecode-d4-d10-vec-k4` → **`pass-f203-alg6-bytedecode-d-vec-k4`**；验收 d 扩至 `{4,5,10,11}`。  
**FIPS 203**：Algorithm 6 `ByteDecode_d`；§4.2 + §2.4.8  
**配对探针**：[`pass-f203-byteencode-d-vec-k4`](../pass-f203-byteencode-d-vec-k4/)；d=12 见 [`pass-fix-f203-2s1e-byteencode12-vec-k4`](../pass-fix-f203-2s1e-byteencode12-vec-k4/)
...
**Decrypt 路径**（Alg.15）：`u' ← Decompress_du( ByteDecode_du(c₁) )` — 本探针只做 **ByteDecode**（**d=4/5/10/11**）；Decompress 见 [`pass-f203-decompress-d-vec-k4`](../pass-f203-decompress-d-vec-k4/)。

### 1.2 Encrypt 相关 d（k=4 polyvec）

| d | 输入字节/poly | Decrypt 用途 |
|---|---------------|--------------|
| 11 | 352 | ML-KEM-1024 c₁ |
| 10 | 320 | ML-KEM-512/768 c₁ |
| 5 | 160 | ML-KEM-1024 c₂ |
| 4 | 128 | ML-KEM-512/768 c₂ |
...
```text
pass-f203-alg6-bytedecode-d-vec-k4/
...
8 系数/组分组 unpack（与 encode 对称；抄 mlkem-native `poly_decompress_d5/d11` 比特重组半部）。d=5/11 **PASS**（2026-07-08）。
