#!/usr/bin/env bash
# pass-f203-compress-unified-int-vec-k4 — 统一整数舍入 Compress_d（d=1/4/5/10/11）
#
# 默认 COMPRESS_UNIFIED_INT_VEC=1（纯 int32 向量 limb 宽乘：C0/C1 Muls + 进位合并）；=0 为 int64 lane 对照
# 数学见 docs/notes/F203-Compress-Decompress-统一整数舍入技术总结.md
#
# Usage（默认）:
#   bash run.sh -r cpu -v Ascend910B4
#   F203_UNIFIED_ROUND_D=11 bash run.sh -r sim -v Ascend910B4
#
# 环境变量：
#   F203_UNIFIED_ROUND_D — 1/4/5/10/11（默认 4）
#   COMPRESS_UNIFIED_INT_VEC — 0=int64 lane 对照 / 1=纯 int32 向量 limb（默认 1）
#
...
export F203_UNIFIED_ROUND_D="${F203_UNIFIED_ROUND_D:-4}"
export COMPRESS_UNIFIED_INT_VEC="${COMPRESS_UNIFIED_INT_VEC:-1}"
...
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} F203_UNIFIED_ROUND_D=${F203_UNIFIED_ROUND_D} COMPRESS_UNIFIED_INT_VEC=${COMPRESS_UNIFIED_INT_VEC}"
...
cmake -B build \
    ...
    -DF203_UNIFIED_ROUND_D="${F203_UNIFIED_ROUND_D}" \
    -DCOMPRESS_UNIFIED_INT_VEC="${COMPRESS_UNIFIED_INT_VEC}"
cmake --build build -j
cmake --install build
...
echo "[SUCCESS] pass-f203-compress-unified-int-vec-k4 d=${F203_UNIFIED_ROUND_D} (${RUN_MODE})"
