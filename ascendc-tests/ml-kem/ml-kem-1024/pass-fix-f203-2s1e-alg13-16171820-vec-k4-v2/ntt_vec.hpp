...
/** Stage1 输出对：x0=低 6-bit limb（lo）、x1=高 6-bit limb（hi），均为 int8 LocalTensor。 */
struct Tensor_int8x4 {
    LocalTensor<int8_t> &x0;
    LocalTensor<int8_t> &x1;
};

/** LocalTensor 类型重解释（如 int8 缓冲按 int32 打包写回）。 */
...
/**
 * 标量：单系数 a → (d0=lo, d1=hi) 各 6-bit。
 * lo = a & 0x3f；hi = (a>>6) & 0x3f（与向量路径 Sub 语义在非负域一致）。
 */
...
/**
 * Stage1 标量路径：按 4 系数一组 GetValue/SetValue 拆 limb。
 * @param dst.x0/x1 输出 lo/hi int8；@param src 输入 int32；@param count 须为 4 的倍数
 */
...
constexpr int32_t kLimbScale = static_cast<int32_t>(1) << kKyberLimbBits; /**< 64 = 2^6 */

/** int32→int8：经 int16、half 两级 Cast（CAST_NONE），供 limb 落盘。 */
...
/**
 * 向量 limb6：hi = v>>6；lo = v - hi*64（Sub，禁止 And(v,63)）。
 * @param hi/lo/tmp 均为 int32 临时；@param v 输入系数；@param count 元素数
 */
...
/**
 * 单 tile：src[off..] 拆成 dst.x0/x1 的 lo/hi int8。
...
/**
 * Stage1 入口（无 scratch）：仅 F203_STAGE1_SPLIT==0 走标量；向量档位须用带 scratch 重载。
 */
 ...
 * @param tileLen 方案 2 的分块长度（通常 32）；方案 1 忽略
 */
__aicore__ inline void split_vec(Tensor_int8x4 &dst, LocalTensor<int32_t> &src, const int32_t count,
                                 AscendC::TBuf<AscendC::TPosition::VECCALC> &scratch, const int32_t tileLen)
{
...
/**
 * 条件 wrap：若 src≥q 则 dst=src-q，否则 dst=src（用符号位掩码实现，无分支）。
 * Barrett / Stage3 约化后收尾共用。
 */
...
/**
 * Barrett 乘约化运行时（参数化 q/k/mu）；Stage3 固定参数见 stage3_mod_variants。
 */
...
/**
 * Stage3 RouteA Horner 合并（无 mod）：dst = ((hh<<6)+(hl+lh))<<6 + ll。
 * 平面读入的四 limb 半行；后续由 combine_limb6_routea_mod_vec 做 mod q。
 */
...
/** Stage3 入口（方案 0）：Horner+每步 Barrett → dst[count]，q 通常 3329。 */
...
/** Stage3 入口（方案 1）：Horner raw + 标量 int64 floor mod。 */
__aicore__ inline void combine_limb6_routea_mod_vec(LocalTensor<int32_t> &dst, LocalTensor<int32_t> &hh,
                                                    LocalTensor<int32_t> &lh, LocalTensor<int32_t> &hl,
                                                    LocalTensor<int32_t> &ll, LocalTensor<int32_t> &t1,
                                                    LocalTensor<int32_t> &t2, int32_t q, int32_t count)
...
#else
/** Stage3 入口（方案 2）：Horner raw + float Div 取模。 */
__aicore__ inline void combine_limb6_routea_mod_vec(LocalTensor<int32_t> &dst, LocalTensor<int32_t> &hh,
                                                    LocalTensor<int32_t> &lh, LocalTensor<int32_t> &hl,
                                                    LocalTensor<int32_t> &ll, LocalTensor<int32_t> &t1,
