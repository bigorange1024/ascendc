 ...
 *
 * 本探针用途（ByteEncode₁₂ vec-k4）：
 *   - ReadFile：input 下各 .bin（poly / tiling 等）；
 *   - WriteFile：output 下各 .bin（ek|sk_polyvec 等）；
 *   - CHECK_ACL：SIM/NPU 路径 ACL 错误打印。
 *
 * 与 golden 关系：读写缓冲尺寸须与本探针 tiling / 配置头一致；
 * 数值对拍由 scripts/verify_result.py 完成，本头不参与计算。
 *
