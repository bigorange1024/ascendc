...
串行占用 **1 颗 AI Core**（非双核并行）。CPU `[SUCCESS][AIC_x]` 为 tikicpu 拓扑 artifact；**勿**将 KAT/CPU 路径上「6+3 行 SUCCESS」读成「2AIC+4AIV 占满多核」— 见 [F203-KeyGen-prep双AIV与SHAKE内嵌技术总结.md §4.1](../../docs/notes/F203-KeyGen-prep双AIV与SHAKE内嵌技术总结.md#41-cpu-successaic_x-与2aic4aiv误读)；以 `sim_log/profile_*.toml` 为准，实机再 msprof。
