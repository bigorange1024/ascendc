...
def _check_a_hat_rows() -> None:
    """G3 后逐行 a_hat 诊断（并行 shard1 失败时行 8–15 先错）。"""
    a = np.fromfile(OUT / "a_hat.bin", dtype=np.int32).reshape(16, 256)
    g = np.fromfile(OUT / "golden_a_hat.bin", dtype=np.int32).reshape(16, 256)
    bad = []
    for r in range(16):
...
def stage_g3(seed_d: int, kg: dict) -> None:
    ...
    _verify_gate("g3")
    _check_a_hat_rows()
    print("[staged] g3 PASS (a_hat rows 0–15, rho via verify_result)")
