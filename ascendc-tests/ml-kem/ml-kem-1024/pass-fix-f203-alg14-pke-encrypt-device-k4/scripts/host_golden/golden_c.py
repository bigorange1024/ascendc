...
"""
@file golden_c.py
@brief Alg.14 全链 host golden：ek,m,coins → c.bin（1568B）。
...
EK_T_BYTES = 1536  # ByteEncode₁₂(t̂) 不含 ρ
...
def decode_t_hat(ek: bytes) -> np.ndarray:
    """
    行 2：从 ek_pke 前 1536B 解出 t̂[K,N]。
    @param ek  完整 ek_pke（1568B）；本函数只用前 EK_T_BYTES
    """
...
def build_re(coins: bytes) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    行 7–15：coins → r[K,N], e₁[K,N], e₂[N]（nonce 0..8）。
    """
...
def golden_encrypt(ek: bytes, m: bytes, coins: bytes) -> bytes:
    """
    Alg.14 全链参考：ek+m+coins → c（1568B）。
