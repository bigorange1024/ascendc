 ...
 * @brief FIPS 203 Alg.19 ML-KEM.KeyGen host 驱动（device-k4：2 launch 全链）。
 *
 * 设备全链（2 launch，Alg.19→Alg.16→Alg.13 同 session，无第 3 launch）：
 *   L1 f203_keygen_prep  — stable Alg.13 行 3–15（d 在 device UB；源码编译期引用 stable）
 *   L2 mmad_custom       — stable Alg.13 行 16–21 + 内嵌 Alg.16 尾（F203_KEM_KEYGEN_TAIL=1）
 *
 * 工程接线（P1，2026-07-10 定案）：
 *   - L2 编译 stable `compute/mmad_custom.cpp`（非 fork），尾段在 kem/f203_kem_kg_tail_fuse.hpp；
 *   - ek_kem 与 ek_pke 共用 ekPkeGm/ekPkeDev，零 GM 拷贝；
 *   - 相对 fork mmad 版 SIM tick ~+1.8%，I/O 与 correctness 一致，保留宏方案以降低 merge 债。
 *
 * 生产 I/O：input/seed_d.bin(4B)+LUT → output/ek_kem.bin(1568)+dk_kem.bin(3168)。
 *
 * L2 mmad 实参（KEM 构建）：除 stable PKE 外追加 seed_d_gm、dk_kem_gm（见 stable mmad 宏文档）。
 */
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    // NPU/SIM 路径原先不检查 ReadFile：LUT 缺失时 workspace 全 0 → NTT 错 → ek/dk 与 golden 大面积不一致（「结果不对」）。
    rs = lutFileSize;
