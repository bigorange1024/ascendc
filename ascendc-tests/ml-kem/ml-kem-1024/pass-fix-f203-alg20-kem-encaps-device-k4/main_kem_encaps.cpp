 ...
 * @brief Alg.20/17 KEM Encaps host：stable Encrypt 2/5 launch + prep 前段设备 H/G。
 *
 * 生产 I/O：input/ek_kem.bin + m.bin + LUT → output/c.bin + K.bin。
 * coins/r 由设备 G 写入 workspace，Host 不读 coins.bin。
 * 方案见 INTEGRATION_PLAN.md；Encrypt 编译期引用 stable。
 */
...
/**
 * Host：CPU 五 launch / SIM 两 launch；产出 c.bin + K.bin。
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    ACLRT_LAUNCH_KERNEL(f203_encrypt_l18_l19)(1, stream, uDev, vDev, yDev, yHatDev, uNttDev, uTrDev, aHatDev, ekPkeDev,
                                              ...
                                              traceDev);
    CHECK_ACL(ascendc_acl::SynchronizeStreamMaybeTrace(stream, traceDev, traceHost, kFusedTraceStages));
