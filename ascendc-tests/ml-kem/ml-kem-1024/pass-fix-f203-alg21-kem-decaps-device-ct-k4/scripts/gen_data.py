#!/usr/bin/env python3
"""
gen_data.py — Alg.21 Decaps **全链** golden 与 input 生成（device 探针）。
...
（G(m‖H(ek)) + stable Encrypt 的 golden_c.golden_encrypt；拒绝分支纯 SHAKE256）。
背景：借入实机装不了 thirdparty/liboqs，原先硬依赖会让本探针直接跑不起来。
例外：外部 C_SRC 且未给 K_ENC_SRC 时需完整 Decaps（重加密比对），回落不覆盖，须有 liboqs。

## 路径解析（device 探针）
ROOT = 本用例目录；REPO = ascendc-tests 的上一级（仓库根）。
HOST_GOLDEN 指向 stable Encrypt 的 host_golden（探针不 vendored，借 stable 参考实现）。
"""
...
# 用例根：ascendc-tests/ml-kem/ml-kem-1024/pass-fix-f203-alg21-kem-decaps-device-ct-k4/
...
# KeyGen stash 默认位置（可由 KEM_KEYPAIR_STASH 覆盖）
...
def _gen_luts(inp: Path) -> None:
    ...
    # Encrypt 兼容别名（部分 run 脚本仍读 lut_even/odd_stacked）
...
def main() -> None:
    ...
    # dk 布局：末 64B = h(32) ‖ z(32)（Alg.19 KeyGen 输出）
    h = dk[3104:3136]
    z = dk[3136:3168]

    reject = os.environ.get("KEM_DECAPS_REJECT", "0") == "1"
    # 全链 input 公共段：dk、ek、h、z、LUT
    ...
    # m 来源：KAT 须 M_FILE/M_HEX；否则随机 32B
    ...
    if os.environ.get("C_SRC"):
        # 外部给定合法 c（如 roundtrip）；golden K 可来自 K_ENC_SRC 或 Decaps
        # 注：不带 K_ENC_SRC 的这条分支需要完整 Decaps（含重加密比对），回落路径不覆盖 →
    ...
    # m' 参考（正确 Decrypt 应恢复 m）；coins 供 golden_v 计算
    (inp / "m_prime_ref.bin").write_bytes(m)
    _k_ref, coins = g_mh(m, h)
    (inp / "coins.bin").write_bytes(coins)

    # golden_v：v = embed(INTT(tr̂), m) + e₂ (mod q)，与 Encrypt Phase-E CPU 对拍一致
