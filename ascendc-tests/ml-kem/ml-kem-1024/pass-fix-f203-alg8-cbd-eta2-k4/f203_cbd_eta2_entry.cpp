 ...
 *
 * 流水线位置：本探针（`pass-fix-f203-alg8-cbd-eta2-k4`）的唯一设备核编译单元，
 * 被 `main.cpp` 通过 `ICPU_RUN_KF`（CPU 孪生）或 `f203_cbd_eta2_batch8_do`
 * （SIM/NPU）拉起。核内直接调用 `f203_cbd_eta2.hpp::SamplePolyCbd2Batch8`
...
/**
 * Alg.8 CBD η=2 设备核函数。
 * @param prf_gm GM 指针，输入 PRF 输出，形状 [ROWS=8, PRF_BYTES=128] uint8
 * @param src_gm GM 指针，输出 CBD 采样结果，形状 [ROWS=8, N=256] int32
 *               （行 0-3=ŝ 各分量，行 4-7=ê 各分量）
 * 前置条件：编译期 F203_CBD_BLOCK_DIM 决定分支——
 *   =1（P1b-single）：仅 blockIdx==0 执行，串行 8 行；
 *   其他（P2 默认）：blockIdx 超出实际启动核数（GetBlockNum）时直接返回，
 *     否则按 blockIdx 走双 AIV 分片（见 SamplePolyCbd2Batch8 内部实现）。
 */
