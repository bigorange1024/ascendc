...
本文件在流水线中的位置：pass-fix-f203-alg8-cbd-eta2-k4 探针的 Host 侧数据生成
脚本，由 run.sh 在设备编译/运行前调用。职责：
  1. 用固定 SEED_D 播种的伪随机数生成 8 行 x 128 字节的 PRF 输出（模拟真实
...
ROWS = 8         # 8 行：前 4 行对应 ŝ 各分量，后 4 行对应 ê 各分量（k=4）
...
def main() -> None:
    ...
    # 8 行 x 128B 的伪随机字节流，模拟 PRF 输出（Alg.8 输入契约为任意合法 128B 数据）
