#!/usr/bin/env bash
# pass-fix-f203-stage123-ntt-intt-polyvec8-vec — 8-poly 紧凑三段式 NTT/INTT（MIX）
#
# Usage:
...
#
# 环境变量：
#   F203_NTT_MODE              — ntt（默认）或 intt（切换 LUT）
#   STAGE123_POLYVEC8_MIX_PASS — 0=S1 / 1=S2 / 2=S3 / 3=全链（默认）
#
...
export F203_NTT_MODE="${F203_NTT_MODE:-ntt}"
export STAGE123_POLYVEC8_MIX_PASS="${STAGE123_POLYVEC8_MIX_PASS:-3}"
...
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} F203_NTT_MODE=${F203_NTT_MODE} mixPass=${STAGE123_POLYVEC8_MIX_PASS}"
...
echo "[SUCCESS] pass-fix-f203-stage123-ntt-intt-polyvec8 mode=${F203_NTT_MODE} (${RUN_MODE})"
