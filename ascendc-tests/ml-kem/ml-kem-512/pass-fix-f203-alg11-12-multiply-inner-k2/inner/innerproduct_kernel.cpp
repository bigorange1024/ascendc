 ...
 * @brief 2×2×1 内积：GM 全量 a_hat / s_hat，双 AIV 写 1+1 行 t̂[p]（无 ê）。
 *
 * 语义：t̂[p] = mod_q( Σ_j MultiplyNTTs(Â[p,j], ŝ[j]) )。
 * 分核（512 参数卡）：AIV0 → [0,1)，AIV1 → [1,2)；禁沿用 k=3 的 2+1。
...
constexpr int32_t kPPerAivMax = innerproduct_tiling::kPPerAivMax;
...
/**
 * 双 AIV 半行内积：每核只维护本地 outLine[kPPerAivMax*N]，写回 t̂ 的对应行段。
 */
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR aHat, GM_ADDR sHat, GM_ADDR tHat)
    {
        ...
        // 本核输出行：k=2 明确拆成 1+1；不沿用 k=3 的 2+1，也不靠 P_OUT/2 旁路。
        if (GetBlockIdx() == 0) {
            pBegin_ = 0;
            pEnd_ = 1;
        } else {
    ...
    __aicore__ inline void ProcessHalfRows()
    {
        ...
        // 本地缓冲按 max=1 行分配；实际写回长度为 (pEnd_-pBegin_)
        ...
        LocalTensor<int32_t> outLine = bufI32(innerproduct_tiling::kOffOutLine, kPPerAivMax * kN);
        ...
        // 写回 t̂ 从 pBegin 起的连续 (pEnd-pBegin) 行
        const int32_t nRows = pEnd_ - pBegin_;
        const uint32_t tOff = static_cast<uint32_t>(pBegin_) * static_cast<uint32_t>(kN);
        DataCopy(tGm_[tOff], outLine, nRows * kN);
        ALG11_PIPE_MTE2();
    }
