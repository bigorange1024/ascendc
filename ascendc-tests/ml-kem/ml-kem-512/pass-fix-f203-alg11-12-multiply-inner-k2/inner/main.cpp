/**
 * @file main.cpp
 * @brief Host 入口：ML-KEM-512 Alg.13 行 18 内积探针（双 AIV 1+1 写 t̂，无 ê）。
 *
 * 流水线位置：读 input/a_hat.bin、s_hat.bin → 调内积 kernel → 写 output/t_hat.bin，
 * 由 scripts/verify_result.py 与 golden_t_hat.bin 对拍。
 *
 * GM 为全量 2×2 a_hat / s_hat；blockDim=2，AIV0 写 t̂[0]，AIV1 写 t̂[1]。
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    // 全量 GM 字节数；blockDim=2（k=2 锁定 1+1 输出行分片）
