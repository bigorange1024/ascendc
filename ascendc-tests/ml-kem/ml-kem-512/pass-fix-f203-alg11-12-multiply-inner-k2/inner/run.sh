#!/bin/bash
# pass-fix-f203-alg11-12-multiply-inner-k2/inner — 2×2×1 行 18 内积（双 AIV 1+1；GM a_hat/s_hat 与 alg13 一致）
#
# Usage:
#   bash run.sh -r cpu -v Ascend910B4
#   bash run.sh -r sim -v Ascend910B4
#   # 默认即 2×2×1；调试其它形状须显式覆盖 P_OUT/S_VEC，验收仍以 k2 默认为准。
#
#
...
export INNERPRODUCT_P_OUT="${INNERPRODUCT_P_OUT:-2}"
export INNERPRODUCT_S_VEC="${INNERPRODUCT_S_VEC:-2}"
...
echo "[SUCCESS] pass-fix-f203-alg11-12-multiply-inner-k2/inner (${RUN_MODE})"
