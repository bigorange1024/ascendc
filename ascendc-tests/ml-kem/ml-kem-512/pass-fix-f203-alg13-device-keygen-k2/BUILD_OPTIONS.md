# BUILD_OPTIONS — pass-fix-f203-alg13-device-keygen-k2

**原则**：`bash run.sh` **不加 export** 即走生产路径（`ascendc_keygen_bbit` + 生产 I/O）。

**实现说明**：本目录是 `ascendc-tests/` 探针，不适用 `examples/` customspec 门禁；锁定参数与验收见 [`STATUS.md`](STATUS.md)。
...
| `F203_CBD_BLOCK_DIM` | **1** | prep（Alg.13 s/e 均走 CBD-η3；代码 vendored 于 `prep/alg8/`） |
| `F203_STAGE1_SPLIT` | **1** | compute |
| `F203_KEYGEN_EK_PKE` | **1** | compute |
| `HAT_ALG11_VEC` 等 | polyvec4 生产默认 | compute |
...
全目录源文件含 `@probe pass-fix-f203-alg13-device-keygen-k2` 文件头。重新注入：
