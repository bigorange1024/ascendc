...
// @production_io 默认 run.sh 生产 I/O：input/ 仅 seed_d.bin + lut_even/odd_stacked.bin；output/ ek_pke.bin (800B) + dk_pke.bin (768B)；中间 GM 不落盘。 / Default production I/O: seed+LUT in; ek_pke+dk_pke out; no intermediate GM dumps. 本文件可能用于 legacy/staged I/O 或分阶段调试，非默认生产路径。 / May use legacy staging I/O.
 ...
 *
 * ## 与 golden 关系
 * 验收仅 I/O 等价：`ek_pke` = ByteEncode₁₂(t̂) ‖ ρ（800B）；禁止把本 Host 当作 AscendC 规格。
 *
 * ## 输入 / 输出
 * - 入：`ek_polyvec` 768B、`rho` 32B（均由上游门控或 golden 预置）
 * - 出：`ek_pke` 800B
 */
