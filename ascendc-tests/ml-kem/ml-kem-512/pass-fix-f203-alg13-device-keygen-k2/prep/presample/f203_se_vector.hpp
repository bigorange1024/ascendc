...
// @depends #include: f203_se_stage_config.hpp, f203_se_vector_prf.hpp, f203_se_vector_cbd_ub.hpp, f203_cbd_eta3.hpp
 ...
 * @brief 阶段编排：G → P → C。
 *
 * V3（F203_SE_VECTOR_V3，默认）：shake UB batch PRF + alg8 P1b/P2（SamplePolyCbd3Batch4）。
...
#else
    F203CbdEta3::SamplePolyCbd3Batch4(reinterpret_cast<const __gm__ uint8_t *>(prf_out_gm),
                                      reinterpret_cast<__gm__ int32_t *>(src_gm));
#endif
}
