...
# pass-fix-f203-alg13-device-keygen-k2 — Alg.13 全链 KeyGen（ML-KEM-512，prep 双 AIV 5+4 生成 Â）
#
# 生产 I/O：
#   input/  — seed_d.bin + lut_even/odd_stacked.bin
#   output/ — ek_pke.bin (800B) + dk_pke.bin (768B)
#
...
if [ "${KEYGEN_KAT:-0}" = "1" ]; then
    ...
    if [ "${ek_sz}" -ne 800 ] || [ "${dk_sz}" -ne 768 ]; then
