...
# @production_io 默认 run.sh 生产 I/O：input/ 仅 seed_d.bin + lut_even/odd_stacked.bin；output/ ek_pke.bin (800B) + dk_pke.bin (768B)；中间 GM 不落盘。 / Default production I/O: seed+LUT in; ek_pke+dk_pke out; no intermediate GM dumps. compute 子树可单独跑中间 bin（调试）。 / Compute subtree debug bins optional.
...
"""
"""
gen_data.py — pass-fix-f203-alg13-device-keygen-k2 的 compute golden 生成器。

## 数据契约（与设备 mmad_custom / aiv_func 必须一致）

  src [4,256]   : 行 0..1 为 ŝ，行 2..3 为 ê（polyvec4，禁止 pad 到 6/8）
  mat_c [32,128]: 平面布局，行 = planar_row(slot, limb, half)
  dst [4,256]   : NTT 后 ŝ̂/ê̂，行 0..1 共享给行 18，行 2..3 作 ê̂
...
N = 256
HALF_N = N // 2
K_S = 2          # ŝ 多项式个数（ML-KEM-512 k=2）
K_E = 2          # ê 多项式个数（ML-KEM-512 k=2）
K_E_PER_AIV = 1  # 行 18 / ByteEncode 每个 AIV 负责 1 个 p
K_HAT = 2        # t_hat / ek 行数
K_KK = K_HAT * K_HAT  # a_hat 行数（行 18：2×2）
K_PLANAR_SLOTS = K_S + K_E        # mat_c slot 数 = 4
LIMBS = 4        # hh,lh,hl,ll
MAT_C_PLANAR_ROWS = K_PLANAR_SLOTS * LIMBS * 2  # 32 = 4×4×2(half)
...
def planar_row(slot: int, limb: int, half: int) -> int:
    """与 tiling.h / AivK4PackMatCPlanar 一致：half*16 + slot*4 + limb。"""
    return half * (K_PLANAR_SLOTS * LIMBS) + slot * LIMBS + limb


def encode_polyvec4_s0(src: np.ndarray) -> np.ndarray:
    """
    Stage1 golden：src[4,256] → S0[8,256]。

    上半 4 行写 hi=(v>>6)&63，下半 4 行写 lo=v-hi*64；与设备 `AivK4Split`
    的真实 polyvec4 几何一致，不复制 ŝ bank，也不补零到 6/8 行。
    """
...
def encode_2s1e_s0(s_poly: np.ndarray, e_poly: np.ndarray) -> np.ndarray:
    """兼容旧调用名：把 ŝ‖ê 拼成真实 polyvec4 后走 encode_polyvec4_s0。"""
    return encode_polyvec4_s0(np.vstack([s_poly, e_poly]))
...
def build_src_se() -> np.ndarray:
    ...
    行 0..1：同一 FIXED_POLY ×2（调试路径）；
    行 2..3：基 e 上 (e+0..1) mod Q，对应 ML-KEM-512 两条 ê。
    """
