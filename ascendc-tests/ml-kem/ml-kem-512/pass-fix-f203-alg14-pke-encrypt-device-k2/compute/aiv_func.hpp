 ...
 *
 * 几何：本探针 tiling 中 kInttBatch=4、kPlanarSlots=4；NTT(r) 段仅用前 kK=2 行语义，
...
/** NTT(r) 的真实分片：AIV0 处理 poly 0/1，AIV1 不处理假 poly；分配容量仍按 2 行。 */
...
public:
     ...
     * polyBase_：AIV0→0，AIV1→2；AIV1 仅参与同步，不处理语义 poly。
     */
    ...
    /** 从 GM src 读本核 poly bank → 编码写 S0；k2 AIV1 无语义行，直接返回但保留外层同步。 */
...
public:
    ...
    /** 打包本核 poly bank 的 lo 半平面与 hi 半平面；k2 AIV1 无 NTT 行。 */
    __aicore__ inline void Process()
    {
        if (polyCount_ == 0U) {
            return;
...
public:
    ...
    /** 处理本核 poly bank：先 lo 半系数再 hi 半，写满 [kPolys,N] 后 copyOut；k2 AIV1 无 NTT 行。 */
    __aicore__ inline void Process()
    {
        if (polyCount_ == 0U) {
            return;
