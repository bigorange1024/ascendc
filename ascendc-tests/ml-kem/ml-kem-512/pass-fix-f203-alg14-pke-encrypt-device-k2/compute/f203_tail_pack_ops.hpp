 ...
 * @brief Alg.14 行 22–24 tail pack 可复用设备例程（ML-KEM-512，统一整数 Compress 向量 + ByteEncode 标量 pack）。
 * 供 f203_encrypt_alg14_pack（CPU 独立 launch）与 f203_encrypt_l18_l19（SIM 单 launch 内联 pack）共用。
 * 分片：subBlock0 → c₁[0..1]；subBlock1 → c₂（与 INTT polyvec4 的 [u0,u1] / [v,空] 对齐）。
 * Compress：统一整数 limb 宽乘（d=4/d=10 各 4×int32 scratch，无 float）。
 *
 * 流水线位置：FIPS 203 Alg.14 / ML-KEM-512（k=2）K-PKE.Encrypt。
 ...
 * @param gmU     u GM，行主序 k=2×256
 * @param gmC     密文 GM
 * @param polyIdx poly 下标 0..1
 */
 ...
 * @param uGm       int32[2,256]
 * @param vGm       int32[256]（仅 subBlock1 读）
 * @param cGm       uint8[768]；nullptr 则跳过
