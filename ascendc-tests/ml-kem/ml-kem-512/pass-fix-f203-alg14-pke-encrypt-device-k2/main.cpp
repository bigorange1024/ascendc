/**
 * @file main.cpp
 * @brief pass-fix-f203-alg14-pke-encrypt-device-k2 — 完整 Encrypt host 编排（Alg.14 行 1–22）。
 *
 * 输入 ek_pke + m + coins（锁定 SEED_D=20260619）→ 设备产 c = c₁‖c₂（768B）。
 * 两上游 PASS 探针在**单 device session、GM handoff（零拷贝）**下串联：
 *   prep（f203_encrypt_prep，AIV blockDim=2）：ek+coins → a_hat[4,256] + re[5,256]
 *   compute+tail（f203_encrypt_l18_l19，MIX blockDim=1）：a_hat + re(切片 y/e₁/e₂) + ek + m → u,v,c
 *
 * | 模式 | launch | 说明 |
 * |------|--------|------|
 * | SIM  | **2×** | prep → l18_l19（含 e₂+=μ 与内联 tail pack）；c 全设备 |
 * | CPU  | **5×** | prep + ntt_y/at_jp/intt_e1（MIX 三段产 u）+ pack；v=golden_v |
 *
 * handoff 契约见 f203_encrypt_full_layout.h（编译期 static_assert 绑定 prep↔compute 尺寸）。
 * golden 见 scripts/gen_data.py（本地 k2 derand + golden_c；本地派生 LUT/v）。
 */
...
/** PRF(SHAKE256) tiling：host 仍按 batch8 分配最大 UB；设备逐 nonce 覆盖 batch=1 写 7 行。 */
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    // re 切片：y=r（0）、e₁（2048）；compute 读同一 GM
    ...
    // v 注入：CPU 三 launch 仅产 u，不产 batch4 的 v，读 gen_data 生成的注入 golden（含 μ+e₂）；
