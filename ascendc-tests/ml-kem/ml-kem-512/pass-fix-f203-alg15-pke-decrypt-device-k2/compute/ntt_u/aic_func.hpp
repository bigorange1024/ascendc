 ...
 *
 * 作用：封装官方 LoadDataWithTranspose + Mmad + Fixpipe；本探针逻辑 m=4（紧凑 HI₂+LO₂，硬件按 16 对齐）、
 ...
 *
 * 语义：poly-batch 下 S0 已含全部 2 poly 的 HI/LO；Cube 按 16 行硬件粒度执行，不分 limbsplit。
 */
 ...
 *
     * 形状约定（本探针）：
     *   - A：S0 [m=4, k=256] int8（ND，进 Cube 时 m 向上对齐到 16）
 *   - B：LUT 半块 [k=256, n=128] int8（even/odd × top/bottom 之一）
     *   - C：临时 [m=4, n=128] int32，写到 ws+MAT_C_TMP_*
 */
class AicMmad {
public:
    /**
     * @param m  左矩阵逻辑行数（本探针 4，硬件按 16 对齐）
     * @param k  内积维（256）
