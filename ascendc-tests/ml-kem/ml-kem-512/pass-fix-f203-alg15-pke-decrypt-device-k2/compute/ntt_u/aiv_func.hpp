 ...
 *
 * 几何：本探针 tiling 中 kK=2、kPlanarSlots=2；NTT/INTT 均只处理真实 2 条 poly。
 * poly-batch：AIV0 处理 poly0，AIV1 处理 poly1；每 AIV 握完整 poly 的 hi+lo（非 limbsplit）。
 *
...
/** NTT/INTT 的真实分片：AIV0 处理 poly0，AIV1 处理 poly1，禁止读写假 poly。 */
__aicore__ inline uint32_t ntt_poly_base(int32_t subCoreIdx)
{
    return (subCoreIdx == 0) ? 0U : 1U;
}

/** @return 当前 AIV 在 k=2 阶段的真实 poly 数；每核固定 1 条完整 poly。 */
 ...
 * 前置：subCoreIdx∈{0,1}；本核处理 polyBase 起 1 条真实 poly。
 */
class AivK8Split {
public:
    /**
     * @param subCoreIdx AIV 子核 0/1；@param coeffN 系数个数（通常 256）
     * polyBase_：AIV0→0，AIV1→1；每核仅处理 1 条 poly。
     */
...
private:
     ...
     * lo 行偏移 = kK + polyBase + lp；k2 无 ZERO 填充，也无假 poly 行。
     */
