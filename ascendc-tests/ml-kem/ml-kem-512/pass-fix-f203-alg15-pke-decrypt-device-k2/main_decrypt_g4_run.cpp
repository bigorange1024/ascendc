 ...
 * 对齐 customspec §编排；基线 pass-fix-f203-alg15-pke-decrypt-device-k2。
 *
 * Host 职责（非密码学）：
                       ...
                       const uint8_t *lut_intt_even, const uint8_t *lut_intt_odd, uint8_t *m_out)
{
    ...
    tilingData.kPolys = static_cast<int32_t>(kK);    /* k=2 */
                            ...
                            const uint8_t *lut_intt_odd, uint8_t *m_out)
{
    ...
    std::printf("[main_decrypt] device-k2 **1-kernel** done m.bin=%uB (no mid D2H)\n", F203_MSG_BYTES);
    return 0;
}
