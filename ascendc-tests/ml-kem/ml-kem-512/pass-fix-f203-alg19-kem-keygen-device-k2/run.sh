#!/usr/bin/env bash
# pass-fix-f203-alg19-kem-keygen-device-k2 — Alg.19 ML-KEM.KeyGen（2 launch · D13 k2 PKE + 内嵌 Alg.16 尾）
#
# 生产 I/O：
#   input/  — seed_d.bin + lut_even/odd_stacked.bin
#   output/ — ek_kem.bin (800B) + dk_kem.bin (1632B)
#
# Usage（默认 = 生产全量 + golden 对拍，无需额外 env）：
#   bash run.sh -r cpu -v Ascend910B4
#   bash run.sh -r sim -v Ascend910B4
#
# PKE 源码：本目录复制活跃 D13 k2 PKE KeyGen 源码；scripts/compute 与 thirdparty 软链到 D13。
#
# W3/D19 工程（2026-07-26）：
#   - mmad：本目录 D13 k2 compute 副本开启 F203_KEM_KEYGEN_TAIL，不 fork mmad_custom_kem.cpp
#   - ROM：scripts/prep/gen_alg7_*.py → 本目录 prep/alg7/（PYTHONPATH 引 D13 k2 的 alg7_geom）
#   - SIM tick 以本探针验收登记为准；首版正确性优先，非性能基线
#
...
D13_KEYGEN="${REPO_ROOT}/ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg13-device-keygen-k2"
SCRIPTS_PREP="${CURRENT_DIR}/scripts/prep"
INSTALL_PREFIX=""
mkdir -p "${CURRENT_DIR}/scripts"
ln -sfn "${D13_KEYGEN}/scripts/compute" "${CURRENT_DIR}/scripts/compute"
ln -sfn "${D13_KEYGEN}/thirdparty" "${CURRENT_DIR}/thirdparty"
...
if [ "${KEM_KEYGEN_KAT:-0}" != "1" ]; then
    echo "[kem_keygen device-k2] RUN_MODE=${RUN_MODE} SEED_D=${SEED_D} profile=${BUILD_PROFILE} BUDGET_SEC=${KERNEL_COMPUTE_BUDGET_SEC}"
fi

_keygen_gen_alg7_roms() {
    # ROM 写入本探针 prep/alg7/，避免污染 D13 k2 树（与 D13 k2 脚本逻辑同型，仅 out 路径不同）
    mkdir -p "${CURRENT_DIR}/prep/alg7"
    export PYTHONPATH="${D13_KEYGEN}/scripts/prep:${PYTHONPATH:-}"
...
if [ "${KEM_KEYGEN_KAT:-0}" = "1" ]; then
    ...
    if [ "${ek_sz}" -ne 800 ] || [ "${dk_sz}" -ne 1632 ]; then
    ...
    if [ "${KEM_KEYGEN_KAT:-0}" != "1" ]; then
        echo "[kem_keygen device-k2] output OK ek_kem=${ek_sz}B dk_kem=${dk_sz}B"
    fi
fi

if [ "${KEM_KEYGEN_KAT:-0}" != "1" ]; then
    echo "[SUCCESS] pass-fix-f203-alg19-kem-keygen-device-k2 (${RUN_MODE}) SEED_D=${SEED_D}"
fi
