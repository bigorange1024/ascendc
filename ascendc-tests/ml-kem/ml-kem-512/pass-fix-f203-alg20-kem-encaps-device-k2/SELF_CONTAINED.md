# 自包含与设备全链约束 — pass-fix-f203-alg20-kem-encaps-device-k2

## 密码学契约

- **Alg.17 形态**：`ek_kem(800)` + **`m(32)`（GM 输入）** → device `H`/`G` → D14 k2 Encrypt → `c(768)`/`K(32)`。
...
## 工程（对齐 D14 k2 Encrypt）

| 项 | 约定 |
|----|------|
| Encrypt 源码 | 编译期引用活跃 [`pass-fix-f203-alg14-pke-encrypt-device-k2`](../pass-fix-f203-alg14-pke-encrypt-device-k2/) |
| KEM 增量 | 本目录 `kem/` + `f203_kem_enc_prep_entry.cpp` |
| Launch | SIM **2**（prep_kem → l18_l19）；CPU **5**（prep_kem + D14 分段 compute） |
| 参数 | ML-KEM-512：`k=2`、`du=10`、`dv=4`、D14 INTT batch4 |
