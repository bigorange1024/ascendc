#!/usr/bin/env bash
# pass-fix-f203-alg20-kem-encaps-device-k2 — Alg.20 Encaps（prep 前段 H/G + D14 k2 Encrypt）
#
# 生产 I/O：input/{ek_kem,m,lut_*} → output/{c,K}.bin；coins 设备自产，不读 Host coins 为权威。
# PKE：编译期引用 ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg14-pke-encrypt-device-k2
#
# Usage（默认全量）:
...
# 批测 quiet：log → output/kat_kem_encaps_k2.log
if [ "${KEM_ENCAPS_KAT:-0}" = "1" ]; then
    mkdir -p "${CURRENT_DIR}/output"
    export CI=1
    exec >>"${CURRENT_DIR}/output/kat_kem_encaps_k2.log" 2>&1
fi
...
ENCRYPT_ROOT="${REPO_ROOT}/ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg14-pke-encrypt-device-k2"
...
echo "[kem_encaps device-k2] RUN_MODE=${RUN_MODE} profile=${BUILD_PROFILE} BUDGET_SEC=${KERNEL_COMPUTE_BUDGET_SEC}"
...
if [ "${KEM_ENCAPS_KAT:-0}" = "1" ]; then
    ...
    if [ "${c_sz}" -ne 768 ] || [ "${k_sz}" -ne 32 ]; then
...
if [ "${KEM_ENCAPS_KAT:-0}" != "1" ]; then
    echo "[SUCCESS] pass-fix-f203-alg20-kem-encaps-device-k2 (${RUN_MODE})"
fi
