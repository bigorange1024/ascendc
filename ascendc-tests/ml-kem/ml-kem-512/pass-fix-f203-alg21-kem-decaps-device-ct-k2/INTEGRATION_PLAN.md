# INTEGRATION_PLAN — pass-fix-f203-alg21-kem-decaps-device-ct-k2

**定位**：`ascendc-tests/` **Alg.21 `ML-KEM.Decaps` CT 设备探针**（**ml_kem_512 / k=2**）。内部 **Alg.18 `Decaps_internal`**；PKE 段 **编译期引用** D15 k2 Decrypt / D14 k2 Encrypt，**禁止** frozen G4/G5 `vendor_sync`。

**基线对照**（只读判决 / STATUS；**禁止**抄 frozen 源码）：

| 路径 | 角色 |
|------|------|
| [`pass-fix-f203-alg15-pke-decrypt-device-k2`](../pass-fix-f203-alg15-pke-decrypt-device-k2/) | Phase-D Decrypt 权威 |
| [`pass-fix-f203-alg14-pke-encrypt-device-k2`](../pass-fix-f203-alg14-pke-encrypt-device-k2/) | Phase-E Encrypt 权威 |
...
|----|------|
| **参数集** | ml_kem_512（**k=2**） |
| **I/O** | 与 D21 delivery sibling 相同：`dk_kem=1632B` + `c=768B` → `K=32B`；禁止 pad-to-4/8，禁止 k4 字节长度 |
...
| **F1** | D→E 串联；SIM 默认 2-session（CT；单库） | `K` max=0 | **PASS**（accept D**163069**+E**407638**） |
| **F2** | 拒绝路径 | `KEM_DECAPS_REJECT=1` CPU+SIM；`K=J(z‖c)` 且 `reject≠accept` | **PASS**（reject SIM D**163025**+E**408344**） |
...
```bash
cd ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg21-kem-decaps-device-ct-k2
...
| **T19i：SIM `fo_only` → `l18_l19` 尾（4→3）** | **PASS**：继承 D21 k2 形态，本 CT 探针复验 accept/reject |
| D+E 单 launch 融合 | 更后 |
...
本探针 CMake 现将 `COMPUTE_DIR` 指到 [`pass-fix-f203-alg14-pke-encrypt-device-k2/compute`](../pass-fix-f203-alg14-pke-encrypt-device-k2/compute/)。**禁止**在该共享树上改 `f203_encrypt_l18_l19_kernel.cpp` 签名或行为（Encaps / PKE Encrypt 同编此文件）。
...
| SIM 全链 | **PASS**；默认 `decaps_2session`；tick D**163069**+E**407638**；3 launch |
| E3 拒绝 | CPU+SIM **PASS**；SIM tick D**163025**+E**408344**；`K=J(z‖c)` 且 `reject≠accept` |

### T2 / CT 落地要点

1. **问题**：D15 k2 Decrypt/Encrypt 同名头 → ascendc precompile 无法 per-TU 隔离；CT 探针还要求 D/E fresh ACL session。
