#!/usr/bin/env bash
# pass-fix-f203-alg21-kem-decaps-device-ct-k2 — Alg.21 Decaps CT 全链（D+E）；可选 Phase-E-only
#
# 生产 I/O：input/{dk_kem,c,lut_*} → output/K.bin
# PKE：编译期引用 D15 k2 Decrypt + D14 k2 Encrypt；见 INTEGRATION_PLAN.md
#
# Usage（默认全链）:
...
STABLE_ENCRYPT="${REPO_ROOT}/ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg14-pke-encrypt-device-k2"
...
echo "[SUCCESS] pass-fix-f203-alg21-kem-decaps-device-ct-k2 ${LABEL} (${RUN_MODE})"
