# INTEGRATION_PLAN — ML-KEM-512 Alg.7 SampleNTT k2（Min+mod / 交错 / compact）

**探针**：`pass-fix-f203-alg7-sample-ntt-k2` — **单 poly** `(j,i)` 一次 Alg.7 全链 → `â[256]`；矩阵域锁定 `{0,1}²`。
**来源**：仅从活跃 `ml-kem-768/pass-fix-f203-alg7-sample-ntt-k3/` 参数化到 k=2；不引用已关闭目录实现。
**讨论**：[`qa/2026-07/2026-07-27-768收尾复盘与文档刷新.md`](../../../../qa/2026-07/2026-07-27-768收尾复盘与文档刷新.md)（512 W1 下一刀）。
...
| **建议** | **值得做 POC** | 分阶段门禁：先 golden 对拍，再谈 tick；单 poly 成功后服务 512 D13 prep 的 2×2 `Â` batch |
...
- 2×2 全矩阵 `Â` batch 集成（后继 D13 prep 探针负责）
...
| interleave 范本 | 本目录 `scripts/gen_alg7_interleave_rom.py` + `f203_alg7_interleave_rom.h`（由活跃 k3 模板参数化） |
