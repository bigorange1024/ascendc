#!/usr/bin/env python3
# coding=utf-8
"""Golden 生成器：ML-KEM-512 Alg.7 单 poly SampleNTT — xof[672]、d1/d2[224]、â[256]。

本脚本是探针 pass-fix-f203-alg7-sample-ntt-k2 的**黑盒 oracle**：
...
流水线（与 main.cpp / f203_alg7_sample_ntt_entry 一致）：
  ...
  2. ρ = G(d) = SHA3-512(d || byte(k)) 前 32B，k=2（ML-KEM-512）
...
def hash_g_rho(d: bytes, k: int = KYBER_K) -> bytes:
  ...
  对应 FIPS 203 / Kyber 的矩阵种子扩展；k 为安全参数（ML-KEM-512/768/1024 等用 k=2/3/4）。
  """
