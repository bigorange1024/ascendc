 ...
 * @brief Host 侧布局常量（ML-KEM-512 Alg.8 CBD η=2，k=2）。
 *
 * 本文件只给 `main.cpp` 使用：计算 input/output 文件大小、打印探针形状与确定
 * Host 下发 blockDim。设备侧同名语义常量在 `f203_cbd_eta2.hpp` 内独立定义，
 * 因为设备头依赖 `kernel_operator.h`，Host 侧不能直接 include。两边常量必须
 * 同步：k=2，N=256，q=3329，ROWS=4（s0,s1,e0,e1），单 poly PRF=128B。
 */
#pragma once
...
constexpr uint32_t PRF_BYTES = (ETA * N) / 4U;          // 单行 PRF 输出字节数：2*256/4=128
constexpr uint32_t ROWS = 2U * K;                       // 4 行：s0,s1,e0,e1（T-B2 polyvec4）
constexpr uint32_t PRF_TOTAL_BYTES = ROWS * PRF_BYTES;  // 全部 4 行 PRF 输出总字节数：4*128=512
