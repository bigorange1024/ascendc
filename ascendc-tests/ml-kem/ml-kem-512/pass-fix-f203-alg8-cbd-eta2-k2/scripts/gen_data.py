#!/usr/bin/env python3
# coding=utf-8
"""Alg.8 CBD η=2 探针 golden：prf_out[4,128] → src[4,256] int32（ML-KEM-512，k=2）。

本文件在流水线中的位置：`run.sh` 在编译完成、kernel 启动前调用它生成输入与
期望输出。输入是 4 行 × 128B 的伪随机 PRF 字节流（Alg.8 的输入契约就是 PRF
输出字节，不在本探针内复现 PRF 派生）；golden 调用共享 Python oracle
`golden_se_sampling.sample_poly_cbd2`。

与 AscendC 的关系：golden 仅提供合法输入与期望输出；设备实现可使用 load32、
...
PRF_BYTES = 128   # η=2：2*256/4
N = 256           # 每个多项式系数个数
SEED = int(os.environ.get("SEED_D", "20260619"))


def main() -> None:
    """生成 PRF 输入并逐行调用 CBD_2 golden，落盘 input/prf_out.bin 与 output/golden_src.bin。"""
    rng = np.random.default_rng(SEED)
    # 直接生成 Alg.8 输入域的 PRF 字节；行与设备/Host 契约保持 [4,128] 行优先。
    prf = rng.integers(0, 256, size=ROWS * PRF_BYTES, dtype=np.uint8)
    src = np.zeros((ROWS, N), dtype=np.int32)
    for row in range(ROWS):
        # sample_poly_cbd2 内部执行 load32 + 0x55555555 抽取，并把负差映射到 [0,q)。
