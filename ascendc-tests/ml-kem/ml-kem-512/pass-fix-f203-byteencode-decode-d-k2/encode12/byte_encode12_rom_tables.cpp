 ...
 * @brief ByteEncode₁₂ 固定 n=256 Gather 字节索引（与 Alg.11 even/odd 公式一致：8i / 8i+4）。
 *
 * 流水线位置：设备 GM ROM 表；custom.cpp 在 prefetch 路径下 #include 本文件以链接进 kernel。
 * 与 golden 关系：仅提供解交错索引，编码结果仍须与 golden_ek/sk 字节一致。
 * 作用：定义 even/odd 各 128 项 int32 字节偏移常量数组。
 */
#include "byte_encode12_rom_tables.h"
#include "kernel_operator.h"

/** even：第 i 对偶系数在 poly 行内的字节偏移 = 8*i（每 int32 占 4B，偶下标 2i → 字节 8i） */
...
/** odd：第 i 对奇系数字节偏移 = 8*i+4（奇下标 2i+1 → 字节 8i+4） */
