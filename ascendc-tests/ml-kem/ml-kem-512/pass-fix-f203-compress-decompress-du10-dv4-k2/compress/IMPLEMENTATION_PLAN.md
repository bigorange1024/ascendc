# IMPLEMENTATION_PLAN — pass-fix-f203-compress-decompress-du10-dv4-k2/compress
...
**Encrypt 用法**（Alg.14，ML-KEM-512 `k=2` 默认参数）：
...
```text
pass-fix-f203-compress-decompress-du10-dv4-k2/compress/
...
**与 ByteEncode 关系**：本探针输出 **256×int32 压缩域**（值 < 2^d）；后续 W0/B2 `pass-fix-f203-byteencode-decode-d-k2/` 负责 `ByteEncode_d`。本 B1 不做 bit-pack，只保证 Compress_d 的 per-lane I/O 与 golden 一致。
...
| A1 | 向量 d=4 + d=5 |
| A2 | 向量 d=10（u64 方案定稿） |
| A3 | k=2 密文域 batch（u 为 2×poly，v 为 1×poly；本 B1 仍按单 poly 子探针验收） |
| B | fused `ByteEncode_d(Compress_d(·))` with 后续 B2 探针 |
