# IMPLEMENTATION_PLAN — pass-fix-f203-compress-decompress-du10-dv4-k2/decompress

**状态**：ML-KEM-512 W0/B1 子探针；顶层验收 d=**4/10**（`d_v=4,d_u=10`）  
**可编译 d**：**`{4, 5, 10, 11}`**；本轮默认矩阵只跑 `{4,10}`，d=5/11 保留作 1024 对照  
**FIPS 203**：§4.2.1 `Decompress_d`（Eq 4.8）；Alg.15 行 4  
**配对**：同级 `compress/` 子探针（近似逆，非精确）
...
```text
pass-fix-f203-compress-decompress-du10-dv4-k2/decompress/
...
ML-KEM-512 的 c1 对应 2 个 poly（2×256 lanes），c2 对应 1 个 poly；本 B1 子探针按单 poly 验收，后续 PKE 链接时再决定一次 launch 内是否合并多 poly。

### 3.4 Fused：`Decompress_d(ByteDecode_d(B))`

Phase B 与后续 W0/B2 `pass-fix-f203-byteencode-decode-d-k2/` 合并：
...
## 5. Encrypt / Decrypt 参数（k=2）

| 参数 | d_u | d_v | c₁ 字节 | c₂ 字节 |
|------|-----|-----|---------|---------|
| ML-KEM-512 主路径 | 10 | 4 | 640 | 128 |
| ML-KEM-1024 备选 | 11 | 5 | 1408 | 160 |

CMake：`F203_DECOMPRESS_D` 与 compress、后续 B2 ByteDecode/ByteEncode 探针同名参数同步。
