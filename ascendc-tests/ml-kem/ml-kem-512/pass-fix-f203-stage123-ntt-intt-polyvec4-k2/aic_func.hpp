 ...
 *
 * 语义：poly-batch 下 S0 已含全部 4 poly 的 HI/LO；Cube 一次吃满逻辑 8 行，
 * Mmad 参数按硬件粒度 pad 到 16 行（不是假 poly / ZERO poly），不分 limbsplit。
 */
 ...
 *
 * 形状约定（本探针）：
 *   - A：S0 [m=8, k=256] int8（ND；AIC 内部硬件 pad 到 16）
 *   - B：LUT 半块 [k=256, n=128] int8（even/odd × top/bottom 之一）
 *   - C：临时 [m=8, n=128] int32，写到 ws+MAT_C_TMP_*；本地 CO1 缓冲按 16 行分配
 */
class AicMmad {
public:
    /**
     * @param m  左矩阵逻辑行数（本探针 8；硬件执行时 pad 到 16）
     * @param k  内积维（256）
