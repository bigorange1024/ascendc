 ...
 * @brief 4-poly 紧凑三段式 NTT/INTT 设备入口（MIX 1×AIC + 2×AIV）；LUT 由 host 选择 NTT/INTT。
 *
 ...
 *
 * 与 golden 关系：dst [4,256] int32 对拍 golden_dst；NTT/INTT 仅差 workspace 内 LUT bin，本 FSM 不变。
 *
...
volatile int g_stage123_k4_mix_pass = 3;
#endif
 ...
 * MIX kernel 入口。
 *
 * @param dst     输出 GM [4,256] int32
 * @param src     输入 GM [4,256] int32
...
extern "C" __global__ __aicore__ void mmad_custom(GM_ADDR dst, GM_ADDR src, GM_ADDR ws, TilingData tiling)
{
...
#ifdef ASCENDC_CPU_DEBUG
    const int32_t mixPass = g_stage123_k4_mix_pass;
#else
    ...
    if (AIC) {
        ...
        STATE = AIC_MMAD;
        // m=8 行逻辑 S0 × k=256 × n=128 半列；Cube 内部硬件 pad 到 16 行，不引入假 poly
        ...
        if (runS3) {
            AivK4RouteAMod merge(subBlockID, coeffN);
