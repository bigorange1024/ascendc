# BUILD_OPTIONS — pass-fix-f203-alg13-device-keygen-k3

**原则**：`bash run.sh` **不加 export** 即走生产路径（`ascendc_keygen_bbit` + 生产 I/O）。

**实现方案**：[`pass-fix-f203-alg13-device-keygen-k3-实现方案-customspec.tex`](pass-fix-f203-alg13-device-keygen-k3-实现方案-customspec.tex)
...
全目录源文件含 `@probe pass-fix-f203-alg13-device-keygen-k3` 文件头。重新注入：
