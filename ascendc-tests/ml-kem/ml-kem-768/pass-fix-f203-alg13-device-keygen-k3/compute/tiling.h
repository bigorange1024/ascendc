/**
 * @file tiling.h
 * @brief 6-poly 紧凑 Tag5T Stage1–3 的 host/device 共享布局与 workspace 偏移表。
 *
 * 流水线位置：main.cpp（读 tiling.bin / 算 GM 偏移）、mmad_custom.cpp（S0/LUT/mat_c 指针）、
 * aiv_func.hpp（poly-batch 切分与平面行号）、gen_data.py（文件字节与形状约定）。
 *
 * 作用：
 *   - 输入 polyvec `src` [6,256] int32；输出 NTT/INTT `dst` [6,256] int32；
 *   - Stage1 紧凑 S0：[HI₆, LO₆] 共 12 行 × 256 列 int8（无插零，真 polyvec6，禁 pad 到 8）；
 *   - Stage2 四路 Cube 临时 mat_c_tmp → 平面 mat_c [48,128] int32（每 poly 握有完整 hi+lo limbs）；
 ...
 *
 * 不变量（poly-batch）：`kPolysPerAiv=3`，AIV0 处理 poly 0..2，AIV1 处理 3..5；单 poly 的 HI/LO 同属一核。
 */
#ifndef STAGE123_POLYVEC6_TILING_H
#define STAGE123_POLYVEC6_TILING_H
...
struct TilingData {
    ...
    int32_t kPolys;     /**< 6，本探针固定 polyvec6 长度 */
    /** 0=生产全链；非 0 仅调试 */
    int32_t mixPass;
};
...
constexpr size_t kK = 6;                  /**< polyvec 条数 */
constexpr size_t kS = 3;                  /**< ŝ poly 数：ML-KEM-768 k=3 */
constexpr size_t kE = 3;                  /**< ê poly 数：ML-KEM-768 k=3 */
constexpr size_t kPolysPerAiv = kK / 2;   /**< 每 AIV 3 条 poly（poly-batch） */
constexpr size_t kEPerAiv = 2;            /**< 行18/ByteEncode 每 AIV 最大输出行数：AIV0=2，AIV1=1 */
...
/** 紧凑 S0：行 0..5 = HI，行 6..11 = LO（无假 poly / ZERO 填充） */
...
/** 平面 mat_c：每 slot 4 limb × 2 half（lo/hi 半）→ 6×4×2 = 48 行 */
...
constexpr size_t dstSOffAiv1 = kS;       /**< 仅保留调试兼容；生产共享 ŝ 从 dst[0..2] 读 */
constexpr size_t dstEOff = kS;
constexpr size_t dstEOffAiv0 = dstEOff;
constexpr size_t dstEOffAiv1 = dstEOff + 2;
...
/** mixPass=6 调试兼容：Cube sanity 仍按硬件 16 行对齐，不代表 polyvec6 语义补行。 */
