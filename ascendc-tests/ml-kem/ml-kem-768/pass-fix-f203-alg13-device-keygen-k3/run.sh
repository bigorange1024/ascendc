...
# pass-fix-f203-alg13-device-keygen-k3 — Alg.13 全链 KeyGen（ML-KEM-768，prep 双 AIV 5+4 生成 Â）
#
# 生产 I/O：
#   input/  — seed_d.bin + lut_even/odd_stacked.bin
#   output/ — ek_pke.bin (1184B) + dk_pke.bin (1152B)
#
...
if [ "${KEYGEN_KAT:-0}" = "1" ]; then
    ...
    if [ "${ek_sz}" -ne 1184 ] || [ "${dk_sz}" -ne 1152 ]; then
