 ...
 * @brief Alg.14 PRF：coins → 7× SHAKE256；逐 nonce 单行写出（ML-KEM-768）。
 *
 * 流水线位置（Encrypt prep 行 8–15 前半）：
 *   coins[32] → PRF(coins, nonce)=SHAKE256(coins‖byte(nonce)).squeeze(128)
 *   nonce 0..2 → r；3..5 → e₁；6 → e₂；写出 prf_out_gm[7,128]
 *
 * 编排原因：k3 仅 7 行，不能用 batch8 写越界；复用同一 SHAKE UB，但每次将 tiling.batch 改为 1。
 *
...
/** Encrypt k3 需要 7 行 PRF（r3 + e₁3 + e₂1）。 */
constexpr uint32_t PRF_TOTAL_BATCH = 7U;
...
/**
 * coins → prf_out[7,128]：逐 nonce 单行 SHAKE，避免 batch8 越界。
 *
 * @param coins       32B 随机性
 * @param prf_out_gm  GM 输出 [7,128] uint8，行主序
 ...
 */
__aicore__ inline void RunShakePrfEncrypt7UbWithUb(const uint8_t coins[32], __gm__ uint8_t *prf_out_gm,
                                                   GM_ADDR tiling_gm,
                                                   AscendC::TBuf<AscendC::TPosition::VECCALC> &xBuf,
                                                   AscendC::TBuf<AscendC::TPosition::VECCALC> &lenBuf,
                                                   AscendC::TBuf<AscendC::TPosition::VECCALC> &stagingBuf,
