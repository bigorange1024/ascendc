/**
 * @file main.cpp
 * @brief pass-fix-f203-alg14-pke-encrypt-device-k3 — 完整 Encrypt host 编排（Alg.14 行 1–22）。
 *
 * 输入 ek_pke + m + coins（锁定 SEED_D=20260619）→ 设备产 c = c₁‖c₂（1088B）。
 * 两上游 PASS 探针在**单 device session、GM handoff（零拷贝）**下串联：
 *   prep（f203_encrypt_prep，AIV blockDim=2）：ek+coins → a_hat[9,256] + re[7,256]
 ...
 * golden 见 scripts/gen_data.py（复用 correctness 输入 + golden_c；本地派生 LUT/v）。
 */
