...
"""
@file gen_data.py
@brief pass-fix-f203-alg14-pke-encrypt-device-k3 — 全链 Encrypt golden 生成（自包含）。

流水线（INTEGRATION_PLAN §4.1、§8）：
  * 锁死 SEED_D=20260619；本地生成 ML-KEM-768 ek/m/coins/golden_c
...
def _gen_locked_inputs_local(inp: str) -> None:
    """本目录自生成 k3 ek/m/coins（与参数卡 derand 前缀一致）。"""
...
def _write_golden_c(gold: str, ek: bytes, m: bytes, coins: bytes, prefer_corr: bool) -> tuple[bytes, str]:
    """写 golden/c.bin；prefer_corr 保留签名兼容，k3 始终本地重算。"""
...
def _compute_golden_v(ek: bytes, m: bytes, coins: bytes) -> np.ndarray:
    """v = INTT(Σ_j t̂[j]∘r̂[j]) + e₂ + μ(m)（与设备 e₂+=μ 折叠、k3 batch4 INTT 结果等价）。"""
