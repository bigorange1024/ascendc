...
def stage123_transform(polys: np.ndarray, mode: str) -> np.ndarray:
    """k=3 polyvec Stage123 NTT/INTT；D14 对 INTT 另用 batch=4 包住 u0/u1/u2/v。"""
    if polys.shape != (K, N):
...
def compress_d_scalar(u: int, d: int) -> int:
    """Compress_d 标量（ML-KEM-768 密文域默认 d=10/4；保留通用整数公式）。"""
    u = int(u) % Q
    if d in (4, 10):
...
def pack_ciphertext(u: np.ndarray, v: np.ndarray) -> bytes:
    """ML-KEM-768：c₁=3×ByteEncode₁₀(Compress₁₀(u))，c₂=ByteEncode₄(Compress₄(v))。"""
    c1 = bytearray(960)
    for p in range(K):
