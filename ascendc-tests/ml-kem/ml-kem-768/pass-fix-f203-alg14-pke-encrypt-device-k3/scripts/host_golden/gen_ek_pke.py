...
"""
@file gen_ek_pke.py
@brief host_golden：KeyGen 语义生成 ek_pke（1184B，自包含，禁止 liboqs）。

流水线：Alg.14 Encrypt 输入公钥；逻辑对齐 pass-fix-f203-alg13-device-keygen-k3
keygen_golden（ρ→Â、σ→s/e→NTT→t̂→ByteEncode₁₂‖ρ）。仅 golden，非设备实现。
golden I/O：SEED_D → ek_pke.bin（1152B t̂ ‖ 32B ρ）。
"""
...
def derand_bytes_from_seed(seed_d: int) -> bytes:
    ...
    msg = f"exp-mlkem-f203-2s1e-k3:SEED_D={seed_d}".encode()
