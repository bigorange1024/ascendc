 ...
 * @brief k3 行 18 布局：每 AIV 读取完整 ŝ[0..2]、ê 局部行号、p 区间与 Â 偏移辅助。
 *
 ...
 *
 * 不变量：
 *   - AIV0 负责 p={0,1}，AIV1 负责 p={2}（参数卡 §3.2 / B6），禁止 P/2 假设；
 *   - ŝ 行号即 j∈[0,2]，由 compute launch 内 GM 暂存 + SyncAll 后共享读取。
 *
 * Golden：src.bin 行 0..2 为 ŝ、3..5 为 ê；t_hat [3,256]。
 *
