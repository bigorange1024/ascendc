 ...
 * @brief FIPS 203 Alg.19（ML-KEM.KeyGen）I/O 尺寸与 dk_kem 内存布局常量（ml_kem_768 / k=3）。
 *
 * 本探针流水线（device-k3，2 launch）：
 *   Host 仅喂 seed_d + LUT → D13 k3 PKE prep + mmad（F203_KEM_KEYGEN_TAIL=1 内嵌 Alg.16 尾）
 *   → output/ek_kem.bin（=ek_pke 别名）、dk_kem.bin。
 *
 * dk_kem 采用 **liboqs 展开布局**（非 FIPS 紧凑 ByteEncode 串联）：
 *
 *   偏移 0      : dk_pke[1152]   — ByteEncode₁₂(ŝ)，来自 mmad sk_out
 *   偏移 1152   : ek[1184]       — ek_PKE 别名（与 ek_kem.bin 同内容）
 *   偏移 2336   : H(ek)[32]      — SHA3-256(ek)，设备尾段计算
 *   偏移 2368   : z[32]          — Alg.19 隐式拒绝秘密，设备 UB 派生
 *   总长 2400B
 *
...
/** Alg.13 PKE 私钥字节数（ByteEncode₁₂(ŝ)×k=3）。 */
constexpr uint32_t kDkPkeBytes = 1152U;
/** KEM/PKE 公钥字节数（ek_PKE = ByteEncode₁₂(t̂)‖ρ）。 */
constexpr uint32_t kEkKemBytes = 1184U;
...
constexpr uint32_t kDkKemBytes = kDkPkeBytes + kEkKemBytes + kHashEkBytes + kZBytes;  // 2400
