 ...
 *
 * ## 对齐与 golden
 * FIPS 203 Alg.13，ML-KEM-768（k=3）；输出 a_hat/src/ρ 与 Host golden 中间态 I/O 等价。
 *
...
/** aHatQue 先承载单 poly â[256] int32，后复用为 PRF Y[6,128] uint8；按两者最大值分配。 */
 ...
 * @param blockIdx    逻辑分片：0→Â poly 0–4，1→5–8；PRF/CBD 仅 block0 执行
 * @param a_hat_gm    输出 Â[9,256] int32，行主序
 * @param prf_out_gm  PRF 中间态 [6,128] uint8（block0 写入，供 CBD 读）
 * @param src_gm      输出 ŝ‖ê [6,256] int32
 * @param rho_gm      输出 ρ[32]（block0 写入）
 * @param tiling_gm   presample SHAKE batch tiling（host 填充）
 *
 * 双 AIV（F203_AHAT16_BLOCK_DIM=2）：
 * - block0/1 并行跑 BuildAHat16ShardWithUb，按 5+4 poly 写 Â[9,256]
                                                 ...
                                                 GM_ADDR tiling_gm)
{
    ...
    // 双 AIV：blockIdx 0→poly 0–4、1→5–8 并行写 GM；GM 为真实 Â[9,256]，禁止补行到 16。
    // 背景：D13 锁定 5+4 分片；队列按单 poly 1024B 分配，随后复用给 PRF Y[6,128]。
    ...
    if (AscendC::GetBlockIdx() == 0U) {
        ...
        // 行 8–11：σ → PRF → prf_out_gm[6,128]
        F203SeVector::RunShakePrfBatchUbWithUb(sigma, prf_out_gm, tilingLocal, shakeXBuf, shakeLenBuf,
                                               shakeStagingBuf, aHatQue);
        F203_PREP_PIPE_ALL();

        // 行 12–15：CBD_η=2 → src_gm[6,256]；前 3 行 ŝ，后 3 行 ê（polyvec6，禁 pad）。
        F203CbdEta2::SamplePolyCbd2Batch6WithUb(0U, prf_out_gm, src_gm, scratchBuf, aHatQue);
    }
