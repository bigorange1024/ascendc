 ...
 * @brief FIPS 203 Alg.8 SamplePolyCBD_η=2 — batch 6 poly 入口与编译期契约（ML-KEM-768，k=3）。
 *
 * 数据流（P1b/P2 默认路径）：
 ...
 *
 * 布局：polyvec6（s‖e，前 K=3 行 ŝ、后 K=3 行 ê）；双 AIV 分片见 RowForBlock。
 */
#pragma once
...
constexpr uint32_t K = 3U;         // ML-KEM-768 模块秩 k
...
/** 6 行（polyvec6）：前 K 行 s，后 K 行 e（语义标注；CBD 各行独立）。 */
constexpr uint32_t ROWS = 2U * K;
constexpr uint32_t PRF_TOTAL_BYTES = ROWS * PRF_BYTES;  // 6 行 PRF 总字节数：6*128=768
constexpr uint32_t SRC_COEFFS = ROWS * N;               // 输出 src 系数总数：6*256=1536

/** P2：每 AIV 处理行数（6 行 / 2 个 AIV = 3 行）。 */
constexpr uint32_t ROWS_PER_AIV = ROWS / 2U;
constexpr uint32_t CBD_BLOCK_DIM = 2U;

/**
 * P2 行表 — polyvec6（s‖e）交叉分片（T-B）：
 *   AIV0 → {0,1,3}  // s0,s1,e0
 *   AIV1 → {2,4,5}  // s2,e1,e2
 * 行号 0–2 对应 ŝ 的 3 个分量，3–5 对应 ê 的 3 个分量；交叉分配使每个 AIV
 * 同时持有 ŝ 与 ê 的部分行，与 ML-KEM-768 下游消费方对分片方式的假设一致。
 */
constexpr uint32_t kAiv0Rows[ROWS_PER_AIV] = {0U, 1U, 3U};
constexpr uint32_t kAiv1Rows[ROWS_PER_AIV] = {2U, 4U, 5U};

/**
 * 由 blockIdx（0 或 1）与 AIV 内局部行号，查表得到全局 src/prf 行号（0..5）。
 ...
 * @param prf_gm [in]  GM 指针，PRF 输出，形状 [ROWS=6, PRF_BYTES=128] uint8，行优先
 * @param src_gm [out] GM 指针，CBD 采样结果，形状 [ROWS=6, N=256] int32，行优先
 *               （前 3 行 = ŝ 各分量，后 3 行 = ê 各分量）
 ...
 */
__aicore__ inline void SamplePolyCbd2Batch6(__gm__ const uint8_t *prf_gm, __gm__ int32_t *src_gm)
{
 ...
 * @param prf_gm [in]  GM 指针，PRF 输出，形状 [ROWS=6, PRF_BYTES=128] uint8
 * @param src_gm [out] GM 指针，CBD 采样结果，形状 [ROWS=6, N=256] int32
 * 实际分片与流水线细节见 `f203_cbd_eta2_ub_io.hpp::SamplePolyCbd2Batch6DataCopy`
 * （按当前 blockIdx 决定单核串行 6 行还是双 AIV 各 3 行）。
 */
__aicore__ inline void SamplePolyCbd2Batch6(__gm__ const uint8_t *prf_gm, __gm__ int32_t *src_gm)
{
    const uint32_t blockIdx = AscendC::GetBlockIdx();
    SamplePolyCbd2Batch6DataCopy(blockIdx, prf_gm, src_gm);
}
