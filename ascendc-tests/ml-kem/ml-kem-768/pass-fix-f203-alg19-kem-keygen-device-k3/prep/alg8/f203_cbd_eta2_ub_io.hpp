 ...
 * @brief P1b/P2：GM↔UB MTE DataCopy + PipeBarrier（ML-KEM-768，polyvec6 / ROWS=6）。
 *
 ...
 *   内核见 `GetBlockNum()==1` 时 block0 串行 6 行，语义与 P2 对拍一致。
 *
                                              ...
                                              AscendC::TQue<AscendC::TPosition::VECOUT, 1> &rowQue)
{
    ...
    F203_CBD_SYNC_AFTER_COPYIN();  // 等待 MTE2 搬运完成，避免计算读到未写完的 UB 数据
    ...
    F203_CBD_SYNC_BEFORE_COPYOUT();  // 等待向量计算完成，避免 CopyOut 搬出未写完的结果
...
/**
 * P1b/P2 批处理主体：按编译期 F203_CBD_BLOCK_DIM 与运行时 blockDim 决定单核串行 6 行
 * 还是双 AIV 各 3 行，逐行调用 SamplePolyCbd2OneRowUb。
 ...
 * 前置条件：调用侧（Batch6DataCopy）已完成 pipe.InitBuffer 对 scratchBuf/rowQue 的初始化。
 */
__aicore__ inline void SamplePolyCbd2Batch6WithUb(uint32_t blockIdx, __gm__ const uint8_t *prf_gm,
                                                   ...
                                                   AscendC::TQue<AscendC::TPosition::VECOUT, 1> &rowQue)
{
...
#if F203_CBD_BLOCK_DIM == 1
    /* P1b-single：本核（block0）串行处理全部 6 行 */
    ...
    if (AscendC::GetBlockNum() == 1U) {
         ...
         * 退化为单核串行 6 行，语义与 P2 双核结果一致，避免 tikicpu 按 launch
        ...
        /* P2 双 AIV：按 RowForBlock 查表得到本 AIV 负责的 3 个全局行号，逐行处理 */
...
/**
 * P1b/P2 入口：初始化本核所需 UB 资源（TPipe/TQue/TBuf）后调用 SamplePolyCbd2Batch6WithUb。
 * @param blockIdx [in]  当前核的 block 编号
 * @param prf_gm   [in]  GM 指针，PRF 输出，形状 [ROWS, PRF_BYTES] uint8
 * @param src_gm   [out] GM 指针，CBD 结果，形状 [ROWS, N] int32
 * 被 f203_cbd_eta2.hpp 中的默认 SamplePolyCbd2Batch6（P1b/P2 分支）调用。
 */
__aicore__ inline void SamplePolyCbd2Batch6DataCopy(uint32_t blockIdx, __gm__ const uint8_t *prf_gm,
                                                    __gm__ int32_t *src_gm)
{
#if F203_CBD_BLOCK_DIM == 1
    if (blockIdx != 0U) {
    ...
    SamplePolyCbd2Batch6WithUb(blockIdx, prf_gm, src_gm, scratchBuf, rowQue);
}
