#!/usr/bin/env bash
# pass-fix-f203-alg19-kem-keygen-device-k3 — Alg.19 ML-KEM.KeyGen（2 launch · D13 k3 PKE + 内嵌 Alg.16 尾）
#
# 生产 I/O：
...
#
# PKE 源码：本目录复制活跃 D13 k3 PKE KeyGen 源码；scripts/compute 与 thirdparty 软链到 D13。
#
# W3/D19 工程（2026-07-26）：
#   - mmad：本目录 D13 k3 compute 副本开启 F203_KEM_KEYGEN_TAIL，不 fork mmad_custom_kem.cpp
#   - ROM：scripts/prep/gen_alg7_*.py → 本目录 prep/alg7/（PYTHONPATH 引 D13 k3 的 alg7_geom）
...
D13_KEYGEN="${REPO_ROOT}/ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg13-device-keygen-k3"
...
_keygen_gen_alg7_roms() {
    # ROM 写入本探针 prep/alg7/，避免污染 D13 k3 树（与 D13 k3 脚本逻辑同型，仅 out 路径不同）
...
if [ "${KEM_KEYGEN_KAT:-0}" != "1" ]; then
    echo "[SUCCESS] pass-fix-f203-alg19-kem-keygen-device-k3 (${RUN_MODE}) SEED_D=${SEED_D}"
fi
