#!/usr/bin/env python3
"""
gen_data.py — Alg.19 KEM KeyGen 的 ML-KEM-768 host golden 生成器。

用途（仅 KEM_KEYGEN_VERIFY=1 / 对拍路径）：
  复用活跃 D13 k3 PKE KeyGen oracle 生成 ek_pke/dk_pke，再按 FIPS 203
...
D13_ROOT = REPO / "ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg13-device-keygen-k3"
_KEYGEN_GOLDEN = D13_ROOT / "scripts/keygen_golden.py"

_spec = importlib.util.spec_from_file_location("mlkem768_d13_keygen_golden", _KEYGEN_GOLDEN)
...
def derand_z_from_seed(seed_d: int) -> bytes:
    """与设备 DerandZFromSeedD 同式：SHA3-256("exp-mlkem-f203-kem-k3:SEED_Z="‖十进制)。"""
    msg = f"exp-mlkem-f203-kem-k3:SEED_Z={seed_d}".encode()
