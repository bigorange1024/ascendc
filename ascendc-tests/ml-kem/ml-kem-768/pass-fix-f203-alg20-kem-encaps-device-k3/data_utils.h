/**
 * @file data_utils.h
 * @brief Host 侧 bin 读写、ACL 错误检查宏与调试打印；D20 Encaps host 使用。
 *
 * ## 流水线位置
 * FIPS 203 Alg.20 / ML-KEM-768 Encaps 的 host 编排层（非设备核）。
 * 负责把 input 目录下各 .bin 装入 host 缓冲、把设备回写结果落到 output/{c,K}.bin。
 *
 * ## 与 golden 关系
