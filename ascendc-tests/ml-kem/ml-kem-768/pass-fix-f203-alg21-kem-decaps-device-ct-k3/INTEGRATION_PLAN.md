# INTEGRATION_PLAN — pass-fix-f203-alg21-kem-decaps-device-ct-k3

**定位**：`ascendc-tests/` **Alg.21 `ML-KEM.Decaps` CT 设备探针**（**ml_kem_768 / k=3**）。内部 **Alg.18 `Decaps_internal`**；PKE 段 **编译期引用** D15 k3 Decrypt / D14 k3 Encrypt，**禁止** frozen G4/G5 `vendor_sync`。

**基线对照**（只读判决 / STATUS；**禁止**抄 frozen 源码）：

| 路径 | 角色 |
|------|------|
| [`pass-fix-f203-alg15-pke-decrypt-device-k3`](../pass-fix-f203-alg15-pke-decrypt-device-k3/) | Phase-D Decrypt 权威 |
| [`pass-fix-f203-alg14-pke-encrypt-device-k3`](../pass-fix-f203-alg14-pke-encrypt-device-k3/) | Phase-E Encrypt 权威 |
| [`pass-fix-f203-alg20-kem-encaps-device-k3`](../pass-fix-f203-alg20-kem-encaps-device-k3/) | Encaps「头融 prep」工程范式 |
...
## 0. 用户锁定（2026-07-26 / D21ct）

| 项 | 约定 |
|----|------|
| **参数集** | ml_kem_768（**k=3**） |
| **I/O** | 与 D21 delivery sibling 相同：`dk_kem=2400B` + `c=1088B` → `K=32B`；禁止 pad-to-4/8，禁止 k4 字节长度 |
| **开工顺序** | **先 Phase-E（Alg.18 行 6–12），后 Phase-D（行 1–5）**；两段绿后再谈 D∥E 合段 |
| **行 1–4** | 从 `dk_kem` 切 `dk_pke`/`ek`/`h`/`z` → **并入 Decrypt 入口**；不另开 launch |
| **行 5** | `m' ← Decrypt` → Phase-D 主体（D15 k3 Decrypt fused） |
| **行 6** | `(K', r') ← G(m' ‖ h)` → **并入 Encrypt prep 前段**（对齐 Encaps 的 H/G；**不**挂在 Decrypt 尾） |
| **行 7** | `c' ← Encrypt(ek, m', r')` → Phase-E 主体（D14 k3 Encrypt） |
...
| **PKE 源** | 编译期引用活跃 D14/D15 k3；**禁止** `#include` / rsync frozen |
| **SIM session** | **默认 `ASCENDC_SIM_HOST_MODE=decaps_2session`**（CT 锁定）；`decaps_1session` 仅作排障覆盖；禁止回改 D21 delivery 的 `decaps_1session` 默认 |
...
```text
输入：dk ∈ B^{2400}，c ∈ B^{1088}
dk = dk_pke(1152) ‖ ek(1184) ‖ h(32) ‖ z(32)
...
| `ek` | 1184 | `input/ek_kem.bin` | dk 切片 |
| `c` | 1088 | `input/c.bin`（FO 比对用） | 调用方密文 |
| `K'` / `r'` | 32+32 | 设备 G 写 workspace（禁止 Host 预填 r'） | 同左 |
| `c'` | 1088 | workspace / 可选 dump | Encrypt 输出 |
...
Host：读 dk_kem、c、LUT
  │
  ├─ Phase-D：D15 k3 Decrypt（入口前段切 dk）→ m'_gm
...
对齐 D20/D14 k3 device：**prep 前段并入 G**；Encrypt 余下同 D14 k3 几何。
...
Host（Phase-E-only）：灌 m'/h/z/ek/c + LUT；分配 K'/coins/a_hat/re/c'/K workspace
  ...
  │
  ├─ Launch-E2…: 同 D14 k3 Encrypt compute
...
| 路径 | 内容 |
|------|------|
| `cmake/decaps/CMakeLists.txt` | `STABLE_ENCRYPT_ROOT` 指向 D14 k3；KERNEL=`kem/` Phase-E 入口 +本地覆盖 compute；**单库** |
...
Phase-D：`main_kem_decaps_phase_d_run.*` + D15 k3 `f203_decrypt_device_fused`；全链 `main_kem_decaps.cpp` + `scripts/gen_data.py`。
...
| **E3** | 拒绝路径：随机假密文 → `K` ≡ `J(z‖c)` | CPU/SIM max=0 | **PASS**（reject≠accept） |
| **D0–D2** | Phase-D / 全链内 Decrypt | `m'` 可用；全链 `K` max=0 | **PASS**（并入 F1） |
| **F1** | D→E 串联；SIM 默认 2-session（CT；单库） | `K` max=0 | **PASS**（accept D**220868**+E**605590**） |
| **F2** | 拒绝路径 | `KEM_DECAPS_REJECT=1` CPU+SIM；`K=J(z‖c)` 且 `reject≠accept` | **PASS**（reject SIM D**220680**+E**602322**） |
| **T2** | SIM 单库 + 2-session CT 默认 | 无 `…_dec_sim`；`K` max=0；根无 stray dump | **PASS**（根无 stray） |
...
```bash
cd ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg21-kem-decaps-device-ct-k3
bash run.sh -r cpu -v Ascend910B4          # 默认全链
SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4 # 默认单库 + decaps_2session
...
ASCENDC_SIM_HOST_MODE=decaps_1session bash run.sh -r sim -v Ascend910B4
```
...
| 项 | 归属 |
|----|------|
| **T2：SIM 单库合库 + CT 2-session 默认** | **PASS**：`prepare_dec_shim.sh` + 单 `ascendc_library`；默认 `decaps_2session` |
| **T19i：SIM `fo_only` → `l18_l19` 尾（4→3）** | **PASS**：继承 D21 k3 形态，本 CT 探针复验 accept/reject |
| D+E 单 launch 融合 | 更后 |
| `examples/stable/ml-kem/ml-kem-768` Decaps | 非本轮；本轮仅 `ascendc-tests` 探针 |
...
**背景**：k4 交付树曾经历 4 launch 过渡（D + prep + `l18_l19` + `fo_only`）。本 k3 探针直接落到 T19i 形态：`l18_l19` 内联 Encrypt pack 写出完整 `c'` 后同核调用 `KemDecFo`。目标与 INTEGRATION §0「行 8–12 并入 Encrypt pack 尾」对齐。
...
本探针 CMake 现将 `COMPUTE_DIR` 指到 [`pass-fix-f203-alg14-pke-encrypt-device-k3/compute`](../pass-fix-f203-alg14-pke-encrypt-device-k3/compute/)。**禁止**在该共享树上改 `f203_encrypt_l18_l19_kernel.cpp` 签名或行为（Encaps / PKE Encrypt 同编此文件）。

| 做法 | 说明 |
|------|------|
| **采用** | 探针本地覆盖：`kem/f203_encrypt_l18_l19_kernel.cpp`（自 Encrypt 拷贝后仅本目录改）；CMake 对该 TU 改指本地路径 |
| **禁止** | 直接改 D14 k3 共享 `compute/f203_encrypt_l18_l19_kernel.cpp` |
...
| tick | 记 D+E；本 CT 探针登记 accept 与 reject SIM |
...
| SIM 全链 | **PASS**；默认 `decaps_2session`；tick D**220868**+E**605590**；3 launch |
| E3 拒绝 | CPU+SIM **PASS**；`K=J(z‖c)` 且 `reject≠accept` |

### T2 / CT 落地要点

1. **问题**：D15 k3 Decrypt/Encrypt 同名头 → ascendc precompile 无法 per-TU 隔离；CT 探针还要求 D/E fresh ACL session。
2. **做法**：`scripts/prepare_dec_shim.sh` 自 D15 k3 Decrypt 复制可达子树，冲突 basename → `dec_*` 并改写 `#include`；SIM 单 `ascendc_library` 吃 shim + Encrypt + kem。
3. **验收**：CPU 与 CT 默认 SIM 均绿；`KEM_DECAPS_REJECT=1` CPU+SIM 仍绿；仅 `libascendc_kernels_sim.so`；用例根无 stray dump。
