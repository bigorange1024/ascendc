 ...
 * - 本 TU **仅**由 `pass-fix-f203-alg21-kem-decaps-device-ct-k3` CMake 编译；**禁止**回写共享 Encrypt/D21 delivery 树。
 *
 * ## 与 Encaps 共享核的差异（本文件为探针本地覆盖）
 * | 项 | D14 k3 Encrypt 共享核 | 本 Decaps 覆盖 |
 * |----|----------------------------------------|----------------|
 ...
 * @param cGm pack 输出的 c'（1088B 分片写）；traceGm 可选进度
                                                           ...
                                                           GM_ADDR KprimeGm, GM_ADDR KoutGm)
{
    ...
    if (aic) {
        ...
        /* ── 行 18/19 内积段：k3 按 2+1 行分片，uTr batch4 驻留 UB，接 INTT polyvec4 ── */
        {
            FusedTraceMark(traceGm, TR_AIV_AT_JP_START, aic, subBlockID);
            // ML-KEM-768：AIV0 负责 u0/u1，AIV1 负责 u2 和 v 的 tr̂，禁止沿用 k4 2+2。
            ...
            // k3 D14 基线：仅 AIV1 解码 ek→t̂ 并计算 tr̂；其 INTT 分片同时产出 v。
        ...
        /* 行 19：u ← u + e₁ (mod q)，按 k3 2+1 poly 行分片 */
        AscendC::PipeBarrier<PIPE_ALL>();
        {
            const int32_t pBegin = (subBlockID == 0) ? 0 : 2;
            const int32_t pEnd = (subBlockID == 0) ? 2 : 3;
