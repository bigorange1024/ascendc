 ...
 * @brief Phase-D Host：launch `f203_decrypt_device_fused`（编译期引用 D15 k3 Decrypt / 本树 shim）。
 *
 * 背景（CT / qa）：
 *   - Alg.18 行 1–4 切分不另开 launch：dk_pke = dk_kem[0:1152)；ek/h/z 留给全链 Host 偏移给 Phase-E。
 *   - SIM：本函数完整 aclInit→launch→aclFinalize，供 CT 默认 `decaps_2session` 与 Phase-E 隔离 session。
 *
                       ...
                       uint8_t *m_out)
{
    ...
    // 行 1–4：仅把前缀当作 dk_pke 上传（H2D 1152B）
