#!/usr/bin/env bash
# pass-fix-f203-alg21-kem-decaps-device-ct-k3 — Alg.21 Decaps CT 全链（D+E）；可选 Phase-E-only
#
# 生产 I/O：input/{dk_kem,c,lut_*} → output/K.bin
# PKE：编译期引用 D15 k3 Decrypt + D14 k3 Encrypt；见 INTEGRATION_PLAN.md
#
# Usage（默认全链）:
#   bash run.sh -r cpu -v Ascend910B4
#   bash run.sh -r sim -v Ascend910B4   # 默认单库 + decaps_2session；CT 锁定，T19i 后 SIM 3 launch
#
# 调试（非默认）:
#   KEM_DECAPS_PHASEE_ONLY=1 …          # 仅 Phase-E（灌 m'）
#   KEM_DECAPS_REJECT=1 …               # Gate E3：随机假密文；K vs liboqs Decaps≡J(z‖c)
#   KEM_DECAPS_TAMPER_C=1 …             # 同 REJECT，兼容旧调试命名
#   ASCENDC_SIM_HOST_MODE=decaps_1session …  # SIM 单 session 排障（非默认）
...
STABLE_ENCRYPT="${REPO_ROOT}/ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg14-pke-encrypt-device-k3"
ln -sfn "${STABLE_ENCRYPT}/scripts/host_golden" "${CURRENT_DIR}/scripts/host_golden"
# D14 k3 不 vendored `thirdparty`；host golden 会向上找到仓库级 thirdparty。
...
# TAMPER_C 是历史 correctness/CT 笔记里的同义调试名；生产验收仍使用 KEM_DECAPS_REJECT=1。
...
# SIM 全链默认 2-session（CT 锁定）；单 session 只作显式排障覆盖，不回改 D21 delivery 默认。
...
echo "[SUCCESS] pass-fix-f203-alg21-kem-decaps-device-ct-k3 ${LABEL} (${RUN_MODE})"
