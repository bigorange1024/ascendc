# INTEGRATION_PLAN — pass-fix-f203-alg21-kem-decaps-device-k3

**定位**：`ascendc-tests/` **Alg.21 `ML-KEM.Decaps` 无 vendor 设备主线**（**ml_kem_768 / k=3**）。内部 **Alg.18 `Decaps_internal`**；PKE 段 **编译期引用** D15 k3 Decrypt / Encrypt（或 Encaps 树），**禁止** frozen G4/G5 `vendor_sync`。
...
| **F1** | D→E 串联；SIM 默认 1-session（单库） | `K` max=0 | **PASS**（D**220767**+E**597518**） |
...
```bash
cd ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg21-kem-decaps-device-k3
...
| **T19i：SIM `fo_only` → `l18_l19` 尾（4→3）** | **本探针 PASS**（2026-07-26；D**220767**+E**597518**） |
| D+E 单 launch 融合 | 更后 |
...
| tick | 记 D+E；k3 首版登记为 D**220767**+E**597518** |
...
| SIM 全链 | **PASS**；tick D**220767**+E**597518**；3 launch |
| E3 拒绝 | CPU+SIM **PASS** |
...
3. **验收**：CPU 仍绿；SIM 全链 `K` max=0（D**220767**+E**597518**）；`KEM_DECAPS_REJECT=1` CPU+SIM 仍绿；仅 `libascendc_kernels_sim.so`；用例根无 stray dump。
