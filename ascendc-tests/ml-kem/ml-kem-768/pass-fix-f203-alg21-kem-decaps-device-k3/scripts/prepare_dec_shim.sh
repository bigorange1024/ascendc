#!/usr/bin/env bash
# prepare_dec_shim.sh — 从 D15 k3 Decrypt 生成 SIM 合库用头隔离树（幂等）
#
# 背景（T2）：ascendc_library precompile 只认 target 级 -I，无法 per-TU 隔离；
# D15 k3 Decrypt/Encrypt 有大量同名头且内容分歧。合进单设备库前，须把 Decrypt
# 侧冲突 basename 改名为 dec_*，并改写 shim 内 #include。
#
# 源：ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg15-pke-decrypt-device-k3（编译期权威；禁止 frozen）
...
SRC="${REPO_ROOT}/ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg15-pke-decrypt-device-k3"
DST="${CASE_DIR}/shim/pke_decrypt"

if [[ ! -d "${SRC}/compute/g4_full" ]]; then
    echo "[dec_shim] ERROR: missing D15 k3 Decrypt at ${SRC}" >&2
    exit 1
fi
...
# 与 D14 k3 Encrypt 同名且可能内容分歧的 basename → dec_*（合库单 -I 必需）
...
echo "[dec_shim] OK ${DST} (from D15 k3 Decrypt; conflict headers → dec_*)"
