 ...
 *
 * ML-KEM-768：k=3，polyvec6（ROWS=6）。
 */
#pragma once
...
constexpr uint32_t ROWS = 2U * K;                        // 6 行：前 K 行对应 ŝ，后 K 行对应 ê（polyvec6）
constexpr uint32_t PRF_TOTAL_BYTES = ROWS * PRF_BYTES;   // 全部 6 行 PRF 输出总字节数：6*128=768
constexpr uint32_t SRC_COEFFS = ROWS * N;                // 输出 src 的系数总数：6*256=1536
 ...
 * F203_CBD_BLOCK_DIM 分支保持一致（=1 时单 AIV 串行 6 行，其余走双 AIV 分片）。 */
