#!/usr/bin/env python3
# coding=utf-8
"""Alg.8 CBD η=2 探针对拍：output/src.bin vs golden_src.bin（ML-KEM-768，k=3，ROWS=6）。

本文件在流水线中的位置：pass-fix-f203-alg8-cbd-eta2-k3 探针的最终验收脚本，
...
ROWS = 6              # 6 行（polyvec6）：ŝ(3) + ê(3)
N = 256                # 每行系数个数
SRC_BYTES = ROWS * N * 4  # int32 每元素 4 字节，总字节数 6*256*4=6144
...
def main() -> None:
    ...
    print(f"[verify] src[6,256] PASS ({SRC_BYTES} bytes exact match)")
