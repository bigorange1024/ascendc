...
def _find_v2_gen_data() -> str:
    """定位 1024 Tag5T v2 的 gen_data（ByteEncode₁₂ golden 链）。

    背景：768 W0/B2 的 encode12 仍复用 v2 Alg.13 mixPass=7 几何（k=4）做 d=12 算法探针；
    嵌套布局下不可再写相对「同级 sibling」路径。
    结论：自本文件向上 walk，优先命中 ml-kem/ml-kem-1024/.../vec-k4-v2/scripts/gen_data.py。
    未采用：在 768 树内复制整份 v2 gen_data（避免双份维护）。
    """
    cur = _CASE_DIR
    for _ in range(10):
        cand = os.path.join(
            ...
            "ml-kem",
            "ml-kem-1024",
            "pass-fix-f203-2s1e-alg13-16171820-vec-k4-v2",
            "scripts",
            "gen_data.py",
        )
        if os.path.isfile(cand):
            return os.path.normpath(cand)
        # 兼容历史扁平布局（ascendc-tests 下直接放探针）
        cand2 = os.path.join(
            cur,
            "pass-fix-f203-2s1e-alg13-16171820-vec-k4-v2",
            "scripts",
        ...
        cur = parent
    raise SystemExit(
        "[gen_data] missing v2 gen_data under "
        "ml-kem/ml-kem-1024/pass-fix-f203-2s1e-alg13-16171820-vec-k4-v2/scripts/"
    )


_V2_GEN = _find_v2_gen_data()
