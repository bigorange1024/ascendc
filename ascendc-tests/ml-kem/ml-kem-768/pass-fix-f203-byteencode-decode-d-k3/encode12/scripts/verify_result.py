...
@brief ByteEncode₁₂-only 对拍：输入契约同 Alg.13 mixPass=7 preset；验 ek/sk。

流水线位置：run.sh 在 kernel 写出 output/ek|sk_polyvec.bin 后调用。
与 golden 关系：将设备输出与 gen_data 的 golden_ek/sk（及可选 golden_dst/t_hat）逐元素比较。
作用：检查 preset 自洽 + ByteEncode₁₂(t̂)/ByteEncode₁₂(ŝ) 字节一致；全过则打印 PASS。
"""
import os
import sys
...
def check(label: str, got: np.ndarray, ref: np.ndarray) -> int:
    """
    int32（或可转 int64）数组对拍：max_abs_diff==0 则 SUCCESS。
...
def check_bytes(label: str, got: np.ndarray, ref: np.ndarray) -> int:
    """
    uint8 字节流对拍：形状须一致，且 max_abs_diff==0。

    参数：
        label: 日志标签（如 ek_polyvec / sk_polyvec）
        got / ref: dtype uint8，通常长度 POLYVEC_BYTES
    返回：
        0 通过，1 失败（失败时打印首个最大差字节下标）
    """
...
def main() -> int:
    """
    按存在的文件做分段对拍（缺文件则跳过该段）。

    检查项：
      - input dst vs golden_dst，以及 dst 内两份 ŝ 副本
      - input t_hat vs golden_t_hat
      - output ek/sk vs golden_ek/sk（本探针主验收）
    返回：
        0 全部通过，非 0 有失败
    """
    rc = 0

    # preset 自洽：gen_data 写出的 input 应等于其 golden 副本
