#!/usr/bin/env bash
# pass-fix-f203-mlkem768-sub-decompress-d-vec-k4 — FIPS 203 Decompress_d（验收 d=4/5/10/11）
#
...
#
# 环境变量：
#   F203_DECOMPRESS_D — 4/5/10/11（默认 4）
#   DECOMPRESS_D_VEC  — 0 标量 / 1 向量（默认 1；全档 int32 向量）
#
# Usage:
#   bash run.sh -r cpu -v Ascend910B4
#   F203_DECOMPRESS_D=10 bash run.sh -r sim -v Ascend910B4
#
# 环境变量：
#   F203_DECOMPRESS_D — 4（默认，Decrypt v）或 10（Decrypt u）
#   DECOMPRESS_D_VEC  — 0 标量 / 1 向量（d=4/10 均支持向量）
#
...
export F203_DECOMPRESS_D="${F203_DECOMPRESS_D:-4}"
...
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} F203_DECOMPRESS_D=${F203_DECOMPRESS_D} DECOMPRESS_D_VEC=${DECOMPRESS_D_VEC}"
...
echo "[SUCCESS] pass-fix-f203-compress-decompress-du10-dv4-k3/decompress d=${F203_DECOMPRESS_D} (${RUN_MODE})"
