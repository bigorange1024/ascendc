 ...
 *
 * 作用：封装官方 LoadDataWithTranspose + Mmad + Fixpipe；本探针 m=12（紧凑 HI₆+LO₆）、
 * k=256、n=128（半列），与平面 mat_c 偶/奇半对应。
 *
 * 与 golden 关系：四路临时经 AivK6PackMatCPlanar 拼成平面 mat_c 后与 golden_mat_c 对拍；
 * 本类不直接写 dst。NTT/INTT 仅差 LUT 内容，本类逻辑相同。
 *
 * 语义：poly-batch 下 S0 已含全部 6 poly 的 HI/LO；Cube 一次吃满逻辑 12 行（Cube pad 16），不分 limbsplit。
 */
...
private:
     ...
     * 循环次数 = ceil(m/16)；本探针 m=12（Cube mmadParams.m 仍 pad 到 16） 故仅 1 次。
     */
    template <int debug_val = 0>
