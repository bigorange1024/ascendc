...
"""
@file mlkem_ref.py
@brief F203 / sepolyvec6_ntt_f203 交付语义的 Python 参考（Stage1+MatMul+RouteA+mod）。

流水线位置：被 gen_data.py import stage31_mod；亦可独立做 Tag3 风格 [16,512] mat_c 路径对照。
本探针设备侧用平面 mat_c [48,128]，与本文件 f203_stage3_route_a（偶奇列 deinterleave）
...
_F203_CASE = _REPO / "thirdparty/ntt_onnx/deliverables/sepolyvec6_ntt_f203"
...
def gen_fixed_se_polyvec() -> "np.ndarray":
    """
    生成固定可复现的 s||e 风格 [6,256] int32（非随机）。
...
def load_f203_se_and_lut_fp16():
    """
    读交付用例 input0.bin（se [6,256] int32）与 input1.bin（LUT [256,512] fp16）。
    @return (se, lut_fp16)
    """
    import numpy as np
...
def validate_se_polyvec_in_zq(se: "np.ndarray") -> None:
    ...
    与交付 sepolyvec6_ntt_f203 一致：图内仅 hi/lo 拆分，无 ReduceToZq。
...
def encode_mat_a(se: "np.ndarray") -> "np.ndarray":
    """F203 Stage1：双 polyvec [HI(8), LO(8)] -> [12,256] int8。
...
def matmul_int8_i32(a: "np.ndarray", b: "np.ndarray") -> "np.ndarray":
    """int8×int8 累加到 int32：a @ b，形状 [12,256]×[256,512]→[16,512]。"""
    import numpy as np
...
def f203_stage3_route_a(mat_c: "np.ndarray") -> "np.ndarray":
    """RouteA：mat_c [16,512] -> raw [6,256]，对齐 sepolyvec6_ntt_f203 ONNX。
...
def f203_three_stage_batch(se: "np.ndarray", mat_b: "np.ndarray | None" = None) -> "np.ndarray":
    """完整 F203 int8 Cube 路径（Tag3 风格 mat_c）-> [6,256] int32。
...
def golden_mlkem_ntt_batch(se: "np.ndarray") -> "np.ndarray":
    """对 [6,256] 逐 poly 调用 mlkem_ntt，得到标量 NTT golden。"""
    import numpy as np
