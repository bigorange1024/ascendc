...
| 允许 | 禁止 |
|------|------|
| 本目录源码 + `thirdparty/ntt_onnx/`（LUT 头） + `scripts/mlkem_ref.py` | Python/`#include` 引用其它探针或 `examples/` |
| `library/shared/`（编译期 merged_kyber 等） | 仓库根 `thirdparty/ntt_onnx` 未 vendored 路径 |
