#!/bin/bash
# pass-fix-f203-alg11-12-innerproduct-k4-halfrows — 4×4×1 半行内积（双 AIV；GM a_hat/s_hat 与 alg13 一致）
#
# Usage:
#   bash run.sh -r cpu -v Ascend910B4
#   bash run.sh -r sim -v Ascend910B4
#   # 2×2×1 对照：INNERPRODUCT_P_OUT=2 INNERPRODUCT_S_VEC=2 bash run.sh -r sim -v Ascend910B4
#
#
...
export INNERPRODUCT_P_OUT="${INNERPRODUCT_P_OUT:-4}"
export INNERPRODUCT_S_VEC="${INNERPRODUCT_S_VEC:-4}"
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} P_OUT=${INNERPRODUCT_P_OUT} S_VEC=${INNERPRODUCT_S_VEC} ALG11_IMPL=${ALG11_IMPL} ALG11_VEC_VARIANT=${ALG11_VEC_VARIANT} ALG11_VEC_OPTS=${ALG11_VEC_OPTS} ALG11_MEM_OPS=${ALG11_MEM_OPS}"
...
cmake -B build \
    ...
    -DINNERPRODUCT_P_OUT=${INNERPRODUCT_P_OUT} \
    -DINNERPRODUCT_S_VEC=${INNERPRODUCT_S_VEC}
cmake --build build -j
cmake --install build
...
echo "[SUCCESS] pass-fix-f203-alg11-12-innerproduct-k4-halfrows (${RUN_MODE})"
