 ...
 * | `Aiv2s1eRouteAMod` | S3 独立 | 平面 merge+mod → dst（生产用 `Aiv2s1eUbPipeline` 内联） |
 *
 * ## 平面行号
 *
...
public:
    ...
    /** AIV0：ŝ 行 0..3 + ê 4..5；AIV1：ŝ 另一套 S0 行 + ê 6..7 */
...
private:
    /** int32 poly → limb6 int8：hi 行 rowBase..，lo 行 rowBase+kPolys.. */
...
/** 平面 mat_c 行号：half×48 + slot×4 + limb（与 gen_data.py::planar_row 一致） */
...
/**
 * Stage3：平面 mat_c，bulk DataCopy 四 limb 行，无 Gather。
 */
...
public:
    ...
    /** 平面 slot 批量 merge+mod → local_dst，再 copyOut 到 gm_dst（独立 S3 调试用） */
