...
PROGRESS_EVERY = max(1, int(os.environ.get("KAT_PROGRESS", "10")))
...
def run_ascendc_keygen(seed_d: int, run_mode: str) -> tuple[np.ndarray, np.ndarray]:
  ...
  if ek.size != EK_PKE_BYTES or dk.size != DK_PKE_BYTES:
    raise SystemExit(f"g4 output size ek={ek.size} dk={dk.size}")
  return ek, dk


def compare_pke(seed_d: int, label: str, ek_oqs, dk_oqs, ek_dev, dk_dev) -> None:
  if not np.array_equal(ek_dev, ek_oqs):
    idx = int(np.argmax(ek_dev != ek_oqs))
    raise SystemExit(
      f"[KAT FAIL] {label} seed_d={seed_d} ek_pke @ {idx}: ascendc={ek_dev.flat[idx]} liboqs={ek_oqs.flat[idx]}"
    )
  if not np.array_equal(dk_dev, dk_oqs):
    idx = int(np.argmax(dk_dev != dk_oqs))
    raise SystemExit(
      f"[KAT FAIL] {label} seed_d={seed_d} dk_pke @ {idx}: ascendc={dk_dev.flat[idx]} liboqs={dk_oqs.flat[idx]}"
    )


def one_round(seed_d: int, run_mode: str, label: str, idx: int, total: int) -> None:
  ...
  if (idx + 1) % PROGRESS_EVERY == 0 or idx + 1 == total:
    print(f"[kat] {label} seed_d={seed_d} OK ({idx + 1}/{total})", flush=True)
...
def main() -> int:
  ...
  print(f"[kat] start CPU×{cpu_count} + SIM×{sim_count} (random SEED_D per round)")
  print(f"[kat] cpu seeds: {cpu_seeds}")
  print(f"[kat] sim seeds: {sim_seeds}")
  os.environ.setdefault("KEYGEN_SKIP_REBUILD", "0")
  for i, s in enumerate(cpu_seeds):
    one_round(s, "cpu", "CPU", i, cpu_count)
    if i == 0:
      os.environ["KEYGEN_SKIP_REBUILD"] = "1"
  for i, s in enumerate(sim_seeds):
    os.environ.pop("KEYGEN_SKIP_REBUILD", None)
    one_round(s, "sim", "SIM", i, sim_count)
