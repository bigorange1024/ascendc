...
"""
@file gen_data.py
@brief 8-poly 紧凑三段式 NTT/INTT 的 golden / 输入生成（与设备 Tag5T 同构）。
...
作用（黑盒 oracle，非 AscendC 规格）：
  ...
  2. 随机 src [8,256] → 紧凑 S0 [16,256] → 四路 matmul → 平面 mat_c [64,128] → RouteA+stage31_mod → dst；
  3. 可选与 ntt_study deliverable 交叉核对（仅 NTT 且 src 碰巧一致时）。

与 golden 关系：verify_result.py 只比 dst vs golden_dst；s0/mat_c 供分段调试。
环境变量：F203_NTT_MODE=ntt|intt；STAGE123_POLYVEC8_MIX_PASS（写入 tiling.bin）。
"""
import os
import re
import struct
...
HALF_N = N // 2
K = 8
K_PER_AIV = 4          # poly-batch：每 AIV 4 条
M_ROWS = 2 * K         # 紧凑 S0：HI₈+LO₈
LIMBS = 4
MAT_C_PLANAR_ROWS = K * LIMBS * 2  # 64
LIMB_MASK = 0x3F
LIMB_BITS = 6
Q = 3329
SEED = 20260628
...
def encode_k8_s0(polys: np.ndarray) -> np.ndarray:
    """分配零 S0 [16,256] int8 并 encode_compact_k8 全 8 poly。"""
...
def mat_c_tmp_golden(s0: np.ndarray, lut: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    Stage2 四路 matmul golden：s0[16,256] @ LUT 四半块 → 各 [16,128] int32。
...
def pack_bank_planar_k8(
    ...
    与 AivK8PackMatCPlanar 同构：half=0 用 c_lo_*，half=1 用 c_hi_*；
...
def pack_mat_c_planar_k8(c_lo_even, c_lo_odd, c_hi_even, c_hi_odd) -> np.ndarray:
    """全 8 poly：先 AIV0 的 0..3，再 AIV1 的 4..7，得到 [64,128] 平面。"""
...
def golden_dst_from_planar(mat_planar: np.ndarray) -> np.ndarray:
    """对 slot 0..7 调用 merge_planar_poly，得到 golden dst [8,256]。"""
