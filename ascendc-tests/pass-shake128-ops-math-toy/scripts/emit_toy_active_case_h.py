#!/usr/bin/env python3
# coding=utf-8
"""由 cases/<active>/ 生成 auto_gen/toy_active_case.h（设备 UB 用例 + 内嵌 golden）。"""
...
def emit_toy_active_case_h(case_dir: Path, out_h: Path, ns: str = "Shake128ToyActive") -> None:
    ...
    if len(x) != batch * max_msg_len:
        raise SystemExit(f"x.bin size mismatch")
    if len(golden) != y_bytes:
        raise SystemExit(f"golden_y.bin size mismatch")
