 ...
 *
 * 流水线位置：NTT / INTT 段 AIC 在 CrossCore WAIT(1) 之后、SET(3) 之前各调用一次，
 * 给握手提供「真 Cube 活」；形状取 int8 Cube 最小粒度，避免 SIM 过慢。
 * GATE 段仅 WAIT(4)/SET(8)，不用本类。WAIT 期间禁止 SyncAll。
 * API：DataCopy+Nd2Nz / LoadData / LoadDataWithTranspose / Mmad / Fixpipe
 * （查阅索引已有记录，本玩具复用）。
 */
#ifndef PASS_TOY_DECRYPT_FSM_FUSED_SKEL1_AIC_FUNC_HPP
#define PASS_TOY_DECRYPT_FSM_FUSED_SKEL1_AIC_FUNC_HPP
 ...
 * 前置：k、n 为 32 的倍数（本玩具 k=n=32）；m 允许 ≤16（内部按 16 对齐）。
 */
class AicMmad {
public:
