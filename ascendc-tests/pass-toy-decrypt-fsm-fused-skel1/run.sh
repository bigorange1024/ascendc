#!/bin/bash
# pass-toy-decrypt-fsm-fused-skel1：Decrypt Soft×2+GATE×2+NTT/INTT1/3 融合握手骨架
#
# Usage（默认 = 生产全量；本玩具无调试分段开关）：
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4   # 验收（只认 SIM；TOY_LAUNCH_REPEAT 默认 16）
#   TOY_LAUNCH_REPEAT=16 SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4  # DGT-4 加压显式写法
#   bash run.sh -r cpu -v Ascend910B4                # 仅编译壳；不作验收证据
#
# 不对算法正确性；kernel 退出码 0 + Host 见完成标记 + 无 stray dump 即过。
# KERNEL_COMPUTE_BUDGET_SEC 默认 600（16 轮同进程 launch；防挂死，非性能定标）
...
echo "[SUCCESS] pass-toy-decrypt-fsm-fused-skel1 (${RUN_MODE}) kernel exit=0 (no correctness cmp)"
