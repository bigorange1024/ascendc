 ...
 *
 * 流水线位置：INTT 段 AIC 在 CrossCore WAIT(1) 之后、SET(3) 之前调用，给握手提供
 * 「真 Cube 活」；形状取 int8 Cube 最小粒度，避免 SIM 过慢。GATE 段不用本类。
 * API：DataCopy+Nd2Nz / LoadData / LoadDataWithTranspose / Mmad / Fixpipe
 * （查阅索引已有记录，本玩具复用）。
 */
#ifndef PASS_TOY_ENCRYPT_FSM_GATE_INTT1_AIC_FUNC_HPP
#define PASS_TOY_ENCRYPT_FSM_GATE_INTT1_AIC_FUNC_HPP
