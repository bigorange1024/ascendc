#ifndef PASS_TOY_ENCRYPT_FSM_GATE_INTT1_AIV_FUNC_HPP
#define PASS_TOY_ENCRYPT_FSM_GATE_INTT1_AIV_FUNC_HPP

/**
 * @file aiv_func.hpp
 * @brief AIV 侧：桩「哈希」（禁止真 SHAKE）+ 半片写 S0 + 完成标记写 out。
 *
 * 背景（GT-20260903-2）：Encrypt μ 前缀有 SHAKE；本玩具用 SetValue 常数填充代替
 * （J-FAIL-DUPLICATE-INT8：A2 上 Duplicate 不支持 int8），只保留写半片意图。
