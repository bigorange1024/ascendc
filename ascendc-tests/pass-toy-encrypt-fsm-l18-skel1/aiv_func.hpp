#ifndef PASS_TOY_ENCRYPT_FSM_L18_SKEL1_AIV_FUNC_HPP
#define PASS_TOY_ENCRYPT_FSM_L18_SKEL1_AIV_FUNC_HPP

/**
 * @file aiv_func.hpp
 * @brief AIV 侧：桩「哈希」（禁止真 SHAKE）+ 半片写 S0 + 完成标记。
 *
 * 背景（GT-20260903-3/4）：Encrypt μ 前缀有 SHAKE；本玩具用 SetValue 常数填充代替
 * （J-FAIL-DUPLICATE-INT8：A2 上 Duplicate 不支持 int8）。
 * TRACE 写槽已改在 mmad_custom.cpp 的 ToyTraceMark（DataCopy，禁标量写 GM）。
