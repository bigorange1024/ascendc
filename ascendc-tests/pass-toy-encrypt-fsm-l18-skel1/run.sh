#!/bin/bash
# pass-toy-encrypt-fsm-l18-skel1：同核 NTT 1/3 → GATE 4↔8 → INTT 1/3 + TRACE
#
# Usage（默认 = 单 launch 全链路；与 GT-5/6 兼容）：
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4   # 验收（只认 SIM）
#   bash run.sh -r cpu -v Ascend910B4                # 仅编译壳；不作验收证据
#
# 调试 / 非默认（GT-20260903-7 两段 Host launch，模仿 F-SIM-LAUNCH）：
#   TOY_SPLIT_2LAUNCH=1 TOY_LAUNCH_REPEAT=16 SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#   # 每轮：launch0=NTT_ONLY → Sync → launch1=GATE_INTT_ONLY → Sync；TRACE 每轮开始清一次
#
# 不对算法正确性；kernel 退出码 0 + Host TRACE 打印 + 无 stray dump 即过。
# KERNEL_COMPUTE_BUDGET_SEC 默认 240（防挂死，非性能定标；允许 180–300）
# 两段×16 组建议可抬：KERNEL_COMPUTE_BUDGET_SEC=600（本刀默认仍 240，不够再抬）
CURRENT_DIR=$(
...
if [ "${RUN_MODE}" = "sim" ]; then
    export KERNEL_COMPUTE_BUDGET_SEC="${KERNEL_COMPUTE_BUDGET_SEC:-240}"
    # shellcheck source=/dev/null
...
echo "[SUCCESS] pass-toy-encrypt-fsm-l18-skel1 (${RUN_MODE}) kernel exit=0 (no correctness cmp)"
