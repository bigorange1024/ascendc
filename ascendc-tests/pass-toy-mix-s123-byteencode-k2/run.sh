#!/bin/bash
# pass-toy-mix-s123-byteencode-k2：S1 双 AIV → S2 Cube 64³ → S3 UB encode
# Usage:
#   bash run.sh -r cpu -v Ascend910B4
#   TOY_MIX_PASS=1 bash run.sh -r cpu   # 仅 Stage1
#   TOY_MIX_PASS=2 bash run.sh -r cpu   # 仅 Stage2（需 s0 preset）
#   TOY_MIX_PASS=3 bash run.sh -r cpu   # 仅 Stage3（需 mat_c preset）
#
...
# CANN 环境：统一走仓库 scripts/env.sh（多候选 + source 后回写 CANN_HOME）
# 禁止写死 ${HOME}/ascendc（借入 root 机 / 非标准 clone 路径会挂）
set +e
# shellcheck source=/dev/null
...
export TOY_MIX_PASS="${TOY_MIX_PASS:-0}"
# 实机 ACL 设备号：npu 缺省 0；SIM 强制 0（CAModel 仅设备 0）
if [ "${RUN_MODE}" = "npu" ]; then
    export ASCEND_DEVICE_ID="${ASCEND_DEVICE_ID:-0}"
elif [ "${RUN_MODE}" = "sim" ]; then
    export ASCEND_DEVICE_ID=0
fi
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} TOY_MIX_PASS=${TOY_MIX_PASS} CANN=${_ASCEND_INSTALL_PATH} ASCEND_DEVICE_ID=${ASCEND_DEVICE_ID:-n/a}"
...
if [ "${RUN_MODE}" = "sim" ]; then
    ...
    echo "[npu] ASCEND_DEVICE_ID=${ASCEND_DEVICE_ID}  CANN_HOME=${_ASCEND_INSTALL_PATH}"
fi
...
if [ "${RUN_MODE}" = "sim" ]; then
    # 本用例 CAModel 全量 S123 实测约 55–70s；预算须留余量（防挂死≠性能定标）
...
echo "[SUCCESS] pass-toy-mix-s123-byteencode-k2 (${RUN_MODE})"
