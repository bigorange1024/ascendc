...
#   - 必须含 scripts/（sim_env.sh、xelatex-clean.sh 等）与 HOME-KEYGEN-DEBUG.md
...
# --- 根文档与脚本 ---
for f in README.md AGENT_HANDOFF.md HOME-KEYGEN-DEBUG.md Makefile backup-project.sh; do
  if [ -f "$f" ]; then
    rsync -a "$f" "$DEST/"
  fi
done
...
- HOME-KEYGEN-DEBUG.md、backup-project.sh
