...
> 本系统免费向开发者提供算力体验环境。为合理利用资源，**连续 1 小时未使用**的环境将自动关机；**连续 2 周未开机**的环境将自动释放。  
> **卡时配额**：每人初始配额为 **100 卡时**，请及时关注剩余用量，避免超额影响作业运行。  
> **`/workspace`** 为服务器本地目录，具备高 IO 性能，适用于代码编译，容量上限为 **300 GB**。注意：个人数据请及时备份以防丢失。  
> **`/shared_assets`** 目录存放有常用模型权重、软件包等文件，请优先取用。

| 约束 | 操作含义 |
|------|----------|
| 连续 1h 未使用 → 自动关机 | 长编译保持终端活动，或拆短任务 / `nohup`+分段回看 |
| 连续 2 周未开机 → 自动释放 | 重要产物以 **Git push** 为权威备份；勿只信本地盘 |
| 100 卡时初始配额 | 默认只跑冒烟 + 必要刀；盯控制台剩余用量 |
| `/workspace` ≤300G、高 IO、可能丢 | 工程放 `/workspace/ascendc`；编译产物可本地，源码以远程 Git 为准 |
| `/shared_assets` 公共只读优先 | 权重/公共包优先从这里取；**不要**当私有仓写 |
