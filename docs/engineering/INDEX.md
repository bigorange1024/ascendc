...
| [thirdparty-本地依赖.md](thirdparty-本地依赖.md) | `thirdparty/` 不进 Git；仓 URL + `clone-thirdparty.sh`（**默认 build liboqs**）；含 `cannbot-skills`；禁止 Agent 误删 |

| [thirdparty-本地依赖.md](thirdparty-本地依赖.md) | `thirdparty/` 不进 Git；仓 URL + `clone-thirdparty.sh`（**默认 build liboqs**）；含 `cannbot-skills`；**Drive** `reasoning-graph-skill`；禁止 Agent 误删 |
