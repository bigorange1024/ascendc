...
| [Encrypt卡死重写-实机一次测清单.md](Encrypt卡死重写-实机一次测清单.md) | **Encrypt 卡死**专用一次测：`npu_hang_rewrite_one_trip.sh`（N0–N10 + FORCE_REBUILD + L18_TRACE）；回传 `FEEDBACK.md` + `BRING_BACK.tar.gz` |
| [Encrypt卡死重写-sync清单.txt](Encrypt卡死重写-sync清单.txt) | 上机最小 sync/tar 路径列表（toys + stable 1024 四档 + 脚本/acl_session） |
