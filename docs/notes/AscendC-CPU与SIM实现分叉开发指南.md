...
| 模式 | 角色 |
|------|------|
| **SIM** | **主参考点**（与设备同构路径、tick、交付结论） |
| **CPU** | **辅助正确性**（快回归 / gdb；可与 SIM 不同构 launch，如 Encrypt 的 `golden_v`） |

Encrypt 交付细则：[`F203-Alg14-Encrypt-交付口径-CPU辅助与SIM主参考.md`](F203-Alg14-Encrypt-交付口径-CPU辅助与SIM主参考.md)。
