...
| 约束 | 说明 |
|------|------|
| **禁止回传文件** | 用户无法传 log / md / 截图包；主控不得再要求「填模板传回」 |
| **只打字** | 对话里发 `REPORT:` / 三位码即可（例 `C1 HANG 110`） |
| **主控义务** | 缺信息只追问**一个**数字；按 `BRANCHING.md` 推理，不索文件 |
| **NPU 设备 TRACE** | 设备 `AscendC::printf`（2xx/4xx）常不进 host stdout；挂因套件 **只认 Host 1xx**；`L1 TRACE 200 missing` 在 Host 已有 `111` 时 **≠ 粘性挂** |
