# ML-KEM-512：从 0 到 incubating exp 完整实现计划（草案）

**状态**：**草案待用户锁 P0**（2026-07-27）· 未写码  
**参数组**：真 **ML-KEM-512**（\(k=2,\eta_1=3,\eta_2=2,d_u=10,d_v=4\)）  
**实验意图**：**方法论压力床 3**——参照 768 闭环，在用户**极少介入**下自主推进至 incubating + 测试 + 文档刷新  
**权威参数卡（草案）**：[docs/specs/fips203-mlkem512-parameter-card.md](../specs/fips203-mlkem512-parameter-card.md)  
...
**一句话**：用真 \(k=2\) + **单 AI Core** 几何，把 768 已验证的「参数卡 → 补洞 → 波次绿线 → incubating → glue」再跑通一遍，检验方法论能否指导下一参数组。
...
| P1 | — | 相对 768/1024 补洞图；必建用例表定稿 |
| P2 | W0 | 编码/压缩 + **CBD \(\eta=3\)** 积木 |
...
```text
P0 文书锁定 ──► P1 补洞+用例表 ──► P2 按波次实现 ──► P3 exp + glue
     │                │                   │                    │
...
| **P1** | 补洞图；必建用例表（另文 `…-p1-gap-and-cases.md`） | 表定稿 |
...
| **T-A2** | 真 2-poly 批更细切 | 目录更多，收益有限 |

**NTT 不变量（继承 Tag5T，与 k 无关）**：每个参与计算的 AIV **握完整 poly 的 hi+lo**（禁 limbsplit）；S1–S3 **禁 `Gather`**；平面 mat_c。

> **草案默认（待用户确认）**：**S-1 + T-B2 + C-1（512 树自建 compress/byteencode）+ 保留 D19–D21 + CT + PKE exp 必做 + `-k2`**。

### 3.3 Compress / Launch（草案默认）

| 项 | 默认 |
|----|------|
| Compress/ByteEncode | **C-1**：512 树自建；主路径 \(d\in\{10,4\}\)，密钥域 d=12 |
...
| CBD \(\eta=3\) | **无** | **新建** Alg.8 \(\eta=3\) 探针（KeyGen 主路径） | **高** |
...
| **B3b** | P2 | W0 | `…-alg8-cbd-eta3-k2` | **CBD \(\eta=3\)**（512 新洞） |
| **B4** | P2 | W1 | `…-alg7-sample-ntt-k2` | SampleNTT \(i,j\in\{0,1\}\) |
...
> **怎么确认**：在编辑器里点下面的 `- [ ]`，或**直接在对话里回复**例如：`§9 全部按草案默认` / `1–9 同意`（有异议只写要改的编号）。Agent 收到后会把对应项改成 `- [x]` 并锁定参数卡。

### 待确认清单

- [ ] **#1 单 AI Core** — 草案：**S-1**（单 cube，`blockDim=1`；cube 内可用 2 AIV；禁跨 cube 切 poly）
- [ ] **#2 噪声批** — 草案：**T-B2** polyvec4；\(\hat A\) 独立 prep；禁零垫
- [ ] **#3 KEM device** — 草案：**保留** D19–D21
- [ ] **#4 reject / CT** — 草案：**要求** D21ct + E21ct
- [ ] **#5 PKE exp** — 草案：**要做** E13–E15
- [ ] **#6 命名** — 草案：**`-k2`**
- [ ] **#7 liboqs-512 交叉** — 草案：**本阶段必达**（含修 helper）
- [ ] **#8 stable-512** — 草案：**本阶段不建**
- [ ] **#9 自主授权** — 草案：锁本表后 Agent **自主推进至有条件完成**；仅计划 §0.1 停问点打断
...
**下一刀**：用户确认 §9（对话或勾选均可）→ Agent 落 P0 文书与目录壳 → P1 定稿 → 开 W0。
