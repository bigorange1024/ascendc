...
| # | 议题 | 草案默认 | 用户决议 |
|---|------|----------|----------|
| 1 | 单 AI Core | **S-1**（单 cube，`blockDim=1`；cube 内可用 2 AIV） | □ |
| 2 | 噪声批 | **T-B2** polyvec4；\(\hat A\) 独立 prep；禁零垫 | □ |
| 3 | KEM device | **保留** D19–D21 | □ |
| 4 | reject / CT | **要求** D21ct + E21ct | □ |
| 5 | PKE exp | **要做** E13–E15 | □ |
| 6 | 命名 | **`-k2`** | □ |
| 7 | liboqs-512 交叉 | **本阶段必达**（含修 helper） | □ |
| 8 | stable-512 | **本阶段不建** | □ |
| 9 | 自主授权 | 锁本表后 Agent **自主推进至有条件完成**；仅 §0.1 停问点打断 | □ |

**下一刀**：用户勾选 §9 → Agent 落 P0 文书与目录壳 → P1 定稿 → 开 W0。
