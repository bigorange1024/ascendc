...
| **补洞** | 把缺项逐步变成已获证、使目标闭包已满的过程 |
| **补洞图** | P1 工程表：列出缺项与补齐策略（**不是**新对象；勿再称「洞 / 新洞 / 洞级」） |
| **闭包 / 已获证 / 最小补洞** | 同教材 |

**禁止**：把「洞」当缺项的同义词另立定义；registry「缺项」仍指登记表未覆盖的计算块（Rule 原义，勿混称「洞」）。
...
| P1  | —    | 相对 768/1024 **缺项对照（补洞图）**；必建用例表定稿                             |
...
```text
P0 文书锁定 ──► P1 缺项对照（补洞图）+用例表 ──► P2 按波次实现 ──► P3 exp + glue
     │                │                   │                    │
...
| **P1** | 缺项对照（补洞图）；必建用例表（另文 `…-p1-gap-and-cases.md`）                                                 | 表定稿                       |
...
### 4.1 缺项对照（相对 768；即补洞图）


| 能力块                | 768                | 512 策略（补洞）                             | 备注（高/中/低，非正式） |
| ------------------ | ------------------ | ---------------------------------- | --------- |
