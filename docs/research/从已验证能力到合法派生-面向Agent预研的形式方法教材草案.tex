...
\newenvironment{fipsalgo}[1]{%
  \begin{tcolorbox}[
    ...
    boxed title style={colback=gray!12, colframe=gray!55, boxrule=0.4pt}
  ]
  \small
...
标准把 KEM 密钥生成拆成三层。
下面给出\textbf{与 FIPS~203 一致的步骤编号}（ml\_kem\_1024：$k{=}4$，$\eta_1{=}\eta_2{=}2$）。
正文引用 NIST FIPS~203 Algorithm~13 / 16 / 19。

\begin{fipsalgo}{Algorithm 19 \texttt{ML-KEM.KeyGen()}（FIPS~203 §7.1）}
  \item $d \leftarrow\$ \mathbb{B}^{32}$
  \item $z \leftarrow\$ \mathbb{B}^{32}$
  \item \textbf{if} $d = \mathrm{NULL}$ \textbf{or} $z = \mathrm{NULL}$ \textbf{then return} $\bot$
...
\begin{fipsalgo}{Algorithm 13 \texttt{K-PKE.KeyGen}$(d)$（FIPS~203 §5.1；节选）}
...
\subsection{我们前期是怎样一步步探出来的}

这张清单不是一天长出来的。
我们用\textbf{图}把两条时间线画在一起：先长 PKE，再在 PKE 上接 KEM 尾段。
...
只看 Alg.19$\to$16$\to$13，数据流可以画成下面这样（与 §2.2 步骤编号对照）：

\begin{center}
\begin{tikzpicture}[
  node distance=5mm and 10mm,
  var/.style={draw, ellipse, minimum width=1.8cm, minimum height=0.7cm, font=\small},
  op/.style={draw, rectangle, rounded corners=2pt, align=center, font=\small,
             minimum width=2.4cm, minimum height=0.75cm},
  arr/.style={-{Stealth[length=2mm]}}
]
  \node[var] (d) {$d$};
  \node[var, right=of d] (z) {$z$};
  \node[op, below=8mm of d, xshift=8mm] (pke) {Alg.13\\K-PKE.KeyGen};
  \node[var, below=of pke] (ekpke) {$\mathit{ek}_{\mathrm{PKE}}$};
  \node[var, left=12mm of ekpke] (dkpke) {$\mathit{dk}_{\mathrm{PKE}}$};
  \node[op, right=18mm of ekpke] (H) {$H(\mathit{ek})$};
  \node[var, below=of dkpke] (dk) {$\mathit{dk}_{\mathrm{KEM}}$};
  \node[var, right=of ekpke] (ek) {$\mathit{ek}_{\mathrm{KEM}}$};
  ...
  \draw[arr] (H) -- (dk);
  \draw[arr] (z) -- (dk);
  \draw[arr, dashed] (z) to[bend left=15] (pke.south east);
\end{tikzpicture}
\end{center}

注意：NTT、CBD、MMAD 等节点并不只属于 KeyGen——Encrypt、Encaps 也会用到它们。
