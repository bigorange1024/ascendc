...
本节是后文「闭包表」的数学核心。
以下固定有向图 $G=(V,E)$。方法论中取边语义为：
\[
u\to v \quad\text{表示}\quad\text{「$v$ \textbf{依赖} $u$」}
\quad\text{（$u$ 为先决，$v$ 为后继）}.
\]
这与 §2 图二「数据从上游流到下游」的箭头方向\textbf{可能相反}；
\textbf{凡计算 $\mathrm{Dep}^*$，一律以本节依赖语义为准}，示意图须注明。

\subsubsection{边关系的幂、转置与传递闭包}

\begin{definition}[边关系及其复合]
把边集 $E$ 视为 $V$ 上的二元关系。
对关系 $R,S\subseteq V\times V$，定义\textbf{复合}
\[
R\circ S \;=\; \{\,(u,w)\mid \exists v\in V,\; (u,v)\in R,\;(v,w)\in S\,\}.
\]
归纳定义 $E^{1}=E$，$E^{k+1}=E^{k}\circ E$（$k\ge 1$）。
约定 $E^{0}=\Delta_V=\{(v,v)\mid v\in V\}$（恒等关系）。
\end{definition}

\begin{definition}[转置图（反图）]
$G$ 的\textbf{转置图}为 $G^{\top}=(V,E^{\top})$，其中
$E^{\top}=\{(v,u)\mid (u,v)\in E\}$。
直观上：把每条箭头反向。
\end{definition}

\begin{definition}[自反传递闭包]
关系 $E$ 的\textbf{自反传递闭包}为
\[
E^{*} \;=\; \bigcup_{k\ge 0} E^{k}.
\]
其\textbf{传递闭包}为 $E^{+}=\bigcup_{k\ge 1}E^{k}$。
于是 $(u,v)\in E^{*}$ 当且仅当 $u\rightsquigarrow_G v$。
\end{definition}

\begin{proposition}[路径长度与关系幂]
$(u,v)\in E^{k}$ 当且仅当存在长度为 $k$ 的有向途径从 $u$ 到 $v$。
\end{proposition}

\begin{remark}[为何先讲 $E^{*}$]
「$u$ 是 $T$ 的（间接）依赖」精确地说就是 $(u,T)\in E^{*}$。
依赖闭包不过是把该事实改写成集合语言。
\end{remark}

\subsubsection{直接依赖、祖先与依赖闭包}

\begin{definition}[直接前驱 / 直接依赖]
对 $v\in V$，定义
\[
\mathrm{Pred}_G(v) \;=\; N^-(v) \;=\; \{u\in V\mid (u,v)\in E\}.
\]
称 $\mathrm{Pred}_G(v)$ 为 $v$ 的\textbf{直接前驱集}（直接依赖集）。
\end{definition}

\begin{definition}[直接后继]
$\mathrm{Succ}_G(v)=N^+(v)=\{u\in V\mid (v,u)\in E\}$。
\end{definition}

\begin{definition}[祖先与后代]
称 $u$ 为 $v$ 的\textbf{祖先}，若 $u\rightsquigarrow_G v$；
称 $v$ 为 $u$ 的\textbf{后代}，若 $u\rightsquigarrow_G v$。
（按约定，每个顶点是自己的祖先与后代。）
\end{definition}

\begin{definition}[单点依赖闭包]
对目标 $T\in V$，定义其\textbf{依赖闭包}
\[
\mathrm{Dep}^*_G(T)
\;=\;
...
\{u\in V\mid (u,T)\in E^{*}\}.
\]
由定义恒有 $T\in\mathrm{Dep}^*_G(T)$。不致混淆时略去下标 $G$。
\end{definition}

\begin{proposition}[三种等价说法]
对任意 $u,T\in V$，下列命题等价：
\begin{enumerate}
  \item $u\in\mathrm{Dep}^*(T)$；
  \item $(u,T)\in E^{*}$；
  \item 存在路径 $u\to\cdots\to T$，或 $u=T$；
  \item 在转置图 $G^{\top}$ 中 $T\rightsquigarrow_{G^{\top}} u$。
\end{enumerate}
因而 $\mathrm{Dep}^*(T)$ 就是 $T$ 在依赖语义下的\textbf{全体祖先（含自身）}。
\end{proposition}

\subsubsection{迭代构造与算法}
...
\begin{property}[闭包算子的基本代数性质]
把 $\mathrm{Dep}^*$ 视为 $V\to 2^{V}$ 的算子时，有：
\begin{enumerate}
  \item \textbf{扩张性}：$T\in\mathrm{Dep}^*(T)$；
  \item \textbf{单调性}：若从 $T$ 增加可达边使更多点通向 $T$，则闭包不缩小；
        对目标集推广后（见下）亦有 $A\subseteq B\Rightarrow \mathrm{Dep}^*(A)\subseteq\mathrm{Dep}^*(B)$；
  \item \textbf{幂等性}：$\mathrm{Dep}^*(\mathrm{Dep}^*(T))=\mathrm{Dep}^*(T)$
        （祖先的祖先仍是祖先；形式表述见目标集版本）。
\end{enumerate}
\end{property}
...
\begin{proposition}[闭包算子公理]
映射 $\mathrm{Cl}:2^{V}\to 2^{V}$，$\mathrm{Cl}(A)=\mathrm{Dep}^*(A)$ 满足：
\begin{enumerate}
  \item $A\subseteq \mathrm{Cl}(A)$（扩张）；
  \item $A\subseteq B \Rightarrow \mathrm{Cl}(A)\subseteq\mathrm{Cl}(B)$（单调）；
  \item $\mathrm{Cl}(\mathrm{Cl}(A))=\mathrm{Cl}(A)$（幂等）。
\end{enumerate}
因而 $\mathrm{Cl}$ 是集合上的一个\textbf{闭包算子}。
\end{proposition}
