# FIPS 203 ML-KEM-512 Alg.21 ML-KEM.Decaps — baseline-registry（骨架）

**主题**：Alg.21 Decaps（含 CT）（k=2）交付/预研侧 golden / KAT 计算块登记  
**适用（目标）**：`examples/incubating/ml-kem/ml-kem-512/exp-fips203-mlkem-kem-decaps-k2（及 `-ct`）/`  
...
| 角色 | 尺寸 | 说明 |
|------|------|------|
| 输入 `dk_kem`/`c` | 1632+768 | |
| 输出 `K` | **32 B** | accept / reject |
...
| FO / J(z‖c) / CT | 合法与拒绝 | 待 D21/D21ct 获证后登记 | 待补 |
