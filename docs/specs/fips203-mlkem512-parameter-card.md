# FIPS 203 ML-KEM-512 参数卡（P0 草案）

**状态**：**草案待锁**（2026-07-27）· **未实现**  
**参数组**：ML-KEM-512（\(k=2\)）  
**范围**：待用户确认 §0 决议后锁定；本阶段默认 **不建** stable-512  
...
> 与 [计划 §9](../research/MLKEM-512-从0到exp完整实现计划.md) 同步。确认方式：勾选下列 `- [ ]`，或对话回复 `§9 全部按草案默认`。

- [ ] **#1 单 AI Core** — **S-1**：单 cube（MIX `blockDim=1`）；cube 内可用 2 AIV；禁跨 cube 切 poly
- [ ] **#2 噪声批** — **T-B2**：polyvec4（\(s‖e\)）；\(\hat A\)（\(2\times2\)）独立 prep；**禁零垫**
- [ ] **#3 KEM device** — **保留** D19–D21
- [ ] **#4 reject / CT** — **要求** device-ct + incubating-ct
- [ ] **#5 PKE exp** — **要做**（三个 `exp-…-pke-*-k2`）
- [ ] **#6 命名后缀** — **`-k2`**
- [ ] **#7 liboqs-512 交叉** — **本阶段必达**
- [ ] **#8 stable-512** — **本阶段不建**
- [ ] **#9 自主推进** — 锁本卡后 Agent 按计划推进至有条件完成（停问点见计划 §0.1）

附加默认（随 #1–#9 一并锁定）：

| 项 | 锁定草案 |
|----|----------|
| Compress/ByteEncode | **C-1**：512 树自建，默认 \(d\in\{10,4\}\)（密钥域 d=12） |
