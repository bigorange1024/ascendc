...
| 计算块 | 用途 | 已验证来源 | 状态 |
|--------|------|------------|------|
| liboqs `ML-KEM-768` oracle | 可选 KAT / 交付级外部 oracle | `OQS_KEM_ml_kem_768_*` / 拟 `liboqs_kem_ref` | **未验证胶水**（后续 P3 补；E19 不依赖该项 claim） |
...
每通过一块 → 把「未验证」改为具体路径与提交；进 INDEX 时同步日期。本表当前仅支撑 768 E19 incubating 预研，不表示 stable-768 可晋级。
