# ML-KEM-768 P1：缺项对照（补洞图）与必建用例表（已定稿）
...
以教材为准：**缺项** = 闭包中尚未获证的先决；**补洞** = 把缺项变成已获证的过程；本文「补洞图」= 缺项与补齐策略对照表。  
**禁止**另立「洞 / 新洞 / 洞级」等说法。
...
| 符号 | 含义 |
|------|------|
| **P0–P3** | 方法论主阶段（文书 → 缺项对照/补洞图 → 探针实现 → incubating 闭环） |
...
## 1. 相对 ML-KEM-1024 的缺项对照（补洞图）

| 能力块 | 1024（参考模式） | 768 策略（补洞） | 备注（高/中/低，非正式） |
|--------|------------------|----------|------|
