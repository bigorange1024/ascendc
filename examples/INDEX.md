...
| [stable-fips203-mlkem-kem-decaps-k4/](stable/stable-fips203-mlkem-kem-decaps-k4/) | FIPS 203 **Alg.21 ML-KEM-1024 KEM Decaps**（k=4）：`dk`+`c`→**仅** `K`（设备 G/FO） | **T19i SIM 3** / CPU 6；tick **1050620**（CPU/SIM/KAT/roundtrip ✓） | **定型交付算子**（`scripts/` 默认） |
| [stable-fips203-mlkem-kem-decaps-ct-k4/](stable/stable-fips203-mlkem-kem-decaps-ct-k4/) | FIPS 203 **Alg.21 ML-KEM-1024 KEM Decaps**（k=4，CT 专题副本）：`dk`+`c`→**仅** `K`（设备 FO） | SIM `decaps_2session`；D**286829**+E**763658**（CPU/SIM ✓） | **CT 实验副本**（`research/formal-lang-dag`；非 scripts 默认） |
