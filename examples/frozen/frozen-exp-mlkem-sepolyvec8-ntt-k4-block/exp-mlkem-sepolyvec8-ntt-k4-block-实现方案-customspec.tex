...
Alg.13 行 16–17 之前的编码（及 \texttt{exp-fips203-mlkem-pke-stage1-encode-vec}）规定左矩阵 \texttt{mat\_a [16,256] int8} 行序为：
\[
...
\begin{itemize}
  ...
  \item 选用块紧凑布局的理由：与 Alg.13 行 16–17 前编码（\texttt{mat\_a [HI(8),LO(8)]}）、\texttt{exp-fips203-mlkem-pke-stage1-encode-vec}、\texttt{ntt\_study} F203 交付语义一致；避免 KeyGen 流水线中 Stage1 encode 与 Stage2/NTT 混用两种行序。
...
\begin{itemize}
  ...
  \item Stage1 encode：\texttt{exp-fips203-mlkem-pke-stage1-encode-vec-实现方案-customspec.pdf}
