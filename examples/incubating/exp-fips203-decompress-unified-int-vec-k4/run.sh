#!/usr/bin/env bash
# exp-fips203-decompress-unified-int-vec-k4 — 统一整数舍入 Decompress_d（d=1/4/5/10/11）
#
# 规格：exp-fips203-decompress-unified-int-vec-k4-实现方案-customspec.pdf
# 默认 DECOMPRESS_UNIFIED_INT_VEC=1（int32 全向量 Muls+Adds+ShiftRight）。
...
#   DECOMPRESS_UNIFIED_INT_VEC — 0/1（默认 1）
...
export DECOMPRESS_UNIFIED_INT_VEC="${DECOMPRESS_UNIFIED_INT_VEC:-1}"
...
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} F203_UNIFIED_ROUND_D=${F203_UNIFIED_ROUND_D} DECOMPRESS_UNIFIED_INT_VEC=${DECOMPRESS_UNIFIED_INT_VEC}"
...
cmake -B build \
    ...
    -DDECOMPRESS_UNIFIED_INT_VEC="${DECOMPRESS_UNIFIED_INT_VEC}"
cmake --build build -j
cmake --install build
...
echo "[SUCCESS] exp-fips203-decompress-unified-int-vec-k4 d=${F203_UNIFIED_ROUND_D} (${RUN_MODE})"
