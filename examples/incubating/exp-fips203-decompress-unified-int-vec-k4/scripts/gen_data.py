#!/usr/bin/env python3
"""gen_data — 统一整数 Decompress_d：随机 d-bit comp + C ref golden（d=1/4/5/10/11）。"""
import os
import struct
import subprocess
...
_REF_C = os.path.join(_CASE_DIR, "decompress_unified_int_ref.c")
_REF_SO = os.path.join(_CASE_DIR, "scripts", "libdecompress_unified_int_ref.so")
...
def _decompress_ref(comp: np.ndarray) -> np.ndarray:
    _build_ref_so()
    lib = CDLL(_REF_SO)
    out = np.zeros(N, dtype=np.int32)
    fn = lib.poly_decompress_unified_ref
