...
\begin{itemize}
  ...
  \item $h=H(\mathrm{ek})$、$r$（$\equiv$\texttt{coins}，$G$ 后半）\textbf{仅} device
        UB/workspace；\textbf{禁止} Host 预填 \texttt{coins.bin}；\textbf{禁止}将
        $h$/$r$/$a\_hat$/$re$ 等中间态作为交付契约落盘。
\end{itemize}
...
\begin{itemize}
  ...
  \item Host \texttt{tiny\_sha3} / liboqs 参与\textbf{生产}路径的 $H$/$G$（KAT /
        golden 脚本除外）；\textbf{禁止} Host 算 $H$/$G$/\texttt{coins} 冒充设备。
  \item 从 \dirname{ascendc-tests/frozen/} 或 correctness \texttt{vendor/}（G5）
        \textbf{抄实现} / \texttt{vendor\_sync}。
  \item 为 KEM 头增加独立 launch；Host 预填 \texttt{coins.bin}；将 $h$/$r$ 落盘为交付物。
\end{itemize}
...
Prep Â / CBD & 与 stable Encrypt：\texttt{F203\_AHAT16\_BLOCK\_DIM=2}；
               双 AIV 分片 SampleNTT；PRF+CBD$_{\eta=2}$ 产 $r\Vert e_1\Vert e_2$
               （$r$ 种子 $=\texttt{coins}$，$G$ 设备写出） \\
...
\textbf{禁止}将「Host 注入 coins」「关 ByteEncode」「分段只验 Encrypt」等作为默认
...
\begin{align*}
  ...
  c &\leftarrow \texttt{K-PKE.Encrypt}(\mathrm{ek},\,m,\,r)
    && \text{Alg.14；$r\equiv\texttt{coins}$} \\
  &\textbf{输出}\ (K,\,c)
\end{align*}
...
\begin{itemize}
  ...
  \item $r$/\texttt{coins}：\textbf{仅} device 写 workspace GM；Launch-2 与 prep CBD
        消费；\textbf{不}落盘为交付物。
...
\midrule
生产输入 & $\mathrm{ek}$+$m$+\texttt{coins} &
  $\mathrm{ek}$+$m$（\textbf{无} Host \texttt{coins}） \\
\texttt{coins}/$r$ & Host 预填 & 设备 $G$ 后半写出 \\
...
（头+$a\_hat$+$re$）；\#2--\#5 同 Encrypt（NTT / 内积 / INTT / pack）。
...
\texttt{coins} & 可能 Host/oracle & \textbf{仅} device $G$ \\
...
\texttt{coins\_gm} & 32 & 头写；$= r$；prep CBD 读；\textbf{不}D2H \\
\texttt{a\_hat} / \texttt{re} & 同 Encrypt & L1 写 / L2 读；禁落盘 \\
\texttt{ws}+LUT & 同 Encrypt & L2 \\
\texttt{c\_gm} & 1568 & L2 写；D2H \\
\bottomrule
\end{longtable}

\textbf{不新增}：\texttt{h\_gm}、独立 Host \texttt{coins.bin} 输入契约。
...
\begin{enumerate}
  ...
  \item 写 \texttt{K\_gm}、$r\to\texttt{coins\_gm}$（标量逐字节或小块 \texttt{DataCopy}）。
\end{enumerate}
...
\begin{itemize}
  \item SIM/NPU：\texttt{GetBlockIdx()==0} 先完成 \texttt{KemEncInitHead}，再与
        ...
        SampleNTT 只依赖 $\mathrm{ek}$ 尾 $\rho$，\textbf{不}依赖 \texttt{coins}；
        CBD/PRF \textbf{仅} block0，故须保证 block0 在 CBD 前已写完 \texttt{coins\_gm}。
  ...
  \item \textbf{不}要求 KeyGen 式 Encode 后 \texttt{SyncAll}（本用例无双 AIV 分片写
        再由他核读完整 sk 的场景）；但本核写 \texttt{coins} 后、CBD 读前须
...
条款 & 要求 \\
\midrule
禁 Host 伪头 & 禁止 Host SHA3 写 \texttt{K}/\texttt{coins} 再 launch Encrypt 冒充 Encaps \\
禁预填 coins & 生产 \texttt{gen\_data} \textbf{不得}写 \texttt{input/coins.bin} 契约 \\
...
Host 预填 \texttt{coins} + 纯 Encrypt & 非 Alg.17 设备 Encaps；\textbf{禁止}作生产 \\
...
$G(m\Vert h)\to K\Vert r$ & SHA3-512 共享库 & 标量 & 写 \texttt{K\_gm}/\texttt{coins} \\
...
G1 & 固定 \texttt{ek}/$m$：头后 \texttt{coins}/$K$ 可对照 host tiny\_sha3（调试） \\
