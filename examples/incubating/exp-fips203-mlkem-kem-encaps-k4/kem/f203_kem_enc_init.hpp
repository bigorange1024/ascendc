 ...
 * @brief FIPS 203 Alg.17 设备头：读外部 m，算 H(ek)/G(m‖h)，写 K 与 r。
 *
 * 并入 prep 入口前段（不另开 launch）。
 * 背景：customspec 锁定 m 为 GM 输入；r 禁止 Host 预填。
 ...
 * @param m_gm   输入 32B 消息 m（外部读入；生产勿导出）
 * @param K_gm   输出共享秘密 K（32B）
 * @param r_gm   输出 r=G 后半（32B；喂 Encrypt prep PRF+CBD）
 */
