#!/bin/bash
# exp-fips203-mlkem-pke-stage3-routea-mod-vec: Stage3 向量预研（cpu / sim / npu）
# Usage:
...
compare_golden() {
    ...
    md5sum output/out_gm.bin output/golden.bin
    if ! cmp -s output/out_gm.bin output/golden.bin; then
        echo "[FAILED] output differs from golden (profile=${profile})"
        exit 1
    fi
    python3 scripts/verify_result.py output/out_gm.bin output/golden.bin
}
