% exp-mlkem-f203-alg13-16171820-2s1e-k4 — Alg.13 行 16–20（2s1e UB 融合，FIPS 合规采样）
% 编译：bash ../../../../scripts/xelatex-clean.sh exp-mlkem-f203-alg13-16171820-2s1e-k4-实现方案-customspec.tex
...
\title{exp-mlkem-f203-alg13-16171820-2s1e-k4\\FIPS 203 Alg.13 行 16–20：2s1e MIX + UB 融合\\（FIPS 合规 $\mathbf{s}$/$\mathbf{e}$ 采样版）}
...
\begin{abstract}
本文档为 \dirname{examples/incubating/exp-mlkem-f203-alg13-16171820-2s1e-k4/} 的\textbf{实现规格说明书}（customspec）。
...
\textbf{工程壳}以已调通探针
\dirname{ascendc-tests/pass-fix-f203-2s1e-alg13-16171820-vec-k4-v2/} 为\textbf{唯一 AscendC 实现参照}（平面 \texttt{mat\_c}、单 TPipe UB 融合、\texttt{stageHatDotOnly} j$\rightarrow$p）。
...
项 & 内容 \\
\midrule
用例路径 & \dirname{examples/incubating/exp-mlkem-f203-alg13-16171820-2s1e-k4/} \\
规格书 & 本文件 \texttt{*-customspec.tex} / \texttt{.pdf} \\
实现参照 & \dirname{ascendc-tests/pass-fix-f203-2s1e-alg13-16171820-vec-k4-v2/}（\texttt{IMPLEMENTATION\_REFERENCE.md}） \\
原理文档 & \dirname{docs/notes/F203-2s1e-NTT内积UB融合技术总结.md} \\
ByteEncode 原理 & \dirname{docs/notes/F203-ByteEncode12-prefetch技术总结.md} \\
探针（encode-only） & \dirname{ascendc-tests/pass-fix-f203-2s1e-byteencode12-vec-k4/} \\
...
\subsection{与探针 \dirname{vec-k4-v2} 的差异}
\begin{table}[h]
  \centering
...
\begin{enumerate}
  \item \textbf{壳}：自 \dirname{ascendc-tests/pass-fix-f203-2s1e-alg13-16171820-vec-k4-v2/} 复制
...
\begin{itemize}
  \item 探针：\dirname{ascendc-tests/pass-fix-f203-2s1e-alg13-16171820-vec-k4-v2/IMPLEMENTATION\_REFERENCE.md}
