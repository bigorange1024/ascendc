% exp-mlkem-f203-pke-encrypt-k4 — FIPS 203 Alg.14 ML-KEM-1024 PKE Encrypt (k=4)
% 编译：bash ../../../scripts/xelatex-clean.sh exp-mlkem-f203-pke-encrypt-k4-实现方案-customspec.tex
...
\title{exp-mlkem-f203-pke-encrypt-k4\\FIPS 203 Alg.14 ML-KEM-1024 PKE Encrypt 实现方案（k=4）}
\author{ascendc 工程（预研 / 交付示例）}
\date{2026-07-09}
...
\begin{abstract}
本文档描述 \textbf{预研用例} \dirname{examples/incubating/exp-mlkem-f203-pke-encrypt-k4/}：
...
写法对齐 \dirname{exp-mlkem-f203-pke-keygen-k4-实现方案-customspec.tex}。
\end{abstract}
...
组件 & 锁定配置 \\
\midrule
Prep Â / CBD & 与 prep 探针一致：\texttt{F203\_AHAT16\_BLOCK\_DIM=2}，双 AIV 分片 SampleNTT；PRF+CBD$_{\eta=2}$ 产 $r\Vert e_1\Vert e_2$ \\
...
\begin{verbatim}
exp-mlkem-f203-pke-encrypt-k4/
  exp-mlkem-f203-pke-encrypt-k4-实现方案-customspec.tex/.pdf  # 本规格
...
\dirname{ascendc-tests/pass-fix-f203-alg14-pke-encrypt-device-k4/}。
...
\begin{enumerate}
  ...
  \item 活跃 customspec：\dirname{examples/incubating/exp-mlkem-f203-pke-encrypt-k4/exp-mlkem-f203-pke-encrypt-k4-实现方案-customspec.tex}（及同名 PDF）。
  \item 晋级 \dirname{examples/stable/stable-mlkem-f203-pke-encrypt-k4} 须另走 \texttt{\#交付\#}，且须 baseline-registry 齐备。
\end{enumerate}
