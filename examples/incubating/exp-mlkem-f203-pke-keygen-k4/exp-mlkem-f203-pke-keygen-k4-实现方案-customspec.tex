% exp-mlkem-f203-pke-keygen-k4 — FIPS 203 Alg.13 ML-KEM-768 KeyGen (k=4)
% 编译：bash ../../../scripts/xelatex-clean.sh exp-mlkem-f203-pke-keygen-k4-实现方案-customspec.tex
...
\title{exp-mlkem-f203-pke-keygen-k4\\FIPS 203 Alg.13 ML-KEM-768 PKE KeyGen 实现方案（k=4）}
...
\begin{abstract}
本文档描述 \textbf{交付示例} \dirname{examples/incubating/exp-mlkem-f203-pke-keygen-k4/}：
...
\begin{itemize}
  ...
  \item \textbf{liboqs} 及 \texttt{kat\_liboqs\_vs\_ascendc.sh} —— 可选外部对照，非 \texttt{run.sh} 契约。
...
\midrule
Prep Â & \texttt{F203\_AHAT16\_BLOCK\_DIM=2}，内嵌 SHAKE \texttt{ProcessInline} \\
...
exp-mlkem-f203-pke-keygen-k4/
  ...
  thirdparty/ntt_study/        # Host golden LUT 表（vendored，非 AscendC 编译依赖）
  run.sh
\end{verbatim}
...
\midrule
Prep & AIV\_ONLY；\texttt{blockDim=2}；SIM 无 AIC cycle；block0 重活 \\
...
原理与实验记录见 \dirname{docs/notes/}；编排 macro 对照见 \dirname{f203\_keygen\_prep\_sync.hpp}。
...
\section{性能（SIM 910B4 参考，2026-06-28）}

Total tick $\approx$ \textbf{886801}（prep $\approx$806k + mmad $\approx$81k；单进程 2 launch）。
prep 段 Opt-4 双 AIV + Opt-5 部分 Pipe 窄化；详见 \dirname{docs/notes/F203-KeyGen-prep-Pipe细同步技术总结.md}。
