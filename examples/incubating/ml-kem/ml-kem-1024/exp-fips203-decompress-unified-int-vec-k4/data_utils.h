 ...
 * input 目录下 bin 文件（如 comp.bin）与写出 output 目录下 bin 文件（如 poly.bin），不包含任何
 ...
 * @return read result
 *
 * 中文说明：从 filePath 读取整个文件内容到 buffer（如 input/comp.bin），
 ...
 * @return write result
 *
 * 中文说明：把 buffer 中的 size 字节原样写入 filePath（如 output/poly.bin），
