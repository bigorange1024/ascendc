#!/usr/bin/env bash
# exp-fips203-mlkem-kem-keygen-k4 — Alg.19 ML-KEM.KeyGen（2 launch · vendored PKE + 内嵌 Alg.16 尾）
#
...
#   bash run.sh -r sim -v Ascend910B4          # sim 内默认 SIM_DIRECT=1；勿手写
...
if [ "${KEM_KEYGEN_KAT:-0}" != "1" ]; then
    echo "[SUCCESS] exp-fips203-mlkem-kem-keygen-k4 (${RUN_MODE}) SEED_D=${SEED_D}"
fi
