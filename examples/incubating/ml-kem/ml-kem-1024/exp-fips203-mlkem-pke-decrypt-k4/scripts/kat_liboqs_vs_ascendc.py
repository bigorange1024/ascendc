...
def main() -> int:
    ...
    if not VERBOSE:
        LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
        LOG_PATH.write_text("[kat] exp-decrypt vs fixture\n", encoding="utf-8")
    ...
    print(
        ...
        + " (liboqs keygen + host c → dk/c/m ↔ exp-decrypt output/m)"
    )
    ...
    if sim_count > 0:
        ...
        print(f"[kat] PASS CPU×{cpu_count} + SIM×{sim_count} (exp-decrypt vs fixture)")
    else:
        print(f"[kat] PASS CPU×{cpu_count} (exp-decrypt vs fixture)")
    return 0
