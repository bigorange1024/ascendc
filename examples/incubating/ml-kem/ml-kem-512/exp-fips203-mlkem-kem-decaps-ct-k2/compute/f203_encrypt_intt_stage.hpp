 ...
 * @brief Alg.14 行 19/21：uTr[4] → polyvec4 INTT Stage1–3（ML-KEM-512：u0,u1,tr̂,空槽）。
 *
 * UB 分片（INTT S1 输入，每 AIV 2 行）：
 *   AIV0: uTr[0], uTr[1]
 *   AIV1: tr̂, 空槽
 *
 * Stage3 输出 scatter：
 *   AIV0 local0→u[0], local1→u[1]；AIV1 local0→v（行 21），local1 为空槽丢弃。
 */
...
/**
 * INTT 平面 mat_c 行号（kInttBatch=4 槽）。
...
/**
 * INTT S1：batch=4 几何；loRow 基准为 kInttBatch（非 NTT 的 kK）。
 * 输入：ProcessFromLocal 的 batch4 UB；输出：S0 [8,N] int8。
 */
class AivInttK8Split {
public:
    /**
     * @param subCoreIdx AIV 0/1；@param coeffN 通常 256
     * polyBase_：AIV0→0，AIV1→2（每核 2 行）
     */
...
private:
     ...
     * 与 NTT AivK8Split 的差异：lo 基准用 kInttBatch=4，不是 kK=3。
     */
...
/** INTT S2 后 pack：batch=4 平面 mat_c（loRow 用 kInttBatch）。 */
...
/**
 * INTT S3：平面 mat_c → u[2] + v 分片写回。
 * AIV0: local0/1 → u[0]/u[1]；AIV1: local0 为 tr̂ 时域 → vOut，local1 空槽丢弃。
 */
...
public:
    ...
    /**
     * @param uOut u[2,N]；@param vOut v[N]（仅 AIV1 写）；@param matPlanar 平面 mat_c
     */
    ...
    /** RouteA 合并本核 2 行 → scatter 到 u/v（无 pad 行） */
    __aicore__ inline void Process()
    {
...
private:
    ...
    /**
     * 将 local_dst[lp] scatter 到 u / v GM。
     * AIV0: lp0/1→u[0]/u[1]；AIV1: lp0→v，lp1 为空槽丢弃。
     */
