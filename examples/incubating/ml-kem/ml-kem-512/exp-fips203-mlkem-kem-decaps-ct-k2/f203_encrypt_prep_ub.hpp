 ...
 * @brief Alg.14 Encrypt prep 单 TPipe：ρ→Â（双 AIV 2+2）→ coins→PRF+CBD batch5（block0）。
 *
 * 流水线位置（行 3–15）：
...
/** PRF batch5 的 x/len UB 尺寸（≥ 单 poly SampleNTT 消息缓冲，可复用）。 */
...
/** 从 ek_pke GM 读取 ρ[32]（偏移 kRhoOffset=768）。 */
                                                  ...
                                                  __gm__ uint8_t *prf_out_gm, __gm__ int32_t *re_gm, GM_ADDR tiling_gm)
{
    ...
    if (AscendC::GetBlockIdx() == 0U) {
        ...
        // tiling 由 RunShakePrfEncryptReUbWithUb 自 tiling_gm 加载；每个 nonce 覆盖 batch=1。
        F203EncryptRePrf::RunShakePrfEncryptReUbWithUb(coins, prf_out_gm, tiling_gm, shakeXBuf, shakeLenBuf,
