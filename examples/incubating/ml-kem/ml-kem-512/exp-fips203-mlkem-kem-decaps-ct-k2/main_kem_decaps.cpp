 ...
 *
 * 流水线（Alg.21 Decaps / D21ct；本探针 CT 行为基线）：
 ...
 *
 * SIM Host：生产默认 `ASCENDC_SIM_HOST_MODE=decaps_2session`（D/E 各一 ACL session，CT 锁定）；
 * 排障可设 `decaps_1session`，但不改变本目录默认，也不回改 D21 delivery 探针。
 *
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    if (ascendc::SimHostDecapsUse2Session()) {
        std::fprintf(stderr, "[kem-decaps] SIM host mode=decaps_2session（CT 生产默认）\n");
    } else {
        std::fprintf(stderr, "[kem-decaps] SIM host mode=decaps_1session（排障；非默认）\n");
    }
#endif
