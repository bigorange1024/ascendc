 ...
 * @brief Phase-D Host：launch `f203_decrypt_device_fused`（编译期引用 D15 k2 Decrypt / 本树 shim）。
 *
 * 背景（CT / qa）：
 *   - Alg.18 行 1–4 切分不另开 launch：dk_pke = dk_kem[0:768)；ek/h/z 留给全链 Host 偏移给 Phase-E。
                       ...
                       uint8_t *m_out)
{
    ...
    // 行 1–4：仅把前缀当作 dk_pke 上传（H2D 768B）
