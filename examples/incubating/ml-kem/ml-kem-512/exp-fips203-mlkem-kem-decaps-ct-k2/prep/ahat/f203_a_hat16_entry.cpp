...
/** 独立 Â 探针入口；D14 全链由 f203_encrypt_prep 内联 BuildAHat16ShardWithUb（函数名保留历史 16poly ABI）。 */
// @probe exp-fips203-mlkem-kem-decaps-ct-k2
// @file prep/ahat/f203_a_hat16_entry.cpp
// @layer prep
// @role prep/ahat：设备侧生成 Encrypt 用矩阵 A_hat（FIPS203 Alg.14 行 3–7）；本入口仅供独立调试，全链由 f203_encrypt_prep 调用 UB helper。
...
/**
 * 本文件在 Encrypt prep 子模块中的位置：独立 Â[4,256] 分片调试入口。
 * 对齐：FIPS 203 Alg.14 / ML-KEM-512（k=2）。
 * 与 golden 关系：仅 I/O 等价验收；禁止把 Host/参考源码当作 AscendC 实现规格。
 * 文件：prep/ahat/f203_a_hat16_entry.cpp
 */
/**
 * @file f203_a_hat16_entry.cpp
 * @brief Alg.14 行 3–7 设备核：ρ → 向量 SampleNTT → a_hat[4,256] GM（1 或 2 AIV）。
 */
