 ...
 * @brief Alg.20/17 KEM Encaps host：D14 k2 Encrypt 2/5 launch + prep 前段设备 H/G。
 *
 * 生产 I/O：input/ek_kem.bin + m.bin + LUT → output/c.bin + K.bin。
 * coins/r 由设备 G 写入 workspace，Host 不读 coins.bin。
 * 方案见 INTEGRATION_PLAN.md；Encrypt 编译期引用活跃 D14 k2 绿树。
 */
...
/** PRF(SHAKE256) tiling：与 D14 k2 prep 一致；UB 预留 batch8，实际写 re[5]。 */
...
/**
 * CPU 分段路径：INTT LUT 覆盖写入同一 LUT_NTT_* 区（phased 复用，与 D14 k2 探针一致）。
...
/**
 * Host：CPU 五 launch / SIM 两 launch；产出 c.bin(768B) + K.bin(32B)。
