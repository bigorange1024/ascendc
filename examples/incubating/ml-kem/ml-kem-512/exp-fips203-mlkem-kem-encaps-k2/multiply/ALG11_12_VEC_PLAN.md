# Alg.11/12 向量化实现方案（B6 k2 multiply）
...
**定位**：本探针为 **单次 MultiplyNTTs(f, g)** 的活跃实现，在 B6 k2 中由 `inner/` 作为 UB 计算模块复用。
...
### 0.5 复用到 k2 InnerProduct 的清单

- `inner/` 复用 **`RomUbLuts` + `init_rom_luts_ub` + `alg11_rom_tables`**，每个 AIV 初始化一次 ROM UB。
...
面向 **行 18 全链路**（本目录 `inner/` 的 `P_OUT=S_VEC=2` 内积）。
