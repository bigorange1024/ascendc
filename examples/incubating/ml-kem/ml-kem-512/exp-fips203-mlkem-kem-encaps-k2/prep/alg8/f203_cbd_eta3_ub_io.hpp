/**
 * @file f203_cbd_eta3_ub_io.hpp
 * @brief 默认路径：GM↔UB DataCopy + PipeBarrier + CBD_3 SWAR/LUT（ROWS=4）。
 *
 * 每行流水线：
 *   1. DataCopy 192B PRF 行：GM → UB
 *   2. PipeBarrier<PIPE_MTE2>：等待 CopyIn 完成
 *   3. load24 + SWAR + CBD3 LUT：UB → UB rowLocal[256] int32
...
#define F203_CBD3_SYNC_AFTER_COPYIN() AscendC::PipeBarrier<PIPE_MTE2>()
#define F203_CBD3_SYNC_BEFORE_COPYOUT() AscendC::PipeBarrier<PIPE_V>()
...
/**
 * 单行 CBD_3 的完整 GM↔UB 流水线。
 * @param row      [in]  全局行号，范围 [0,4)
 * @param prfGm    [in]  GM 上的 prf_out 视图，长度 PRF_TOTAL_BYTES=768 uint8
 * @param srcGm    [out] GM 上的 src 视图，长度 SRC_COEFFS=1024 int32
 * @param prfLocal [in/out] VECCALC UB 暂存区，容量至少 192B
 * @param rowQue   [in/out] VECOUT 队列，容量为一行 256×int32
 */
__aicore__ inline void SamplePolyCbd3OneRowUb(uint32_t row, AscendC::GlobalTensor<uint8_t> &prfGm,
                                              AscendC::GlobalTensor<int32_t> &srcGm,
                                              AscendC::LocalTensor<uint8_t> &prfLocal,
                                              AscendC::TQue<AscendC::TPosition::VECOUT, 1> &rowQue)
{
    /* CopyIn：按行搬 192B，192 是 32B 的 6 倍，满足本探针连续 DataCopy 的块对齐。 */
    AscendC::DataCopy(prfLocal, prfGm[row * PRF_BYTES], PRF_BYTES);
    F203_CBD3_SYNC_AFTER_COPYIN();

    /* 计算：每 3 字节产 4 系数；rowQue 只保存当前一行，逐行复用 UB。 */
    AscendC::LocalTensor<int32_t> rowLocal = rowQue.AllocTensor<int32_t>();
    SamplePolyCbd3RowSwLutUb(rowLocal, prfLocal);
    F203_CBD3_SYNC_BEFORE_COPYOUT();
 ...
 */
__aicore__ inline void SamplePolyCbd3Batch4WithUb(uint32_t blockIdx, __gm__ const uint8_t *prf_gm,
                                                   ...
                                                   AscendC::TQue<AscendC::TPosition::VECOUT, 1> &rowQue)
{
    ...
    for (uint32_t row = 0; row < ROWS; ++row) {
        SamplePolyCbd3OneRowUb(row, prfGm, srcGm, prfLocal, rowQue);
    }
#else
 ...
 * @param prf_gm   GM 输入，prf_out[4,192] uint8
 * @param src_gm   GM 输出，src[4,256] int32
 */
__aicore__ inline void SamplePolyCbd3Batch4DataCopy(uint32_t blockIdx, __gm__ const uint8_t *prf_gm,
                                                    __gm__ int32_t *src_gm)
{
#if F203_CBD_BLOCK_DIM == 1
    if (blockIdx != 0U) {
    ...
    SamplePolyCbd3Batch4WithUb(blockIdx, prf_gm, src_gm, scratchBuf, rowQue);
}
