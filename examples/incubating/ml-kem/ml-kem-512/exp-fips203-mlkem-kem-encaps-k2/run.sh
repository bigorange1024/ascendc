#!/usr/bin/env bash
# exp-fips203-mlkem-kem-encaps-k2 — Alg.20 Encaps（prep 前段 H/G + D14 k2 Encrypt）
#
# 生产 I/O：input/{ek_kem,m,lut_*} → output/{c,K}.bin；coins 设备自产，不读 Host coins 为权威。
# PKE：编译期引用 ascendc-tests/ml-kem/ml-kem-512/exp-fips203-mlkem-kem-encaps-k2
#
# Usage（默认全量）:
...
if [ "${KEM_ENCAPS_KAT:-0}" != "1" ]; then
    echo "[SUCCESS] exp-fips203-mlkem-kem-encaps-k2 (${RUN_MODE})"
fi
