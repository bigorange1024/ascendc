 ...
 *
 * ## 对齐与 golden
 * ML-KEM-512（k=2）；验收仅 I/O 等价于 `keygen_golden.py`，不要求与 Host 源码同构。
 *
 * ## 流水线（mixPass=0 生产路径）
 *
 *   host src[4,256] + LUT + a_hat
 *     → [AIV] AivK4Split          Stage1：polyvec4 limb6 拆分 → workspace S0
 *     → [AIC] AicMmad ×4          Stage2：int8 MMAD（even/odd × lo/hi）
 *     → [AIV] AivK4PackMatCPlanar    竖堆临时 → 平面 mat_c [32,128]
 *     → [AIV] Aiv2s1eUbPipeline   Stage3 merge/mod + 行18 + 行19–20
 *     → [AIV0] FuseEkPke          行 21 ek_polyvec ‖ ρ（F203_KEYGEN_EK_PKE=1）
 *     → [AIV0] KemKgTailFused     D19 专用：H(ek)+z+dk_kem 拼接（F203_KEM_KEYGEN_TAIL=1）
 *
 ...
 * ## Host 数据语义
 *
 *   - src 行 0..1：ŝ；行 2..3：ê（prep CBD_η=3 输出）。
 *   - Stage3 后按真实 polyvec4 写 dst[0..3]，SyncAll 后每个 AIV 读取完整 ŝ[0..1]。
 ...
 * @param dst     输出/中间：NTT 后 ŝ/ê [4,256] int32
 * @param t_hat   输出/中间：t̂ [2,256] int32
 * @param ek_out  输出：ByteEncode₁₂(t̂) 768B（亦作 Fuse 输入）
 * @param sk_out  输出：ByteEncode₁₂(ŝ) 即 dk_pke 768B
 * @param src     输入：prep 的 ŝ‖ê [4,256] int32
 * @param a_hat   输入：Â[4,256] int32
 ...
 * @param seed_d_gm / dk_kem_gm 仅 D19：seed_d 与 dk_kem 输出；尾段复用 ek_pke/sk_out
                                                  ...
                                                  )
{
    ...
    /* ========== mixPass=6：AicMmad sanity（tag5t 同维 14×254×192，单路 lo_even）========== */
    if (mixPass == 6) {
        if (AIC) {
            STATE = AIC_MMAD;
    ...
    if (AIC) {
        ...
        STATE = AIC_MMAD;
        /** MMAD 仅消费 S0 有效行 mRowsLogic(8)；Cube 内部 m 参数按 16 对齐，非假 poly。 */
        ...
        if (runS1) {
            STATE = AIV_SPLIT;
            AivK4Split split(subBlockID, coeffN);
        ...
        if (runS2) {
            STATE = AIV_PACK;
            WAIT
            AivK4PackMatCPlanar pack(subBlockID, coeffN);
...
#if F203_KEYGEN_EK_PKE >= 1
    if (!AIC && runEncode) {
        /* 双 AIV 均写完 ek/sk 分片后再让 AIV0 读取完整 k2 ek_polyvec，避免读取未完成的 p1。 */
        AscendC::SyncAll</*isAIVOnly=*/true>();
    }
    /* KeyGen：行 21 ek_PKE = ek_polyvec ‖ ρ；双 AIV 编码分片完成后由 AIV0 融合。 */
    if (!AIC && subBlockID == 0 && runEncode) {
        F203KeygenEkPke::FuseEkPke(ek_out, rho_gm, ek_pke_gm);
        KYBER_PIPE_ALL();
#if F203_KEM_KEYGEN_TAIL >= 1
        /*
         * D19 尾段：Alg.16 要求 dk_kem = dk_pke‖ek‖H(ek)‖z。
         * 此处紧跟 FuseEkPke 之后执行，仍在 compute MIX launch 内，无第三次 kernel。
         */
