 ...
 *
 * 流水线位置（device-k2，2 launch）：
 *   L1 D13 k2 f203_keygen_prep  — Alg.13 行 3–15（d 在 device UB）
 *   L2 D13 k2 mmad_custom       — Alg.13 行 16–21 + 本尾段（F203_KEM_KEYGEN_TAIL=1）
 *
 * 在 FuseEkPke 已写出 ek_pke_gm / dk_pke_gm（sk_out）之后，本函数完成 Alg.16 剩余步骤：
 *   1) UB 内派生 z（DerandZFromSeedD，或旁路 A 读 kem_seed[32:64]）；
 *   2) H(ek) = SHA3-256(ek_pke)，输入 800B，输出 32B；
 *   3) 按 liboqs 展开布局拼接 dk_kem_gm = dk_pke ‖ ek ‖ H(ek) ‖ z（1632B）。
 *
 ...
 *
 * 本 k2 首版正确性验收 SIM tick 为 510775；I/O 与 D13 PKE 段一致，KEM 尾按 768 布局拼接。
 *
 * ## SHA3-256 替换指南（日后第三方 AscendC 实现）
 *
 * 本文件仅 **一处** 哈希调用，须保持语义：
 *   `Sha3OneShot(h, 32, ek_ub, kEkKemBytes)` — 对 800B ek 做 FIPS SHA3-256，输出 32B。
 *
 * 替换步骤：
 ...
 * @param ek_pke_gm  GM：800B ek_PKE，亦为 ek_kem 别名（只读）
 * @param dk_pke_gm  GM：768B PKE 私钥（= mmad sk_out / ByteEncode₁₂(ŝ)）
 * @param dk_kem_gm  GM：1632B 输出，liboqs 展开布局
 */
                                      ...
                                      __gm__ uint8_t *dk_kem_gm)
{
    ...
    // —— 步骤 2：H(ek) = SHA3-256(ek_pke[800]) → h[32] ——
    ...
    // 【SHA3 替换点 #2】mdlen=32 → SHA3-256；输入长 kEkKemBytes=800
