 ...
 *   而是复用其构件（BuildAHat16ShardWithUb / RunShakePrfBatchUbWithUb / SamplePolyCbd3Batch4WithUb）
 *   与常量；仅复制 ~40 行编排以替换 d 的派生点。若 D13 k2 prep 编排变更，本变体需同步。
 *
 * 数学契约：G(d‖byte(k=2)) = ρ‖σ（一次 SHA3-512，与 F203Alg7::HashGFull 同式）；
 ...
 * @param blockIdx   逻辑分片：0→poly 0–1，1→2–3；PRF/CBD 仅 block0 执行
                                                     ...
                                                     __gm__ int32_t *src_gm, __gm__ uint8_t *rho_gm, GM_ADDR tiling_gm)
{
    ...
    // 旁路 A 差异点：ρ‖σ = G(d_ext‖byte(k=2))，一次 SHA3-512（替代 BuildRhoSigmaFromSeedD 的
    ...
    // 双 AIV：blockIdx 0→poly 0–1、1→2–3 并行写 Â（同生产路径）
    ...
    if (AscendC::GetBlockIdx() == 0U) {
        ...
        // ML-KEM-512 prep 仅采样 2s+2e=4 个 η=3 多项式，禁止沿用 k3/k4 的行数或 η=2 路径。
