 ...
 * @brief FIPS 203 Alg.19 ML-KEM.KeyGen host 驱动（device-k2：2 launch 全链）。
 *
 * 设备全链（2 launch，Alg.19→Alg.16→Alg.13 同 session，无第 3 launch）：
 *   L1 f203_keygen_prep  — D13 k2 Alg.13 行 3–15（d 在 device UB；源码编译期引用本目录 D13 k2 副本）
 *   L2 mmad_custom       — D13 k2 Alg.13 行 16–21 + 内嵌 Alg.16 尾（F203_KEM_KEYGEN_TAIL=1）
 *
 * 工程接线（W3/D19，2026-07-26）：
 *   - L2 编译 本目录 `compute/mmad_custom.cpp`（复制自活跃 D13 k2），尾段在 kem/f203_kem_kg_tail_fuse.hpp；
 *   - ek_kem 与 ek_pke 共用 ekPkeGm/ekPkeDev，零 GM 拷贝；
 *   - 相对 D13，D19 仅追加 H(ek)、z 与 dk_kem 拼接；PKE 几何保持 k2 锁定值。
 *
 * 生产 I/O：input/seed_d.bin(4B)+LUT → output/ek_kem.bin(800)+dk_kem.bin(1632)。
 *
 * L2 mmad 实参（KEM 构建）：除 D13 k2 PKE 外追加 seed_d_gm、dk_kem_gm（见 D13 k2 mmad 宏文档）。
 */
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    std::cout << "[main] kem_keygen device-k2 SEED_D=" << seed_d << " prep_blockDim=" << kPrepBlockDim
    ...
    // L2：D13 k2 mmad + F203_KEM_KEYGEN_TAIL 内嵌尾；ek 与 ek_kem 同缓冲
