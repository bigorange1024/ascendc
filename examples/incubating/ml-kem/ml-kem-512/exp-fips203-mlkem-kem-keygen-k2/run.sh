#!/usr/bin/env bash
# exp-fips203-mlkem-kem-keygen-k2 — Alg.19 ML-KEM.KeyGen（2 launch · D13 k2 PKE + 内嵌 Alg.16 尾）
#
# 生产 I/O：
...
# 自包含：本目录已 vendored D13 k2 PKE 主体；不再软链 ascendc-tests 探针。
D13_KEYGEN="${CURRENT_DIR}"
...
if [ "${KEM_KEYGEN_KAT:-0}" != "1" ]; then
    echo "[SUCCESS] exp-fips203-mlkem-kem-keygen-k2 (${RUN_MODE}) SEED_D=${SEED_D}"
fi
