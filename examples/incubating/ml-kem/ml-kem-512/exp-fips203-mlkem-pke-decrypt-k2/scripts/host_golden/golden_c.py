...
"""
@file golden_c.py
@brief Alg.14 全链 host golden：ek,m,coins → c.bin（768B）。
...
PRF_BYTES = 128
PRF_BATCH = 2 * K + 1  # r(k)+e₁(k)+e₂(1)
EK_T_BYTES = K * 384  # ByteEncode₁₂(t̂) 不含 ρ
EK_BYTES = EK_T_BYTES + 32
CT_BYTES = K * 320 + 128
...
def decode_t_hat(ek: bytes) -> np.ndarray:
    """
    行 2：从 ek_pke 前 K×384B 解出 t̂[K,N]。
    @param ek  完整 ek_pke（ML-KEM-512 为 800B）；本函数只用前 EK_T_BYTES
    """
...
def golden_encrypt(ek: bytes, m: bytes, coins: bytes) -> bytes:
    """
    Alg.14 全链参考：ek+m+coins → c（768B）。
    分段：decode → SampleNTT/CBD → NTT(r) → û/tr̂ → INTT+噪声 → μ 嵌入 → pack。
    """
    # --- 行 2–3：拆 ρ / t̂ ---
    rho = ek[EK_T_BYTES:EK_BYTES]
