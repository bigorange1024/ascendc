 ...
 * @brief Alg.14 行 2/16–24：ML-KEM-512 **SIM 单 launch** 融合 MIX + μ 折叠 + 内联 tail pack。
 *
 ...
 *   行 16–17 NTT(r) → 行 2 decode → 行 18 uTr polyvec4(u0,u1,tr̂,空) → GATE → 行 19/21 INTT → pack
 */
 ...
 * @param mRows NTT 用 nttMRowsLogic(6)，INTT batch4 用 inttMRowsLogic(8)
 */
                                                           ...
                                                           GM_ADDR traceGm)
{
    ...
    if (aic) {
        ...
        AscendC::PipeBarrier<PIPE_ALL>();
        // e₂ GM 由 AIV0 原地叠加 μ；AIV1 后续负责 vOut += e₂ 和 c₂ pack。
        // PipeBarrier 只约束本核，双 AIV 需硬同步后才能让 AIV1 读取最终 e₂。
        ...
        /* ── 行 18/19：k2 内积 uTr polyvec4 驻留 UB（u0,u1,tr̂,空）→ INTT ── */
        {
        ...
        if (subBlockID == 1 && e2 != nullptr && vOut != nullptr) {
            // vOut 由本 AIV1 scatter 得到；继续在本 AIV 叠加 e₂+μ，避免 AIV0/AIV1 读写竞态。
