...
\begin{enumerate}
  ...
  \item 行 8--15：从 \texttt{coins} 采样 \(r\Vert e_1\Vert e_2\) 共 5 个 poly（GM 锁定 \texttt{[5,256]}）：
        \(r\) 用 \(\eta_1=3\)（PRF 192B/行），\(e_1\Vert e_2\) 用 \(\eta_2=2\)（PRF 128B/行；GM 行 stride 统一 192B，XOF 前缀性质）。
        背景：2026-07-27 glue-c 相对 liboqs Encaps \(c\) 缺项根因曾为 5 行全走 \(\eta=2\)；按参数卡补缺。
...
采样 \(r\Vert e_1\Vert e_2[5]\) & AIV\_ONLY + CBD \(\eta_1{=}3\)/\(\eta_2{=}2\) & 100\% 设备 & \(r\) 2 行 η1；$e$ 3 行 η2 \\
