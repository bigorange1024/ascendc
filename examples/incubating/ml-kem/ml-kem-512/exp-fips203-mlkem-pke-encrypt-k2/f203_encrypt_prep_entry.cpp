 ...
 *
 * 流水线位置：
 *   行 3–7：ρ = ek_pke[768:800] → SampleNTT → a_hat[4,256]
 *   行 8–15：coins → PRF+CBD(η=2) batch5 → re[5,256]（nonce 0–1=r, 2–3=e₁, 4=e₂）
 *
 * 编排：双 AIV 并行 Â 分片（block0→poly 0–1，block1→2–3）；PRF/CBD 仅 block0。
 * CPU tikicpu：blockDim=2 时单 block 串行跑两次分片（见 ASCENDC_CPU_DEBUG 分支）。
 * 代码来源：stable KeyGen prep vendoring；禁止抄 correctness 全链。
 *
 * 与 golden：main 写 a_hat.bin / re.bin，scripts/gen_data.py 生成 c golden 并由 run.sh 对拍。
 */
 ...
 * @param tiling_gm         SHAKE tiling（PRF 单 nonce 覆盖 batch=1）
 */
