...
def decode_t_hat(ek: bytes) -> np.ndarray:
    """
    行 2：从 ek_pke 前 768B 解出 t̂[K,N]。
    @param ek  完整 ek_pke（800B）；本函数只用前 EK_T_BYTES
    """
...
def golden_encrypt(ek: bytes, m: bytes, coins: bytes) -> bytes:
    ...
    # --- 行 19：INTT + e₁；tr 单 poly经语义 Stage123 变换；device 侧用 polyvec4 空槽 ---
