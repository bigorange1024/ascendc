...
public:
    ...
    __aicore__ inline void Init(GM_ADDR matPlanar, GM_ADDR a_hat, GM_ADDR ek_out, GM_ADDR sk_out, GM_ADDR dst_dump,
                                GM_ADDR t_dump)
    {
        /* GM 绑定：平面 mat_c、a_hat[4,256]、ek/sk 输出、对拍 dump dst/t_hat */
         ...
         *   ubNttBuf_     — ŝ[0..1] + ê 本核最多 2 行（AIV0=1，AIV1=1）
                                   ...
                                   bool loadThatPreset)
    {
        ...
        /* k2 的 NTT 共享 dst 已在 stageS3Into 按全局行写入；这里不再按本地 UB 布局覆盖。 */
...
private:
    ...
    /** mixPass 4/7 或 S3 后共享：从 dst_dump 灌完整 ŝ[0..1] + 本核 ê[p]，跳过/接续 S3。 */
    __aicore__ inline void loadNttPresetInto(LocalTensor<int32_t> &ub_ntt)
    {
        // k2 polyvec4 NTT 的物理分片为 AIV0:s0,s1 / AIV1:e0,e1；
        // 行18 要求每个 AIV 都有完整 ŝ，因此从 GM 共享区统一读 dst[0..1]。
    ...
    /** S3：本核按连续 polyvec4 slot merge 后写入 dst GM；SyncAll 后重装完整 ŝ + 本核 ê。 */
    __aicore__ inline void stageS3Into(LocalTensor<int32_t> &ub_ntt)
    {
        ...
        KYBER_PIPE_ALL();
        /*
         * 背景：k2 D13 的 B5 NTT 锁定为连续 polyvec4，AIV1 只本地得到 e0,e1；
         * 结论：两 AIV 先把各自 3 poly 写入 dst GM，再硬同步，随后都读取完整 ŝ[0..1]。
         * 未采用：为复用 k4 而复制 ŝ 到两个 AIV 或把 polyvec4 pad 成 8 行。
         */
