 ...
 *
 * ## 流水线位置
 * 行 3–15：Â[4×256] + ŝ/ê[4×256] + ρ[32] + PRF 中间态；
 * Host `main_keygen` 与设备 `BuildKeygenPrepSinglePipe` 共用本命名空间尺寸。
 *
 * ## 对齐
 * FIPS 203 Alg.13，ML-KEM-512（k=2，N=256）；与 golden 中间态形状一致（生产默认不落盘）。
 *
 * ## 分核
 * F203_AHAT16_BLOCK_DIM=2：双 AIV 按 2+2 写 4 个 Â poly；PRF/CBD 仅 block0。
 */
#pragma once
...
/** 与 `F203_AHAT16_BLOCK_DIM` 一致：1=单 AIV 4 poly；2=双 AIV 2+2（D13 锁定）。 */
