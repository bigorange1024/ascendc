...
/**
 * @file main_keygen.cpp
 * @brief FIPS 203 Alg.13 PKE KeyGen 全链 Host 入口（ML-KEM-512，k=2）。
 *
 * ## 流水线位置
 * 本文件是 Alg.13 KeyGen device 探针的唯一 Host `main`：串联两次设备 Launch，管理 GM 与生产 I/O。
 ...
 *
 * ## 与 golden 关系
 * 验收仅要求 I/O 等价：`output/ek_pke.bin`(800B) / `dk_pke.bin`(768B)
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    // presample SHAKE batch tiling：batch=4、PRF 输出 192B/条（CBD_η=3），blockDim 与 prep 双 AIV一致
     ...
     * GM 输出：a_hat[4,256]、src[4,256]、rho[32]、prf_out（CBD 中间态，仍占 GM 但默认不落盘）。
     ...
     * 写 ek_pke（800B）、dk_pke（768B）；dst/t_hat/ek_poly 为内核中间 GM，生产路径不写盘。
     */
