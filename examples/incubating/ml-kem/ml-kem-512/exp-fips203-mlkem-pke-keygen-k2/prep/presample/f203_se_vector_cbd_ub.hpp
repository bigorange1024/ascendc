...
/** CBD_η=3 的 UB 路径：读 prf_out，写 src poly。 */
...
// @depends #include: f203_cbd_eta3.hpp, kernel_operator.h
...
{
    const uint32_t base = row * F203CbdEta3::PRF_BYTES;
    for (uint32_t b = 0; b < F203CbdEta3::PRF_BYTES; ++b) {
        prfRowLocal.SetValue(b, prfAllUb.GetValue(base + b));
    }
}

/** 单 TPipe：bulk prf GM→UB + 4 行 SWAR+LUT + src DataCopy。 */
__aicore__ inline void BuildSrcFromPrfGmUb(__gm__ const uint8_t *prf_out_gm, __gm__ int32_t *src_gm)
{
    ...
    pipe.InitBuffer(prfAllQue, 1, F203CbdEta3::PRF_TOTAL_BYTES);
    pipe.InitBuffer(prfRowQue, 1, F203CbdEta3::PRF_BYTES);
    pipe.InitBuffer(rowQue, 1, static_cast<uint32_t>(F203CbdEta3::N) * sizeof(int32_t));

    AscendC::LocalTensor<uint8_t> prfAllUb = prfAllQue.AllocTensor<uint8_t>();
    AscendC::GlobalTensor<uint8_t> prfGm;
    prfGm.SetGlobalBuffer(const_cast<__gm__ uint8_t *>(prf_out_gm), F203CbdEta3::PRF_TOTAL_BYTES);
    AscendC::DataCopy(prfAllUb, prfGm, F203CbdEta3::PRF_TOTAL_BYTES);
    F203_SE_PIPE_ALL();

    AscendC::GlobalTensor<int32_t> srcGm;
    srcGm.SetGlobalBuffer(src_gm, F203CbdEta3::SRC_COEFFS);

    for (uint32_t row = 0; row < F203CbdEta3::ROWS; ++row) {
        ...
        F203CbdEta3::SamplePolyCbd3RowSwLutUb(rowLocal, prfRowLocal);
        prfRowQue.FreeTensor(prfRowLocal);
        F203_SE_PIPE_ALL();

        rowQue.EnQue(rowLocal);
        rowLocal = rowQue.DeQue<int32_t>();
        AscendC::DataCopy(srcGm[row * F203CbdEta3::N], rowLocal, F203CbdEta3::N);
        F203_SE_PIPE_ALL();
