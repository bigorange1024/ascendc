...
"""
"""
verify_result.py — D13 k2 polyvec4 compute 对拍脚本。
...
def main() -> int:
    ...
    if os.path.isfile("./output/golden.bin") and os.path.isfile("./output/dst.bin"):
        ...
        rc |= check("dst vs golden (polyvec4)", d, g)
        s0 = d[:K_S]
        s_hat_ref = g[:K_S]
        rc |= check("s_hat vs golden s_hat", s0, s_hat_ref)
        e_ref = g[K_S : K_S + K_E]
        e_got = d[K_S : K_S + K_E]
