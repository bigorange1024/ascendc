 ...
 */
/**
 * liboqs ML-KEM-512（k=2）PKE KeyGen 黑盒参考：Alg.13 indcpa_keypair_derand。
 *
 * 本仓 PKE 输出尺寸与 FIPS ML-KEM-512 参数集一致：
 *   ek_PKE = INDCPA pk = polyvec(t)‖ρ，800 B
 *   dk_PKE = INDCPA sk = polyvec(s)，768 B
 *
 * 输入：32 字节 coins = FIPS 的 d（由 SEED_D 经 derand 得到）。
 * 调用 OQS_KEM_ml_kem_512_keypair_derand；其 coins[0:32] 即传入 indcpa。
 */
#include <oqs/oqs.h>
...
#define PKE_PK_BYTES OQS_KEM_ml_kem_512_length_public_key
#define PKE_SK_BYTES (OQS_KEM_ml_kem_512_length_secret_key - OQS_KEM_ml_kem_512_length_public_key - 64)
#define KEM_SK_BYTES OQS_KEM_ml_kem_512_length_secret_key
#define KEM_SEED_BYTES OQS_KEM_ml_kem_512_length_keypair_seed
...
int main(int argc, char **argv)
{
    ...
    if (OQS_KEM_ml_kem_512_keypair_derand(pk, sk, kem_seed) != OQS_SUCCESS) {
        fprintf(stderr, "OQS_KEM_ml_kem_512_keypair_derand failed\n");
        OQS_destroy();
        return 1;
    }
