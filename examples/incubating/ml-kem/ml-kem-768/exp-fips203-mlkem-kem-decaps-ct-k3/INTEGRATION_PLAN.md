# INTEGRATION_PLAN — exp-fips203-mlkem-kem-decaps-ct-k3

**定位**：`examples/incubating/` **Alg.21 `ML-KEM.Decaps` CT 自包含预研实现**（**ml_kem_768 / k=3**）。内部 **Alg.18 `Decaps_internal`**；PKE 段使用本目录 vendored E15 k3 Decrypt / E14 k3 Encrypt，**禁止** frozen G4/G5 `vendor_sync`。

**基线对照**（只读判决 / STATUS；**禁止**抄 frozen 源码）：

| 路径 | 角色 |
|------|------|
| `ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg15-pke-decrypt-device-k3/` | Phase-D Decrypt 行为基线 |
| `ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg14-pke-encrypt-device-k3/` | Phase-E Encrypt 行为基线 |
| `ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg20-kem-encaps-device-k3/` | Encaps「头融 prep」工程范式 |
...
| **PKE 源** | 本目录 vendored 活跃 E14/E15/D14/D15 k3；**禁止** `#include` / rsync frozen |
| **SIM session** | **默认 `decaps_2session`**（CT 锁定）；`ASCENDC_SIM_HOST_MODE=decaps_1session` 仅作排障覆盖 |
...
| 路径 | 内容 |
|------|------|
| `cmake/decaps/CMakeLists.txt` | `STABLE_ENCRYPT_ROOT` / `STABLE_DECRYPT_ROOT` 指向本目录；KERNEL=`kem/` Phase-E 入口 + 本地覆盖 compute；**单库** |
...
| **F1** | D→E 串联；SIM 默认 2-session（CT 锁定） | `K` max=0 | **PASS**（accept D**220727**+E**605388**；reject D**220787**+E**605049**） |
| **F2** | 分项 kat / roundtrip | 按 scripts 约定 | **未纳入本轮** |
| **T2** | SIM 单库 + 2-session CT 默认 | 无 `…_dec_sim`；`K` max=0；根无 stray dump | **PASS**（2026-07-26 Cloud） |
...
| 项 | 归属 |
|----|------|
| **T2：SIM 单库合库 + 2-session CT 默认** | 继承 D21ct：`prepare_dec_shim.sh` + 单 `ascendc_library`；默认 `decaps_2session` |
| **T19i：SIM `fo_only` → `l18_l19` 尾（4→3）** | **本 exp PASS**（2026-07-26；accept D**220727**+E**605388**） |
| D+E 单 launch 融合 | 更后 |
| `examples/stable/ml-kem/ml-kem-768` Decaps | 非本轮；本轮仅 incubating exp |
...
本 exp CMake 现将 `COMPUTE_DIR` 指到本目录 vendored `compute/`，并用 `kem/f203_encrypt_l18_l19_kernel.cpp` 覆盖 Decaps 尾核。**禁止**回写共享 D14/E14 树或其它活跃探针树。
...
| tick | 记 D+E；本 CT exp 登记为 accept D**220727**+E**605388**，reject D**220787**+E**605049** |
...
| SIM 全链 | **PASS**；accept tick D**220727**+E**605388**；3 launch |
| E3 拒绝 | CPU+SIM **PASS** |

### T2 落地要点（已完成）

1. **问题**：D15 k3 Decrypt/Encrypt 同名头 → ascendc precompile 无法 per-TU 隔离 → 曾被迫 SIM 双库 + `decaps_2session`。
...
3. **验收**：CPU 仍绿；SIM 全链 `K` max=0（accept D**220727**+E**605388**）；`KEM_DECAPS_REJECT=1` CPU+SIM 仍绿且 reject≠accept（reject D**220787**+E**605049**）；仅 `libascendc_kernels_sim.so`；用例根无 stray dump。
