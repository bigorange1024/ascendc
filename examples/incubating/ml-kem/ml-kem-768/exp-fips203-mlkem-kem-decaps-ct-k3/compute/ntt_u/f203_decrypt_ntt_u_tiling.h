 ...
 * @brief Alg.15 NTT(u') / INTT(ŵ) 共用 workspace 与几何常量（ML-KEM-768 k=3 poly-batch）。
 *
 * 流水线位置：ntt_u / intt_w / fused 的 nttWsGm、inttWsGm 布局。
 * 语义：Stage1 紧凑 [HI₃‖LO₃] → AIC MMAD → 平面 mat_c → Stage3 merge；
 * AIV 分片为 2+1，Cube m 按硬件对齐但 GM 语义不补假 poly。mixPass=3 为生产全链。
 *
...
constexpr size_t kPolysPerAiv = 2; /* 分配容量按 2 行；真实分片 AIV0=2、AIV1=1（非 limbsplit） */
...
constexpr size_t s0RowsLogic = 2 * kK; /* Stage1 输出：HI₃+LO₃，共 6 行 */
