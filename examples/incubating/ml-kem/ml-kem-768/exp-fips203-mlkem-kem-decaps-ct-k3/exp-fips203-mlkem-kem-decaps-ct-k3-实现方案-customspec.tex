% exp-fips203-mlkem-kem-decaps-ct-k3 — FIPS 203 Alg.21 ML-KEM-768 KEM Decaps
% 编译：bash scripts/xelatex-clean.sh examples/incubating/ml-kem/ml-kem-768/exp-fips203-mlkem-kem-decaps-ct-k3/exp-fips203-mlkem-kem-decaps-ct-k3-实现方案-customspec.tex
...
\title{exp-fips203-mlkem-kem-decaps-ct-k3\\FIPS 203 Alg.21 ML-KEM-768 KEM Decaps 实现方案}
\author{ascendc 工程（ML-KEM-768 W4b / E21ct）}
\date{2026-07-26}
...
\begin{abstract}
本文档描述 \dirname{examples/incubating/ml-kem/ml-kem-768/exp-fips203-mlkem-kem-decaps-ct-k3/}：
在 \textbf{Atlas A2 / Ascend910B4} 上以 AscendC 实现 FIPS 203 \textbf{Algorithm 21}，即 ML-KEM-768（$k=3$）\textbf{KEM Decaps CT 专题副本}。

\textbf{参数锁定}：几何、I/O 与 launch 全部锁定到
\dirname{docs/specs/fips203-mlkem768-parameter-card.md} §3.3 D21ct：
\texttt{dk\_kem=2400B}，\texttt{c=1088B}，输出 \texttt{K=32B}；合法路径 K max=0，拒绝路径必须输出 $J(z\Vert c)$ 且 reject $\neq$ accept；SIM/生产默认 \textbf{\texttt{decaps\_2session}}（CT 锁定），T19i 形态 \textbf{3 launch}。
\textbf{行为基线}：活跃探针
\dirname{ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg21-kem-decaps-device-ct-k3/}。
...
来源 & 用途 \\
\midrule
\dirname{ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg21-kem-decaps-device-ct-k3/} & 活跃 D21ct k3 行为基线；可复制后自包含适配到本 exp，最终不得保留对探针目录的编译期 include/import \\
...
\begin{itemize}
  ...
  \item 禁止为 $G$ 或 FO 新增与 Encrypt/Decrypt 无关的产品化独立 launch；默认路径须保持 D21ct T19i 三段结构，并且 reject 必须与 accept 不同。
...
\midrule
Phase-D & MIX，\texttt{blockDim=1} & 继承 E15/D15：Decode12、unpack \(c\)、NTT/Inner/INTT、Compress1+ByteEncode1，输出 \(m'\) 到 GM workspace。选择 1 个 AI Core 是参数卡 §3.3 D21ct 对 D15 fused 的锁定继承。 \\
...
\subsection{同步与流水}
Phase-D 与 Phase-E 之间由 Host launch 顺序保证 GM 可见性；SIM 默认在同一 ACL session 内连续执行，保持 \texttt{decaps\_2session}，即 Phase-D 与 Phase-E 分 session 执行。
Phase-E prep 内 \(G\) 写出 \(K'\Vert r'\) 后，PRF/CBD 必须读取完整 \(r'\)；沿用 D21ct 已验顺序与 barrier。
...
本实现不引入 E21ct 独有的新 AscendC 矢量/矩阵 API；KEM 头尾使用 shared Keccak \texttt{Sha3OneShot}/\texttt{Shake256OneShot}。下表 API 均已有索引记录，使用约束按
...
期望：\texttt{K.bin} 为 32B，合法路径与 golden 对拍 PASS；SIM 默认 \texttt{decaps\_2session}，Total tick 登记到 \texttt{qa/active\_sim\_regress\_summary.md}，并检查用例根目录无 stray \texttt{core*.dump}、\texttt{profile\_*log*.toml}。
...
\section{产出物}
\begin{itemize}
  \item \dirname{exp-fips203-mlkem-kem-decaps-ct-k3-实现方案-customspec.tex}
  \item \dirname{exp-fips203-mlkem-kem-decaps-ct-k3-实现方案-customspec.pdf}
