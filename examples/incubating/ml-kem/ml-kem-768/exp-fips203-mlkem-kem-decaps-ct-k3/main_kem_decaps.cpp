 ...
 *
 * SIM Host：CT 生产默认 `ASCENDC_SIM_HOST_MODE=decaps_2session`，D/E 分两次
 * KernelLaunch；`decaps_1session` 仅保留为排障覆盖。
 *
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    if (ascendc::SimHostDecapsUse2Session()) {
        std::fprintf(stderr, "[kem-decaps] SIM host mode=decaps_2session（CT 默认）\n");
    } else {
        std::fprintf(stderr, "[kem-decaps] SIM host mode=decaps_1session（排障覆盖；非 CT 默认）\n");
    }
#endif
