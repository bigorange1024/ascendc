 ...
 * @brief Alg.13 行 3–7：9×（SHAKE + 向量 d12/rej），UB 全链，链末写 GM a_hat[9,256]。
 *
 * 复用 pass-fix-f203-alg7-sample-ntt-k3 的 F203Alg7 模块（Mins+Gather+标量 compact）。
...
/** D13 锁定：Â[9] 由双 AIV 按 5+4 分片，禁止补到 16。 */
constexpr uint32_t kShardPolysAiv0 = 5U;
...
/** 填 batch=9（或 2 AIV 时最大 batch=5）全量消息；等价于 range(0, kShakeActiveBatch)。 */
...
/**
 * SEED_D → ρ → 本分片 poly（1 AIV×9 或 2 AIV×5/4）SHAKE + 向量 d12/rej → GM a_hat 对应块。
 *
 * 2 AIV：blockIdx 0 → polyIdx 0–4；blockIdx 1 → 5–8；GM 偏移互不重叠。
 */
/**
 * 单分片：blockIdx 0→poly 0–4，1→5–8（BLOCK_DIM=2）；BLOCK_DIM=1 时仅 blockIdx=0 有效。
