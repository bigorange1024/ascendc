#!/usr/bin/env bash
# exp-fips203-mlkem-kem-decaps-ct-k3 — Alg.21 Decaps CT 全链（D+E）；可选 Phase-E-only
#
...
#   bash run.sh -r sim -v Ascend910B4   # 默认单库 + decaps_2session；T19i 后 SIM 3 launch（pack+FO 同核）
...
# SIM 全链默认 2-session（CT 锁定）；单 session 仅用于排障覆盖。
...
echo "[SUCCESS] exp-fips203-mlkem-kem-decaps-ct-k3 ${LABEL} (${RUN_MODE})"
