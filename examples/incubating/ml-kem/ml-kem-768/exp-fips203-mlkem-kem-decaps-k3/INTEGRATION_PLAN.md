# INTEGRATION_PLAN — exp-fips203-mlkem-kem-decaps-k3

**定位**：`examples/incubating/` **Alg.21 `ML-KEM.Decaps` 自包含预研实现**（**ml_kem_768 / k=3**）。内部 **Alg.18 `Decaps_internal`**；PKE 段使用本目录 vendored E15/D15 k3 Decrypt 与 E14/D14 k3 Encrypt，**禁止** frozen G4/G5 `vendor_sync`。
...
| **F1** | D→E 串联；SIM 默认 1-session（单库） | `K` max=0 | **PASS**（D**221059**+E**599171**） |
...
| **T19i：SIM `fo_only` → `l18_l19` 尾（4→3）** | **本 exp PASS**（2026-07-26；D**221059**+E**599171**） |
| D+E 单 launch 融合 | 更后 |
...
| tick | 记 D+E；本 exp 登记为 accept D**221059**+E**599171**，reject D**220692**+E**601808** |
...
| SIM 全链 | **PASS**；tick D**221059**+E**599171**；3 launch |
| E3 拒绝 | CPU+SIM **PASS** |
...
3. **验收**：CPU 仍绿；SIM 全链 `K` max=0（D**221059**+E**599171**）；`KEM_DECAPS_REJECT=1` CPU+SIM 仍绿（D**220692**+E**601808**）；仅 `libascendc_kernels_sim.so`；用例根无 stray dump。
