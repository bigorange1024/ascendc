 ...
 * @brief Alg.14 行 19/21：uTr[4] → batch=4 的 INTT Stage1–3（ML-KEM-768 真 polyvec4）。
 *
 ...
 *
 * Stage3 输出 scatter：
 *   AIV0 local0→u[0], local1→u[1]；AIV1 local0→u[2], local1→v（行 21）。
 */
...
/**
 * INTT S3：平面 mat_c → u[3] + v 分片写回。
 * AIV0: local0/1 → u[0]/u[1]；AIV1: local0 → u[2]，local1 为 tr̂ 时域 → vOut。
 */
...
public:
    ...
    /**
     * @param uOut u[3,N]；@param vOut v[N]（仅 AIV1 写）；@param matPlanar 平面 mat_c
     */
...
private:
     ...
     * AIV0: lp0/1→u[0]/u[1]；AIV1: lp0→u[2]，lp1→v。
     */
