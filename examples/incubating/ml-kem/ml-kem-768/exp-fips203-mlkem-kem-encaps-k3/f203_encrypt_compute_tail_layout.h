 ...
 * @brief Alg.14 compute（行 2/16–21）+ tail（行 20/22–24）**统一 GM 契约**（ML-KEM-768 / k=3）。
 *
 * 用途：本目录 host 单 session 分配 device arena 时，compute launch 写、tail launch 读的
 *       **权威偏移**须与此一致。尺寸锁定到 ML-KEM-768：k=3、du=10、dv=4、
 *       c1=960B、c2=128B，与 E20 customspec §3.3 I/O/GM 契约一致。
 *
...
/** workspace 字节数：与 f203_l18_l19_tiling.h `tiling::wssize` 同值（按 k3 polyvec4 几何派生） */
#define F203_ECT_WS_BYTES 296960U
