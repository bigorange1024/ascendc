 ...
 * @brief Alg.20/17 Encaps（k=3）I/O 常量：与 D14 k3 Encrypt 公钥/密文几何对齐。
 *
 * 生产 I/O：input/ek_kem.bin(1184) + m.bin(32) + LUT → output/c.bin(1088) + K.bin(32)。
...
constexpr uint32_t kEkKemBytes = F203EncryptPrep::kEkPkeBytes;       // 1184
constexpr uint32_t kSharedSecretBytes = 32U;
constexpr uint32_t kCtBytes = F203_TAIL_C_BYTES;                     // 1088
