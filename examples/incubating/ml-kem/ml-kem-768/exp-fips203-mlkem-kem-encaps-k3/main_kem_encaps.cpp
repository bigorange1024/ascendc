 ...
 * @brief Alg.20/17 KEM Encaps host：E20 自包含 Encrypt 2/5 launch + prep 前段设备 H/G。
 *
 * 生产 I/O：input/ek_kem.bin + m.bin + LUT → output/c.bin + K.bin。
 * coins/r 由设备 G 写入 workspace，Host 不读 coins.bin。
 * 方案见 customspec；Encrypt 源码来自本目录 vendored E14 k3 副本，不在编译期引用其它用例。
 */
...
/**
 * CPU 分段路径：INTT LUT 覆盖写入同一 LUT_NTT_* 区（phased 复用，与本地 Encrypt 几何一致）。
