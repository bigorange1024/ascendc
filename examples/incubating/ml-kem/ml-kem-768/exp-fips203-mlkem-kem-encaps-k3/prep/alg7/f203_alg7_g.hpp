 ...
 *
 * 数学契约：
 *   - DerandFromSeedD：SHA3-256("exp-mlkem-f203-2s1e-k3:SEED_D=<decimal>") → d[32]
 *   - HashGRho：SHA3-512(d || byte(k=3)) 取前 32B → ρ
 *   - HashGSigma：同一次 SHA3-512 取后 32B → σ（见 BuildRhoSigmaFromSeedD）
 *
 * 与 golden 关系：gen_data.py 须使用相同域分离串与 k=3 后缀；ρ 不一致则 XOF 全链偏离。
 */
#pragma once
...
__aicore__ inline void DerandFromSeedD(uint32_t seed_d, uint8_t d[32])
{
    char msg[48];
    int pos = 0;
    // 固定前缀 "exp-mlkem-f203-2s1e-k3:SEED_D="
    msg[pos++] = 'e';
    msg[pos++] = 'x';
    msg[pos++] = 'p';
    msg[pos++] = '-';
