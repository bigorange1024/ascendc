#!/usr/bin/env python3
"""
gen_data.py — Alg.20 Encaps device-k3：ek + m + LUT + golden(c/K) + CPU golden_v。

流水线位置：
  1. 默认自生成 ML-KEM-768 ek_kem（1184B），也允许 EK_KEM_SRC 覆盖；
...
def _gen_default_ek(inp: Path) -> bytes:
    """调用本目录 host_golden 生成 ek_pke；Alg.20 中 ek_kem 与 ek_pke 同字节。"""
    import subprocess
...
def main() -> None:
    ...
    print(f"[gen_data] ek←{src_label} m={m.hex()[:16]}… golden c={len(c_ref)}B K=32B via local host_golden")
