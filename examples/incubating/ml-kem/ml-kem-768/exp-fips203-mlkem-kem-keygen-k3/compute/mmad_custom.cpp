 ...
 *
 * ## 对齐与 golden
 * ML-KEM-768（k=3）；验收仅 I/O 等价于 `keygen_golden.py`，不要求与 Host 源码同构。
 *
 * ## 流水线（mixPass=0 生产路径）
 *
 *   host src[6,256] + LUT + a_hat
 *     → [AIV] AivK6Split          Stage1：polyvec6 limb6 拆分 → workspace S0
 *     → [AIC] AicMmad ×4          Stage2：int8 MMAD（even/odd × lo/hi）
 *     → [AIV] AivK6PackMatCPlanar    竖堆临时 → 平面 mat_c [48,128]
 ...
 * ## Host 数据语义
 *
 *   - src 行 0..2：ŝ；行 3..5：ê（prep CBD 输出）。
 *   - Stage3 后按真实 polyvec6 写 dst[0..5]，SyncAll 后每个 AIV 读取完整 ŝ[0..2]。
 ...
 * @param dst     输出/中间：NTT 后 ŝ/ê [6,256] int32
 * @param t_hat   输出/中间：t̂ [3,256] int32
 * @param ek_out  输出：ByteEncode₁₂(t̂) 1152B（亦作 Fuse 输入）
 * @param sk_out  输出：ByteEncode₁₂(ŝ) 即 dk_pke 1152B
 * @param src     输入：prep 的 ŝ‖ê [6,256] int32
 * @param a_hat   输入：Â[9,256] int32
                                                  ...
                                                  )
{
    ...
    if (AIC) {
        ...
        STATE = AIC_MMAD;
        /** MMAD 仅消费 S0 有效行 mRowsLogic(12)；Cube 内部 m 参数按 16 对齐，非假 poly。 */
        ...
        if (runS1) {
            STATE = AIV_SPLIT;
            AivK6Split split(subBlockID, coeffN);
        ...
        if (runS2) {
            STATE = AIV_PACK;
            WAIT
            AivK6PackMatCPlanar pack(subBlockID, coeffN);
...
#if F203_KEYGEN_EK_PKE >= 1
    if (!AIC && runEncode) {
        /* 双 AIV 均写完 ek/sk 分片后再让 AIV0 读取完整 ek_polyvec，避免拷贝到未完成的第 3 行。 */
