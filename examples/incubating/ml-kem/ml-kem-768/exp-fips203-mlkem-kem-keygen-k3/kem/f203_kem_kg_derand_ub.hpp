 ...
 *
 * 流水线分工（device-k3，2 launch）：
 *   - d（KeyGen 主随机性）由 L1 D13 k3 prep 内 DerandFromSeedD / HashG 消费，不在本文件；
 ...
 *   `"exp-mlkem-f203-kem-k3:SEED_Z=" ‖ decimal(seed_d)`（见 scripts/gen_data.py）。
 *
 * 替换步骤：
 ...
 * D13 k3 prep 内 SHA3-512（ρ‖σ）走同库不同 mdlen，替换第三方时 prep 与 kem 可一并评估。
 */
#pragma once
...
/**
 * 将 uint32 写成十进制 ASCII（无堆分配；与 D13 k3 Alg.7 DerandFromSeedD 同型）。
 ...
 *
 * 消息 = 固定前缀 "exp-mlkem-f203-kem-k3:SEED_Z=" ‖ 十进制(seed_d)，
