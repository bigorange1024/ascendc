 ...
 *
 * 流水线位置（device-k3，2 launch）：
 *   L1 D13 k3 f203_keygen_prep  — Alg.13 行 3–15（d 在 device UB）
 *   L2 D13 k3 mmad_custom       — Alg.13 行 16–21 + 本尾段（F203_KEM_KEYGEN_TAIL=1）
 *
 * 在 FuseEkPke 已写出 ek_pke_gm / dk_pke_gm（sk_out）之后，本函数完成 Alg.16 剩余步骤：
 *   1) UB 内派生 z（DerandZFromSeedD，或旁路 A 读 kem_seed[32:64]）；
 *   2) H(ek) = SHA3-256(ek_pke)，输入 1184B，输出 32B；
 *   3) 按 liboqs 展开布局拼接 dk_kem_gm = dk_pke ‖ ek ‖ H(ek) ‖ z（2400B）。
 *
 * ek_kem 与 ek_pke_gm **同址**（Alg.16「ek ← ek_PKE」），本函数不写 ek 缓冲。
 *
 * ## 工程接线（W3/D19，2026-07-26 定案）
 *
 * 尾段通过 本目录 `compute/mmad_custom.cpp` 的宏 `F203_KEM_KEYGEN_TAIL` 挂接，而非 fork 整份 mmad：
 *   - 避免 D13 NTT/Alg.11/行21 演进时手工 merge `mmad_custom_kem.cpp`；
 *   - D19 CMake 默认宏=1；D13 绿树不受本目录副本影响；
 *   - 尾段算法仍集中在本目录 kem 头文件，便于 alg20/21 复用模式。
 *
 * 本 k3 首版正确性验收 SIM tick 为 510775；I/O 与 D13 PKE 段一致，KEM 尾按 768 布局拼接。
 *
 * ## SHA3-256 替换指南（日后第三方 AscendC 实现）
 *
 * 本文件仅 **一处** 哈希调用，须保持语义：
 *   `Sha3OneShot(h, 32, ek_ub, kEkKemBytes)` — 对 1184B ek 做 FIPS SHA3-256，输出 32B。
 *
 * 替换步骤：
 *   1) 保持函数签名与 dk_kem 拼接偏移不变（见 f203_kem_kg_layout.h）；
 *   2) 将下行 `F203SeDeviceKeccak::Sha3OneShot` 换为第三方 `__aicore__` 一次性摘要 API；
 *   3) 验收：`bash run.sh -r cpu/sim` + `verify_kem.py` golden output；可选对比 H(ek) 中间量。
 *
 ...
 * @param ek_pke_gm  GM：1184B ek_PKE，亦为 ek_kem 别名（只读）
 * @param dk_pke_gm  GM：1152B PKE 私钥（= mmad sk_out / ByteEncode₁₂(ŝ)）
 * @param dk_kem_gm  GM：2400B 输出，liboqs 展开布局
 */
                                      ...
                                      __gm__ uint8_t *dk_kem_gm)
{
    ...
    // —— 步骤 2：H(ek) = SHA3-256(ek_pke[1184]) → h[32] ——
    ...
    // 【SHA3 替换点 #2】mdlen=32 → SHA3-256；输入长 kEkKemBytes=1184
