 ...
 * @brief 阶段编排：G → P → C。
 *
 * V3（F203_SE_VECTOR_V3，默认）：shake UB batch PRF + alg8 P1b/P2（SamplePolyCbd2Batch6）。
...
#else
    F203CbdEta2::SamplePolyCbd2Batch6(reinterpret_cast<const __gm__ uint8_t *>(prf_out_gm),
                                      reinterpret_cast<__gm__ int32_t *>(src_gm));
#endif
}
