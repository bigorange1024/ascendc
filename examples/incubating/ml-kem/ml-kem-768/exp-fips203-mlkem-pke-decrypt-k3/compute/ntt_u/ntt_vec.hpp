 ...
 *
 * 流水线位置：
 *   - Stage1：AivK8Split.encodeBank → split_vec → S0 [16,256] int8；
 *   - Stage3：AivK8RouteAMod → combine_limb6_routea_mod_vec → dst [8,256] int32。
 *
 * 作用：
 ...
 *
 * 与 golden 关系：gen_data 平面 mat_c + stage31_mod 语义；dst.bin [8,256] NTT/INTT 后系数。
 *
 ...
 * @param count 元素个数，须为 4 的倍数（本探针 bank 至多 2*256）
