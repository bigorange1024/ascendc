...
constexpr int32_t kUnifiedDecompressBias10 = 512;
constexpr int32_t kUnifiedDecompressBias4 = 8;
constexpr int32_t kUnifiedDecompressShift10 = 10;
constexpr int32_t kUnifiedDecompressShift4 = 4;
...
{
    return decompress_unified_scalar(u, kUnifiedDecompressBias10, kUnifiedDecompressShift10);
}

__aicore__ inline uint32_t decompress_d4_u32(uint32_t u)
{
    return decompress_unified_scalar(u, kUnifiedDecompressBias4, kUnifiedDecompressShift4);
}

/**
 * ByteDecode_d：从字节流按 LSB-first 抽出 N 个 d-bit 整数。
 * @param out  系数缓冲 [N]
 * @param in   编码字节（d=10→320B / d=4→128B）
 * @param dBits 10（c₁）或 4（c₂）
 */
                                          ...
                                          AscendC::LocalTensor<int32_t> &tmp)
{
    ...
    const int32_t bias = (dBits == 10U) ? kUnifiedDecompressBias10 : kUnifiedDecompressBias4;
    const int32_t shift = (dBits == 10U) ? kUnifiedDecompressShift10 : kUnifiedDecompressShift4;
    ...
    for (int32_t i = 0; i < kUnpackN; ++i) {
        ...
        if (dBits == 10U) {
            outLocal.SetValue(i, static_cast<int32_t>(decompress_d10_u32(u)));
        } else {
            outLocal.SetValue(i, static_cast<int32_t>(decompress_d4_u32(u)));
        }
    }
#endif
    AscendC::PipeBarrier<PIPE_ALL>();
...
__aicore__ inline void unpack_c_impl(GM_ADDR cGm, GM_ADDR uGm, GM_ADDR vGm)
{
    ...
    /* c₁：k 个 poly，各 320B → Decompress₁₀ → u'[p] */
    for (int32_t p = 0; p < kUnpackK; ++p) {
        ...
        unpack_one_poly_ub(cPolyLocal, uOut + p * kUnpackN, 10U, inLocal, outLocal, tmp);
    }
    ...
    unpack_one_poly_ub(c2Local, vOut, 4U, inLocal, outLocal, tmp);
