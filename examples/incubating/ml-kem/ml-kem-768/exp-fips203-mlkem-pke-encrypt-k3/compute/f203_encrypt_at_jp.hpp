...
/**
 * halfrows 内积（û）+ 可选 tr̂ → k3 batch4 UB。
 *
 * @param doTrHat 仅 AIV1 为 true 时算 tr̂；@param tHatUbOpt 行 2 decode 驻留（优先于 tHat GM）
 * @param unifiedUTrPad8 true：沿用旧参数名；dst 布局 [û0,û1] / [û2,tr̂]
...
/**
 * 统一 uTr[4] 对拍：从 batch4 UB scatter 到 GM u_tr[4,256]。
 * AIV0: ub[0,1]→u_tr[0,1]；AIV1: ub[0,1]→u_tr[2,3]（第 3 行为 tr̂）。
 */
