 ...
 * @brief Alg.14 行 18 NTT 域内积：Âᵀ∘r̂（3 列 û）+ 可选 tr̂（INTT batch=4 第 4 行）。
 *
 * 流水线：单 launch 内由 `f203_encrypt_l18_l19` 在 NTT(r) 后调用；输出驻留 UB 供 INTT batch=4。
 * 对齐：pass-fix-f203-alg11-12-innerproduct-k4-halfrows 的 ProcessHalfRows；
 *       GM 索引用 encrypt_at_jp_layout（flat(j,p)，Encrypt 读 Âᵀ 索引）。
 *
 * k3 batch4 UB 布局（`ProcessToUbMaybeTrHat(..., unifiedUTrPad8=true)` 沿用旧参数名）：
 *   AIV0 dstUb[2,N]: [û0, û1]
 *   AIV1 dstUb[2,N]: [û2, tr̂]
 * INTT 后 scatter：slot 0–2→u+e₁；slot 3 时域→v+e₂。
 *
 ...
 * ④ final mod → ⑤ 写 dstUb（k3 batch4 或紧凑 halfrows）。
 *
 * @param unifiedUTrPad8 true：沿用旧形参名；k3 下 AIV0 [û0,û1]，AIV1 [û2,tr̂]
 */
                                                ...
                                                bool unifiedUTrPad8 = false)
{
        ...
        // ⑤ 写出 dstUb：k3 batch4 布局或紧凑 halfrows
        if (unifiedUTrPad8) {
