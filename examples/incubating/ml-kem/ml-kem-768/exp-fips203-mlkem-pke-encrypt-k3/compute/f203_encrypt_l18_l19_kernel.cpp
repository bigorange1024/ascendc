 ...
 * @brief Alg.14 行 2/16–24：ML-KEM-768 **SIM 单 launch** 融合 MIX + μ 折叠 + 内联 tail pack。
 *
 ...
 *   行 16–17 NTT(r) → 行 2 decode → 行 18 uTr batch4 → GATE → 行 19/21 INTT → pack
 */
                                                           ...
                                                           GM_ADDR traceGm)
{
    ...
    if (aic) {
        ...
        /* ── 行 18/19：k3 内积 uTr batch4 驻留 UB → INTT polyvec4 ── */
        {
