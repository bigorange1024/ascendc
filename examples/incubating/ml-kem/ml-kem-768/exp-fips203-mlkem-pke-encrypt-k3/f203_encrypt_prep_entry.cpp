 ...
 *
 * 流水线位置：
 *   行 3–7：ρ = ek_pke[1152:1184] → SampleNTT → a_hat[9,256]
 *   行 8–15：coins → PRF+CBD(η=2) batch7 → re[7,256]（nonce 0–2=r, 3–5=e₁, 6=e₂）
 *
 * 编排：双 AIV 并行 Â 分片（block0→poly 0–4，block1→5–8）；PRF/CBD 仅 block0。
