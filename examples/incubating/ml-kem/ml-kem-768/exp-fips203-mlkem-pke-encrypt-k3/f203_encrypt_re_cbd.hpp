 ...
 * @brief Alg.14：PRF batch7 → CBD_η=2 batch7 → re GM（r‖e₁‖e₂ 连续 7 行）。
 *
 * 流水线位置（Encrypt prep 行 8–15 后半）：
 *   prf_out[7,128] → SamplePolyCBD_η=2 ×7 → re[7,256] int32
 *   行语义：row 0..2→r，3..5→e₁，6→e₂
 *
 * 复用 k3 prep/alg8 SWAR+LUT 单行 CBD；batch=7、block0 串行（F203_CBD_BLOCK_DIM=1）。
...
/**
 * 7 行 CBD：row 0..2→r，3..5→e₁，6→e₂；re GM 行主序 int32[7,256]。
 *
 * @param prf_gm     PRF 输出 [7,128] uint8
 * @param re_gm      输出 [7,256] int32 扁平
 * @param scratchBuf 复用为单行 prfLocal[128]（uint8 视图）
 * @param rowQue     单行 int32[256] 输出队列（与 prep 的 prfYQue 共用）
 */
__aicore__ inline void SamplePolyCbd2Batch7WithUb(__gm__ const uint8_t *prf_gm, __gm__ int32_t *re_gm,
