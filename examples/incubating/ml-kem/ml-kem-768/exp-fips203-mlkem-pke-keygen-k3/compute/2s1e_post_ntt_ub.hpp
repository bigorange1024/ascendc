...
// @file compute/2s1e_post_ntt_ub.hpp
// @layer compute
// @role compute/：Tag5T NTT + Alg.11 basemul + 行18–20 UB 融合 MMAD 内核与 host 驱动；第二次 launch，读 prep 写 GM + LUT，写 ek/sk 与 ek_pke。 / Full keygen compute (mmad_custom) sources. 本文件 `2s1e_post_ntt_ub.hpp` 为该子模块组件。 / Component: 2s1e_post_ntt_ub.hpp.
...
public:
    __aicore__ inline Aiv2s1eUbPipeline(int32_t subCoreIdx, uint32_t coeffN)
        : subCoreIdx_(subCoreIdx), coeffN_(coeffN), halfLen_(coeffN / 2), pairCount_(halfLen_ / 2),
          ...
          kPolysE_(static_cast<uint16_t>(twos1e::p_end(subCoreIdx) - twos1e::p_begin(subCoreIdx))),
          kUbPolys_(static_cast<uint16_t>(tiling::kS + tiling::kEPerAiv)),
          limbTileLength_(4 * (coeffN / 2)),
          sBatchPlaneLength_(static_cast<uint32_t>(tiling::kPolysPerAiv) * limbTileLength_),
          eBatchPlaneLength_(static_cast<uint32_t>(tiling::kPolysPerAiv) * limbTileLength_),
          ...
          localSlotBase_((subCoreIdx == 0) ? 0U : static_cast<uint32_t>(tiling::kPolysPerAiv))
    {
    }
    ...
    __aicore__ inline void Init(GM_ADDR matPlanar, GM_ADDR a_hat, GM_ADDR ek_out, GM_ADDR sk_out, GM_ADDR dst_dump,
                                GM_ADDR t_dump)
    {
        /* GM 绑定：平面 mat_c、a_hat[9,256]、ek/sk 输出、对拍 dump dst/t_hat */
         ...
         *   ubNttBuf_     — ŝ[0..2] + ê 本核最多 2 行（AIV0=2，AIV1=1）
         *   ubThatBuf_    — t̂ 本核最多 2 行
                                   ...
                                   bool loadThatPreset)
    {
        ...
        /* k3 的 NTT 共享 dst 已在 stageS3Into 按全局行写入；这里不再按本地 UB 布局覆盖。 */
...
private:
    ...
    /** mixPass 4/7 或 S3 后共享：从 dst_dump 灌完整 ŝ[0..2] + 本核 ê[p]，跳过/接续 S3。 */
    __aicore__ inline void loadNttPresetInto(LocalTensor<int32_t> &ub_ntt)
    {
        // k3 polyvec6 NTT 的物理分片为 AIV0:s0..s2 / AIV1:e0..e2；
        // 行18 要求每个 AIV 都有完整 ŝ，因此从 GM 共享区统一读 dst[0..2]。
        DataCopy(ub_ntt, gm_dst_dump_[0], static_cast<uint32_t>(tiling::kS) * coeffN_);
        KYBER_PIPE_ALL();
        for (uint16_t p = pBegin_; p < pEnd_; ++p) {
            const uint32_t localE = static_cast<uint32_t>(tiling::kS) +
                                    (static_cast<uint32_t>(p) - static_cast<uint32_t>(pBegin_));
            const uint32_t globalE = static_cast<uint32_t>(tiling::dstEOff) + static_cast<uint32_t>(p);
            DataCopy(ub_ntt[localE * coeffN_], gm_dst_dump_[globalE * coeffN_], coeffN_);
            KYBER_PIPE_ALL();
        }
    }

    /** S3：本核按连续 polyvec6 slot merge 后写入 dst GM；SyncAll 后重装完整 ŝ + 本核 ê。 */
    __aicore__ inline void stageS3Into(LocalTensor<int32_t> &ub_ntt)
    {
        mergeBankInto(ub_ntt, 0U, localSlotBase_, static_cast<uint16_t>(tiling::kPolysPerAiv),
                      sBatchPlaneLength_);
        DataCopy(gm_dst_dump_[localSlotBase_ * coeffN_], ub_ntt,
                 static_cast<uint32_t>(tiling::kPolysPerAiv) * coeffN_);
        KYBER_PIPE_ALL();
        /*
         * 背景：k3 D13 的 B5 NTT 锁定为连续 polyvec6，AIV1 只本地得到 e0..e2；
         * 结论：两 AIV 先把各自 3 poly 写入 dst GM，再硬同步，随后都读取完整 ŝ[0..2]。
         * 未采用：为复用 k4 而复制 ŝ 到两个 AIV 或把 polyvec6 pad 成 8 行。
         */
                                         ...
                                         uint16_t kPolys, uint32_t batchPlaneLength)
    {
        ...
        const uint32_t loPlaneBase = planar_k6::mat_row(slotBase, 0U, 0U) * halfLen_;
        const uint32_t hiPlaneBase = planar_k6::mat_row(slotBase, 0U, 1U) * halfLen_;
...
#else
        /** legacy：half-row basemul + 每 half 一次 MOD_Q（历史半行路线，勿作生产） */
    ...
    /** 对拍 dump：S3 已在 stageS3Into 写全局 dst；保留空函数避免旧调试入口误用。 */
