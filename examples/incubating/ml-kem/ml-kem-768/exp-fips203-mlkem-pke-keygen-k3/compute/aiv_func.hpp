/**
 * @file aiv_func.hpp
 * @brief Tag5T 6-poly 向量侧：Stage1 紧凑 split、Stage2 后平面 pack、Stage3 RouteA merge/mod。
 *
 * 流水线位置（MIX 1×AIC + 2×AIV，见 mmad_custom.cpp）：
 *   1. AivK6Split：src [6,256] int32 → S0 [12,256] int8（[HI₆,LO₆]）；
 *   2. AIC 四路 MMAD → mat_c_tmp；
 *   3. AivK6PackMatCPlanar：四临时 → 平面 mat_c [48,128] int32；
 *   4. AivK6RouteAMod：平面 → dst [6,256] int32（Horner+mod，无 Gather）。
 *
 * 语义（强制）：
 *   - poly-batch：AIV0 处理 poly 0..2，AIV1 处理 3..5；每核握有完整 poly 的 HI+LO；
 *   - 平面 mat_c：每 slot 四 limb × 两 half（lo/hi 半系数）；
 *   - 三段式内禁止 Gather。
 *
 * 与 golden 关系：s0 / mat_c / dst 分别对应 golden_s0、golden_mat_c、golden_dst（gen_data）。
 */
#ifndef STAGE123_POLYVEC6_AIV_FUNC_HPP
#define STAGE123_POLYVEC6_AIV_FUNC_HPP
...
/**
 * 平面 mat_c 行号：half∈{0,1}（lo/hi 半）、slot∈[0,6)、limb∈[0,4)。
 * 布局：先 half 大块（各 32 行），块内按 slot 连续 4 limb 行。
 *
 * @return 行索引 ∈ [0, 64)
 */
...
/**
 * Stage1：k=6 紧凑 [HI, LO] → S0 int8 [12,256]。
 *
 * 输入：GM src 中本 AIV 的 3 条 poly（polyBase..+2），各 256 int32。
...
public:
     ...
     */
    __aicore__ inline AivK6Split(int32_t subCoreIdx, uint32_t coeffN)
    ...
    /** 对本 AIV 的 3-poly bank 执行 encodeBank */
...
private:
     ...
     *
     * 索引：hiRow = polyBase+lp；loRow = K + polyBase+lp（K=6）。
    ...
    __aicore__ inline void encodeBank(uint32_t polyBase, uint16_t kPolys)
    {
        ...
        // 按 poly 写回：HI 在上半 6 行，LO 在下半 6 行
...
/**
 * Stage2 后：Cube 四路临时 → 平面 mat_c [48,128]。
 *
 ...
 */
class AivK6PackMatCPlanar {
public:
    __aicore__ inline AivK6PackMatCPlanar(int32_t subCoreIdx, uint32_t coeffN)
...
private:
                                        ...
                                        AscendC::GlobalTensor<int32_t> &gmOdd)
    {
        for (uint16_t lp = 0; lp < kPolys; ++lp) {
            ...
            KYBER_PIPE_ALL();
            const uint32_t dstBase = planar_k6::mat_row(slot, 0U, half) * halfLen_;
...
/**
 * Stage3：平面 mat_c → dst [6,256]（与 vec-k4-v2 Aiv2s1eRouteAMod 同构）。
 *
 ...
 */
class AivK6RouteAMod {
public:
    __aicore__ inline AivK6RouteAMod(int32_t subCoreIdx, uint32_t coeffN)
