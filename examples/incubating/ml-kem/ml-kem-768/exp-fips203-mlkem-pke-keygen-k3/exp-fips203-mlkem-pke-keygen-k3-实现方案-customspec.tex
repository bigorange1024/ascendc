% exp-fips203-mlkem-pke-keygen-k3 — FIPS 203 Alg.13 ML-KEM-768 PKE KeyGen
% 编译：bash scripts/xelatex-clean.sh examples/incubating/ml-kem/ml-kem-768/exp-fips203-mlkem-pke-keygen-k3/exp-fips203-mlkem-pke-keygen-k3-实现方案-customspec.tex
...
\title{exp-fips203-mlkem-pke-keygen-k3\\FIPS 203 Alg.13 ML-KEM-768 PKE KeyGen 实现方案}
\author{ascendc 工程（ML-KEM-768 W4a / E13）}
\date{2026-07-26}
...
\begin{abstract}
本文档描述 \dirname{examples/incubating/ml-kem/ml-kem-768/exp-fips203-mlkem-pke-keygen-k3/}：
在 \textbf{Atlas A2 / Ascend910B4} 上以 AscendC 实现 FIPS 203 \textbf{Algorithm 13}，即 ML-KEM-768（$k=3$）\textbf{PKE KeyGen}。

\textbf{参数锁定}：几何、I/O 与 launch 全部锁定到
\dirname{docs/specs/fips203-mlkem768-parameter-card.md} §3.2 D13。
\textbf{行为基线}：活跃探针
\dirname{ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg13-device-keygen-k3/}。
\textbf{生产 I/O}：\texttt{input/} 仅 \texttt{seed\_d.bin} 与 NTT LUT；
\texttt{output/} 仅 \texttt{ek\_pke.bin}（1184B）与 \texttt{dk\_pke.bin}（1152B）。
\textbf{计划 AI Core 数}：2 launch；Launch 1 为 AIV\_ONLY \texttt{blockDim=2}，Launch 2 为 MIX \texttt{blockDim=1}（1 AIC + 2 AIV）。
\textbf{禁止}：创建 \dirname{examples/stable/ml-kem/ml-kem-768/}；从任何 \dirname{frozen/} 目录复制、改写或移植源码；用 k4 的 ek/dk 长度或 16/8 路几何替代 k3 几何。
\end{abstract}
...
来源 & 用途 \\
\midrule
\dirname{ascendc-tests/ml-kem/ml-kem-768/pass-fix-f203-alg13-device-keygen-k3/} & 活跃 D13 k3 行为基线；可复制后自包含适配到本 exp，最终不得保留对探针目录的编译期 include/import \\
\dirname{examples/incubating/ml-kem/ml-kem-1024/exp-fips203-mlkem-pke-keygen-k4/} & 活跃 k4 incubating 结构参考；仅 retarget 到 k3 锁定几何与字节长 \\
...
\begin{itemize}
  ...
  \item 禁止把 k4 的 \texttt{ek=1568}、\texttt{dk=1536}、\texttt{\^A[16]}、polyvec8 或零垫 4/8 当作默认。
  \item 禁止新建或写入 stable-768；本目录只作为 incubating 预研实现。
\end{itemize}

\section{数学语义（Alg.13，$k=3$，$n=256$，$q=3329$）}
...
\texttt{output/ek\_pke.bin} & 1184B，$\text{ByteEncode}_{12}(\hat t)$ 的 1152B 后拼接 $\rho$ 的 32B \\
\texttt{output/dk\_pke.bin} & 1152B，$\text{ByteEncode}_{12}(\hat s)$，其中 $\hat s$ 为前 3 个秘密多项式 \\
\bottomrule
\end{longtable}
...
\begin{enumerate}
  \item $G(d\Vert \texttt{byte}(3))\to(\rho,\sigma)$；$k$ 字节锁定为 3。
  \item 行 3--7：对 $(i,j)\in\{0,1,2\}^2$ 执行 \texttt{SampleNTT}，生成 $\hat A\in R_q^{3\times3}$，GM 形状锁定为 \texttt{[9,256] int32}。
  \item 行 8--15：用 $\eta_1=\eta_2=2$ 采样 $s\Vert e$ 共 6 个 poly；GM 形状锁定为 \texttt{[6,256]}。
  \item 行 16--20：polyvec6 NTT 后计算 $\hat t=\hat A\circ\hat s+\hat e$；InnerProduct 输出 $P\_OUT=3$，AIV 分片为 2+1。
...
\midrule
1 prep & AIV\_ONLY，\texttt{blockDim=2} & $\hat A$ 共 9 poly 按 5+4 分给两个 AIV；$s\Vert e$ 共 6 poly 可先按 B3 交叉分片写 GM，compute 前按 B5 连续 polyvec6 语义消费。选择 2 AIV 是参数卡 §3.2 D13 锁定值，继承 D13 探针已验同步与 UB 分配。 \\
2 compute & MIX，\texttt{blockDim=1} & 1 AIC + 2 AIV；polyvec6 NTT 使用连续 3+3 AIV 映射；InnerProduct 输出 3 行，AIV0 负责 $\hat t[0,1]$，AIV1 负责 $\hat t[2]$。选择单 MIX AI Core 是 D13 锁定生产档，避免跨 AI Core GM 交换。 \\
\bottomrule
\end{longtable}
...
ByteEncode12 & $3\times384=1152$B；分别用于 $\hat t$ 与 $\hat s$ \\
\bottomrule
\end{longtable}
...
Compute 内部遵守 B5/B6 已验模式：AIV 写入平面 Stage1/Stage3 GM 后以 CrossCore flag 与 AIC 同步；AIC 写完 \texttt{mat\_c} 后 AIV 等待再做 Stage3 merge、InnerProduct 与 ByteEncode12。
...
API / 路线 & 不采用原因 \\
\midrule
\apiname{Compares}/\apiname{GatherMask} compact 链 & Alg.13 E13 不需要新增 reject compact；沿用 D13 已验 \texttt{Mins}/标量 rej 路径，不新增比较掩码风险 \\
高阶 \apiname{Matmul<>} & 本路线继承 D13 手写 MMAD + 平面 mat\_c 契约；高阶 Matmul 输出布局与 MIX 交接不作为本轮变量 \\
...
ByteEncode12 + $ek\Vert\rho$ & ByteEncode12 向量 + 尾部标量 pack & 部分向量 & pack 尾部标量缺口沿用 D13/k4 已验实现 \\
\bottomrule
\end{longtable}
...
\begin{verbatim}
cd examples/incubating/ml-kem/ml-kem-768/exp-fips203-mlkem-pke-keygen-k3
bash run.sh -r cpu -v Ascend910B4
SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
\end{verbatim}

期望：\texttt{ek\_pke.bin} 为 1184B，\texttt{dk\_pke.bin} 为 1152B；若开启 \texttt{KEYGEN\_VERIFY=1}，两者均与 golden 对拍 PASS。
...
后续可补 \texttt{kat\_liboqs\_vs\_ascendc.sh}，但 E13 本轮门禁为 CPU + \texttt{SIM\_DIRECT=1} sim。
liboqs 仅作为黑盒 I/O oracle；禁止把 liboqs 源码实现同构移植进 AscendC。

\section{产出物}

\begin{itemize}
  \item \dirname{exp-fips203-mlkem-pke-keygen-k3-实现方案-customspec.tex}
  \item \dirname{exp-fips203-mlkem-pke-keygen-k3-实现方案-customspec.pdf}
