...
/**
 * @file main_keygen.cpp
 * @brief FIPS 203 Alg.13 PKE KeyGen 全链 Host 入口（ML-KEM-768，k=3）。
 *
 * ## 流水线位置
 ...
 *
 * ## 与 golden 关系
 * 验收仅要求 I/O 等价：`output/ek_pke.bin`(1184B) / `dk_pke.bin`(1152B)
...
int32_t main(int32_t argc, char *argv[])
{
     ...
     * GM 输出：a_hat[9,256]、src[6,256]、rho[32]、prf_out（CBD 中间态，仍占 GM 但默认不落盘）。
     ...
     * 写 ek_pke（1184B）、dk_pke（1152B）；dst/t_hat/ek_poly 为内核中间 GM，生产路径不写盘。
     */
