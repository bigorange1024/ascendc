...
#
# 源：本用例 decrypt/（vendored stable Decrypt；禁止 frozen）
# 目标：本用例 shim/pke_decrypt/（生成物，可 rm -rf 后重跑本脚本）
#
# 不改 Alg.18 语义；不修改 decrypt/ 源码（仅复制+改名）。

set -euo pipefail

CASE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="${CASE_DIR}/decrypt"
...
echo "[dec_shim] OK ${DST} (from vendored decrypt/; conflict headers → dec_*)"
