% stable-fips203-mlkem-kem-encaps-k4 — FIPS 203 Alg.20 ML-KEM-1024 KEM Encaps (k=4)
% 编译：bash ../../../scripts/xelatex-clean.sh stable-fips203-mlkem-kem-encaps-k4-实现方案-customspec.tex
...
\title{stable-fips203-mlkem-kem-encaps-k4\\FIPS 203 Alg.20 ML-KEM-1024 KEM Encaps 实现方案（k=4）}
...
\textbf{晋级（2026-07-15）}：自
\dirname{examples/incubating/ml-kem/ml-kem-1024/exp-fips203-mlkem-kem-encaps-k4/}
\textbf{复制}晋级（\texttt{\#验收\#}）；预研副本保留。登记表：
\dirname{docs/specs/fips203-mlkem1024-kem-encaps-baseline-registry.md}。

\textbf{行为对照（只读；禁止把探针当编译依赖）}：
\dirname{ascendc-tests/ml-kem/ml-kem-1024/pass-fix-f203-alg20-kem-encaps-device-k4/}
...
写法对齐
\dirname{stable-fips203-mlkem-kem-keygen-k4-实现方案-customspec.tex}、
\dirname{stable-fips203-mlkem-pke-encrypt-k4-实现方案-customspec.tex}。
...
\begin{verbatim}
stable-fips203-mlkem-kem-encaps-k4/
  stable-fips203-mlkem-kem-encaps-k4-实现方案-customspec.tex/.pdf
...
\dirname{examples/stable/ml-kem/ml-kem-1024/stable-fips203-mlkem-pke-encrypt-k4/} 的 \dirname{prep/}、
\dirname{compute/}、Encrypt layout 头与必要 Host 胶水；
KEM 增量对照（\textbf{只读行为}，禁止编译依赖）
\dirname{ascendc-tests/ml-kem/ml-kem-1024/pass-fix-f203-alg20-kem-encaps-device-k4/kem/}。
...
已晋级 stable & 2026-07-15 \texttt{\#验收\#} 自 incubating 复制；预研副本保留 \\
...
\begin{enumerate}
  ...
  \item incubating 经验收阶梯稳定后，用户 \texttt{\#交付\#} 再\textbf{复制}晋级
        \dirname{examples/stable/ml-kem/ml-kem-1024/stable-fips203-mlkem-kem-encaps-k4/}（名称待交付时确认）；
        \textbf{禁止}未绿先晋级。
\end{enumerate}
