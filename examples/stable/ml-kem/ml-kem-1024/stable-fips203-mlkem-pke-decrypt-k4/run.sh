#!/usr/bin/env bash
# stable-fips203-mlkem-pke-decrypt-k4 — FIPS 203 Alg.15 PKE Decrypt（k=4）
#
# customspec：stable-fips203-mlkem-pke-decrypt-k4-实现方案-customspec.tex
...
RUN_MODE="cpu"
# 默认不写死 SEED_D；未 export 时 gen_data 用 SHA3（fips203_host_rng）
if [ -n "${SEED_D:-}" ]; then
    export SEED_D
fi
...
# 实机 ACL 设备号：npu 缺省 0（借入机 device1 上 l18_l19 复跑死锁；避让时再 ASCEND_DEVICE_ID=1）；SIM 强制 0（CAModel 仅设备 0）
...
echo "[SUCCESS] stable-fips203-mlkem-pke-decrypt-k4 (${RUN_MODE}) DECRYPT_VERIFY=${DECRYPT_VERIFY} COMPRESS_1_VEC=${COMPRESS_1_VEC} DECOMPRESS_D_VEC=${DECOMPRESS_D_VEC}"
