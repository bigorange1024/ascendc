# 自包含与设备全链约束 — stable-fips203-mlkem-pke-encrypt-k4

**customspec**：[`stable-fips203-mlkem-pke-encrypt-k4-实现方案-customspec.tex`](stable-fips203-mlkem-pke-encrypt-k4-实现方案-customspec.tex)  
...
**规则**：自 [`pass-fix-f203-alg14-pke-encrypt-device-k4`](../../../ascendc-tests/ml-kem/ml-kem-1024/pass-fix-f203-alg14-pke-encrypt-device-k4/) vendor 后切断外链；`alg11_*` / `multiply_ntts_*` 在 `compute/`。
...
**CPU**：辅助正确性孪生（5 launch + `golden_v`）；**不得**单独作为「设备全链已证」依据。无 NPU 前以 **SIM** 为交付主参考。
