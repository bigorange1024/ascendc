...
| 项 | 值 |
|----|-----|
| **状态** | **定型交付**（CPU + SIM + roundtrip + liboqs KAT 验收中） |
...
| **SIM tick（incubating 晋级前）** | **1032789**（D **286999** + E **745790**） |
...
| 门禁 | 结果 |
|------|------|
| incubating CPU / SIM | PASS（晋级前；`K` max=0） |
| incubating roundtrip（含拒绝） | PASS CPU+SIM |
| incubating liboqs KAT CPU×10 + SIM×3 | PASS |
| **stable** `bash run.sh -r cpu` | ⏳ 复验中 |
| **stable** `bash run.sh -r sim` | ⏳ 复验中 |
| **stable** liboqs KAT CPU×10 + SIM×3 | ⏳ 复验中 |
| **stable** roundtrip CPU+SIM | ⏳ 复验中 |
