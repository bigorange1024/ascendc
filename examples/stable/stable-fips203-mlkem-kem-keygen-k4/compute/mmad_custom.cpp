...
#if F203_KEYGEN_EK_PKE >= 1
    /*
     * 行 21 FuseEkPke（+ 可选 Alg.16 尾）：须读完整 sk_out[1536] / ek_out。
     * ByteEncode 由双 AIV 分片写；本核 PipeBarrier **不能**等待对端 AIV。
     *
     * 强制（customspec §landmines / 2026-07-13）：
     *   - SIM/设备：双 AIV SyncAll<isAIVOnly=true>() 后由 subBlockID==0 做 Fuse/尾；
     *   - CPU：tikicpu 偶发并行 → 仅靠「AIV1 做尾」仍可能抢跑 AIV0 的 poly；
     *     用 dk_kem_gm[0:2] 作 Encode-done 软旗（Tail 随后整段覆盖），AIV1 等齐再 Fuse/尾。
     */
    if (!AIC && runEncode) {
#ifdef ASCENDC_CPU_DEBUG
        // 软汇合：两 AIV 各自置位；AIV1 自旋等待 AIV0，避免并行时拷到未写完的 sk_out
        __gm__ volatile uint8_t *encDone = reinterpret_cast<__gm__ volatile uint8_t *>(dk_kem_gm);
        encDone[static_cast<uint32_t>(subBlockID)] = 1U;
        AscendC::PipeBarrier<PIPE_ALL>();
        if (subBlockID == 1) {
            while (encDone[0] != 1U) {
                AscendC::PipeBarrier<PIPE_ALL>();
