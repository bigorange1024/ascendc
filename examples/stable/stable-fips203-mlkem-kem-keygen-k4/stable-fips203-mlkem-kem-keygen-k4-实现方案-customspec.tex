% stable-fips203-mlkem-kem-keygen-k4 — FIPS 203 Alg.19 ML-KEM-1024 KEM KeyGen (k=4)
% 编译：bash ../../../scripts/xelatex-clean.sh stable-fips203-mlkem-kem-keygen-k4-实现方案-customspec.tex
...
\title{stable-fips203-mlkem-kem-keygen-k4\\FIPS 203 Alg.19 ML-KEM-1024 KEM KeyGen 实现方案（k=4）}
...
\begin{abstract}
本文档描述 \textbf{定型交付算子} \dirname{examples/stable/stable-fips203-mlkem-kem-keygen-k4/}：
...
\textbf{晋级（2026-07-14）}：自
\dirname{examples/incubating/exp-fips203-mlkem-kem-keygen-k4/}
\textbf{复制}晋级（\texttt{\#交付\#}）；预研副本保留。踩坑与强制同步见 \S\ref{sec:landmines}。

\textbf{行为对照（只读；禁止把探针当编译依赖）}：
\dirname{ascendc-tests/pass-fix-f203-alg19-kem-keygen-device-k4/}
（CPU+SIM PASS；SIM Total tick $\approx$ \textbf{713227}）。
定稿原理：\dirname{docs/notes/F203-KEM-Alg19-KeyGen设备全链技术总结.md}。
PKE 主体行为对齐 \dirname{examples/stable/stable-fips203-mlkem-pke-keygen-k4/}
...
\textbf{自包含原则（与 device 探针 CMake 策略不同）}：
...
写法对齐 \dirname{stable-fips203-mlkem-pke-keygen-k4-实现方案-customspec.tex}。
\end{abstract}
...
组件 & 锁定配置 \\
\midrule
Prep Â & \texttt{F203\_AHAT16\_BLOCK\_DIM=2}，双 AIV 并行 \texttt{BuildAHat16ShardWithUb} \\
...
\begin{verbatim}
stable-fips203-mlkem-kem-keygen-k4/
  stable-fips203-mlkem-kem-keygen-k4-实现方案-customspec.tex/.pdf
...
\dirname{fix-f203-alg19-kem-keygen-correctness-k4} 的 \texttt{ek\_kem}/\texttt{dk\_kem}
...
\begin{itemize}
  ...
  \item 本 stable 验收目标：与 device \textbf{同量级}（实测 $\approx$707k）；\textbf{远超}视为回归。
...
\begin{enumerate}
  ...
  \item incubating 经验收阶梯（含 G4b）稳定后，用户 \texttt{\#交付\#}/\texttt{\#验收\#}
        再\textbf{复制}晋级 \dirname{examples/stable/stable-fips203-mlkem-kem-keygen-k4/}；
