...
// @file compute/hat_gammas.hpp
// @layer compute
// @role compute/：Tag5T NTT + Alg.11 basemul + 行18–20 UB 融合 MMAD 内核与 host 驱动；第二次 launch，读 prep 写 GM + LUT，写 ek/sk 与 ek_pke。 / Full keygen compute (mmad_custom) sources. 本文件 `hat_gammas.hpp` 为该子模块组件。 / Component: hat_gammas.hpp.
 ...
 *
 * 用途：multiply_ntts_half_scalar、hat_reduce 路径；与 alg11_gammas.h / hat_inner_product_ref.c 数值一致。
 *
 * 调用方：hat_vec.hpp（标量 basemul）；向量路径优先 gAlg11GammasGm ROM。
 *
 * 不变量：128 个 γ；kHatQ=3329。
 *
...
/** FIPS 203 kMlkemGammas[128]，与 hat_inner_product_ref.c / mlkem_ntt_tables.h 一致。 */
constexpr int32_t kHatGammas[128] = {
