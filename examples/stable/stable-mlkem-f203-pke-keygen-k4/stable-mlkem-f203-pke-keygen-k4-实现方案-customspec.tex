% stable-mlkem-f203-pke-keygen-k4 — FIPS 203 Alg.13 ML-KEM-768 KeyGen (k=4)
% 编译：bash ../../../scripts/xelatex-clean.sh stable-mlkem-f203-pke-keygen-k4-实现方案-customspec.tex
...
\title{stable-mlkem-f203-pke-keygen-k4\\FIPS 203 Alg.13 ML-KEM-768 PKE KeyGen 实现方案（k=4）}
\author{ascendc 工程（定型交付算子）}
\date{2026-06-29}
...
\begin{abstract}
本文档描述 \textbf{定型交付算子} \dirname{examples/incubating/stable-mlkem-f203-pke-keygen-k4/}：
...
SIM 910B4 参考 Total tick $pprox$ 	extbf{542393}（prep $\approx$462k + mmad $\approx$80k）。
...
\midrule
Prep Â & \texttt{F203\_AHAT16\_BLOCK\_DIM=2}，\textbf{双 AIV 并行} \texttt{BuildAHat16ShardWithUb}（\texttt{blockIdx} 分片），内嵌 SHAKE \texttt{ProcessInline} \\
...
\begin{verbatim}
stable-mlkem-f203-pke-keygen-k4/
...
\begin{verbatim}
cd examples/incubating/stable-mlkem-f203-pke-keygen-k4
...
与 \dirname{ascendc-tests/pass-fix-f203-alg13-device-keygen-k4/} 同脚本契约；liboqs 0.15.0 PKE ref 静态链。

\section{性能（SIM 910B4 参考，2026-06-29）}

Total tick $pprox$ 	extbf{542393}（prep $\approx$462161 + mmad $\approx$80475；单进程 2 launch；\texttt{profile\_subtask\_log*.toml} 求和）。
