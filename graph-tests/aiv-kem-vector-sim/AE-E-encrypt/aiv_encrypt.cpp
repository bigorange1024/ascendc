/**
 * @file aiv_encrypt.cpp
 * @brief AE-E：ML-KEM-1024 单 AIV 单 launch 全 AscendC Encrypt。
 *
 * 流水线（同 launch）：
 *   (a) coins → SHAKE256+CBD η=2 → y/e₁/e₂（写 workspace GM）
 *   (b) ρ=ek[1536:1568] → SampleNTT×16 → Â（写 workspace GM）
 *   (c) ek[0:1536] → ByteDecode₁₂×4 → t̂（写 workspace GM）
 *   (d) NTT(y) → Âᵀ∘ŷ / ⟨t̂,ŷ⟩ → INTT → +e/+μ → Compress+ByteEncode → c
 *
 * Host 仅喂 ek|m|coins + NTT 常量表；Â/t̂/y/e/ŷ 为设备写 workspace。
 * 约束：KERNEL_TYPE_AIV_ONLY、blockDim=1、禁 Cube/CrossCore。
 */
...
constexpr int COINS_BYTES = 32;
/** Alg.7 固定挤出：224 候选对 × 3B = 672B（与 host golden / EN09 一致）。 */
constexpr int XOF_BYTES = 672;
constexpr int CAND_PAIRS = 224;
}  // namespace

/**
 * 单 AIV Encrypt 核：UB 驻留单 poly；采样与编解码标量；NTT/matvec 向量。
 */
class AivEncrypt {
public:
    /**
     * @param ekGm     ek_pke[1568]：前 1536B=ByteEncode₁₂(t̂)，末 32B=ρ
     * @param coinsGm  coins[32]：Encrypt 随机币（CBD 种子）
     * @param mGm      m[32] 消息
     * @param rootsGm…gammaGm  NTT/INTT/basemul 常量表
     * @param yGm…yHatGm  workspace（设备写；Host 仅 GmAlloc）
     * @param cGm      输出密文 uint8[1568]
     */
    __aicore__ inline void Run(GM_ADDR ekGm, GM_ADDR coinsGm, GM_ADDR mGm, GM_ADDR rootsGm,
                               GM_ADDR mapGm, GM_ADDR invRootsGm, GM_ADDR invMapGm,
                               GM_ADDR gammaGm, GM_ADDR yGm, GM_ADDR e1Gm, GM_ADDR e2Gm,
                               GM_ADDR aHatGm, GM_ADDR tHatGm, GM_ADDR yHatGm, GM_ADDR cGm)
    {
        ...
        pipe.InitBuffer(cPackBuf, 352);
        pipe.InitBuffer(ekBuf, EK_BYTES);
        pipe.InitBuffer(coinsBuf, COINS_BYTES);
        ...
        GlobalTensor<uint8_t> mG, cG, ekG, coinsG;
        ...
        coinsG.SetGlobalBuffer((__gm__ uint8_t *)coinsGm, COINS_BYTES);
        ...
        // —— (a)(b)(c)：设备采样与公钥展开，写 workspace ——
        {
            auto ekLocal = ekBuf.Get<uint8_t>();
            auto coinsLocal = coinsBuf.Get<uint8_t>();
            DataCopy(ekLocal, ekG, EK_BYTES);
            DataCopy(coinsLocal, coinsG, COINS_BYTES);
            PipeBarrier<PIPE_ALL>();
            ...
            for (int i = 0; i < COINS_BYTES; ++i) {
                coins[i] = coinsLocal.GetValue(i);
            }

            // (a) PRF=SHAKE256(coins‖nonce)→128B → CBD η=2 → y[0..3]/e1[0..3]/e2
            ...
            // (b) SampleNTT：Â[p,j] ← rej(SHAKE128(ρ‖j‖p))；平坦 (p*K+j)*N
            // 背景：与 FIPS203 / host golden_c.build_a_hat / liboqs 一致（列下标 j 在前）
            uint8_t rho[32];
            for (int i = 0; i < 32; ++i) {
            ...
            // (c) ByteDecode₁₂：ek[poly*384 : +384] → t̂[poly]（shared 标量积木）
...
private:
    /**
     * Alg.8 SamplePolyCBD_η=2：prf[128] → data[256] ∈[0,q)。
     * 与 AE-P / host golden build_re 同式。
     */
    __aicore__ inline void CbdEta2FromPrf(LocalTensor<int32_t> data, const uint8_t prf[128])
    {
        for (int i = 0; i < (N / 8); ++i) {
    ...
    /**
     * Alg.7 SampleNTT 单 poly：SHAKE128(ρ‖j‖p)→672B → 12-bit 拒绝采样满 256 系数。
     * @param j 列下标；@param p 行下标（矩阵 Â[p,j]；种子序与 FIPS/liboqs 一致）
     */
    __aicore__ inline void SampleNttPoly(LocalTensor<int32_t> out, const uint8_t rho[32], int j,
                                         int p)
    {
        uint8_t seed[34];
        for (int i = 0; i < 32; ++i) {
        ...
        seed[32] = static_cast<uint8_t>(j & 0xFF);
        seed[33] = static_cast<uint8_t>(p & 0xFF);
        uint8_t xof[XOF_BYTES];
        F203SeDeviceKeccak::Shake128OneShot(xof, static_cast<uint32_t>(XOF_BYTES), seed, 34);
        int nOut = 0;
        for (int t = 0; t < CAND_PAIRS && nOut < N; ++t) {
            ...
            if (d1 < static_cast<uint32_t>(kQ) && nOut < N) {
                out.SetValue(nOut, static_cast<int32_t>(d1));
                ++nOut;
            }
            if (d2 < static_cast<uint32_t>(kQ) && nOut < N) {
                out.SetValue(nOut, static_cast<int32_t>(d2));
                ++nOut;
            }
        }
        // 672B 在 ML-KEM-1024 上几乎总够满 256；不足时剩余系数置 0（对拍应不触发）
        while (nOut < N) {
            out.SetValue(nOut, 0);
            ++nOut;
        }
    ...
    TBuf<TPosition::VECCALC> ekBuf, coinsBuf;
};

/**
 * @brief 单 AIV Encrypt 入口：ek|coins|m + 表 → c；workspace 由设备填充。
 */
extern "C" __global__ __aicore__ void aiv_encrypt(
    GM_ADDR ekGm, GM_ADDR coinsGm, GM_ADDR mGm, GM_ADDR rootsGm, GM_ADDR mapGm,
    GM_ADDR invRootsGm, GM_ADDR invMapGm, GM_ADDR gammaGm, GM_ADDR yGm, GM_ADDR e1Gm,
    GM_ADDR e2Gm, GM_ADDR aHatGm, GM_ADDR tHatGm, GM_ADDR yHatGm, GM_ADDR cGm)
{
    KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_AIV_ONLY);
    AivEncrypt kernel;
    kernel.Run(ekGm, coinsGm, mGm, rootsGm, mapGm, invRootsGm, invMapGm, gammaGm, yGm, e1Gm,
               e2Gm, aHatGm, tHatGm, yHatGm, cGm);
}
