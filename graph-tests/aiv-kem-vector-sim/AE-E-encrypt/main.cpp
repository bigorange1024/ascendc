/**
 * @file main.cpp
 * @brief AE-E host：读 ek|m|coins + NTT 表 → aiv_encrypt 单 launch → output/c.bin。
 *
 * workspace（y/e1/e2/ahat/that/yhat）仅 GmAlloc，由设备写入；禁 ReadFile 喂采样中间态。
 * CPU：ICPU_RUN_KF + AIV_MODE；SIM：ACLRT_LAUNCH_KERNEL。禁 -r npu。
 */
...
#include "acl/acl.h"
#include "aclrtlaunch_aiv_encrypt.h"
#else
#include "tikicpulib.h"
#ifndef GM_ADDR
#define GM_ADDR int8_t *
#endif
extern "C" void aiv_encrypt(GM_ADDR ekGm, GM_ADDR coinsGm, GM_ADDR mGm, GM_ADDR rootsGm,
                            GM_ADDR mapGm, GM_ADDR invRootsGm, GM_ADDR invMapGm, GM_ADDR gammaGm,
                            GM_ADDR yGm, GM_ADDR e1Gm, GM_ADDR e2Gm, GM_ADDR aHatGm,
                            GM_ADDR tHatGm, GM_ADDR yHatGm, GM_ADDR cGm);
#endif
...
constexpr size_t COINS_BYTES = 32;
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    uint8_t *coins = alloc(COINS_BYTES);
    ...
    size_t rs = 0;
    if (!ReadFile("./input/ek_pke.bin", rs, ek, EK_BYTES) || rs != EK_BYTES) return 1;
    if (!ReadFile("./input/coins.bin", rs, coins, COINS_BYTES) || rs != COINS_BYTES) return 2;
    if (!ReadFile("./input/m.bin", rs, m, M_BYTES) || rs != M_BYTES) return 3;
    if (!ReadFile("./input/roots.bin", rs, roots, ROOT_BYTES) || rs != ROOT_BYTES) return 4;
    if (!ReadFile("./input/indices.bin", rs, map, MAP_BYTES) || rs != MAP_BYTES) return 5;
    if (!ReadFile("./input/inv_roots.bin", rs, invRoots, ROOT_BYTES) || rs != ROOT_BYTES) return 6;
    if (!ReadFile("./input/inv_indices.bin", rs, invMap, MAP_BYTES) || rs != MAP_BYTES) return 7;
    if (!ReadFile("./input/gammas.bin", rs, gamma, GAMMA_BYTES) || rs != GAMMA_BYTES) return 8;

    ICPU_RUN_KF(aiv_encrypt, blockDim, ek, coins, m, roots, map, invRoots, invMap, gamma, y, e1,
                e2, ahat, that, yhat, c);

    if (!WriteFile("./output/c.bin", c, C_BYTES)) return 9;
    AscendC::GmFree((void *)ek);
    AscendC::GmFree((void *)coins);
    ...
    size_t rs = 0;
    if (!ReadFile("./input/ek_pke.bin", rs, hek, EK_BYTES) || rs != EK_BYTES) return 1;
    if (!ReadFile("./input/coins.bin", rs, hcoins, COINS_BYTES) || rs != COINS_BYTES) return 2;
    if (!ReadFile("./input/m.bin", rs, hm, M_BYTES) || rs != M_BYTES) return 3;
    if (!ReadFile("./input/roots.bin", rs, hroots, ROOT_BYTES) || rs != ROOT_BYTES) return 4;
    if (!ReadFile("./input/indices.bin", rs, hmap, MAP_BYTES) || rs != MAP_BYTES) return 5;
    if (!ReadFile("./input/inv_roots.bin", rs, hinvR, ROOT_BYTES) || rs != ROOT_BYTES) return 6;
    if (!ReadFile("./input/inv_indices.bin", rs, hinvM, MAP_BYTES) || rs != MAP_BYTES) return 7;
    if (!ReadFile("./input/gammas.bin", rs, hgamma, GAMMA_BYTES) || rs != GAMMA_BYTES) return 8;

    CHECK_ACL(aclrtMemcpy(dek, EK_BYTES, hek, EK_BYTES, ACL_MEMCPY_HOST_TO_DEVICE));
    CHECK_ACL(aclrtMemcpy(dcoins, COINS_BYTES, hcoins, COINS_BYTES, ACL_MEMCPY_HOST_TO_DEVICE));
    ...
    ACLRT_LAUNCH_KERNEL(aiv_encrypt)
    (blockDim, stream, dek, dcoins, dm, droots, dmap, dinvR, dinvM, dgamma, dy, de1, de2, dahat,
     dthat, dyhat, dc);
    CHECK_ACL(aclrtSynchronizeStream(stream));

    CHECK_ACL(aclrtMemcpy(hc, C_BYTES, dc, C_BYTES, ACL_MEMCPY_DEVICE_TO_HOST));
    if (!WriteFile("./output/c.bin", hc, C_BYTES)) return 9;

    CHECK_ACL(aclrtFree(dek));
    CHECK_ACL(aclrtFree(dcoins));
    ...
    CHECK_ACL(aclrtFreeHost(hcoins));
