#!/bin/bash
# AV01-single-aiv-mlkem-ntt：单 AIV ML-KEM 正向 NTT（n=256,q=3329）
#
# Usage（默认 = 全量验收路径）：
...
if [ "${RUN_MODE}" = "npu" ]; then
    echo "[ERROR] AV01 本刀禁 -r npu（仅 cpu + SIM_DIRECT sim；NPU 需用户另授）" >&2
    exit 2
fi
...
echo "[SUCCESS] AV01-single-aiv-mlkem-ntt (${RUN_MODE})"
