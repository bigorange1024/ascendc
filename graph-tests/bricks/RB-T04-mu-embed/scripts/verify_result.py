#!/usr/bin/env python3
"""verify_result — 对拍 output/mu.bin 与 output/golden_mu.bin（max diff=0）。"""
...
def main() -> None:
    ...
    out_path = os.path.join(case, "output", "mu.bin")
    golden_path = os.path.join(case, "output", "golden_mu.bin")
