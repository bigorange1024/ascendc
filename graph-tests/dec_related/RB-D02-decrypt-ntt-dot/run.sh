#!/bin/bash
# RB-D02-decrypt-ntt-dot：Decrypt L2a — NTT(u)+su_dot → û/ŵ
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=120 / sim=600（标量 Alg.9×4 + Alg.11）
# DRW-D02 门禁：仅 CPU；本刀 subagent 禁止 -r npu
CURRENT_DIR=$(
...
echo "[SUCCESS] RB-D02-decrypt-ntt-dot (${RUN_MODE})"
