/**
 * @file light_cube.hpp
 * @brief RB-D03 极轻 Cube：单趟 16×32×32 int8→int32 MMAD（无算法正确性要求）。
 *
 * 作用：在 AIC Wait(1) 之后、Set(3) 之前占用 Cube 流水，证明握手中间可插入计算。
 * 输入/输出：GM 上 A[m,k] int8、B[k,n] int8 → C[m,n] int32（ND）。
 * 前置：仅在 AIC 路径调用；m=16、k=n=32。
 *
 * API 依据：查阅索引已有 DataCopy(Nd2Nz)/LoadData/Mmad/Fixpipe；本文件不引入新 API。
 */
#ifndef RB_D03_LIGHT_CUBE_HPP
#define RB_D03_LIGHT_CUBE_HPP
 ...
 * 与 û/ŵ golden 无关；仅要求 SIM/CPU 不挂死。
 */
class LightCube {
public:
