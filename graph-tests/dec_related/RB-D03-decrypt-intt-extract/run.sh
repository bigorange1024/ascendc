#!/bin/bash
# RB-D03-decrypt-intt-extract：Decrypt L2b — INTT(ŵ)+extract → m[32]
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=120 / sim=600（标量 Alg.10 + Compress₁）
# DRW-D03 门禁：仅 CPU；本刀 subagent 禁止 -r npu
CURRENT_DIR=$(
...
echo "[SUCCESS] RB-D03-decrypt-intt-extract (${RUN_MODE})"
