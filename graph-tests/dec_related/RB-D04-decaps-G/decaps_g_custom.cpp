/**
 * @file decaps_g_custom.cpp
 * @brief RB-D04-decaps-G / DRW-K01：单 AIV launch 设备侧 G(m'‖h) → (K'‖r')。
 *
 * 本文件在流水线中的位置：Decaps 重建战役第一刀（K01）。Decrypt 链产出的 m'[32]
 * 与 dk_kem 切片 h[32] 经本核做 FIPS 203 G=SHA3-512，写出 K'[32] 与 r'/coins[32]，
 * 供后续 K02 Re-Encrypt / K03 FO 消费。golden 仅 Host 对拍，不参与设备路径。
 *
 * 契约（S0B FEEDBACK §3 / Decrypt KB §A2）：
 *   - 输入：m'[32] ‖ h[32]；h 语义为 dk_kem[3104:3136) 切片（本刀测试可直接喂 h.bin，
 *     但禁止用默认可重算的 H(ek) 冒充本刀权威输入）。
 *   - 输出：K' = G 前 32B；r' = G 后 32B（与 Encaps G 约定对齐）。
 *   - 写出：UB + DataCopy → GM（X12）；禁 GlobalTensor::SetValue 写业务 GM。
 *   - 形态：KERNEL_TYPE_AIV_ONLY、blockDim=1；无 CrossCore / 无 flag 5/7。
 *
 * 背景：T22 FEEDBACK 思路「设备 G、Host 不预喂 coins」——本刀只继承该契约，
 * 未抄 RB-T22/T25–T27 / alg15/21 / examples decrypt|decaps / frozen 源码。
 * 设备原语：library/shared Keccak → F203SeDeviceKeccak::Sha3OneShot(mdlen=64)。
 */
 ...
 */
extern "C" __global__ __aicore__ void decaps_g_custom(GM_ADDR mPrimeGm, GM_ADDR hGm,
