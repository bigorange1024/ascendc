/**
 * @file main.cpp
 * @brief RB-D04 Host：三 launch 全链 Decrypt — L1 prep → L2a NTT+dot → L2b INTT+extract → m[32]。
 *
 * 本文件在流水线中的位置：仅 Host 编排（H2D/D2H + 三次 launch）；算法在三个独立设备核。
 * 对齐 S0A / DRW-D04：
 *   L1  ACLRT_LAUNCH(dec_prep_custom)         → SynchronizeStream
 *   L2a ACLRT_LAUNCH(dec_ntt_dot_custom)      → SynchronizeStream
 *   L2b ACLRT_LAUNCH(dec_intt_extract_custom) → SynchronizeStream
 *   D2H m[32]
 *
...
/**
 * 主流程：读 dk_pke+c → 三 launch → 落盘 m + 两段 TRACE + 中间量（诊断）。
 */
int32_t main(int32_t argc, char *argv[])
{
    ...
    constexpr size_t kWsL2a = tiling::wssize;
    ...
    // ---------- L2a：MIX NTT+dot ----------
    INFO_LOG("Host: Launch2a dec_ntt_dot_custom (CPU MIX twin)");
    ...
    if (!LoadWsL2b(wsL2b, wHatBuf, vBuf)) {
        ERROR_LOG("LoadWsL2b failed");
        return 4;
    }

    // ---------- L2b：MIX INTT+extract ----------
    INFO_LOG("Host: Launch2b dec_intt_extract_custom (CPU MIX twin)");
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ICPU_RUN_KF(dec_intt_extract_custom, blockDim, outL2b, mBuf, wsL2b, *tilingPtr);
    {
        auto *tr = reinterpret_cast<uint32_t *>(wsL2b + kD03OffTrace);
        tr[11] = 0x484F5355u; // MAGIC_HOST_POST_SYNC
        PrintTraceL2b(tr);
    }
    INFO_LOG("Host: sync after Launch2b; D2H m[32]");
    ...
    if (!LoadWsL2a(wsL2aHost, uHost, sHatHost)) {
        ERROR_LOG("LoadWsL2a failed");
        return 3;
    }
    ...
    // ---------- L2a ----------
    INFO_LOG("Host: Launch2a dec_ntt_dot_custom");
    ...
    if (!LoadWsL2b(wsL2bHost, wHatHost, vHost)) {
        ERROR_LOG("LoadWsL2b failed");
        return 4;
    }
    ...
    // ---------- L2b ----------
    INFO_LOG("Host: Launch2b dec_intt_extract_custom");
    ACLRT_LAUNCH_KERNEL(dec_intt_extract_custom)
    (blockDim, stream, outL2bDev, mDev, wsL2bDev, tilingPtr);
    CHECK_ACL(aclrtSynchronizeStream(stream));
    INFO_LOG("Host: SynchronizeStream after Launch2b; D2H m[32]");
    ...
    {
        auto *tr = reinterpret_cast<uint32_t *>(wsL2bHost + kD03OffTrace);
        tr[11] = 0x484F5355u;
        PrintTraceL2b(tr);
    }
