/**
 * @file main.cpp
 * @brief RB-D05 Host：Decaps Re-Encrypt 双 launch — prep(AIV) → mid-sync → mix(MIX)。
 *
 * 输入：ek[1568]、m'[32]、r'/coins[32]；输出：c'[1568]（设备 pack，禁 Host 预喂）。
 * Launch1：reenc_prep_custom（AIV-only，无 CrossCore）镜像 ek/coins/ρ。
 * Launch2：reenc_mix_custom（MIX，flag 1/3+4）μ←m' + Encrypt 全链 → c'。
 * 本文件在流水线中的位置：仅 Host 壳（H2D / dual launch / mid-sync / D2H）。
 */
...
#include "acl/acl.h"
#include "aclrtlaunch_reenc_mix_custom.h"
#include "aclrtlaunch_reenc_prep_custom.h"
#include <cstdlib>
#else
...
#endif
extern "C" void reenc_prep_custom(GM_ADDR coinsIn, GM_ADDR ekIn, GM_ADDR ws, TilingData tiling);
extern "C" void reenc_mix_custom(GM_ADDR out, GM_ADDR cOut, GM_ADDR ws, TilingData tiling);
#endif
...
static void PrintTrace(const uint32_t *tr)
{
    ...
    INFO_LOG("  [%u] AIV0_MU_DONE         = 0x%08X", SLOT_AIV0_MU_DONE, tr[SLOT_AIV0_MU_DONE]);
    ...
    const bool muDone = (tr[SLOT_AIV0_MU_DONE] == MAGIC_AIV0_MU_DONE);
    ...
    if (nttDone && muDone) {
        INFO_LOG("causal: device Decompress1(m') wrote mu");
    }
    if (nttDone && bd12Done) {
    ...
    if (nttDone && mulDone && gateSeen && inttDone) {
        INFO_LOG("causal: Launch2 MU->BD12->CBD->AHAT/YHAT->MUL->GATE->INTT linked");
    }
    if (uvDone) {
...
/**
 * 装填 Host 侧 ws：原始 m'→OFF_M + ζ/γ/mat；清零 μ/t̂/Â/ŷ/YEE/u/v/c。
 * 背景：Decaps Re-Encrypt；μ 由设备 Decompress₁(m')；禁预喂最终 c'/μ。
 */
static bool LoadWorkspaceHost(uint8_t *ws, const uint8_t *m)
{
    using namespace tiling;
    size_t got = 0;
    ...
    // Host 仅预装原始消息；OFF_MU 清零，由设备 Decompress₁ 填写
    std::memcpy(ws + OFF_M, m, kMsgBytes);
...
/**
 * 写出诊断产物：μ_dev（设备 Decompress₁(m')）。
 * 权威交叉对象是 c'；此处不转发 Encaps 的 K_host。
 */
static bool WriteReencDiagOutputs(const uint8_t *ws)
{
    using namespace tiling;
    if (!WriteFile("./output/mu_dev.bin", ws + OFF_MU, kMuBytes)) {
        return false;
    }
    return true;
}
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    size_t got = 0;
    ok = ReadFile("./input/r_prime.bin", got, coinsIn, kCoinsBytes);
    ...
    ok = ReadFile("./input/m_prime.bin", got, mIn, 32);
    if (!ok || got != 32) {
        return 4;
    }
    if (!LoadWorkspaceHost(ws, mIn)) {
        return 3;
    }

    INFO_LOG("Host: Decaps Re-Encrypt; Launch1 reenc_prep_custom (CPU AIV twin)");
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(reenc_prep_custom, blockDim, coinsIn, ekIn, ws, *tiling);

    auto *tr = reinterpret_cast<uint32_t *>(ws + OFF_TRACE);
    tr[SLOT_HOST_MID_SYNC] = MAGIC_HOST_MID;
    INFO_LOG("Host: mid-sync after Launch1, Launch2 reenc_mix_custom (CPU MIX twin)");
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ICPU_RUN_KF(reenc_mix_custom, blockDim, out, cOut, ws, *tiling);
    ...
    ok = WriteFile("./output/c_prime.bin", cOut, kCBytes);
    if (!ok) {
        return 15;
    }
    if (!WriteReencDiagOutputs(ws)) {
        return 26;
    }
    ...
    uint8_t *coinsHost = nullptr, *ekHost = nullptr, *mHost = nullptr, *outHost = nullptr,
            *cHost = nullptr, *wsHost = nullptr;
    ...
    size_t got = 0;
    ok = ReadFile("./input/r_prime.bin", got, coinsHost, kCoinsBytes);
    if (!ok || got != kCoinsBytes) {
        return 1;
    }
    ...
    ok = ReadFile("./input/m_prime.bin", got, mHost, 32);
    if (!ok || got != 32) {
        return 4;
    }
    if (!LoadWorkspaceHost(wsHost, mHost)) {
        return 3;
    }
    ...
    INFO_LOG("Host: Decaps Re-Encrypt; Launch1 reenc_prep_custom (SIM/NPU)");
    ACLRT_LAUNCH_KERNEL(reenc_prep_custom)(blockDim, stream, coinsDev, ekDev, wsDev, tiling);
    ...
    INFO_LOG("Host: Launch2 reenc_mix_custom (SIM/NPU)");
    ACLRT_LAUNCH_KERNEL(reenc_mix_custom)(blockDim, stream, outDev, cDev, wsDev, tiling);
    CHECK_ACL(aclrtSynchronizeStream(stream));
    ...
    ok = WriteFile("./output/c_prime.bin", cHost, kCBytes);
    if (!ok) {
        return 15;
    }
    if (!WriteReencDiagOutputs(wsHost)) {
        return 26;
    }
