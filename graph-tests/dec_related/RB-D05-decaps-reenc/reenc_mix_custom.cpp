/**
 * @file reenc_mix_custom.cpp
 * @brief RB-D05 Launch2 MIX：握手 + μ←m' + Decode₁₂→CBD→Â/ŷ→Mul/INTT→pack→c'。
 *
 * 本文件在流水线中的位置：Decaps Re-Encrypt Encrypt 面（接 prep mid-sync）。
 * 生产外形（禁 Host 预喂最终 t̂/y/e/Â/ŷ/u/v/c'/μ）：
 *   AIC：Wait(1)→CubeNTT→Set(3) → Wait(4) → Wait(1)→CubeINTT→Set(3)
 *   AIV：Set(1)→Wait(3) → μ←m' → Decode₁₂ → CBD → Â←SampleNTT(ρ) → ŷ←NTT(y) → Mul → Set(4)
 *        → Set(1)→Wait(3) → INTT+噪 → pack（UB+DataCopy，X12）
 *
 * Flag：1/3 复用 + 4=GATE；永禁 5/7；F203_AHAT16_BLOCK_DIM=1。
 * 背景：S0B 契约表；积木契约对齐 T04/T05/T06 STATUS（非抄 T22–T27）。
 */
 ...
 */
extern "C" __global__ __aicore__ void reenc_mix_custom(GM_ADDR out, GM_ADDR cOut, GM_ADDR ws,
                                                     TilingData tiling)
{
    (void)tiling;
