/**
 * @file reenc_prep_custom.cpp
 * @brief RB-D05 Launch1：AIV-only 头 — coins(r')/完整 ek → workspace；ρ←ek 尾。
 *
 * 本文件在流水线中的位置：Decaps Re-Encrypt 双 launch 之 prep（无 CrossCore）。
 * Host mid-sync 后再启 reenc_mix_custom（MIX Encrypt 面）。
 *
 ...
 * 未采用：与 Decrypt 三核 fused；flag 5/7 SoftSync；Host 预喂最终 c'。
 */
 ...
 */
extern "C" __global__ __aicore__ void reenc_prep_custom(GM_ADDR coinsIn, GM_ADDR ekIn, GM_ADDR ws,
                                                        TilingData tiling)
{
    (void)tiling;
