#!/bin/bash
# RB-D05-decaps-reenc：Decaps Re-Encrypt ek+m'+r'→c'[1568]，双 launch（prep AIV + mix MIX）
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=600 / sim=900
# DRW-K02 门禁：仅 CPU；本刀 subagent 禁止 -r npu / SSH
CURRENT_DIR=$(
...
echo "[SUCCESS] RB-D05-decaps-reenc (${RUN_MODE})"
