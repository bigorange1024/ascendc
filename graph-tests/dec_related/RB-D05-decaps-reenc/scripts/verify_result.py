...
# -*- coding: utf-8 -*-
"""
RB-D05 verify：PASS_SYNC（双 launch TRACE）+ PASS_IO（c'≡Encrypt oracle max=0）。

权威：output/golden_c_prime.bin（gen_data 优先 liboqs_pke_ref；见 cross_backend.txt）。
诊断：μ/u/v/t̂/Â/ŷ/yee/ρ 尽力对拍（host 拓扑）；不同源时 c' 仍为硬门。
"""
...
def _cmp_bytes(got: Path, golden: Path, label: str) -> bool:
    ...
    if a != b:
        ...
        return False
    print(f"[PASS_IO] {label} match max=0 n={len(a)}")
    return True
...
def main() -> int:
    ...
    if len(tr) < 27 * 4:
        ...
        return 1
    vals = list(struct.unpack_from("<27I", tr, 0))
    ...
    print("[PASS_SYNC] dual-launch causal + MU + BD12 + CBD + AHAT/YHAT + dual Cube + pack TRACE ok")

    backend = "unknown"
    cb = OUT / "cross_backend.txt"
    if cb.is_file():
        backend = cb.read_text(encoding="utf-8").strip() or "unknown"

    # 硬门：c' ≡ Encrypt oracle
    c_ok = _cmp_bytes(OUT / "c_prime.bin", OUT / "golden_c_prime.bin", "c_prime")
    if not c_ok:
        print(f"[FAIL] c' mismatch vs {backend} Encrypt oracle", file=sys.stderr)
        return 2

    # 诊断中间量（尽力；不覆盖 c' 门）
    diag_ok = True
    diag_ok = _cmp_i32(OUT / "mu_dev.bin", OUT / "golden_mu.bin", "mu_dev") and diag_ok
    diag_ok = _cmp_i32(OUT / "u.bin", OUT / "golden_u.bin", "u") and diag_ok
    diag_ok = _cmp_i32(OUT / "v.bin", OUT / "golden_v.bin", "v") and diag_ok
    diag_ok = _cmp_i32(OUT / "t_hat_dev.bin", OUT / "golden_t_hat.bin", "t_hat") and diag_ok
    diag_ok = _cmp_i32(OUT / "a_hat.bin", OUT / "golden_a_hat.bin", "a_hat") and diag_ok
    diag_ok = _cmp_i32(OUT / "y_hat.bin", OUT / "golden_y_hat.bin", "y_hat") and diag_ok
    diag_ok = _cmp_bytes(OUT / "y_e1_e2_dev.bin", OUT / "golden_y_e1_e2.bin", "y_e1_e2_dev") and diag_ok
    diag_ok = _cmp_bytes(OUT / "rho_dev.bin", OUT / "golden_rho.bin", "rho_dev") and diag_ok

    if diag_ok:
        print(f"[SUCCESS] PASS_SYNC + PASS_IO — RB-D05 c'≡{backend} + diag max=0")
    else:
        print(
            f"[SUCCESS] PASS_SYNC + PASS_IO(c') — oracle={backend}; "
            "diag partial (c' hard gate already green)"
        )
    return 0
