#!/bin/bash
# RB-D06-decaps-fo：Decaps 设备 FO（c≟c' → K' 或 J(z‖c)）→ K[32]
#
# Usage（默认 = 全量验收；同 binary 跑合法+拒绝两路径）：
#   bash run.sh -r cpu -v Ascend910B4
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# 调试（非默认）：SIM_DIRECT=0 bash run.sh -r sim -v Ascend910B4  # msprof
#   FO_PATH=legit|reject  — 仅由本脚本内部设置；勿手改 golden
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=60 / sim=120（仅 kernel 计算段）
# DRW-K03 门禁：仅 CPU；本刀 subagent 禁止 -r npu / SSH
CURRENT_DIR=$(
    cd $(dirname ${BASH_SOURCE:-$0})
    pwd
)
_ORIG_ARGS=("$@")
# graph-tests/dec_related/<case> → 仓库根 ../../..
...
# 同 binary：合法路径 → 拒绝路径（FO_PATH 选 input/<path>/）
for _fo in legit reject; do
    echo "[run] FO_PATH=${_fo}"
    export FO_PATH="${_fo}"
...
echo "[SUCCESS] RB-D06-decaps-fo both paths (${RUN_MODE})"
