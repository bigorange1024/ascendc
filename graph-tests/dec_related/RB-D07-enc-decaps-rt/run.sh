#!/bin/bash
# RB-D07-enc-decaps-rt：设备 Encaps↔Decaps 往返；多 launch + mid-sync；七核 basename 唯一
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=900 / sim=1200（全链多 launch）
# DRW-K04 门禁：仅 CPU + sync_audit；subagent 禁止 -r npu / ×30 / SSH
CURRENT_DIR=$(
...
echo "[SUCCESS] RB-D07-enc-decaps-rt (${RUN_MODE})"
