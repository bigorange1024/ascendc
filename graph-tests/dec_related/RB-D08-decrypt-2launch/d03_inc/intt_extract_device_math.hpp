                                  ...
                                  const AscendC::LocalTensor<int32_t> &zetas)
{
    ...
    for (int32_t length = 2; length <= 128; length *= 2) {
        for (int32_t start = 0; start < tiling_d03::kPolyN; start += 2 * length) {
    ...
    // 末乘 128^{−1} mod q
    for (int32_t i = 0; i < tiling_d03::kPolyN; ++i) {
        const int32_t x = f.GetValue(static_cast<uint32_t>(i));
        f.SetValue(static_cast<uint32_t>(i), ModQ(x * tiling_d03::kInttScale));
    }
}
...
{
    const int64_t s = tiling_d03::kCompressC * static_cast<int64_t>(u) + tiling_d03::kCompress1Bias;
    return static_cast<int32_t>(s >> tiling_d03::kCompress1Shift) & 1;
}
