#!/bin/bash
# RB-D04-decrypt-full：Decrypt 全链三 launch：prep → NTT+dot → INTT+extract → m[32]
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=180 / sim=600（三核串行）
# DRW-D04 门禁：仅 CPU；本刀 subagent 禁止 -r npu
CURRENT_DIR=$(
...
echo "[SUCCESS] RB-D04-decrypt-full (${RUN_MODE})"
