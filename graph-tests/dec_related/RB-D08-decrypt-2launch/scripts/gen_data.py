#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gen_data — RB-D04 Decrypt 全链：dk_pke+c → golden_m。

权威优先：scripts/liboqs_pke_ref（PKE KeyGen+Encrypt+Decrypt）。
缺库时回落 host FIPS 自洽链（D01+D02+D03 契约拼装），并写 cross_backend.txt 标明。

输入落盘：dk_pke.bin / c.bin / zetas.bin / gammas.bin / mat_a.bin / mat_b.bin
...
def _host_fips_full_chain(inp: Path, out: Path) -> bytes:
    """Host FIPS 全链自洽 oracle（非 liboqs）：造合法 dk/c 并使 decrypt(m)=plain。

    构造：随机 ŝ/u/v 时域；ŵ=Σ MultiplyNTTs(ŝ,NTT(u))；令 plain 经 Compress₁ 嵌入
    到 (v−INTT(ŵ)) 的低比特差；再 encode 成 dk_pke/c。为简化验收，直接用
    Alg.15 正向：给定 ŝ、u、v、m 目标 → 不保证与「真实 encrypt」同分布，但
    Decrypt(ŝ,u,v) 数学自洽。
    """
    rng = np.random.default_rng(SEED_D)

    # ŝ：NTT 域私钥；u/v：时域密文分量（压缩前）
    s_hat = rng.integers(0, Q, size=(K, N), dtype=np.int32)
    u = rng.integers(0, Q, size=(K, N), dtype=np.int32)
    # 先算 w = INTT(Σ MultiplyNTTs(ŝ, NTT(u)))
    ...
    # 设备侧 Decompress 后 ≠ 原 u/v；oracle m 必须用 decompress 后的值重算
    ...
    (out / "plain_m.bin").write_bytes(m)  # 诊断：压缩往返后可能 ≠ golden
    return golden
...
def _liboqs_chain(inp: Path, out: Path, ref: Path) -> bytes:
    ...
    with tempfile.TemporaryDirectory(prefix="rb_d04_") as td:
        tdp = Path(td)
        ek = tdp / "ek.bin"
        dk = tdp / "dk.bin"
        c = tdp / "c.bin"
