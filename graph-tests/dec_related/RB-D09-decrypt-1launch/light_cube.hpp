/**
 * @file light_cube.hpp
 * @brief RB-D09 极轻 Cube：单趟 16×32×32 int8→int32 MMAD（握手段有界真算）。
 *
 * 作用：NTT / INTT 各段 AIC 路径各调用一次；与 Decrypt 数学无关。
 * API：DataCopy(Nd2Nz)/LoadData/Mmad/Fixpipe（查阅索引既有登记）。
 */
#ifndef RB_D09_LIGHT_CUBE_HPP
#define RB_D09_LIGHT_CUBE_HPP
...
public:
    ...
    /**
     * 执行一次 Cube 握手计算。
     * @param dst C 输出 GM（int32 ND）
     * @param a   A 输入 GM（int8 ND）
     * @param b   B 输入 GM（int8 ND）
     */
