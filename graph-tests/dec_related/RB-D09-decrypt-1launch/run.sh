#!/bin/bash
# RB-D09-decrypt-1launch：Alg.15 Decrypt 单 Host launch（prep+NTT+INTT 融合 MIX）
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu/npu=180 / sim=600
# NPU 战役用例：ASCENDC_CASE_SUPPORTS_NPU=1
CURRENT_DIR=$(
...
echo "[SUCCESS] RB-D09-decrypt-1launch (${RUN_MODE})"
