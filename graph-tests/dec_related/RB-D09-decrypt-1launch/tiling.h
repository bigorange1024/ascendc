/**
 * @file tiling.h
 * @brief RB-D09：ML-KEM-1024 Alg.15 Decrypt — 单 Host launch 融合 prep+NTT+INTT。
 *
 * Flag（融合 MIX 内复用，各段完整一轮）：
 *   1 = AIV→AIC 就绪
 *   3 = AIC→AIV Cube 完成
 * 永禁 5 / 7；Wait 环内禁 SyncAll / SoftSync。
 * Prep 后 SyncAll 仅发生在 CrossCore Wait 环外。
 *
 ...
 * 禁抄 examples/stable、frozen、alg15 生产核。
 */
#ifndef RB_D09_DEC_TILING_H
#define RB_D09_DEC_TILING_H

#include <cstddef>
#include <cstdint>

struct D09TilingData {
    int32_t reserved0;
    int32_t reserved1;
};
...
/** Compress₁ 统一整数式：(C·u + 2^{35}) >> 36 & 1（docs/notes 统一整数总结 §2）。 */
...
/** 极轻 Cube：握手段有界真算 16×32×32 int8→int32。 */
...
/**
 * 统一 workspace（Host 预喂 ζ/γ/mat/TRACE；prep 写 ŝ/u/v；NTT 写 û/ŵ；INTT 写 w/m）：
 *   S_HAT → U → V → ZETAS → GAMMAS → U_HAT → W_HAT → W →
 *   MAT_A → MAT_B → MAT_C_NTT → MAT_C_INTT → TRACE → M → PREP_MARK
 *
 * 说明：dk/c 不入 ws 镜像；设备 prep 直读 H2D GM（NPU 安全）。
 */
...
constexpr uint32_t MAGIC_OUT_OK = 0x44303931u; // "D091"
