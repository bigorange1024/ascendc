/**
 * EN01-kem256-ntt-port · Host 启动壳
 *
 * 迁入来源：thirdparty/cann-ntt-author-merged_dsa/main.cpp
 * 作用：读 input/{tiling,src,M4}.bin，一次 launch mmad_custom，写出 output/dst.bin。
 * CPU 孪生走 ICPU_RUN_KF；SIM/NPU 走 ACLRT_LAUNCH_KERNEL（本刀禁 npu）。
 */
/**
 * @file main.cpp
 *
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    const size_t wsBench = tiling->bench < maxBenchTile
                         ? tiling->bench
                         : maxBenchTile;
    const size_t wsFileSize = tiling_one::WorkspaceSizeForBench(wsBench);
    size_t srcFileSize = tiling_one::n * sizeof(int32_t) * tiling->bench;
    size_t dstFileSize = tiling_one::n * sizeof(int32_t) * tiling->bench;
    ...
    if(!ok) return 9;
    ok = ReadFile("./input/M4.bin", matMFileSize, ws + 0, matMFileSize);
    if(!ok) return 10;
    ...
    ok = ReadFile("./input/M4.bin", matMFileSize, wsHost, matMFileSize);
    if(!ok) return 10;
