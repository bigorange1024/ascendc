/**
 * EN01-kem256-ntt-port · 正向 NTT MIX 核（KernelLaunch）
 *
 * 迁入来源：thirdparty/cann-ntt-author-merged_dsa/mmad_custom.cpp
 * 本线角色：Encrypt×cann-ntt 线第一刀积木——独立短 MIX 跑 ŷ=NTT(y)；
 *           q=3329 走 2-digit Kyber 路径（AivSplit2 + AivMergeKyber）；禁 GATE/INTT。
 * CrossCore：保留作者包既有 flag 语义（AIV_SPLIT / AIC_MMAD），勿改。
 */
#include "basic.hpp"
