/**
 * EN02-ntt-intt-2launch · 矩阵化 NTT/INTT MIX 核（与 EN01 同构）
 *
 * 基线：复制自 EN01（作者包 merged_dsa）。Host 两段 launch 共用本核：
 *   Launch1 喂正向 M4；Launch2 喂 inverse M4——核内不区分方向。
 * q=3329 走 2-digit Kyber 路径（AivSplit2 + AivMergeKyber）；禁 GATE 4/8。
 * CrossCore：保留作者包既有 flag 语义（AIV_SPLIT / AIC_MMAD），勿改（B1/B2）。
 */
#include "basic.hpp"
