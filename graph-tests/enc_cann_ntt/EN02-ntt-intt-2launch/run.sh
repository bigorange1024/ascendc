#!/bin/bash
# EN02-ntt-intt-2launch：Host 连续两段 launch（正向 NTT → 逆向 INTT）粘性
# 基线复制自 EN01-kem256-ntt-port；核同构，仅 Host 换 M4+输入。
#
# Usage（默认 = Kyber 档；本刀禁止 -r npu）：
#   bash run.sh -r cpu -v Ascend910B4
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# 主门禁：不挂（两段 kernel 均正常退出）。golden 对拍失败仅标 CORRECTNESS_SOFT_FAIL。
# KERNEL_COMPUTE_BUDGET_SEC：CPU 默认 180；SIM 默认 900（双 launch，防挂死）
...
_ORIG_ARGS=("$@")
# graph-tests/enc_cann_ntt/EN02-… → 仓库根为三层上
...
if [ "${RUN_MODE}" = "npu" ]; then
    echo "[ERROR] EN02 本刀禁止 -r npu（NPU 长期占用；任务书仅 CPU+SIM）" >&2
    exit 2
fi
...
if [ "${_rc}" -ne 0 ]; then
    echo "[FAIL] kernel exit=${_rc} (${RUN_MODE}) — dual-launch hang or error" >&2
    exit "${_rc}"
fi
...
if [ "${_vr}" -ne 0 ]; then
    ...
    echo "[SUCCESS] EN02-ntt-intt-2launch (${RUN_MODE}) dual-launch exit=0 + golden match"
fi
exit 0
