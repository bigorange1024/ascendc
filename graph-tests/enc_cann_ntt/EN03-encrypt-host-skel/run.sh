#!/bin/bash
# EN03-encrypt-host-skel：Encrypt 形 Host 五段编排（Prep桩→NTT→Matvec桩→INTT→Pack桩）
# 基线：EN02 NTT/INTT 积木 + 新建 AIV 轻桩；禁融胖 MIX GATE。
#
# Usage（默认 = Kyber 档；本刀禁止 -r npu）：
#   bash run.sh -r cpu -v Ascend910B4
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# 主门禁：不挂（五段 kernel 均正常退出）。NTT/INTT golden 对拍失败仅标 CORRECTNESS_SOFT_FAIL。
...
_ORIG_ARGS=("$@")
# graph-tests/enc_cann_ntt/EN03-… → 仓库根为三层上
...
if [ "${RUN_MODE}" = "npu" ]; then
    echo "[ERROR] EN03 本刀禁止 -r npu（NPU 长期占用；任务书仅 CPU+SIM）" >&2
    exit 2
fi
...
# 正确性次要：仅软对拍 NTT/INTT；失败不否决「不挂」门禁
set +e
python3 "${CURRENT_DIR}/scripts/verify_result.py"
_vr=$?
set -e
if [ "${_vr}" -ne 0 ]; then
    echo "[CORRECTNESS_SOFT_FAIL] NTT/INTT golden 对拍未过（${RUN_MODE}）；kernel 已不挂 exit=0"
else
    echo "[SUCCESS] EN03-encrypt-host-skel (${RUN_MODE}) five-segment exit=0 + NTT/INTT golden match"
fi
exit 0
