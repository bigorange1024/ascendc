/**
 * EN01-kem256-ntt-port · 设备侧调试宏
 * 迁入来源：thirdparty/cann-ntt-author-merged_dsa/basic.hpp
 */
#ifndef __BASIC_HPP_
#define __BASIC_HPP_
...
{\
	...
	if (__check_diff_num == 0) /*AscendC::printf("\033[1;92mNo diff between %s and %s\033[0m %s:%d\n", #x, #y, __FILE__, __LINE__)*/ ;\
	else {AscendC::printf("[%ld]\033[1;31m%d diff between %s and %s(%d), earliest = (%lx, %lx at %d)\033[0m %s:%d\n",AscendC::GetBlockIdx(), __check_diff_num, #x, #y, len, __check_diff_x,__check_diff_y,__check_diff_line, __FILE__, __LINE__);\
		{_catch;}}\
}
