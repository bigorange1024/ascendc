/**
 * EN04-matvec-realbrick · Encrypt 形 Host 五段编排
 *
 * 段序（同进程、同 session；NTT/INTT/Matvec 各自独立 launch，禁融胖 MIX）：
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    // Matvec 已锁：K=4、N=256、q=3329（与 tiling / gen_data 一致）
    ...
    // -------- L1 Prep 桩 --------
    std::printf("[EN04] CPU L1 Prep begin\n");
    ...
    if (!ok) return 32;
    std::printf("[EN04] CPU L1 Prep done\n");

    // -------- L2 NTT 真积木 --------
    std::printf("[EN04] CPU L2 NTT begin\n");
    ...
    if (!ok) return 11;
    std::printf("[EN04] CPU L2 NTT done\n");

    // -------- L3 Matvec 真积木（独立 AIV；Â/ŝ/γ 自 input，不接 NTT 输出布局）--------
    std::printf("[EN04] CPU L3 Matvec begin\n");
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ...
    if (!ok) return 42;
    std::printf("[EN04] CPU L3 Matvec done\n");

    // -------- L4 INTT 真积木 --------
    std::printf("[EN04] CPU L4 INTT begin\n");
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ...
    if (!ok) return 21;
    std::printf("[EN04] CPU L4 INTT done\n");

    // -------- L5 Pack 桩 --------
    std::printf("[EN04] CPU L5 Pack begin\n");
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ...
    if (!ok) return 52;
    std::printf("[EN04] CPU L5 Pack done\n");
    ...
    uint8_t *dstHost, *srcHost, *wsHost, *sHatHost, *gammaHost;
    uint8_t *dstDevice, *srcDevice, *wsDevice, *sHatDevice, *gammaDevice;
    ...
    std::printf("[EN04] SIM L1 Prep begin\n");
    ...
    if (!ok) return 32;
    std::printf("[EN04] SIM L1 Prep done\n");

    // -------- L2 NTT --------
    std::printf("[EN04] SIM L2 NTT begin\n");
    ...
    if (!ok) return 11;
    std::printf("[EN04] SIM L2 NTT done\n");

    // -------- L3 Matvec 真积木（NTT 之后；不 recreate stream，禁 B3）--------
    std::printf("[EN04] SIM L3 Matvec begin\n");
    size_t aHatRead = aHatBytes;
    size_t sHatRead = sHatBytes;
    size_t gammaRead = gammaBytes;
    ...
    if (!ok) return 42;
    std::printf("[EN04] SIM L3 Matvec done\n");

    // -------- L4 INTT --------
    std::printf("[EN04] SIM L4 INTT begin\n");
    ...
    if (!ok) return 21;
    std::printf("[EN04] SIM L4 INTT done\n");

    // -------- L5 Pack 桩 --------
    std::printf("[EN04] SIM L5 Pack begin\n");
    ...
    if (!ok) return 52;
    std::printf("[EN04] SIM L5 Pack done\n");
...
#endif
    std::printf("[EN04] five segments finished OK\n");
    return 0;
}
