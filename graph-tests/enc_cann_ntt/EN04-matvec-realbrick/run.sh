#!/bin/bash
# EN04-matvec-realbrick：Encrypt 形 Host 五段（Prep桩→NTT→Matvec真积木→INTT→Pack桩）
# 基线：EN03 壳；L3 换真 NTT 域内积；禁融胖 MIX GATE。
#
...
# 已锁：NTT_N=256 NTT_Q=3329 NTT_REF=kyber NTT_BENCH=4；Matvec K=4；禁擅自改参
CURRENT_DIR=$(
    cd $(dirname ${BASH_SOURCE:-$0})
    pwd
)
_ORIG_ARGS=("$@")
# graph-tests/enc_cann_ntt/EN04-… → 仓库根为三层上
...
if [ "${RUN_MODE}" = "npu" ]; then
    echo "[ERROR] EN04 本刀禁止 -r npu（NPU 长期占用；任务书仅 CPU+SIM）" >&2
    exit 2
fi
...
# 正确性次要：软对拍 NTT/Matvec/INTT；失败不否决「不挂」门禁
set +e
python3 "${CURRENT_DIR}/scripts/verify_result.py"
_vr=$?
set -e
if [ "${_vr}" -ne 0 ]; then
    echo "[CORRECTNESS_SOFT_FAIL] NTT/Matvec/INTT golden 对拍未过（${RUN_MODE}）；kernel 已不挂 exit=0"
else
    echo "[SUCCESS] EN04-matvec-realbrick (${RUN_MODE}) five-segment exit=0 + NTT/Matvec/INTT golden match"
fi
exit 0
