/**
 * EN05-prep-sample-realbrick · Encrypt 形 Host 五段编排
 *
 * 段序（同进程、同 session；NTT/INTT/Matvec/Prep 各自独立 launch，禁融胖 MIX）：
 *   L1 Prep   — enc_prep_cbd_real （真 PRF+CBD η=2；独立 AIV）
 *   L2 NTT    — mmad_custom      （EN02 正向积木 + M4_ntt；本刀仍读独立 src_ntt）
 *   L3 Matvec — enc_matvec_real  （真 NTT 域 4×4×1 内积；Â Host 喂）
 *   L4 INTT   — mmad_custom      （换 M4_intt）
 *   L5 Pack   — enc_pack_stub    （AIV 轻桩）
 *
 * Prep→NTT 转接：Prep 输出布局 = [k=4,n=256] 行主序 int32，与 NTT src 同构；
 * 本刀 Host 不自动链（L2 仍用 gen_data 的 src_ntt 对拍）；需要链式时 Host memcpy 即可、无转置。
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    // 已锁：K=4、N=256、q=3329；Prep nonce0=0 → y 四 poly
    ...
    uint8_t *dst = (uint8_t *)AscendC::GmAlloc(std::max(dstFileSize, std::max(tHatBytes, (size_t)1024)));
    uint8_t *src = (uint8_t *)AscendC::GmAlloc(std::max(srcFileSize, std::max(aHatBytes, (size_t)1024)));
    ...
    std::printf("[EN05] CPU L1 Prep(CBD η=2) begin\n");
    ...
    if (!ok) return 32;
    // Host 转接说明产物：与 NTT src 同布局，链式时可直接作 L2 输入（本刀不自动链）
    ok = WriteFile("./output/prep_ntt_layout.bin", dst, prepOutBytes);
    if (!ok) return 33;
    std::printf("[EN05] CPU L1 Prep done (y elems=%zu)\n", prepOutBytes / sizeof(int32_t));

    // -------- L2 NTT 真积木（独立 src_ntt；布局契约见上）--------
    std::printf("[EN05] CPU L2 NTT begin\n");
    ...
    if (!ok) return 11;
    std::printf("[EN05] CPU L2 NTT done\n");

    // -------- L3 Matvec 真积木（Â Host 喂 = SampleNTT 本刀范围外）--------
    std::printf("[EN05] CPU L3 Matvec begin\n");
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ...
    if (!ok) return 42;
    std::printf("[EN05] CPU L3 Matvec done\n");

    // -------- L4 INTT 真积木 --------
    std::printf("[EN05] CPU L4 INTT begin\n");
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ...
    if (!ok) return 21;
    std::printf("[EN05] CPU L4 INTT done\n");

    // -------- L5 Pack 桩 --------
    std::printf("[EN05] CPU L5 Pack begin\n");
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ...
    if (!ok) return 52;
    std::printf("[EN05] CPU L5 Pack done\n");
    ...
    uint8_t *dstHost, *srcHost, *wsHost, *sHatHost, *gammaHost, *sigmaHost;
    uint8_t *dstDevice, *srcDevice, *wsDevice, *sHatDevice, *gammaDevice, *sigmaDevice;
    ...
    ResolveNttSizes(tiling, &srcFileSize, &dstFileSize, &wsFileSize);
    const int32_t nElem = static_cast<int32_t>(srcFileSize / sizeof(int32_t));
    const size_t hostScratch = std::max(dstFileSize, std::max(tHatBytes, aHatBytes));
    ...
    // -------- L1 Prep 真 CBD --------
    std::printf("[EN05] SIM L1 Prep(CBD η=2) begin\n");
    size_t sigmaRead = kSigmaBytes;
    ...
    if (!ok) return 33;
    std::printf("[EN05] SIM L1 Prep done\n");

    // -------- L2 NTT --------
    std::printf("[EN05] SIM L2 NTT begin\n");
    ...
    if (!ok) return 11;
    std::printf("[EN05] SIM L2 NTT done\n");

    // -------- L3 Matvec --------
    std::printf("[EN05] SIM L3 Matvec begin\n");
    size_t aHatRead = aHatBytes;
    size_t sHatRead = sHatBytes;
    size_t gammaRead = gammaBytes;
    ...
    if (!ok) return 42;
    std::printf("[EN05] SIM L3 Matvec done\n");

    // -------- L4 INTT --------
    std::printf("[EN05] SIM L4 INTT begin\n");
    ...
    if (!ok) return 21;
    std::printf("[EN05] SIM L4 INTT done\n");

    // -------- L5 Pack 桩 --------
    std::printf("[EN05] SIM L5 Pack begin\n");
    ...
    if (!ok) return 52;
    std::printf("[EN05] SIM L5 Pack done\n");
...
#endif
    std::printf("[EN05] five segments finished OK\n");
    return 0;
}
