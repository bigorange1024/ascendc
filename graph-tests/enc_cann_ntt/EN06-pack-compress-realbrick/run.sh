#!/bin/bash
# EN06-pack-compress-realbrick：Encrypt 形 Host 五段（Prep真CBD→NTT→Matvec→INTT→Pack真Compress+ByteEncode）
# 基线：EN05 壳；L5 换真 Compress₁₁/₅ + ByteEncode；禁融胖 MIX GATE。
#
# Usage（默认 = Kyber 档；本刀禁止 -r npu）：
#   bash run.sh -r cpu -v Ascend910B4
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# 主门禁：不挂（五段 kernel 均正常退出）。golden 对拍失败仅标 CORRECTNESS_SOFT_FAIL。
# KERNEL_COMPUTE_BUDGET_SEC：CPU 默认 300；SIM 默认 1200（五段，防挂死）
# 已锁：NTT_N=256 NTT_Q=3329 NTT_REF=kyber NTT_BENCH=4；Prep/Matvec/Pack K=4；du=11 dv=5；禁擅自改参
CURRENT_DIR=$(
    cd $(dirname ${BASH_SOURCE:-$0})
    pwd
)
_ORIG_ARGS=("$@")
# graph-tests/enc_cann_ntt/EN06-… → 仓库根为三层上
...
if [ "${RUN_MODE}" = "npu" ]; then
    echo "[ERROR] EN06 本刀禁止 -r npu（NPU 长期占用；任务书仅 CPU+SIM）" >&2
    exit 2
fi
...
# 锁定 Kyber / ML-KEM 造数参数（可被环境覆盖，默认即任务书）
...
if [ "${_rc}" -ne 0 ]; then
    echo "[FAIL] kernel exit=${_rc} (${RUN_MODE}) — five-segment hang or error" >&2
    exit "${_rc}"
fi

# 正确性次要：软对拍 Prep/NTT/Matvec/INTT/Pack；失败不否决「不挂」门禁
set +e
python3 "${CURRENT_DIR}/scripts/verify_result.py"
_vr=$?
set -e
if [ "${_vr}" -ne 0 ]; then
    echo "[CORRECTNESS_SOFT_FAIL] Prep/NTT/Matvec/INTT/Pack golden 对拍未过（${RUN_MODE}）；kernel 已不挂 exit=0"
else
    echo "[SUCCESS] EN06-pack-compress-realbrick (${RUN_MODE}) five-segment exit=0 + Prep/NTT/Matvec/INTT/Pack golden match"
fi
exit 0
