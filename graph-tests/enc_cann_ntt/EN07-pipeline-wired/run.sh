#!/bin/bash
# EN07-pipeline-wired：五段真积木 + 段间 GM 贯通（Prep.y→NTT→Matvec.ŷ→INTT→Pack.u）
# 基线：复制 EN06；Â/γ/v 仍 Host；禁融胖 MIX GATE；禁 -r npu。
#
...
if [ "${RUN_MODE}" = "npu" ]; then
    echo "[ERROR] EN07 本刀禁止 -r npu（NPU 长期占用；任务书仅 CPU+SIM）" >&2
    exit 2
fi
...
if [ "${_vr}" -ne 0 ]; then
    ...
    echo "[SUCCESS] EN07-pipeline-wired (${RUN_MODE}) five-segment exit=0 + wired golden match"
fi
exit 0
