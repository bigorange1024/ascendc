#!/usr/bin/python3
# coding=utf-8
# EN07：贯通链软对拍 Prep/NTT/Matvec/INTT/Pack；失败不否决「不挂」
...
if __name__ == "__main__":
    try:
        ...
        # 贯通链尽量 golden；任一段失败 → soft-fail（run.sh 不否决不挂）
        if not (ok_prep and ok_ntt and ok_matvec and ok_intt and ok_pack):
            print("[WARN] wired-chain golden soft-fail (correctness secondary to NOHANG)")
            print(
                f"test soft: Prep={ok_prep} NTT={ok_ntt} Matvec={ok_matvec} "
                f"INTT={ok_intt} Pack={ok_pack}"
            )
            # 仍以非零退出让 run.sh 打印 CORRECTNESS_SOFT_FAIL；kernel 已 exit 0
            sys.exit(1)
        print("test pass (Prep→NTT→Matvec→INTT→Pack wired golden)")
