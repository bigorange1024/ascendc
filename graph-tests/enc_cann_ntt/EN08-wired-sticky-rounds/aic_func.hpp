/**
 * EN01-kem256-ntt-port · AIC Cube MMAD 封装
 * 迁入来源：thirdparty/cann-ntt-author-merged_dsa/aic_func.hpp
 * 背景：BAT 矩阵化 NTT 的右乘在 Cube 上完成；Kyber 仅用两路 digit 结果。
 */
...
public:
    ...
    template <bool keepA = false, bool useA = false, bool keepB = false, bool useB = false> 
    __aicore__ inline void Process(GM_ADDR dst, GM_ADDR a, GM_ADDR b)
    {
        if (!useA) aGM.SetGlobalBuffer((__gm__ int8_t *)a);
        if (!useB) bGM.SetGlobalBuffer((__gm__ int8_t *)b);
        cGM.SetGlobalBuffer((__gm__ int32_t *)dst);
        if (!useA) CopyInA();
        if (!useB) CopyInB();
        if (!useA) SplitA();
        if (!useB) SplitB();
        if (!useA) a2Local = inQueueA2.DeQue<int8_t>();
        if (!useB) b2Local = inQueueB2.DeQue<int8_t>();
        Compute();
        if (!keepA) inQueueA2.FreeTensor(a2Local);
        if (!keepB) inQueueB2.FreeTensor(b2Local);
        CopyOut();
    }
private:
...
private:
    ...
    AscendC::LocalTensor<int8_t> a2Local, b2Local;
