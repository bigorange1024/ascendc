#!/bin/bash
# EN08-wired-sticky-rounds：EN07 贯通五段 × 粘性多轮（同 session 不 recreate stream）
# 基线：复制 EN07；默认 R=4（EN08_ROUNDS）；第 1 轮 golden；禁融胖 MIX GATE；禁 -r npu。
#
# Usage（默认 = Kyber 档；本刀禁止 -r npu）：
#   bash run.sh -r cpu -v Ascend910B4
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# 调试（非默认）：
#   EN08_ROUNDS=2 bash run.sh -r cpu -v Ascend910B4
#
# 主门禁：R 轮全部完成不挂。golden 对拍仅第 1 轮；失败仅标 CORRECTNESS_SOFT_FAIL。
# KERNEL_COMPUTE_BUDGET_SEC：CPU 默认 600；SIM 默认 2400（R=4 防挂死）
# 已锁：NTT_N=256 NTT_Q=3329 NTT_REF=kyber NTT_BENCH=4；Prep/Matvec/Pack K=4；du=11 dv=5
CURRENT_DIR=$(
...
if [ "${RUN_MODE}" = "npu" ]; then
    echo "[ERROR] EN08 本刀禁止 -r npu（NPU 长期占用；任务书仅 CPU+SIM）" >&2
    exit 2
fi
...
export EN08_ROUNDS="${EN08_ROUNDS:-4}"
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} CANN=${_ASCEND_INSTALL_PATH} ASCEND_DEVICE_ID=${ASCEND_DEVICE_ID:-n/a} EN08_ROUNDS=${EN08_ROUNDS}"
...
if [ "${RUN_MODE}" = "sim" ]; then
    export KERNEL_COMPUTE_BUDGET_SEC="${KERNEL_COMPUTE_BUDGET_SEC:-2400}"
    # shellcheck source=/dev/null
...
if [ "${_rc}" -ne 0 ]; then
    echo "[FAIL] kernel exit=${_rc} (${RUN_MODE}) — sticky R=${EN08_ROUNDS} hang or error" >&2
    exit "${_rc}"
fi

# 确认 R 轮完成标记齐全（主门禁补充证据）
_missing=0
for ((_r=1; _r<=EN08_ROUNDS; _r++)); do
    _f=$(printf "output/round_%02d_done.txt" "${_r}")
    if [ ! -f "${_f}" ]; then
        echo "[FAIL] missing ${_f}" >&2
        _missing=1
    else
        echo "[EN08] found ${_f}: $(tr -d '\n' < "${_f}")"
    fi
done
...
set -e
if [ "${_vr}" -ne 0 ]; then
    echo "[CORRECTNESS_SOFT_FAIL] round-1 wired golden 对拍未过（${RUN_MODE}）；R=${EN08_ROUNDS} kernel 已不挂 exit=0"
else
    echo "[SUCCESS] EN08-wired-sticky-rounds (${RUN_MODE}) R=${EN08_ROUNDS} exit=0 + round-1 golden match"
fi
exit 0
