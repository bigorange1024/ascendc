#!/usr/bin/python3
# coding=utf-8
# EN08-wired-sticky-rounds · 五段造数（贯通链 golden；供第 1 轮对拍）
# Host 只喂：σ、M4_ntt/M4_intt、Â、γ、v（噪声形）
# 贯通期望：y(Prep)→ŷ(NTT)→t̂(Matvec)→u(INTT)→Pack(u)；后续轮 Host 扰动不重算 golden
...
if __name__ == "__main__":
    gen_tiling()
    gen_wired_pipeline()
    print("[OK] EN08 wired-pipeline inputs written (round-1 golden; no independent src_ntt/s_hat)")
