#!/usr/bin/python3
# coding=utf-8
# EN08：第 1 轮贯通链软对拍 Prep/NTT/Matvec/INTT/Pack；失败不否决「不挂」
...
if __name__ == "__main__":
    try:
        ...
        # 仅第 1 轮落盘；任一段失败 → soft-fail（run.sh 不否决不挂）
        if not (ok_prep and ok_ntt and ok_matvec and ok_intt and ok_pack):
            print("[WARN] round-1 wired golden soft-fail (correctness secondary to NOHANG)")
            print(
                f"test soft: Prep={ok_prep} NTT={ok_ntt} Matvec={ok_matvec} "
                f"INTT={ok_intt} Pack={ok_pack}"
            )
            sys.exit(1)
        print("test pass (EN08 round-1 Prep→NTT→Matvec→INTT→Pack wired golden)")
