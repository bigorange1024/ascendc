/**
 * EN09-samplentt-device · Encrypt 形 Host 五段贯通 + 设备 SampleNTT(Â)
 *
 * 段序（同进程、同 session；各段独立 launch，禁融胖 MIX GATE）：
 *   L0 SampleNTT — enc_samplentt_real     （ρ → Â[4,4,256]；独立 AIV / X32）
 *   L1 Prep      — enc_prep_cbd_real        （σ → y[4,256]）
 *   L2 NTT       — mmad_custom + M4_ntt     （输入 = L1 的 y）
 *   L3 Matvec    — enc_matvec_real          （ŝ=ŷ；Â = L0 设备输出，禁止 Host 随机冒充）
 *   L4 INTT      — mmad_custom + M4_intt    （输入 = L3 的 t̂）
 *   L5 Pack      — enc_pack_compress_real   （u = L4；v 仍 Host 噪声形）
 *
 * 主门禁：各段正常返回（不挂）。CPU/SIM；禁 npu；R=1。
 */
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    if (srcFileSize != prepOutBytes || srcFileSize != tHatBytes) {
        std::printf("[EN09] FATAL size mismatch src=%zu prep=%zu tHat=%zu\n", srcFileSize,
    ...
    uint8_t *aHat = (uint8_t *)AscendC::GmAlloc(aHatBytes); // 设备 SampleNTT 输出，喂 Matvec
    ...
    // -------- L0 SampleNTT（ρ → Â[4,4,256]）--------
    std::printf("[EN09] CPU L0 SampleNTT(Alg.7) begin (full Â k=4)\n");
    ...
    if (!ok) return 62;
    std::printf("[EN09] CPU L0 SampleNTT done; elems=%zu\n", aHatBytes / sizeof(int32_t));

    // -------- L1 Prep 真 CBD（σ → y[4,256]）--------
    std::printf("[EN09] CPU L1 Prep(CBD η=2) begin\n");
    ...
    if (!ok) return 33;
    std::printf("[EN09] CPU L1 Prep done; wired y→NTT src\n");

    // -------- L2 NTT（src = Prep y）--------
    std::printf("[EN09] CPU L2 NTT begin (src=Prep.y)\n");
    ...
    std::printf("[EN09] CPU L2 NTT done; wired ŷ→Matvec.s_hat\n");

    // -------- L3 Matvec（ŝ=ŷ；Â=设备 SampleNTT）--------
    std::printf("[EN09] CPU L3 Matvec begin (s_hat=NTT.ŷ; A=device SampleNTT)\n");
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    size_t gammaRead = gammaBytes;
    ok = ReadFile("./input/gammas.bin", gammaRead, gammas, gammaBytes);
    if (!ok) return 44;
    ICPU_RUN_KF(enc_matvec_real, stubBlockDim, dst, aHat, sHat, gammas, kMatvecK, kMatvecN, kMatvecQ);
    ok = WriteFile("./output/dst_matvec.bin", dst, tHatBytes);
    if (!ok) return 42;
    std::memcpy(src, dst, tHatBytes);
    std::printf("[EN09] CPU L3 Matvec done; wired t̂→INTT src\n");

    // -------- L4 INTT（src = Matvec t̂）--------
    std::printf("[EN09] CPU L4 INTT begin (src=Matvec.t_hat)\n");
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ...
    std::printf("[EN09] CPU L4 INTT done; wired u→Pack.u\n");

    // -------- L5 Pack（u=INTT；v Host）--------
    std::printf("[EN09] CPU L5 Pack begin (u=INTT; v=Host)\n");
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ...
    if (!ok) return 52;
    std::printf("[EN09] CPU L5 Pack done (c=%zu B)\n", kCipherBytes);
    ...
    // -------- L0 SampleNTT --------
    std::printf("[EN09] SIM L0 SampleNTT(Alg.7) begin (full Â k=4)\n");
    size_t rhoRead = kRhoBytes;
    ...
    if (!ok) return 62;
    std::printf("[EN09] SIM L0 SampleNTT done\n");

    // -------- L1 Prep --------
    std::printf("[EN09] SIM L1 Prep(CBD η=2) begin\n");
    size_t sigmaRead = kSigmaBytes;
    ...
    if (!ok) return 33;
    std::printf("[EN09] SIM L1 Prep done; wired y→NTT src\n");

    // -------- L2 NTT --------
    std::printf("[EN09] SIM L2 NTT begin (src=Prep.y)\n");
    ...
    std::printf("[EN09] SIM L2 NTT done; wired ŷ→Matvec.s_hat\n");

    // -------- L3 Matvec（Â = 设备 aHatDevice）--------
    std::printf("[EN09] SIM L3 Matvec begin (s_hat=NTT.ŷ; A=device SampleNTT)\n");
    size_t gammaRead = gammaBytes;
    ...
    std::printf("[EN09] SIM L3 Matvec done; wired t̂→INTT src\n");

    // -------- L4 INTT --------
    std::printf("[EN09] SIM L4 INTT begin (src=Matvec.t_hat)\n");
    ...
    std::printf("[EN09] SIM L4 INTT done; wired u→Pack.u\n");

    // -------- L5 Pack --------
    std::printf("[EN09] SIM L5 Pack begin (u=INTT; v=Host)\n");
    size_t vRead = packVBytes;
    ...
    if (!ok) return 52;
    std::printf("[EN09] SIM L5 Pack done\n");
...
#endif
    std::printf("[EN09] segments finished OK (SampleNTT+wired pipeline)\n");
    return 0;
}
