#!/bin/bash
# EN09-samplentt-device：设备 Alg.7 SampleNTT(Â) + 五段贯通；R=1；禁 -r npu。
# 基线：复制 EN07；Matvec Â 必须来自设备 SampleNTT；禁融胖 MIX GATE。
#
# Usage（默认 = Kyber 档；本刀禁止 -r npu）：
#   bash run.sh -r cpu -v Ascend910B4
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# 主门禁：不挂。golden 对拍失败仅标 CORRECTNESS_SOFT_FAIL。
# KERNEL_COMPUTE_BUDGET_SEC：CPU 默认 600；SIM 默认 1800（+16×SampleNTT）
...
if [ "${RUN_MODE}" = "npu" ]; then
    echo "[ERROR] EN09 本刀禁止 -r npu（任务书仅 CPU+SIM）" >&2
    exit 2
fi
...
if [ "${_rc}" -ne 0 ]; then
    echo "[FAIL] kernel exit=${_rc} (${RUN_MODE}) — SampleNTT+five-segment hang or error" >&2
    exit "${_rc}"
fi
...
set -e
if [ "${_vr}" -ne 0 ]; then
    echo "[CORRECTNESS_SOFT_FAIL] SampleNTT+贯通 golden 对拍未过（${RUN_MODE}）；kernel 已不挂 exit=0"
else
    echo "[SUCCESS] EN09-samplentt-device (${RUN_MODE}) exit=0 + SampleNTT+wired golden match"
fi
exit 0
