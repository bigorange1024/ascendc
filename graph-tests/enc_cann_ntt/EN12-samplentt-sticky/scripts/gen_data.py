#!/usr/bin/python3
# coding=utf-8
# EN12-samplentt-sticky · 第 1 轮 SampleNTT+贯通 golden（与 EN09 同契约）
# Host 喂：ρ、σ、M4_ntt/M4_intt、γ、v；Â golden 由 Alg.7 从 ρ 生成（与设备同契约）
# 贯通：ρ→Â(SampleNTT) + σ→y→ŷ→t̂(Matvec 用设备 Â)→u→Pack(u)；粘性多轮由 main 编排
...
def gen_wired_pipeline():
    ...
    if ref not in ("kyber", "ml-kem", "mlkem"):
        raise ValueError(f"EN12 锁定 NTT_REF=kyber，收到 {ref!r}")
    if bench != K:
        raise ValueError(f"EN12 贯通要求 NTT_BENCH==K={K}，收到 bench={bench}")
...
if __name__ == "__main__":
    gen_tiling()
    gen_wired_pipeline()
    print("[OK] EN12 SampleNTT+wired-pipeline inputs written (round-1 golden)")
