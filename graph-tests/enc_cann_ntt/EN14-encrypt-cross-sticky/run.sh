#!/bin/bash
# EN14-encrypt-cross-sticky：EN13 Encrypt×liboqs × 粘性多轮（同 session）
# 基线：复制 EN13 核；编排对齐 EN12 sticky；默认 R=8（EN14_ROUNDS）。
# 允许 -r npu（2026-09-12 用户授权）。主门禁：R 轮不挂 + 每轮 c≡liboqs max=0。
#
# Usage（默认）：
...
#
# 调试（非默认）：
#   EN14_ROUNDS=2 bash run.sh -r cpu -v Ascend910B4
#
# KERNEL_COMPUTE_BUDGET_SEC：CPU 默认 1800；SIM 默认 7200（R=8×EN13≈130s）
...
export EN14_ROUNDS="${EN14_ROUNDS:-8}"
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} CANN=${_ASCEND_INSTALL_PATH} ASCEND_DEVICE_ID=${ASCEND_DEVICE_ID:-n/a} EN14_ROUNDS=${EN14_ROUNDS}"
...
if [ "${_rc}" -ne 0 ]; then
    echo "[FAIL] kernel exit=${_rc} (${RUN_MODE}) — sticky R=${EN14_ROUNDS} hang or error" >&2
    exit "${_rc}"
fi
...
set -e
if [ "${_vr}" -ne 0 ]; then
    echo "[FAIL] EN14 sticky c vs liboqs 对拍未过（${RUN_MODE}）R=${EN14_ROUNDS}" >&2
    exit "${_vr}"
fi
echo "[SUCCESS] EN14-encrypt-cross-sticky (${RUN_MODE}) R=${EN14_ROUNDS} exit=0 + each-round c≡liboqs max=0"
exit 0
