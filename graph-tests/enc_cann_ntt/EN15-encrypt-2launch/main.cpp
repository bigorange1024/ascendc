 ...
 *
 * 允许读入：ek_pke、m、coins + LUT（tiling / M4_ntt / M4_intt / gammas）
 * 禁止读入：ρ / t̂ / e1 / e2 / μ / σ 等中间 .bin
 * L1：SampleNTT + CBD(y/e1/e2)；开场一次 H2D 后 L1→L2 无业务 mid-H2D；仅 c 出设备
 */
...
static bool LoadEncryptInputs(uint8_t *rho, int32_t *tHat, uint8_t *coins, uint8_t *m,
                              ...
                              size_t matMFileSize, size_t gammaBytes)
{
    ...
    size_t mR = 32, cR = 32, gR = gammaBytes, nttR = matMFileSize, inttR = matMFileSize;
    if (!ReadFile("./input/ek_pke.bin", ekR, ek.data(), host_ek::kEkBytes) || ekR != host_ek::kEkBytes) {
        std::printf("[EN15] need input/ek_pke.bin\n");
        return false;
    }
    if (!ReadFile("./input/m.bin", mR, m, 32) || mR != 32) {
        std::printf("[EN15] need input/m.bin\n");
        return false;
    }
    if (!ReadFile("./input/coins.bin", cR, coins, 32) || cR != 32) {
        std::printf("[EN15] need input/coins.bin\n");
        return false;
    }
    if (!host_ek::DecodeEkPke(rho, tHat, ek.data(), ekR)) {
        std::printf("[EN15] DecodeEkPke failed\n");
        return false;
    }
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    if (!LoadEncryptInputs(rho, reinterpret_cast<int32_t *>(tPub), sigma, m, wsNtt, wsIntt, gammas,
    ...
    ++g_hostLaunchCount;
    std::printf("[EN15] CPU L1 prep done (no intermediate dump)\n");
    ...
    std::printf("[EN15] CPU L2 compute done\n");
    ...
    if (!LoadEncryptInputs(rhoHost, reinterpret_cast<int32_t *>(tPubHost), sigmaHost, mHost,
    ...
    std::printf("[EN15] SIM L1 done (A/y/e1/e2 on device; no mid H2D)\n");
    ...
    std::printf("[EN15] segments finished OK (Encrypt 2-launch)\n");
    return 0;
}
