#!/bin/bash
# EP01-encaps-host-skel：Encaps Host 壳（H/G + 桩 Encrypt）；禁 -r npu。
#
# Usage（默认）：
#   bash run.sh -r cpu -v Ascend910B4
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# 主门禁：不挂；c=1568 / K=32 / ek 合法；Host H/G soft max=0
# KERNEL_COMPUTE_BUDGET_SEC：CPU 默认 300；SIM 默认 600
CURRENT_DIR=$(
...
if [ "${RUN_MODE}" = "npu" ]; then
    echo "[ERROR] EP01 本刀禁止 -r npu（任务书仅 CPU+SIM）" >&2
    exit 2
fi
...
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} CANN=${_ASCEND_INSTALL_PATH}"
...
set -e
if [ "${_vr}" -ne 0 ]; then
    echo "[FAIL] EP01 verify 未过（${RUN_MODE}）" >&2
    exit "${_vr}"
fi
echo "[SUCCESS] EP01-encaps-host-skel (${RUN_MODE}) exit=0 + lengths/H/G ok"
exit 0
