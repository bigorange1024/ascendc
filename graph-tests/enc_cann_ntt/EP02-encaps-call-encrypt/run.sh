#!/bin/bash
# EP02-encaps-call-encrypt：Alg.14 Encrypt × liboqs PKE 交叉；R=1；禁 -r npu。
...
#
# 主门禁：不挂 + c/K vs encaps-ref max=0（verify 失败则本脚本非 0）。
...
if [ "${RUN_MODE}" = "npu" ]; then
    echo "[ERROR] EP02 本刀禁止 -r npu（任务书仅 CPU+SIM）" >&2
    exit 2
fi
...
set -e
if [ "${_vr}" -ne 0 ]; then
    echo "[FAIL] EP02 c/K vs encaps-ref 对拍未过（${RUN_MODE}）" >&2
    exit "${_vr}"
fi
echo "[SUCCESS] EP02-encaps-call-encrypt (${RUN_MODE}) exit=0 + c/K≡ref max=0"
exit 0
