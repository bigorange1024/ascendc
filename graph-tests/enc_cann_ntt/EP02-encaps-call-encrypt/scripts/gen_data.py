...
# coding=utf-8
"""
EP02-encaps-call-encrypt · Encaps 真调 EN13 形 Encrypt

Alg.16/20：m； (K̄,r)←G(m‖H(ek))；c←Encrypt(ek,m,r)。
设备段序与 EN13 同（SampleNTT…Pack）；coins 槽填 r。
"""
...
def ensure_encaps_inputs(out_dir: Path) -> tuple[bytes, bytes, bytes, bytes, bytes]:
    ...
    再用 liboqs/kem_ref Encaps 得权威 c/K（EP02 自洽门禁；EP04 再硬交叉）。
...
def gen_encrypt_pipeline() -> None:
    ...
    if ref not in ("kyber", "ml-kem", "mlkem"):
        raise ValueError(f"EP02 锁定 NTT_REF=kyber，收到 {ref!r}")
    if bench != K:
        raise ValueError(f"EP02 要求 NTT_BENCH==K={K}，收到 bench={bench}")
    ...
    if bad != 0:
        for i, (a, b) in enumerate(zip(c_host, c_ref)):
            if a != b:
                ...
                break
        raise SystemExit("EP02 Host golden c ≠ encaps ref — 禁止接线设备")
...
if __name__ == "__main__":
    ...
    print("[OK] EP02 encaps-call-encrypt inputs written")
