#!/bin/bash
# EP04-encaps-liboqs-cross：Alg.14 Encrypt × liboqs PKE 交叉；R=1；允许 -r npu（2026-09-12 用户授权）。
...
set -e
if [ "${_vr}" -ne 0 ]; then
    echo "[FAIL] EP04 c/K vs encaps-ref 对拍未过（${RUN_MODE}）" >&2
    exit "${_vr}"
fi
echo "[SUCCESS] EP04-encaps-liboqs-cross (${RUN_MODE}) exit=0 + c/K≡ref max=0"
exit 0
