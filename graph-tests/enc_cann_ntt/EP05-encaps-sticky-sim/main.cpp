/**
 * EP05-encaps-sticky-sim · Encaps sticky 粘性多轮（自 EN13 核 + EN12 编排模式）
 *
 * 同进程、同 acl session / 同一 stream，将 EN13 Alg.14 整链重复 R 轮（默认 8，
 * 环境变量 EP05_ROUNDS 可覆盖）：
 *   for r in 1..R:
 ...
 * 每轮换种子（input/rXX/ 下各 bin，SEED_D+(r-1)*10007）；硬门禁每轮 c/K≡liboqs encaps（verify；可抽样）。
...
/**
 * 解析 EP05_ROUNDS；非法回落默认 8。
 * 背景：任务书默认 R=8；上限 64 防误设墙钟爆炸。
 */
static int32_t ResolveRounds()
{
    const char *env = std::getenv("EP05_ROUNDS");
    ...
    if (end == env || v < 1 || v > 64) {
        std::printf("[EP05] WARN EP05_ROUNDS='%s' invalid → default 16\n", env);
        return 16;
    }
...
/** 确保 output/rXX 目录存在（防挂死/漏 mkdir 导致 WriteFile 失败）。 */
static void EnsureRoundOutDir(int32_t round)
{
    char path[64];
    std::snprintf(path, sizeof(path), "./output/r%02d", round);
    mkdir("./output", 0755);
    mkdir(path, 0755);
}

/** 写 round 完成哨兵：output/round_XX_done.txt（run.sh / verify 依赖）。 */
static bool WriteRoundDone(int32_t round)
{
    char path[64];
    std::snprintf(path, sizeof(path), "./output/round_%02d_done.txt", round);
    char body[96];
    int n = std::snprintf(body, sizeof(body), "EP05 round %d finished OK\n", round);
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    std::printf("[EP05] sticky Encaps sticky begin R=%d (no stream recreate / no re-aclInit)\n",
                rounds);

#ifdef ASCENDC_CPU_DEBUG
    uint8_t *tiling_data = (uint8_t *)AscendC::GmAlloc(tilingSize);
    ReadFile("./input/tiling.bin", tilingSize, tiling_data, tilingSize);
    if (tilingSize != sizeof(TilingData)) return 16;
    ...
    if (srcFileSize != prepOutBytes || srcFileSize != tHatBytes) {
        std::printf("[EP05] FATAL size mismatch src=%zu prep=%zu tHat=%zu\n", srcFileSize,
    ...
    for (int32_t r = 1; r <= rounds; ++r) {
        std::printf("[EP05] CPU round %d/%d begin\n", r, rounds);
        ...
        if (!ok) return 52;
        // 落盘本轮 K（来自 gen_data encaps；与 Host G 一致）
        {
            char kIn[96], kOut[96];
            std::snprintf(kIn, sizeof(kIn), "./input/r%02d/K.bin", r);
            std::snprintf(kOut, sizeof(kOut), "./output/r%02d/K.bin", r);
            uint8_t kBuf[32];
            size_t kR = 32;
            ok = ReadFile(kIn, kR, kBuf, 32);
            if (!ok) return 95;
            ok = WriteFile(kOut, kBuf, 32);
            if (!ok) return 96;
            if (r == 1) {
                ok = WriteFile("./output/K.bin", kBuf, 32);
                if (!ok) return 97;
            }
        }
        if (r == 1) {
        ...
        std::printf("[EP05] CPU round %d/%d finished OK (c=%zu B)\n", r, rounds, kCipherBytes);
    }
    ...
    ResolveNttSizes(tiling, &srcFileSize, &dstFileSize, &wsFileSize);
    if (srcFileSize != prepOutBytes || srcFileSize != tHatBytes) {
        std::printf("[EP05] FATAL size mismatch\n");
        return 7;
    }
    ...
    for (int32_t r = 1; r <= rounds; ++r) {
        std::printf("[EP05] SIM round %d/%d begin (same stream)\n", r, rounds);
        ...
        if (!WriteRoundDone(r)) return 60;
        std::printf("[EP05] SIM round %d/%d finished OK\n", r, rounds);
    }
...
#endif
    std::printf("[EP05] sticky R=%d rounds finished OK (Encaps sticky)\n", rounds);
    return 0;
}
