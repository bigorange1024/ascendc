/**
 * EP04-encaps-liboqs-cross · Alg.14 Encrypt × liboqs 交叉（Host 多段 + cann-ntt）
 *
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    if (srcFileSize != prepOutBytes || srcFileSize != tHatBytes) {
        std::printf("[EP04] FATAL size mismatch src=%zu prep=%zu tHat=%zu\n", srcFileSize,
    ...
    std::printf("[EP04] CPU Host H/G begin\n");
    {
    ...
    std::printf("[EP04] CPU Host H/G done (K written)\n");

    // -------- L0 SampleNTT --------
    std::printf("[EP04] CPU L0 SampleNTT begin\n");
    ...
    if (!ok) return 63;
    std::printf("[EP04] CPU L0 SampleNTT+Host Âᵀ done\n");

    // -------- L1 Prep（coins→y）--------
    std::printf("[EP04] CPU L1 Prep(CBD y) begin\n");
    ...
    std::printf("[EP04] CPU L1 Prep done\n");

    // -------- L2 NTT --------
    std::printf("[EP04] CPU L2 NTT begin\n");
    ...
    std::printf("[EP04] CPU L2 NTT done\n");

    // -------- L3a Matvec（Âᵀ）--------
    std::printf("[EP04] CPU L3a Matvec(Âᵀ∘ŷ) begin\n");
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ...
    std::printf("[EP04] CPU L3a Matvec done\n");

    // -------- L3b Dot --------
    std::printf("[EP04] CPU L3b Dot(⟨t̂,ŷ⟩) begin\n");
    ...
    if (!ok) return 72;
    std::printf("[EP04] CPU L3b Dot done\n");

    // -------- L4a INTT(û) --------
    std::printf("[EP04] CPU L4a INTT(û) begin\n");
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ...
    std::printf("[EP04] CPU L4a INTT(û) done\n");

    // -------- L4b INTT(v̂)（垫成 K poly）--------
    std::printf("[EP04] CPU L4b INTT(v̂ pad) begin\n");
    ...
    if (!ok) return 23;
    std::printf("[EP04] CPU L4b INTT(v̂) done\n");

    // -------- Host 加噪：u+=e1；v+=e2+μ --------
    std::printf("[EP04] CPU Host add e1/e2/μ\n");
    ...
    // -------- L5 Pack --------
    std::printf("[EP04] CPU L5 Pack begin\n");
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ...
    if (!ok) return 53;
    std::printf("[EP04] CPU L5 Pack done (c=%zu B)\n", kCipherBytes);
    ...
    ResolveNttSizes(tiling, &srcFileSize, &dstFileSize, &wsFileSize);
    if (srcFileSize != prepOutBytes || srcFileSize != tHatBytes) {
        std::printf("[EP04] FATAL size mismatch\n");
        return 7;
    }
    ...
    // -------- Host Encaps 头：H/G→K --------
    std::printf("[EP04] SIM Host H/G begin\n");
    {
    ...
    std::printf("[EP04] SIM Host H/G done\n");

    // L0
    std::printf("[EP04] SIM L0 SampleNTT begin\n");
    size_t rhoRead = kRhoBytes;
    ...
    if (!ok) return 63;
    std::printf("[EP04] SIM L0 SampleNTT+Host Âᵀ done\n");

    // L1
    std::printf("[EP04] SIM L1 Prep begin\n");
    size_t sigmaRead = kSigmaBytes;
    ...
    std::printf("[EP04] SIM L1 Prep done\n");

    // L2
    std::printf("[EP04] SIM L2 NTT begin\n");
    ...
    std::printf("[EP04] SIM L2 NTT done\n");

    // L3a
    std::printf("[EP04] SIM L3a Matvec begin\n");
    size_t gammaRead = gammaBytes;
    ...
    std::printf("[EP04] SIM L3a Matvec done\n");

    // L3b
    std::printf("[EP04] SIM L3b Dot begin\n");
    size_t tPubRead = tHatBytes;
    ...
    if (!ok) return 72;
    std::printf("[EP04] SIM L3b Dot done\n");

    // L4a
    std::printf("[EP04] SIM L4a INTT(û) begin\n");
    ...
    std::printf("[EP04] SIM L4a INTT done\n");

    // L4b pad INTT(v̂)
    std::printf("[EP04] SIM L4b INTT(v̂ pad) begin\n");
    ...
    if (!ok) return 23;
    std::printf("[EP04] SIM L4b INTT done\n");

    // Host 加噪
    std::printf("[EP04] SIM Host add e1/e2/μ\n");
    size_t e1Read = packUBytes, e2Read = packVBytes, muRead = packVBytes;
    ...
    // L5
    std::printf("[EP04] SIM L5 Pack begin\n");
    ...
    if (!ok) return 53;
    std::printf("[EP04] SIM L5 Pack done\n");
...
#endif
    std::printf("[EP04] segments finished OK (Encaps→Encrypt pipeline)\n");
    return 0;
}
