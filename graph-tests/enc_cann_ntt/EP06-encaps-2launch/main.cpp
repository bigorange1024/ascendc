/**
 * EP06-encaps-2launch · Alg.14 Encrypt × liboqs · Encaps · Host launch = 2
 *
 * Host FO：H(ek)+G(m‖H)→K̄（无 launch）；Encrypt 外形：
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    // -------- Host Encaps 头：H(ek)、G(m‖H)→(K̄,r)；r 已由 gen_data 写入 sigma --------
    std::printf("[EP06] CPU Host H/G begin\n");
    {
    ...
    std::printf("[EP06] CPU Host H/G done (K written)\n");
    ...
    std::printf("[EP06] CPU L1 prep + Host Âᵀ done\n");
    ...
    std::printf("[EP06] CPU L2 compute done (c=%zu B)\n", kCipherBytes);
    ...
    // -------- Host Encaps 头：H/G→K（无 ACLRT_LAUNCH_KERNEL）--------
    std::printf("[EP06] SIM Host H/G begin\n");
    {
    ...
    std::printf("[EP06] SIM Host H/G done\n");
    ...
    std::printf("[EP06] SIM L1 prep + Host Âᵀ done\n");
    ...
    std::printf("[EP06] segments finished OK (Encaps 2-launch × liboqs)\n");
    return 0;
}
