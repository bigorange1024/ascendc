#!/bin/bash
# EP06-encaps-2launch：Alg.14 Encrypt × liboqs PKE 交叉；R=1；本战役禁 -r npu；Host launch=2。
...
#
# 主门禁：不挂 + c/K vs liboqs Encaps max=0（verify 失败则本脚本非 0）。
...
set -e
if [ "${_vr}" -ne 0 ]; then
    echo "[FAIL] EP06 c/K vs liboqs Encaps 对拍未过（${RUN_MODE}）" >&2
    exit "${_vr}"
fi
echo "[SUCCESS] EP06-encaps-2launch (${RUN_MODE}) exit=0 + c/K≡liboqs max=0"
exit 0
