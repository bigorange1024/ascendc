#!/bin/bash
# ER01-encrypt-shaped-2launch-skel：prep 桩 + MIX 计算壳（NTT 1/3 + 生产 GATE 4/8 真 Vec MAC + INTT 1/3）
#
...
_ORIG_ARGS=("$@")
# graph-tests/enc_related/ER01-… → 仓库根为三层上
...
echo "[SUCCESS] ER01-encrypt-shaped-2launch-skel (${RUN_MODE}) kernel exit=0 (no correctness cmp)"
