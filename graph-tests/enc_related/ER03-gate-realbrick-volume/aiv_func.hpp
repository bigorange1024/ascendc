#ifndef ER03_GATE_REALBRICK_VOLUME_AIV_FUNC_HPP
#define ER03_GATE_REALBRICK_VOLUME_AIV_FUNC_HPP

/**
 * @file aiv_func.hpp
 * @brief AIV 侧：Launch1 prep 桩 → Launch2 S0 写 / GATE 真 Vec MAC / INTT / 完成标记。
 *
 * 背景（ER03 / D-EXP-ER03）：在 ER01 外形 + ER02 同步纪律上，把 GATE 真 Vec MAC
 * 加压到近生产体量（kMacElems=256、kMacRounds=32），验 AIC WAIT(4) 时空等假说（X17）。
 * 禁 X14 空转、禁抄 Encrypt、禁 SoftSync、禁擅自缩小 MAC。
 *
 ...
 * 若编译/UB 放不下 → 停止改参，回报主控（ER03-TASK）。
 */
class AivGateRealBrickMac {
public:
...
#endif /* ER03_GATE_REALBRICK_VOLUME_AIV_FUNC_HPP */
