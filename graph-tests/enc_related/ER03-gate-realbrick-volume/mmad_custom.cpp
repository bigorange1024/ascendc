 ...
 * @brief ER03：Encrypt 外形双 launch + GATE 真积木近生产体量（phase=PREP / COMPUTE）。
 *
 * 图谱：D-EXP-ER03；基线 ER01 外形 + ER02 同步纪律；本刀验 X17（AIC WAIT4 时空等）。
 *
 * Host 串行：
 *   Launch1 phase=PREP：双 AIV 轻量 mixing → SAMPLE_OUT；AIC 立即返回（无 CrossCore）
 *   Launch2 phase=COMPUTE：NTT 1/3 + 生产 GATE 4/8（真 Vec MAC 256×32）+ INTT 复用 1/3
 *
 ...
 * TQue EnQue/DeQue、PipeBarrier 等复用查阅索引（GT-20260903-* / ER01/ER02）；
