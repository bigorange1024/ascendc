#!/bin/bash
# ER03-gate-realbrick-volume：prep 桩 + MIX 计算壳
# （NTT 1/3 + 生产 GATE 4/8 真 Vec MAC 256×32 + INTT 1/3）
#
...
# 已锁：kMacElems=256 kMacRounds=32；禁擅自改参 / 禁 NPU（本刀）
CURRENT_DIR=$(
    cd $(dirname ${BASH_SOURCE:-$0})
    pwd
)
_ORIG_ARGS=("$@")
# graph-tests/enc_related/ER03-… → 仓库根为三层上
...
echo "[SUCCESS] ER03-gate-realbrick-volume (${RUN_MODE}) kernel exit=0 (no correctness cmp)"
