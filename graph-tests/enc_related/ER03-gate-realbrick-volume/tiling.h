#ifndef ER03_GATE_REALBRICK_VOLUME_TILING_H
#define ER03_GATE_REALBRICK_VOLUME_TILING_H

/**
 * @file tiling.h
 * @brief ER03：GATE 真积木近生产体量加压的 GM 布局与 TRACE 槽。
 *
 * 基线：ER01 外形双 launch + ER02 同步纪律。
 * 本刀（D-EXP-ER03）已锁：kMacElems=256、kMacRounds=32；Cube 仍极轻；禁擅自改参。
...
struct TilingData {
    ...
    int32_t phase;      /**< 0=PREP，1=COMPUTE（见 Er03Phase） */
};

/** Host 两次 launch 的 phase 字面量（与 ER01 同值，便于对照）。 */
enum Er03Phase : int32_t {
    ER03_PHASE_PREP = 0,
    ER03_PHASE_COMPUTE = 1,
};

/** 兼容 ER01 脚手架命名（mmad_custom 内仍可写 ER01_PHASE_*）。 */
enum Er01Phase : int32_t {
    ER01_PHASE_PREP = ER03_PHASE_PREP,
    ER01_PHASE_COMPUTE = ER03_PHASE_COMPUTE,
};
...
/** Cube：C[16,32] int32 = A[16,32] int8 @ B[32,32] int8（B=I₃₂）；本刀不加 Cube 体量。 */
...
/**
 * GATE 真积木：近生产体量 int32[256]×32 轮 Mul/Add/Muls（禁 X14 空转）。
 * 已锁（ER03-TASK）：不得静默缩小；UB 放不下须回报主控。
 */
