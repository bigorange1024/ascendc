/**
 * @file aic_func.hpp
 * @brief 极轻 Cube MMAD：C[16,32]=A[16,32]@B[32,32]（int8×int8→int32）。
 *
 * ER04：几何仍 C[16,32]=A[16,32]@B[32,32]；调用方 Process×kCubeRounds=16（ER04-TASK）。
 * 流水线位置：AIC 在 CrossCore WAIT(1)/INTT WAIT(1) 之后、SET(3) 之前调用，
 * 给握手提供「真 Cube 活」；形状取 int8 Cube 最小粒度。
 * API：DataCopy+Nd2Nz / LoadData / LoadDataWithTranspose / Mmad / Fixpipe
 * （查阅索引 GT-20260903-1 已有记录，本玩具复用；无新增 API）。
 */
#ifndef ER04_CUBE_NTT_VOLUME_AIC_FUNC_HPP
#define ER04_CUBE_NTT_VOLUME_AIC_FUNC_HPP
...
/**
 * @class AicMmad
 * @brief Cube 侧 int8 矩阵乘：C[m,n] int32 = A[m,k] int8 @ B[k,n] int8。
 * 前置：k、n 为 32 的倍数（本玩具 k=n=32）；m 允许 ≤16（内部按 16 缩放）。
 */
class AicMmad {
public:
    ...
    /**
     * 执行 CopyIn→SplitA/B→Compute→CopyOut。
     * @param dst [out] C GM [m,n] int32
     * @param a   [in]  A GM [m,k] int8
     * @param b   [in]  B GM [k,n] int8
     */
...
#endif /* ER04_CUBE_NTT_VOLUME_AIC_FUNC_HPP */
