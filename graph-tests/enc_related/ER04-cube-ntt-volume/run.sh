#!/bin/bash
# ER04-cube-ntt-volume：prep 桩 + MIX 计算壳
# （NTT 1/3 Cube×16 + GATE 4/8 MAC 256×32 + INTT 1/3 Cube×16）
#
# Usage（默认 = 生产全量；本骨架无调试分段开关）：
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4   # 验收（只认不挂死）
#   bash run.sh -r cpu -v Ascend910B4                # 编译壳冒烟
#
# 不对算法正确性；kernel 退出码 0 + TRACE 可见 + 无 stray dump 即过。
# KERNEL_COMPUTE_BUDGET_SEC 默认 180（防挂死，非性能定标）
# 已锁：GATE 256×32；Cube 几何 16×32×32；kCubeRounds=16（ER04-TASK）；禁改参 / 禁 NPU
CURRENT_DIR=$(
    cd $(dirname ${BASH_SOURCE:-$0})
    pwd
)
_ORIG_ARGS=("$@")
# graph-tests/enc_related/ER04-… → 仓库根为三层上
...
echo "[SUCCESS] ER04-cube-ntt-volume (${RUN_MODE}) kernel exit=0 (no correctness cmp)"
