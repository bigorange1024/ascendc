/**
 * @file main.cpp
 * @brief Host：装填 seed/LUT/MAC，串行 3 launch（PREP→COMPUTE→COMPUTE），打印 TRACE，落盘完成标记。
 *
 * ER05 粘性假说：生产挂死可能与多 launch 粘性（脏 workspace / CrossCore 残留 / 二次进 MIX）
 * 有关，而非单次体量。本刀在一次 PREP 后连续两次 COMPUTE，共用同一 GM workspace（不重填
 * seed/LUT/MAC），核内 FSM/tiling/MAC/Cube 保持 ER04；仅 Host 启动序列改为三次 launch。
 *
 * Host 编号：110=prep launch 前；120=第一轮 COMPUTE 前；121=第二轮 COMPUTE 前；199=末次 Sync 后。
 * TRACE 约定：每轮 launch 前 Host 清 TRACE 槽，故 120… 与 121… 两段设备 TRACE 可区分。
 * MAC：优先读 gen_data 写出的 mac_{a,b,acc}.bin；缺文件则失败返回。
 */
...
/** 清零 TRACE 缓冲（每轮 launch 前；用于区分两轮 COMPUTE 的 TRACE 段）。 */
...
/**
 * CPU 孪生：PREP → COMPUTE → COMPUTE 各 ICPU_RUN_KF 一次。
 * 粘性：三次共用同一 out/ws；仅 TRACE 在每轮前清零；第二轮 COMPUTE 不得再走 PREP。
 */
static int32_t RunCpuThreeLaunch(uint8_t *out, uint8_t *ws, uint8_t *trace, TilingData *tiling,
                                 uint32_t blockDim, size_t outFileSize, size_t traceFileSize)
{
    ...
    /* ---- Launch3 COMPUTE（第二轮，同 workspace，不重填）---- */
    ClearTraceBuffer(traceSlots, traceFileSize);
    SetPhase(tiling, ER01_PHASE_COMPUTE);
    std::printf("121\n");
    std::fflush(stdout);
...
#else
/**
 * SIM/NPU：PREP → Sync → COMPUTE → Sync → COMPUTE → Sync；每轮 D2H 打印 TRACE。
 * 粘性：wsDevice 全程不重传；仅 TRACE 每轮前清零后 H2D。
 */
static int32_t RunDeviceThreeLaunch(aclrtStream stream, uint8_t *outDevice, uint8_t *wsDevice,
                                    ...
                                    size_t traceFileSize, size_t tilingSize)
{
    ...
    /* ---- Launch3 COMPUTE（第二轮，同 workspace）---- */
    ClearTraceBuffer(traceHost, traceFileSize);
    CHECK_ACL(aclrtMemcpy(traceDevice, traceFileSize, traceHost, traceFileSize,
                          ACL_MEMCPY_HOST_TO_DEVICE));
    SetPhase(tiling, ER01_PHASE_COMPUTE);
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    /* 粘性：装填一次后三次 launch 共用同一 ws；中间不重填。 */
    const int32_t rc =
        RunCpuThreeLaunch(out, ws, trace, tiling, blockDim, outFileSize, traceFileSize);
    ...
    /* 粘性：ws 只 H2D 一次；两轮 COMPUTE 共用同一 wsDevice。 */
    CHECK_ACL(aclrtMemcpy(wsDevice, wsFileSize, wsHost, wsFileSize, ACL_MEMCPY_HOST_TO_DEVICE));

    const int32_t rc =
        RunDeviceThreeLaunch(stream, outDevice, wsDevice, traceDevice, outHost, traceHost, tiling,
