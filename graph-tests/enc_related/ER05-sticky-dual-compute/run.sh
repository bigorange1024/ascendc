#!/bin/bash
# ER05-sticky-dual-compute：Host 三 launch 粘性双 COMPUTE
# （PREP → COMPUTE → COMPUTE；核内保持 ER04：NTT/INTT Cube×16 + GATE MAC 256×32）
#
# Usage（默认 = 生产全量；本骨架无调试分段开关）：
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4   # 验收（只认不挂死）
#   bash run.sh -r cpu -v Ascend910B4                # 编译壳冒烟
#
# 不对算法正确性；kernel 退出码 0 + TRACE 可见（含 120 与 121）+ 无 stray dump 即过。
# KERNEL_COMPUTE_BUDGET_SEC 默认 180（防挂死，非性能定标）
# 已锁：Host 3 launch；核参同 ER04（GATE 256×32；Cube×16）；禁改核参 / 禁 NPU
CURRENT_DIR=$(
    cd $(dirname ${BASH_SOURCE:-$0})
    pwd
)
_ORIG_ARGS=("$@")
# graph-tests/enc_related/ER05-… → 仓库根为三层上
...
echo "[SUCCESS] ER05-sticky-dual-compute (${RUN_MODE}) kernel exit=0 (no correctness cmp)"
