 ...
 * @brief RB-T09 Launch2：compute 壳 = T03 级 MIX 握手 + 有界 Cube + T06 级 pack 半桩。
 *
 * 生产 Encrypt 常见外形（不含算法拓扑真算）：
 *   AIC：Wait(1)→CubeNTT→Set(3) → Wait(4) → Wait(1)→CubeINTT→Set(3)
 *   AIV：Set(1)→Wait(3) → Set(4) → Set(1)→Wait(3) → Compress/BE pack(u,v→c)
 *
 * 背景：T08 拓扑在 Host 预喂 u,v（半桩体）；flag 1/3 复用、4=GATE；永禁 5/7。
 * 结论：单 launch MIX；与 Launch1 同 session 单库（禁双设备库）。
 ...
 *   - 有 POST_WAIT3_INTT 无 PACK_DONE → 卡 pack 段
 */
...
/** CrossCore Wait：mode 0x2，PIPE_MTE2（910B pipe 模板不生效）。 */
...
/** Compress_5：Barrett 标量（与 T06 / compress_d_ref 同式，仅 I/O）。 */
...
/**
 * AIV0：Host 预喂 u,v → Compress₁₁/₅ + ByteEncode → c[1568]（T06 契约；禁抄 encrypt）。
 * 同时写 ws 内 OFF_C 与独立 cOut，便于 Host 落盘。
 */
 ...
 * @param ws     [in/out] 共享 workspace（含 mat_a/b、u/v、TRACE、mat_c）
 * @param tiling [in]  占位
 */
                                                     ...
                                                     TilingData tiling)
{
    ...
    if (isAic) {
        ...
        if (subIdx == 0) {
            ...
            // ---------- T08 半桩体 + T06 pack：Host 预喂 u,v → c ----------
