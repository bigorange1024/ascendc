/**
 * @file main.cpp
 * @brief RB-T09 Host：双 launch — prep(AIV) → Synchronize → compute(MIX) → 落盘。
 *
 * CPU：依次 AIV_MODE + MIX_MODE；SIM：同 session 单库两次 ACLRT_LAUNCH_KERNEL。
 * 验收：不挂 + TRACE 因果 + prep 落盘；c 对齐则 PASS_IO，否则仅 PASS_SYNC。
 */
...
#endif
extern "C" void prep_custom(GM_ADDR yIn, GM_ADDR ekIn, GM_ADDR ws, TilingData tiling);
extern "C" void compute_custom(GM_ADDR out, GM_ADDR cOut, GM_ADDR ws, TilingData tiling);
#endif

/**
 * 打印 TRACE 槽，并据因果链显式打出 PREP / NTT / GATE / INTT / PACK。
 */
static void PrintTrace(const uint32_t *tr)
{
    ...
    if (nttDone && gateSeen && inttDone) {
        INFO_LOG("causal: Launch2 NTT(1/3)->GATE(4)->INTT(1/3 reuse) linked");
    }
    if (packDone) {
        INFO_LOG("causal: AIV0 pack(u,v->c) done after INTT");
    }
}

/**
 * 装填 Host 侧 ws：mat_a/b、u/v、TRACE HOST_PRE。
 * @return false 读盘失败
 */
...
/**
 * 主流程：读 input → Launch1 prep → sync → Launch2 compute → 落盘。
 */
int32_t main(int32_t argc, char *argv[])
{
    ...
    uint8_t *yIn = reinterpret_cast<uint8_t *>(AscendC::GmAlloc(kYe1e2Bytes > 1024 ? kYe1e2Bytes : 1024));
    ...
    size_t got = 0;
    ok = ReadFile("./input/y_e1_e2.bin", got, yIn, kYe1e2Bytes);
    if (!ok || got != kYe1e2Bytes) {
        return 1;
    }
    ...
    ICPU_RUN_KF(prep_custom, blockDim, yIn, ekIn, ws, *tiling);
    ...
    uint8_t *yHost = nullptr, *ekHost = nullptr, *outHost = nullptr, *cHost = nullptr, *wsHost = nullptr;
    uint8_t *yDev = nullptr, *ekDev = nullptr, *outDev = nullptr, *cDev = nullptr, *wsDev = nullptr;

    CHECK_ACL(aclrtMallocHost(reinterpret_cast<void **>(&yHost), kYe1e2Bytes));
    CHECK_ACL(aclrtMalloc(reinterpret_cast<void **>(&yDev), kYe1e2Bytes, ACL_MEM_MALLOC_HUGE_FIRST));
    ...
    std::memset(yHost, 0, kYe1e2Bytes);
    ...
    size_t got = 0;
    ok = ReadFile("./input/y_e1_e2.bin", got, yHost, kYe1e2Bytes);
    if (!ok || got != kYe1e2Bytes) {
        return 1;
    }
    ...
    CHECK_ACL(aclrtMemcpy(yDev, kYe1e2Bytes, yHost, kYe1e2Bytes, ACL_MEMCPY_HOST_TO_DEVICE));
    ...
    ACLRT_LAUNCH_KERNEL(prep_custom)(blockDim, stream, yDev, ekDev, wsDev, tiling);
    CHECK_ACL(aclrtSynchronizeStream(stream));

    // 读回 TRACE 写 HOST_MID，再写回（证明两 launch 之间 Host 可见）
