 ...
 * @brief RB-T09 Launch1：prep 半桩（AIV-only）— 承接 T07 语义 ρ / y‖e1‖e2 落入共享 ws。
 *
 * 本文件在流水线中的位置：Encrypt 外形双 launch 的第一刀；Host 已用 T07 公式预算
 * y_e1_e2 与 ek，本核负责设备侧落盘与 PREP 魔数，证明「prep launch」真实发生。
 *
 * 背景：真 CBD/SHAKE 设备化留给后刀；本刀允许半桩。
 * 结论：blockDim=1、KERNEL_TYPE_AIV_ONLY；无 CrossCore。
 * 未采用：在本核内重算 SHAKE/CBD（墙钟与主验收「不挂」优先）。
 */
