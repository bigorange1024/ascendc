#!/bin/bash
# RB-T09-encrypt-shaped-2launch：外形双 launch（prep AIV + compute MIX）
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=90 / sim=300（双 launch + pack）
...
if [ "${RUN_MODE}" = "sim" ]; then
    ...
    export KERNEL_COMPUTE_BUDGET_SEC="${KERNEL_COMPUTE_BUDGET_SEC:-90}"
fi
...
echo "[SUCCESS] RB-T09-encrypt-shaped-2launch (${RUN_MODE})"
