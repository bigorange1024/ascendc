#!/bin/bash
# RB-T10-uv-device-mix：设备 MIX 真拓扑（MultiplyNTTs + INTT → u,v）
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=120 / sim=600（标量 MultiplyNTTs+INTT）
...
echo "[SUCCESS] RB-T10-uv-device-mix (${RUN_MODE})"
