...
# -*- coding: utf-8 -*-
"""
RB-T10 verify：PASS_SYNC（握手+Cube）+ PASS_IO（u,v 对拍）。

硬条件：
  - out 魔数 T10
  - TRACE Host PRE/POST；AIV 成对槽 NTT/GATE/INTT（勿硬绑 AIV0）
  - mat_c_ntt / mat_c_intt 非全 0
  - u.bin / v.bin 与 golden 逐元素一致（设备真算）

软：AIC TRACE、MUL_DONE/UV_DONE 缺失仅告警（SIM 上 AIV0 应有 MUL/UV）。
"""
...
SOFT = {
    ...
    10: 0xC1011001,
    11: 0xC1031003,
    17: 0x4D554C44,  # MUL_DONE
    18: 0x5556444E,  # UV_DONE
}
...
def main() -> int:
    ...
    pass_io = _cmp_bin(OUT / "u.bin", OUT / "golden_u.bin", "u") and _cmp_bin(
        OUT / "v.bin", OUT / "golden_v.bin", "v"
    )

    if pass_sync:
        print("[PASS_SYNC] NTT(1/3)+GATE(4)+INTT(1/3) causal + dual Cube ok")
    else:
        print("[FAIL_SYNC] handshake/Cube", file=sys.stderr)
    if pass_io:
        print("[PASS_IO] u,v match golden (device MIX)")
    else:
        print("[FAIL_IO] u,v mismatch", file=sys.stderr)

    if pass_sync and pass_io:
        print("[SUCCESS] RB-T10 PASS_SYNC + PASS_IO")
        return 0
    return 1
