 ...
 * @brief RB-T10 设备侧拓扑数学：MultiplyNTTs / Âᵀ∘ŷ / ⟨t̂,ŷ⟩ / InverseNTT + 加噪。
 *
 * 流水线位置：MIX AIV0 在 GATE 前写 û/v̂，在 INTT 握手后写时域 u/v。
 ...
 */
#ifndef RB_T10_UV_DEVICE_MATH_HPP
#define RB_T10_UV_DEVICE_MATH_HPP
...
/**
 * 时域收尾：u = INTT(û)+e₁，v = INTT(v̂)+e₂+μ；写 ws 内 U/V 与独立 uvOut。
 * @param ws     共享 workspace
 * @param uvOut  Host 独立缓冲：前 kUBytes 为 u，后 kVBytes 为 v
 */
__aicore__ inline void ComputeInttAddNoise(GM_ADDR ws, GM_ADDR uvOut)
{
    using namespace tiling;
    ...
    AscendC::GlobalTensor<int32_t> gmUvOut;
    gmUHat.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(ws + OFF_U_HAT),
                           static_cast<uint32_t>(kKem * kPolyN));
    gmVHat.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(ws + OFF_V_HAT),
                           static_cast<uint32_t>(kPolyN));
    ...
    gmV.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(ws + OFF_V),
                        static_cast<uint32_t>(kPolyN));
    gmUvOut.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(uvOut),
                            static_cast<uint32_t>((kUBytes + kVBytes) / sizeof(int32_t)));
    ...
    for (int32_t i = 0; i < kKem; ++i) {
        ...
        // 镜像到 uvOut 前段
        AscendC::DataCopy(gmUvOut[static_cast<uint32_t>(i * kPolyN)], poly,
                          static_cast<uint32_t>(kPolyN));
        AscendC::PipeBarrier<PIPE_ALL>();
    }
    ...
    AscendC::PipeBarrier<PIPE_ALL>();
    AscendC::DataCopy(gmUvOut[static_cast<uint32_t>(kKem * kPolyN)], poly,
                      static_cast<uint32_t>(kPolyN));
    AscendC::PipeBarrier<PIPE_ALL>();
