#!/bin/bash
# RB-T11-encrypt-shaped-device-uv：Encrypt 外形：prep+设备u,v+pack
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=180 / sim=900（标量 MultiplyNTTs+INTT）
...
echo "[SUCCESS] RB-T11-encrypt-shaped-device-uv (${RUN_MODE})"
