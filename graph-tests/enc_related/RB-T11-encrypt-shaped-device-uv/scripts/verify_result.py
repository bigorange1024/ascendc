...
# -*- coding: utf-8 -*-
"""
RB-T11 verify：分栏 PASS_SYNC（主）/ PASS_IO（尽力）。
...
PASS_IO（尽力；失败仅告警 PASS_IO=0，不否决 SYNC）：
  - c / u / v / y_e1_e2_dev / rho_dev 与 golden
...
def main() -> int:
    ...
    if len(tr) < 22 * 4:
        ...
        return 1
    vals = list(struct.unpack_from("<22I", tr, 0))
    ...
    if io_ok:
        print("[SUCCESS] PASS_SYNC + PASS_IO — RB-T11 shaped device-uv")
    else:
