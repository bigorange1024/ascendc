#!/bin/bash
# RB-T12-device-ntt-y：设备 MIX ŷ←NTT(y)（接 prep CBD）
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=120 / sim=600（标量 Alg.9 ×4 poly）
...
echo "[SUCCESS] RB-T12-device-ntt-y (${RUN_MODE})"
