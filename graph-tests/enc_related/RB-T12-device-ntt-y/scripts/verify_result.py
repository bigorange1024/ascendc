...
# -*- coding: utf-8 -*-
"""
RB-T12 verify：PASS_SYNC（握手+Cube）+ PASS_IO（ŷ 对拍）。

硬条件：
  - out 魔数 T12
  - TRACE Host PRE/POST；AIV 成对槽 NTT（勿硬绑 AIV0）
  - mat_c_ntt 非全 0
  - y_hat.bin 与 golden 逐元素一致（设备真算）

软：AIC TRACE、YHAT_DONE 缺失仅告警（SIM 上 AIV0 应有 YHAT）。
"""
...
HARD_AIV_PAIRS = [
    ...
    (5, 0xA1030003, 7, 0xA1130003, "POST_WAIT3_NTT"),
]

SOFT = {
    3: 0xC1010001,
    4: 0xC1030003,
    8: 0x59484154,  # YHAT_DONE
}
...
def main() -> int:
    ...
    pass_io = _cmp_bin(OUT / "y_hat.bin", OUT / "golden_y_hat.bin", "y_hat")

    if pass_sync:
        print("[PASS_SYNC] NTT(1/3) causal + Cube ok")
    else:
        print("[FAIL_SYNC] handshake/Cube", file=sys.stderr)
    if pass_io:
        print("[PASS_IO] ŷ match golden (device MIX)")
    else:
        print("[FAIL_IO] ŷ mismatch", file=sys.stderr)

    if pass_sync and pass_io:
        print("[SUCCESS] RB-T12 PASS_SYNC + PASS_IO")
        return 0
    return 1
