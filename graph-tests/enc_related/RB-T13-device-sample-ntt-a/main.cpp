/**
 * @file main.cpp
 * @brief RB-T13 Host：预喂 ρ + mat → 单 launch MIX → 落盘 Â / TRACE。
 *
 * CPU：ICPU_RUN_KF + MIX_MODE；SIM：ACLRT_LAUNCH_KERNEL。
 * 禁预填最终 Â（aHatOut 启动前清零）；验收靠设备写出后与 golden 对拍。
 */
...
#endif
extern "C" void mmad_custom(GM_ADDR out, GM_ADDR aHatOut, GM_ADDR ws, TilingData tiling);
#endif

/**
 * 打印 TRACE 槽，并据因果链显式打出 SampleNTT 握手 / AHAT。
 */
static void PrintTrace(const uint32_t *tr)
{
    ...
    if (handDone) {
        INFO_LOG("causal: SampleNTT SET1+WAIT3 seen");
    }
    if (handDone && ahatDone) {
        INFO_LOG("causal: device SampleNTT(ρ) wrote Â");
        INFO_LOG("phases: SampleNTT(1/3)->AHAT all linked");
    }
}

/**
 * 装填 Host 侧 ws：ρ + mat_a/b；**不**写入最终 Â。
 * @return false 读盘失败
 */
...
/**
 * 主流程：读 input → launch → 落盘 out/a_hat/trace/mat_c。
 */
int32_t main(int32_t argc, char *argv[])
{
    ...
    INFO_LOG("Host: TRACE HOST_PRE marked, launching MIX device-sample-ntt-a (CPU twin)");
    ICPU_RUN_KF(mmad_custom, blockDim, out, aHatOut, ws, *tiling);
    ...
    uint8_t *outHost = nullptr, *aHatHost = nullptr, *wsHost = nullptr;
    uint8_t *outDev = nullptr, *aHatDev = nullptr, *wsDev = nullptr;
    ...
    INFO_LOG("Host: TRACE HOST_PRE marked, launching MIX device-sample-ntt-a (SIM)");
    ACLRT_LAUNCH_KERNEL(mmad_custom)(blockDim, stream, outDev, aHatDev, wsDev, tiling);
