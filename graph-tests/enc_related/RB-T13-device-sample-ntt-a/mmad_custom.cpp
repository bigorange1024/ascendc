 ...
 * @brief RB-T13：MIX 设备 Â←SampleNTT(ρ) — 握手(1/3) → AIV0 16×Alg.7。
 *
 * 生产 Encrypt 行 3–7 外形（设备算 Â；禁 Host 预喂最终 Â）：
 *   AIC：Wait(1)→CubeSample→Set(3)
 *   AIV：Set(1)→Wait(3) → SampleNTT(ρ‖j‖i)→Â（仅 AIV0 写结果）
 *
 * 背景：T07 Host ρ←ek 尾；T12 MIX 半链；本刀关行 3–7 设备侧。
 * 结论：flag 仅 1/3；永禁 5/7；主验收 Â 对拍。
 ...
 * @param aHatOut [out] 设备算出的 Â（int32，16384B）；Host 禁预填最终值
 * @param ws      [in/out] 预喂 ρ + mat + TRACE；设备写 A_HAT
 * @param tiling  [in] 占位
 */
extern "C" __global__ __aicore__ void mmad_custom(GM_ADDR out, GM_ADDR aHatOut, GM_ADDR ws,
                                                  TilingData tiling)
{
    (void)tiling;
    ...
    if (isAic) {
        rb_t13::LightCube cube;
        cube.Init();

        // ---------- SampleNTT 握手：Wait(1) → Cube → Set(3) ----------
        ...
        if (subIdx == 0) {
            ...
            // ---------- AIV0：Â←SampleNTT(ρ)（16 poly；积木接 lines3-7）----------
            rb_t13::ComputeAHatSampleNtt(ws, aHatOut);
