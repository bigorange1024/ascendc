#!/bin/bash
# RB-T13-device-sample-ntt-a：设备 MIX Â←SampleNTT(ρ)（接 T07 ρ 尾语义）
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=300 / sim=900（16×SampleNTT+MIX）
...
echo "[SUCCESS] RB-T13-device-sample-ntt-a (${RUN_MODE})"
