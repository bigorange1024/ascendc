...
# -*- coding: utf-8 -*-
"""
RB-T13 verify：PASS_SYNC（握手+Cube）+ PASS_IO（Â 对拍）。

硬条件：
  - out 魔数 T13
  - TRACE Host PRE/POST；AIV 成对槽 SampleNTT 握手（勿硬绑 AIV0）
  - mat_c 非全 0
  - a_hat.bin 与 golden 逐元素一致（设备真算）

软：AIC TRACE、AHAT_DONE 缺失仅告警（SIM 上 AIV0 应有 AHAT）。
"""
...
def main() -> int:
    ...
    pass_io = _cmp_bin(OUT / "a_hat.bin", OUT / "golden_a_hat.bin", "a_hat")

    if pass_sync:
        print("[PASS_SYNC] SampleNTT(1/3) causal + Cube ok")
    else:
        print("[FAIL_SYNC] handshake/Cube", file=sys.stderr)
    if pass_io:
        print("[PASS_IO] Â match golden (device MIX)")
    else:
        print("[FAIL_IO] Â mismatch", file=sys.stderr)

    if pass_sync and pass_io:
        print("[SUCCESS] RB-T13 PASS_SYNC + PASS_IO")
        return 0
    return 1
