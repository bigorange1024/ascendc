/**
 * @file main.cpp
 * @brief RB-T14 Host：预喂 ρ/y/ζ + mat → 单 launch MIX → 落盘 Â/ŷ / TRACE。
 *
 * CPU：ICPU_RUN_KF + MIX_MODE；SIM：ACLRT_LAUNCH_KERNEL。
 * 禁预填最终 Â/ŷ；验收靠设备写出后与 golden 双对拍。
 */
...
#endif
extern "C" void mmad_custom(GM_ADDR out, GM_ADDR aHatOut, GM_ADDR yHatOut, GM_ADDR ws,
                            TilingData tiling);
#endif

/**
 * 打印 TRACE 槽，并据因果链打出握手 / AHAT / YHAT。
 */
static void PrintTrace(const uint32_t *tr)
{
    ...
    if (handDone) {
        INFO_LOG("causal: SET1+WAIT3 seen");
    }
    if (handDone && ahatDone) {
        INFO_LOG("causal: device SampleNTT(ρ) wrote Â");
    }
    if (handDone && ahatDone && yhatDone) {
        INFO_LOG("causal: device NTT(y) wrote ŷ");
        INFO_LOG("phases: (1/3)->AHAT->YHAT all linked");
    }
}

/**
 * 装填 Host 侧 ws：ρ + y + ζ + mat；**不**写入最终 Â/ŷ。
 * @return false 读盘失败
 */
...
/**
 * 主流程：读 input → launch → 落盘 out/a_hat/y_hat/trace/mat_c。
 */
int32_t main(int32_t argc, char *argv[])
{
    ...
    uint8_t *aHatOut =
        reinterpret_cast<uint8_t *>(AscendC::GmAlloc(kAHatBytes > 1024 ? kAHatBytes : 1024));
    uint8_t *yHatOut =
    ...
    std::memset(aHatOut, 0, kAHatBytes);
    ...
    INFO_LOG("Host: TRACE HOST_PRE marked, launching MIX device-ahat-yhat (CPU twin)");
    ICPU_RUN_KF(mmad_custom, blockDim, out, aHatOut, yHatOut, ws, *tiling);
    ...
    ok = WriteFile("./output/a_hat.bin", aHatOut, kAHatBytes);
    if (!ok) {
        return 15;
    }
    ...
    uint8_t *outHost = nullptr, *aHatHost = nullptr, *yHatHost = nullptr, *wsHost = nullptr;
    uint8_t *outDev = nullptr, *aHatDev = nullptr, *yHatDev = nullptr, *wsDev = nullptr;
    ...
    INFO_LOG("Host: TRACE HOST_PRE marked, launching MIX device-ahat-yhat (SIM)");
    ACLRT_LAUNCH_KERNEL(mmad_custom)(blockDim, stream, outDev, aHatDev, yHatDev, wsDev, tiling);
    ...
    ok = WriteFile("./output/a_hat.bin", aHatHost, kAHatBytes);
    if (!ok) {
        return 15;
    }
