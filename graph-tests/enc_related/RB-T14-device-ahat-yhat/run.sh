#!/bin/bash
# RB-T14-device-ahat-yhat：设备 MIX Â←SampleNTT(ρ) + ŷ←NTT(y)（同核顺序）
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=600 / sim=1200（16×SampleNTT + Alg.9×4）
...
echo "[SUCCESS] RB-T14-device-ahat-yhat (${RUN_MODE})"
