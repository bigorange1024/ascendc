...
# -*- coding: utf-8 -*-
"""
RB-T14 verify：PASS_SYNC（握手+Cube）+ PASS_IO（Â 与 ŷ 双对拍）。

硬条件：
  - out 魔数 T14
  - TRACE Host PRE/POST；AIV 成对槽握手（勿硬绑 AIV0）
  - mat_c 非全 0
  - a_hat.bin / y_hat.bin 与 golden 逐元素一致

软：AIC TRACE、AHAT_DONE / YHAT_DONE 缺失仅告警。
"""
...
SOFT = {
    3: 0xC1010001,
    4: 0xC1030003,
    8: 0x41484154,  # AHAT_DONE
    9: 0x59484154,  # YHAT_DONE
}
...
def main() -> int:
    ...
    if len(tr) < 10 * 4:
        ...
        return 1
    vals = list(struct.unpack_from("<10I", tr, 0))
    ...
    pass_io_a = _cmp_bin(OUT / "a_hat.bin", OUT / "golden_a_hat.bin", "a_hat")
    pass_io_y = _cmp_bin(OUT / "y_hat.bin", OUT / "golden_y_hat.bin", "y_hat")
    pass_io = pass_io_a and pass_io_y

    if pass_sync:
        print("[PASS_SYNC] SampleNTT+NTT(1/3) causal + Cube ok")
    else:
        print("[FAIL_SYNC] handshake/Cube", file=sys.stderr)
    if pass_io:
        print("[PASS_IO] Â and ŷ match golden (device MIX)")
    else:
        print("[FAIL_IO] Â/ŷ mismatch", file=sys.stderr)

    if pass_sync and pass_io:
        print("[SUCCESS] RB-T14 PASS_SYNC + PASS_IO")
        return 0
    return 1
