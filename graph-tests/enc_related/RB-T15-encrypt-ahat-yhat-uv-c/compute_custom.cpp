 ...
 * @brief RB-T15 Launch2：握手 + 设备 Â/ŷ + Mul/INTT→u,v + pack→c。
 *
 * 生产 Encrypt 半链外形（设备 Â/ŷ/u/v；禁 Host 预喂最终值）：
 *   AIC：Wait(1)→CubeNTT→Set(3) → Wait(4) → Wait(1)→CubeINTT→Set(3)
 *   AIV：Set(1)→Wait(3) → Â←SampleNTT(ρ) → ŷ←NTT(y) → Mul → Set(4)
 *        → Set(1)→Wait(3) → INTT+噪 → pack(u,v→c)
 *
 * 背景：T11 外形但 Host 预喂 Â/ŷ；T14 设备 Â/ŷ 无 Mul/pack；本刀贯通。
 ...
 *   - 有 POST_WAIT3 无 AHAT/YHAT → 卡 SampleNTT / ForwardNTT
 *   - 有 YHAT 无 MUL_DONE → 卡 MultiplyNTTs
 ...
 * @param ws     [in/out] prep ρ/y + Host t̂/e/μ + mat + TRACE；设备写 Â/ŷ/û/v̂/u/v/c
 * @param tiling [in]  占位
 */
                                                     ...
                                                     TilingData tiling)
{
    ...
    if (isAic) {
        ...
        if (subIdx == 0) {
            ...
            // ---------- T14：设备 Â←SampleNTT(ρ)、ŷ←NTT(y)（禁 Host 预喂最终）----------
