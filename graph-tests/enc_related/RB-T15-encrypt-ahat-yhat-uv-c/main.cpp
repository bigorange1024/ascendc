/**
 * @file main.cpp
 * @brief RB-T15 Host：双 launch — prep(AIV) → Synchronize → compute(MIX 设备 Â/ŷ/u,v+pack)。
 *
 * CPU：依次 AIV_MODE + MIX_MODE；SIM：同 session 单库两次 ACLRT_LAUNCH_KERNEL。
 * 禁预填最终 Â/ŷ/u/v；Host 预喂 t̂/e/μ/ζ/γ + prep 输入。
 * 验收：不挂 + TRACE 因果 → PASS_SYNC；c/u/v 对齐 → PASS_IO。
 */
...
/** 打印 TRACE 槽并据因果链打出 PREP / AHAT / YHAT / MUL / GATE / INTT / UV / PACK。 */
static void PrintTrace(const uint32_t *tr)
{
    ...
    if (nttDone && mulDone && gateSeen && inttDone) {
        INFO_LOG("causal: Launch2 NTT->AHAT/YHAT->MUL->GATE->INTT linked");
    }
    if (uvDone) {
...
/**
 * 装填 Host 侧 ws：t̂/e/μ/ζ/γ + mat；**不**写入最终 Â/ŷ/u/v。
 * @return false 读盘失败
 */
