 ...
 * @brief RB-T15 设备侧 Â 薄壳：读 Host ρ → 调用活跃 lines3-7 积木。
 *
 ...
 */
#ifndef RB_T15_SAMPLE_NTT_DEVICE_HPP
#define RB_T15_SAMPLE_NTT_DEVICE_HPP
...
/**
 * AIV0：从 ws 读 Launch1 落盘的 ρ[32]，跑 16× SampleNTT，写 Â 到 OFF_A_HAT。
 *
 * @param ws 共享 workspace（含 OFF_RHO / OFF_A_HAT）
 *
 * 前置：仅 AIV0 调用；`F203_AHAT16_BLOCK_DIM==1`；Host 禁预填最终 Â。
...
__aicore__ inline void ComputeAHatSampleNtt(GM_ADDR ws)
{
    ...
    // ---------- 3) 积木：ρ → 16 poly â → ws+OFF_A_HAT（供后续 Mul）----------
