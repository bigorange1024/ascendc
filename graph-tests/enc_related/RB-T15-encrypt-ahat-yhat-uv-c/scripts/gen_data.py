...
# -*- coding: utf-8 -*-
"""
RB-T15 gen_data：Launch1 prep + 设备 Â/ŷ 契约 golden + Host 预喂 t̂/e/μ + golden u/v/c。

契约：
  - prep：T07 Host 公式（ρ / y‖e1‖e2）；禁抄 encrypt
  - Â golden：16×Alg.7 SampleNTT(ρ)（与 T14 同黑盒）
  - ŷ golden：Alg.9 NTT(y)（y 来自 prep CBD）
  - 拓扑噪声 t̂/e₁/e₂/μ：Host 预喂（设备 CBD 非本刀目标）
  - **不**把最终 Â/ŷ/u/v 写入 input
  - golden_u/v：topology_math.compute_u_v；golden_c：T06 pack
"""
...
def main() -> int:
    ...
    # ---- 设备 Â/ŷ golden（与 T14 同 ρ/y；禁写入 input）----
    ...
    print(
        f"[gen_data] device Â/ŷ golden; t̂ Host-prefed; "
        f"u sum={int(u.sum())} c={len(golden_c)}"
    )
    return 0
