 ...
 * @brief RB-T17 Launch2：握手 + CBD(coins)→y/e + Â/ŷ + Mul/INTT→u,v + pack→c。
 *
 * 生产 Encrypt 全设备外形（禁 Host 预喂最终 y/e/Â/ŷ/u/v/c）：
 *   AIC：Wait(1)→CubeNTT→Set(3) → Wait(4) → Wait(1)→CubeINTT→Set(3)
 *   AIV：Set(1)→Wait(3) → CBD → Â←SampleNTT(ρ) → ŷ←NTT(y) → Mul → Set(4)
 ...
 * @param ws   [in/out] coins/ρ/t̂/μ/ζ/γ/mat；设备写 yee/Â/ŷ/u/v/c
 */
