...
# -*- coding: utf-8 -*-
"""
RB-T17 verify：PASS_SYNC（主）/ PASS_IO（c/u/v/Â/ŷ/yee/ρ）。

PASS_SYNC 硬槽含 CBD_DONE；out 魔数 T17=0x543F0011。
"""
...
def main() -> int:
    ...
    if len(tr) < 25 * 4:
        ...
        return 1
    vals = list(struct.unpack_from("<25I", tr, 0))
    ...
    print("[PASS_SYNC] dual-launch causal + CBD + AHAT/YHAT + dual Cube + pack TRACE ok")
    ...
    if io_ok:
        print("[SUCCESS] PASS_SYNC + PASS_IO — RB-T17 encrypt-full-device")
    else:
