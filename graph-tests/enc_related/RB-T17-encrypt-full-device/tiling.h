/**
 * @file tiling.h
 * @brief RB-T17：全设备 Encrypt — CBD(coins)→y/e + Â/ŷ → u,v → c。
 *
 * Launch1 prep（AIV）：仅 ρ←ek 尾；coins 由 Host 写入 OFF_COINS（禁预喂最终 y/e）。
 * Launch2 compute（MIX）：CBD→y‖e1‖e2 → SampleNTT/NTT → Mul/INTT → pack。
 *
 * Flag（永禁 5/7）：
 ...
 *   AIV: Set(1)→Wait(3) → CBD → Â+ŷ → Mul → Set(4)
 *        → Set(1)→Wait(3) → INTT+噪 → pack
 *
 * 相对 T15：设备 CBD 取代 Host 预喂 y/e；Host 仅 ek/coins/μ/t̂/ζ/γ。
 */
#ifndef RB_T17_ENCRYPT_FULL_DEVICE_TILING_H
#define RB_T17_ENCRYPT_FULL_DEVICE_TILING_H
...
constexpr uint32_t MAGIC_OUT_OK = 0x543F0011u;         // T17
constexpr uint32_t MAGIC_PREP_MARK = 0x543F5052u;      // T17P
