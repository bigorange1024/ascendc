 ...
 * @brief RB-T18：MIX 设备 ByteDecode₁₂(ek)→t̂ — 握手(1/3) → AIV0 Decode₁₂。
 *
 * 生产 Encrypt 行 2 外形（设备解 ek；禁 Host 预喂最终 t̂）：
 *   AIC：Wait(1)→CubeSample→Set(3)
 *   AIV：Set(1)→Wait(3) → ByteDecode₁₂(ek 体) → t̂（仅 AIV0 写结果）
 *
 * 背景：T05 Decode₁₂ 契约；T16 MIX 半链壳；本刀关设备 Decode₁₂(ek)。
 * 结论：flag 仅 1/3；永禁 5/7；主验收 t̂ 对拍。
 ...
 * @param tHatOut [out] 设备算出的 t̂（int32，4096B）；Host 禁预填最终值
 * @param ws      [in/out] 预喂 ek + mat + TRACE
 * @param tiling  [in] 占位
 */
extern "C" __global__ __aicore__ void mmad_custom(GM_ADDR out, GM_ADDR tHatOut, GM_ADDR ws,
                                                  TilingData tiling)
{
    (void)tiling;
    ...
    if (isAic) {
        rb_t18::LightCube cube;
        cube.Init();

        // ---------- Decode 握手：Wait(1) → Cube → Set(3) ----------
        ...
        if (subIdx == 0) {
            ...
            // ---------- AIV0：ek→ByteDecode₁₂→t̂ ----------
            rb_t18::ComputeTHatDecode12(ws, tHatOut);
