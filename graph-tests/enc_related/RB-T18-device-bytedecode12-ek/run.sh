#!/bin/bash
# RB-T18-device-bytedecode12-ek：设备 MIX ByteDecode₁₂(ek)→t̂（接 Encrypt prep）
#
# Usage（默认 = 全量验收路径；本战役 NPU 优先，交 CPU 绿即可）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=120 / sim=300（Decode₁₂×4+MIX）
...
echo "[SUCCESS] RB-T18-device-bytedecode12-ek (${RUN_MODE})"
