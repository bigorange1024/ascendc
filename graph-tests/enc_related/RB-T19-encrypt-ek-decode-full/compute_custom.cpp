 ...
 * @brief RB-T19 Launch2：握手 + Decode₁₂(ek)→t̂ + CBD→y/e + Â/ŷ + Mul/INTT→u,v + pack→c。
 *
 * 生产 Encrypt 全设备外形（禁 Host 预喂最终 t̂/y/e/Â/ŷ/u/v/c）：
 *   AIC：Wait(1)→CubeNTT→Set(3) → Wait(4) → Wait(1)→CubeINTT→Set(3)
 *   AIV：Set(1)→Wait(3) → Decode₁₂ → CBD → Â←SampleNTT(ρ) → ŷ←NTT(y) → Mul → Set(4)
 ...
 * @param ws   [in/out] coins/ρ/ek/μ/ζ/γ/mat；设备写 t̂/yee/Â/ŷ/u/v/c
 */
