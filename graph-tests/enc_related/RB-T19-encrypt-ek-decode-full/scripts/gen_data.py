...
# -*- coding: utf-8 -*-
"""
RB-T19 gen_data：Host 仅 ek/coins/μ/ζ/γ；ek 体=BE₁₂(t̂)；golden 含设备 Decode/CBD/Â/ŷ/u/v/c。

契约：
  - ek[1568]=BE₁₂(t̂)[1536]‖ρ[32]；**不**写 input/t_hat.bin（设备 Decode₁₂）
  - coins：T07 FIXED_COINS；设备 CBD
  - Â：16×Alg.7 SampleNTT(ρ)；ŷ：Alg.9 NTT(y)
  - μ：Host 预喂；禁写最终 Â/ŷ/u/v/c/t̂ 到 input
"""
...
def byte_encode12(coeffs: np.ndarray) -> np.ndarray:
    """coeffs int32[256] → uint8[384]（FIPS Alg.5 d=12）；契约对齐 T18。"""
...
def main() -> int:
    ...
    def rand_hat_poly() -> np.ndarray:
        # Decode₁₂ 只保 12-bit；系数须落在 [0,4095] 且 round-trip 与设备一致。
        # 用 NTT 后 mod 回 [0,q) 再编码，与 T17 拓扑同分布但可无损失 BE₁₂。
    ...
    # ek 体 = BE₁₂(t̂)；ρ 尾 = FIXED_RHO（与 T07 一致，SampleNTT 同源）
    ...
    # 禁写 t_hat / y_e1_e2 / e1 / e2 / a_hat / y_hat / u / v / c 到 input
    ...
    print(
        f"[gen_data] T19 ek=BE12(t_hat)||rho; device Decode+CBD+Â/ŷ; "
