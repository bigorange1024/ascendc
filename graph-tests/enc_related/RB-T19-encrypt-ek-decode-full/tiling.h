/**
 * @file tiling.h
 * @brief RB-T19：全设备 Encrypt — Decode₁₂(ek)→t̂ + CBD + Â/ŷ → u,v → c。
 *
 ...
 *
 * 相对 T17：Host **不再**预喂 t̂；设备 Decode₁₂（T18 契约）写出 OFF_T_HAT。
 * Host 仅 ek/coins/μ/ζ/γ/mat。
 *
 * Flag（永禁 5/7）：
 ...
 */
#ifndef RB_T19_ENCRYPT_EK_DECODE_FULL_TILING_H
#define RB_T19_ENCRYPT_EK_DECODE_FULL_TILING_H
...
constexpr uint32_t MAGIC_OUT_OK = 0x543F0013u;         // T19
constexpr uint32_t MAGIC_PREP_MARK = 0x543F5053u;      // T19P
