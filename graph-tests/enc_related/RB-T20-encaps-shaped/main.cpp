/**
 * @file main.cpp
 * @brief RB-T20 Host：Encaps 外形（μ←m + 转发 K）+ 双 launch Encrypt 全链。
 *
 * Encaps 外形（本刀 Host）：
 *   - μ ← Decompress₁∘ByteDecode₁(m)（T04 契约；不读预喂 μ.bin）
 *   - K_host ← input/K_host.bin（gen_data 已算 G(m‖H(ek))[:32]；设备 K 后刀）
 *   - coins = r 由 gen_data 写入，喂 prep（设备 G 头后刀）
 *
 * Encrypt（同 T19）：prep(AIV: coins+ek+ρ) → Synchronize → compute(MIX)。
 * 禁预填最终 t̂/y/e/Â/ŷ/u/v/c。
 */
...
/**
 * Host Encaps：m[32] → μ[256] int32（FIPS ByteDecode₁ + Decompress₁）。
 * 背景：T04 契约；本刀外形在 Host 完成，设备 μ 嵌入可后刀。
 * 公式：bit_i=(m[i/8]>>(i%8))&1；μ_i=(bit_i*q+1)>>1 ∈ {0,1665}。
 */
static void HostMuEmbedFromM(const uint8_t *m, int32_t *mu)
{
    using namespace tiling;
    for (int32_t i = 0; i < kPolyN; ++i) {
        const int32_t bit = (static_cast<int32_t>(m[i / 8]) >> (i % 8)) & 1;
        mu[i] = (bit * kQ + 1) >> 1;
    }
}
...
/**
 * 装填 Host 侧 ws：Encaps μ←m + ζ/γ/mat；清零 t̂/Â/ŷ/YEE/u/v/c。
 * 背景：相对 T19，μ 由 Host 从 m 嵌入（不读 mu.bin）；禁预喂最终 t̂。
 */
static bool LoadWorkspaceHost(uint8_t *ws, const uint8_t *m)
{
    ...
    // Encaps 外形：Host μ←m（T04）
    HostMuEmbedFromM(m, reinterpret_cast<int32_t *>(ws + OFF_MU));
...
/** 写出 Encaps Host 产物：K_host（转发）+ μ_host（本机嵌入）。 */
static bool WriteEncapsHostOutputs(const uint8_t *ws)
{
    using namespace tiling;
    size_t got = 0;
    uint8_t kHost[32];
    if (!ReadFile("./input/K_host.bin", got, kHost, 32) || got != 32) {
        ERROR_LOG("missing input/K_host.bin");
        return false;
    }
    if (!WriteFile("./output/K_host.bin", kHost, 32)) {
        return false;
    }
    if (!WriteFile("./output/mu_host.bin", ws + OFF_MU, kMuBytes)) {
        return false;
    }
    return true;
}
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    INFO_LOG("Host: Encaps-shaped mu<-m; Launch1 prep_custom (CPU AIV twin)");
    ...
    INFO_LOG("Host: Encaps-shaped mu<-m; Launch1 prep_custom (SIM/NPU)");
    ...
    if (!WriteEncapsHostOutputs(wsHost)) {
        return 26;
    }
