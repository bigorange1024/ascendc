...
# -*- coding: utf-8 -*-
"""
RB-T20 verify：PASS_SYNC（主）/ PASS_IO（c + Host K + μ/u/v/Â/ŷ/t̂/yee/ρ）。

PASS_SYNC 硬槽同 T19（BD12+CBD+AHAT/YHAT+pack）；out 魔数 T20=0x543F0014。
Host K：output/K_host.bin ↔ golden_K；并自洽重算 G(m‖H(ek))。
"""
...
def _recompute_K() -> bytes | None:
    """Host 自洽：K = SHA3-512(m‖SHA3-256(ek))[:32]。"""
    ...
    return hashlib.sha3_512(m + h).digest()[:32]
...
def main() -> int:
    ...
    if len(tr) < 26 * 4:
        ...
        return 1
    vals = list(struct.unpack_from("<26I", tr, 0))
    ...
    print("[PASS_SYNC] dual-launch causal + BD12 + CBD + AHAT/YHAT + dual Cube + pack TRACE ok")

    io_ok = True
    io_ok = _cmp_bytes(OUT / "c.bin", OUT / "golden_c.bin", "c") and io_ok
    io_ok = _cmp_bytes(OUT / "K_host.bin", OUT / "golden_K.bin", "K_host") and io_ok
    recomputed = _recompute_K()
    if recomputed is None:
        print("[PASS_IO=0] cannot recompute Host K from input m/ek")
        io_ok = False
    elif (OUT / "golden_K.bin").read_bytes() != recomputed:
        print("[PASS_IO=0] golden_K != SHA3-512(m||H(ek))[:32]")
        io_ok = False
    else:
        print("[PASS_IO] Host K self-check G(m||H(ek)) ok")
    io_ok = _cmp_i32(OUT / "mu_host.bin", OUT / "golden_mu.bin", "mu_host") and io_ok
    ...
    if io_ok:
        print("[SUCCESS] PASS_SYNC + PASS_IO — RB-T20 encaps-shaped (c + Host K + μ)")
    else:
