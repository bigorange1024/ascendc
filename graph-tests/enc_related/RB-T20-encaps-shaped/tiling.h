/**
 * @file tiling.h
 * @brief RB-T20：Encaps 外形 + T19 Encrypt 全链 — (m,ek)→μ/r/K + c。
 *
 * Host Encaps 头：μ←m（T04）；coins=r←G(m‖H(ek)) 由 gen_data；K Host 对拍。
 * Launch1 prep（AIV）：coins→OFF_COINS；完整 ek→OFF_EK；ρ←ek 尾。
 * Launch2 compute（MIX）：ByteDecode₁₂(ek)→t̂ → CBD→y‖e1‖e2 → SampleNTT/NTT → Mul/INTT → pack。
 *
 * 相对 T19：输入语义升为 Encaps（m→μ + Host K）；设备 Encrypt 链不变。
 *
 * Flag（永禁 5/7）：
 ...
 *   AIV: Set(1)→Wait(3) → Decode₁₂ → CBD → Â+ŷ → Mul → Set(4)
 *        → Set(1)→Wait(3) → INTT+噪 → pack
 *
 * F203_AHAT16_BLOCK_DIM 须为 1（见 sample_ntt_device.hpp / CMake）。
 */
#ifndef RB_T20_ENCAPS_SHAPED_TILING_H
#define RB_T20_ENCAPS_SHAPED_TILING_H
 ...
 *   COINS → Host H2D（禁预喂最终 y/e/Â/ŷ/u/v/c/t̂）
...
constexpr size_t OFF_MU = OFF_T_HAT + kTHatBytes;
...
constexpr uint32_t MAGIC_OUT_OK = 0x543F0014u;         // T20
constexpr uint32_t MAGIC_PREP_MARK = 0x543F5054u;      // T20P
