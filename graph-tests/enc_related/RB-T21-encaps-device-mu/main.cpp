/**
 * @file main.cpp
 * @brief RB-T21 Host：Encaps 外形（转发 K + 预装原始 m）+ 双 launch；设备 μ←m。
 *
 * Encaps 外形：
 *   - Host **仅**把 m[32] 写入 OFF_M；**不**预喂最终 μ（相对 T20 下放到设备）
 *   - K_host ← input/K_host.bin（gen_data 已算 G(m‖H(ek))[:32]；设备 K 后刀）
 *   - coins = r 由 gen_data 写入，喂 prep（设备 G 头后刀）
 *
 * Encrypt：prep(AIV: coins+ek+ρ) → Synchronize → compute(MIX：μ←Decompress₁(m)+全链)。
 * 禁预填最终 t̂/y/e/Â/ŷ/u/v/c/μ。
 */
 ...
 * 背景：相对 T20，μ 改由设备嵌入；Host 只装 m，禁止写最终 μ。
 */
...
/** 写出 Encaps 产物：K_host（Host 转发）+ μ_dev（设备嵌入）。 */
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    INFO_LOG("Host: Encaps device-mu; Launch1 prep_custom (CPU AIV twin)");
    ...
    INFO_LOG("Host: Encaps device-mu; Launch1 prep_custom (SIM/NPU)");
