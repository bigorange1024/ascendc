...
# -*- coding: utf-8 -*-
"""
RB-T21 gen_data：Encaps 外形 + 设备 μ + Encrypt 全链 golden。

Encaps 头（Host / FIPS Alg.17 契约外形）：
  h ← SHA3-256(ek)
  (K ‖ r) ← SHA3-512(m ‖ h)
  coins := r                         # 喂 Encrypt
  μ 仅作 golden（设备 Decompress₁；禁写入 input 最终 μ）

Encrypt 链（同 T20 设备链 + 设备 μ）：
  ek[1568]=BE₁₂(t̂)[1536]‖ρ[32]；设备 μ+Decode₁₂+CBD+Â/ŷ→u,v→c
  禁写最终 t̂/y/e/Â/ŷ/u/v/c/μ 到 input
"""
...
def main() -> int:
    ...
    # —— Encaps 外形头（Host K/r；μ 仅 golden）——
    ...
    print(
        f"[gen_data] T21 Encaps device-mu: H/G→K|r; device μ←m; "
        f"ek=BE12||rho; Encrypt; c={len(golden_c)} K={len(K_bytes)}"
    )
    return 0
