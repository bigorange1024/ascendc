...
# -*- coding: utf-8 -*-
"""
RB-T21 verify：PASS_SYNC（主）/ PASS_IO（c + Host K + 设备 μ/u/v/Â/ŷ/t̂/yee/ρ）。

PASS_SYNC 硬槽：T20 槽 + MU_DONE；out 魔数 T21=0x543F0015。
...
def main() -> int:
    ...
    if io_ok:
        print("[SUCCESS] PASS_SYNC + PASS_IO — RB-T21 encaps device-mu (c + Host K + μ_dev)")
    else:
