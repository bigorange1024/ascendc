/**
 * @file tiling.h
 * @brief RB-T21：T20 Encaps + 设备 μ←Decompress₁(m) — (m,ek)→r/K + 设备μ + c。
 *
 * Host Encaps 头：coins=r←G(m‖H(ek)) 由 gen_data；K Host 对拍；**仅预装原始 m**。
 * Launch1 prep（AIV）：coins→OFF_COINS；完整 ek→OFF_EK；ρ←ek 尾。
 * Launch2 compute（MIX）：μ←Decompress₁(m) → Decode₁₂→CBD→Â/ŷ→Mul/INTT→pack。
 *
 * 相对 T20：μ 改由设备（T04 契约）写出 OFF_MU；Host **不**预喂最终 μ。
 *
 * Flag（永禁 5/7）：
 ...
 */
#ifndef RB_T21_ENCAPS_DEVICE_MU_TILING_H
#define RB_T21_ENCAPS_DEVICE_MU_TILING_H
...
constexpr uint32_t MAGIC_OUT_OK = 0x543F0015u;         // T21
constexpr uint32_t MAGIC_PREP_MARK = 0x543F5154u;      // T21P
