/**
 * @file main.cpp
 * @brief RB-T22 Host：Encaps（设备 G→K/r）+ 双 launch；设备 μ←m；对拍 c 与 K。
 *
 * Encaps：
 *   - Host **仅**预装 m[32]→OFF_M 与 ek；**不**预喂 coins/K/最终 μ
 *   - Launch1：设备 h←H(ek)；(K‖r)←G(m‖h)；coins=r；K 写出对拍
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    INFO_LOG("Host: Encaps device-G; Launch1 prep_custom (CPU AIV twin)");
    ...
    INFO_LOG("Host: Encaps device-G; Launch1 prep_custom (SIM/NPU)");
