...
# -*- coding: utf-8 -*-
"""
RB-T22 gen_data：Encaps + 设备 G/H + 设备 μ + Encrypt 全链 golden。

Encaps 头（设备 / FIPS Alg.17 契约；golden 仅 Host 复算）：
  h ← SHA3-256(ek)
  (K ‖ r) ← SHA3-512(m ‖ h)
  coins := r                         # 设备写出，喂 CBD
  禁写 coins/K/h/最终 μ 到 input（设备路径）
...
def main() -> int:
    ...
    # —— Encaps 头（仅 golden；设备 G 复算；禁写 coins/K 到 input）——
    ...
    print(
        f"[gen_data] T22 Encaps device-G: H/G→K|r on device; μ←m; "
