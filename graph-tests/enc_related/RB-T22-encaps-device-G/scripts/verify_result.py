...
# -*- coding: utf-8 -*-
"""
RB-T22 verify：PASS_SYNC（主）/ PASS_IO（c + 设备 K + coins/h/μ/u/v/Â/ŷ/t̂/yee/ρ）。

PASS_SYNC 硬槽：T21 槽 + G_DONE；out 魔数 T22=0x543F0016。
设备 K：output/K_dev.bin ↔ golden_K；并自洽重算 G(m‖H(ek))。
"""
...
def main() -> int:
    ...
    if io_ok:
        print("[SUCCESS] PASS_SYNC + PASS_IO — RB-T22 encaps device-G (c + K_dev)")
    else:
