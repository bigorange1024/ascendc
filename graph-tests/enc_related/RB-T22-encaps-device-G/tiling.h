/**
 * @file tiling.h
 * @brief RB-T22：T21 + 设备 (K‖r)←G(m‖H(ek)) — (m,ek)→设备 K/r + μ + Encrypt→c。
 *
 * 相对 T21：Host **不**预喂 coins/K；Launch1 prep 设备 SHA3：
 *   h←SHA3-256(ek)；(K‖r)←SHA3-512(m‖h)；coins=r 喂 CBD；K 写出对拍。
 * Launch2 同 T21：μ←Decompress₁(m) → Decode₁₂→CBD→Â/ŷ→Mul/INTT→pack。
 *
 * Flag（永禁 5/7）：
 ...
 *
 * F203_AHAT16_BLOCK_DIM 须为 1（见 sample_ntt_device.hpp / CMake）。
 */
#ifndef RB_T22_ENCAPS_DEVICE_G_TILING_H
#define RB_T22_ENCAPS_DEVICE_G_TILING_H
...
constexpr uint32_t MAGIC_OUT_OK = 0x543F0016u;         // T22
constexpr uint32_t MAGIC_PREP_MARK = 0x543F5155u;      // T22P
