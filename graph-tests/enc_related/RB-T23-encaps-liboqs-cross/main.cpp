/**
 * @file main.cpp
 * @brief RB-T23 Host：T22 设备 Encaps + liboqs 交叉（同 m/ek 对拍 c/K）。
 *
 * Encaps：
 *   - Host **仅**预装 m[32]→OFF_M 与合法 ek（liboqs KeyGen）；**不**预喂 coins/K/μ
 *   - Launch1：设备 h←H(ek)；(K‖r)←G(m‖h)；coins=r；K 写出
 *   - Launch2：μ←Decompress₁(m) + Encrypt 全链 → c
 *   - 验收：c/K ↔ liboqs Encaps（同输入）；缺库勿假绿
 *
 * 禁预填最终 t̂/y/e/Â/ŷ/u/v/c/μ/coins/K。
 */
...
#endif
extern "C" void prep_custom(GM_ADDR ekIn, GM_ADDR ws, TilingData tiling);
...
static void PrintTrace(const uint32_t *tr)
{
    ...
    INFO_LOG("  [%u] AIV0_G_DONE          = 0x%08X", SLOT_AIV0_G_DONE, tr[SLOT_AIV0_G_DONE]);
    ...
    const bool gDone = (tr[SLOT_AIV0_G_DONE] == MAGIC_AIV0_G_DONE);
    ...
    if (gDone) {
        INFO_LOG("causal: device G(m||H(ek)) wrote K||coins");
    }
    if (nttDone && muDone) {
...
/**
 * 装填 Host 侧 ws：原始 m→OFF_M + ζ/γ/mat；清零 coins/K/h/μ/t̂/Â/ŷ/YEE/u/v/c。
 * 背景：相对 T21，coins/K 改由设备 G 填写；Host 禁止预喂。
 */
static bool LoadWorkspaceHost(uint8_t *ws, const uint8_t *m)
{
    ...
    std::memset(ws + OFF_H, 0, kHashBytes);
...
/** 写出 Encaps 产物：设备 K/h/coins + μ_dev。 */
static bool WriteEncapsOutputs(const uint8_t *ws)
{
    using namespace tiling;
    if (!WriteFile("./output/K_dev.bin", ws + OFF_K, kSharedKeyBytes)) {
        return false;
    }
    if (!WriteFile("./output/h_dev.bin", ws + OFF_H, kHashBytes)) {
        return false;
    }
    if (!WriteFile("./output/coins_dev.bin", ws + OFF_COINS, kCoinsBytes)) {
        return false;
    }
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    ok = ReadFile("./input/m.bin", got, mIn, 32);
    if (!ok || got != 32) {
        return 4;
    }
    ...
    INFO_LOG("Host: Encaps liboqs-cross; Launch1 prep_custom (CPU AIV twin)");
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(prep_custom, blockDim, ekIn, ws, *tiling);
    ...
    uint8_t *ekHost = nullptr, *mHost = nullptr, *outHost = nullptr, *cHost = nullptr, *wsHost = nullptr;
    uint8_t *ekDev = nullptr, *outDev = nullptr, *cDev = nullptr, *wsDev = nullptr;
    ...
    ok = ReadFile("./input/m.bin", got, mHost, 32);
    if (!ok || got != 32) {
        return 4;
    }
    ...
    INFO_LOG("Host: Encaps liboqs-cross; Launch1 prep_custom (SIM/NPU)");
    ACLRT_LAUNCH_KERNEL(prep_custom)(blockDim, stream, ekDev, wsDev, tiling);
