#!/bin/bash
# RB-T23-encaps-liboqs-cross：T22 设备 Encaps + liboqs ML-KEM-1024 交叉对拍 c/K
# 缺 liboqs/仓内 ref → gen_data BLOCKED（勿假绿）；SIM 本战役 skip（NPU 优先）
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=600 / sim=900（标量 MultiplyNTTs+INTT）
...
_ORIG_ARGS=("$@")
# graph-tests/enc_related/<case> → 仓库根为 ../../..
...
echo "[SUCCESS] RB-T23-encaps-liboqs-cross (${RUN_MODE})"
