...
# -*- coding: utf-8 -*-
"""
RB-T23 gen_data：T22 设备 Encaps 路径 + liboqs ML-KEM-1024 权威交叉 golden。

权威（本刀硬门禁）：
  1) 用 scripts/liboqs_kem_ref（链 thirdparty/liboqs 或已静态编好的仓内 ref）KeyGen → 合法 ek
  2) 同 m/ek 调 liboqs Encaps → golden_c / golden_K
  缺库 → 写 output/BLOCKED 并 exit 2（勿假绿；禁止 python 冒充权威）

设备路径（同 T22；仅 Host 复算中间量便于诊断）：
...
def main() -> int:
    ...
    if ref is None:
        return _blocked(
            out,
            "缺 liboqs / scripts/liboqs_kem_ref；T23 权威交叉不可用。"
    ...
    # —— 权威：liboqs KeyGen + Encaps（同 m/ek）——
    src_kg = kem_keygen(FIXED_KEM_SEED, ek_path, dk_path)
    if src_kg != "liboqs":
        return _blocked(out, f"kem_keygen 回落为 {src_kg!r}；T23 禁止非 liboqs 权威")
    ...
    if src_enc != "liboqs":
        return _blocked(out, f"kem_encaps 回落为 {src_enc!r}；T23 禁止非 liboqs 权威")
    ...
    print(
        f"[gen_data] T23 Encaps×liboqs cross: via={src_enc}; "
        f"ek=liboqs KeyGen; m fixed; golden_c/K=liboqs Encaps; "
        f"c={len(c_liboqs.read_bytes())} K={len(k_liboqs.read_bytes())}"
    )
    return 0
