...
# -*- coding: utf-8 -*-
"""
RB-T23 verify：PASS_SYNC + PASS_CROSS（主：c/K ↔ liboqs）+ PASS_IO（中间量）。

硬门禁：
  - output/cross_backend.txt 必须为 liboqs（缺库/回落 → FAIL，勿假绿）
  - c.bin / K_dev.bin 与 golden_c / golden_K（liboqs Encaps 同 m/ek）逐字节一致
  - out 魔数 T23=0x543F0017；TRACE 含 G_DONE
"""
...
def main() -> int:
    ...
    if not backend_p.is_file() or backend_p.read_text(encoding="utf-8").strip() != "liboqs":
        print(
            "[FAIL] cross_backend 非 liboqs — T23 禁止无权威交叉的假绿",
            file=sys.stderr,
        )
        return 2
    ...
    # —— 主验收：设备 c/K ↔ liboqs ——
    cross_ok = True
    cross_ok = _cmp_bytes(OUT / "c.bin", OUT / "golden_c.bin", "c↔liboqs") and cross_ok
    cross_ok = _cmp_bytes(OUT / "K_dev.bin", OUT / "golden_K.bin", "K_dev↔liboqs") and cross_ok
    recomputed = _recompute_head()
    if recomputed is None:
        print("[PASS_CROSS=0] cannot recompute G/H from input m/ek")
        cross_ok = False
    else:
        h_ref, k_ref, coins_ref = recomputed
        if (OUT / "golden_K.bin").read_bytes() != k_ref:
            print("[PASS_CROSS=0] liboqs golden_K != SHA3-512(m||H(ek))[:32]")
            cross_ok = False
        else:
            print("[PASS_CROSS] liboqs K ≡ G(m||H(ek))[:32]")
        if (OUT / "K_dev.bin").is_file() and (OUT / "K_dev.bin").read_bytes() != k_ref:
            print("[PASS_CROSS=0] K_dev != recomputed G[:32]")
            cross_ok = False
        if (OUT / "h_dev.bin").is_file():
            if (OUT / "h_dev.bin").read_bytes() != h_ref:
                print("[PASS_CROSS=0] h_dev != H(ek)")
                cross_ok = False
            else:
                print("[PASS_IO] h_dev match H(ek)")
        if (OUT / "coins_dev.bin").is_file():
            if (OUT / "coins_dev.bin").read_bytes() != coins_ref:
                print("[PASS_CROSS=0] coins_dev != G[32:]")
                cross_ok = False
            else:
                print("[PASS_IO] coins_dev match G[32:]")

    if not cross_ok:
        print("[FAIL] PASS_CROSS=0 — 设备 Encaps 与 liboqs 交叉未过", file=sys.stderr)
        return 1
    print("[PASS_CROSS] device Encaps c/K ≡ liboqs ML-KEM-1024 (same m/ek)")
    ...
    if io_ok:
        print("[SUCCESS] PASS_SYNC + PASS_CROSS + PASS_IO — RB-T23 encaps×liboqs")
        return 0
    print("[FAIL] PASS_CROSS ok but PASS_IO=0 — 中间量对拍失败", file=sys.stderr)
    return 1
