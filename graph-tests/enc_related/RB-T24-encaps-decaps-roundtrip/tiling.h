/**
 * @file tiling.h
 * @brief RB-T24：T23 设备 Encaps + liboqs Decaps(sk,c)→K' 与 K 往返对拍。
 *
 * 设备路径同 T23：Host 仅预装 m/ek；Launch1 设备 (K‖r)←G(m‖H(ek))；
 * Launch2 μ←Decompress₁(m)→Decode₁₂→CBD→Â/ŷ→Mul/INTT→pack→c。
 * 验收：liboqs Decaps(sk, device_c)→K' ≡ K_dev；缺库则 BLOCKED。
 *
 * Flag（永禁 5/7）：1/3 复用 + 4=GATE。F203_AHAT16_BLOCK_DIM=1。
 */
#ifndef RB_T24_ENCAPS_DECAPS_ROUNDTRIP_TILING_H
#define RB_T24_ENCAPS_DECAPS_ROUNDTRIP_TILING_H
...
constexpr uint32_t MAGIC_OUT_OK = 0x543F0018u;         // T24
constexpr uint32_t MAGIC_PREP_MARK = 0x543F5157u;      // T24P
