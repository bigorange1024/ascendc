/**
 * @file tiling.h
 * @brief RB-T25：Alg.15 Decrypt 多 launch — prep AIV → NTT MIX → INTT+extract MIX。
 *
 * Flag（各 MIX launch 独立复用）：
 *   1 = AIV→AIC 就绪
 *   3 = AIC→AIV Cube 完成
 * 可选 4 = GATE（本刀不跨段同核，故不用）
 ...
 */
#ifndef RB_T25_DECRYPT_DEVICE_TILING_H
#define RB_T25_DECRYPT_DEVICE_TILING_H
...
// kFlagGate=4 本刀不用（NTT/INTT 已拆 launch）
...
constexpr uint32_t MAGIC_OUT_OK = 0x543F0019u; // T25
