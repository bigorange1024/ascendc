...
/**
 * AIV0：INTT(ŵ)→w；逐系数 Compress₁(v−w) → ByteEncode₁ → OFF_M / mOut。
 * @param mOut 独立 32B 落盘缓冲（与 OFF_M 同内容）
 */
__aicore__ inline void ComputeInttAndExtract(GM_ADDR ws, GM_ADDR mOut)
{
    ...
    // m[i] bit ← Compress₁((v−w) mod q)；字节内 LSB-first
    for (int32_t bi = 0; bi < static_cast<int32_t>(kMBytes); ++bi) {
        ...
        gmMWs.SetValue(static_cast<uint32_t>(bi), byte);
        gmMOut.SetValue(static_cast<uint32_t>(bi), byte);
    }
    AscendC::PipeBarrier<PIPE_ALL>();
