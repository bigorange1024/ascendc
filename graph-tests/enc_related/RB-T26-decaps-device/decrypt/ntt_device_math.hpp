 ...
 * @brief RB-T25 Launch2：û←NTT(u)；ŵ←⟨ŝ,û⟩（MultiplyNTTs 求和）。
 *
 * 契约：Alg.9 ForwardNTT + Alg.11 MultiplyNTTs；poly-batch 整 poly；禁 Gather。
 * 自研标量 UB 路径；数学对齐活跃 T10/T12 契约，非抄 decrypt/encrypt 设备源码。
 */
#ifndef RB_T25_NTT_DEVICE_MATH_HPP
#define RB_T25_NTT_DEVICE_MATH_HPP
                                  ...
                                  const AscendC::LocalTensor<int32_t> &zetas)
{
    ...
    for (int32_t length = 128; length >= 2; length /= 2) {
        for (int32_t start = 0; start < dec_tiling::kPolyN; start += 2 * length) {
...
{
    for (int32_t i = 0; i < dec_tiling::kPolyN / 2; ++i) {
...
{
    AscendC::DataCopy(dst, srcGm[static_cast<uint32_t>(polyIdx * dec_tiling::kPolyN)],
                      static_cast<uint32_t>(dec_tiling::kPolyN));
...
{
    AscendC::DataCopy(dstGm[static_cast<uint32_t>(polyIdx * dec_tiling::kPolyN)], src,
                      static_cast<uint32_t>(dec_tiling::kPolyN));
    AscendC::PipeBarrier<PIPE_ALL>();
}
 ...
 * 背景：Alg.15 行 NTT + 内积；与 Encrypt ⟨t̂,ŷ⟩ 同构，左向量为 ŝ。
 */
