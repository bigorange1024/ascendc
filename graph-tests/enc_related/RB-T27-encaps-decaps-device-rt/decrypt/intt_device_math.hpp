 ...
 * @brief RB-T27 Decrypt Launch3：w←INTT(ŵ)；m←BE₁(Compress₁(v−w))（X12）。
 *
 * NPU：m 经 LocalTensor + DataCopy 写出；禁 GlobalTensor::SetValue 写 m。
 * 背景：T26 复制树对 m' 用 SetValue → CPU 假绿 / NPU K_decaps 错。
 */
#ifndef RB_T27_DEC_INTT_DEVICE_MATH_HPP
#define RB_T27_DEC_INTT_DEVICE_MATH_HPP
                                  ...
                                  const AscendC::LocalTensor<int32_t> &zetas)
{
    ...
    for (int32_t i = 0; i < dec_tiling::kPolyN; ++i) {
        ...
        f.SetValue(static_cast<uint32_t>(i), ModQIntt(x * dec_tiling::kInttScale));
    }
}
...
{
    return ((x * 2 + dec_tiling::kQ / 2) / dec_tiling::kQ) & 1;
}
...
__aicore__ inline void ComputeInttAndExtract(GM_ADDR ws, GM_ADDR mOut)
{
    ...
    // m 先在 UB 组包，再 DataCopy 落 GM（X12：禁 uint8 GlobalTensor::SetValue）
