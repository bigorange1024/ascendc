 ...
 * @brief RB-T27 Decrypt Launch1：dk_pke+c → ŝ/u/v（AIV；X12）。
 *
 * 契约：
 *   - ŝ ← ByteDecode₁₂(dkIn) 直读 H2D（禁 ws SetValue 镜像）
 *   - u/v ← Decompress(ByteDecode(c))：c 先 DataCopy 入 UB 再解
 *   - ŝ/u/v 写出用 DataCopy
 *
 * 背景（NPU FAIL）：T26 复制树仍用 GlobalTensor::SetValue 镜像 dk/c；
 * 实机标量写 GM 不可靠 → Decrypt 读空/脏 → K_decaps≠K_encaps。对齐 T25 X12。
 */
#ifndef RB_T27_DEC_PREP_DEVICE_MATH_HPP
#define RB_T27_DEC_PREP_DEVICE_MATH_HPP
