/**
 * @file tiling.h
 * @brief RB-T27 Encaps/Reenc workspace：设备 G + Encrypt 全链。
 *
 * Host 仅预装 m/ek（及 ζ/γ/mat）；Launch1 设备 (K‖r)←G(m‖H(ek))；
 * Launch2 μ←Decompress₁→Decode₁₂→CBD→Â/ŷ→Mul/INTT→pack→c（DataCopy 写出）。
 * Flag：1/3 复用 + 4=GATE；永禁 5/7；BLOCK_DIM=1。
 */
#ifndef RB_T27_ENC_TILING_H
#define RB_T27_ENC_TILING_H
...
constexpr uint32_t MAGIC_OUT_OK = 0x543F001Bu;         // T27 Encaps↔Decaps RT
constexpr uint32_t MAGIC_PREP_MARK = 0x543F515Bu;      // T27P
