#!/bin/bash
# RB-T27-encaps-decaps-device-rt：设备 Encaps → 设备 Decaps；K'≡K；可选 liboqs 交叉
# 缺 liboqs → gen_data BLOCKED；SIM 本战役 skip（NPU 优先）
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 **180s**（战役口径：3 分钟出结果；超时=有问题）
...
echo "[SUCCESS] RB-T27-encaps-decaps-device-rt (${RUN_MODE})"
