 ...
 * @brief RB-T28 Launch1：dk_pke+c → ŝ/u/v（AIV；NPU 安全路径）。
 *
 ...
 *
 * 背景：T26 误用 GlobalTensor::SetValue 镜像 dk/c 再标量读 —— 实机 GM 标量写不可靠
 * （CPU 假绿、NPU m' 全错）。结论：回迁 T25 NPU 修法——直读 dkIn/cIn + DataCopy 写出。
 * 未采用：继续镜像到 OFF_DK/OFF_C。
 */
#ifndef RB_T28_PREP_DEVICE_MATH_HPP
#define RB_T28_PREP_DEVICE_MATH_HPP
