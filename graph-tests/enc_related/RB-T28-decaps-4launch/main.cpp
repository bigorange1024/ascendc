/**
 * @file main.cpp
 * @brief RB-T28 Host：Alg.21 Decaps 4-launch（Decrypt 融合 + Encaps2）。
 *
 * 数据流：
 *   dk‖c → L1 dec_prep → L2 dec_ntt_intt（融合 MIX）→ m'
 *        → L3 enc_prep → L4 enc_compute → c' / K'
...
#include "acl/acl.h"
#include "aclrtlaunch_dec_ntt_intt_custom.h"
...
extern "C" void dec_ntt_intt_custom(GM_ADDR out, GM_ADDR mOut, GM_ADDR ws, DecTilingData tiling);
...
constexpr uint32_t kMagicOutT28 = 0x543F001Cu;
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    INFO_LOG("Host: mid1 → L2 dec_ntt_intt fused (MIX)");
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ICPU_RUN_KF(dec_ntt_intt_custom, blockDim, out, mPrime, decWs, *decTiling);
    ...
    INFO_LOG("Host: L3 enc_prep G(m'||H(ek)) (AIV)");
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ...
    INFO_LOG("Host: mid2 → L4 enc_compute Encrypt (MIX)");
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ...
    // 写 T28 魔数到 out
    *reinterpret_cast<uint32_t *>(out) = kMagicOutT28;
    ...
    INFO_LOG("Host: L2 dec_ntt_intt fused");
    ACLRT_LAUNCH_KERNEL(dec_ntt_intt_custom)(blockDim, stream, outDev, mDev, decWsDev, decTiling);
    CHECK_ACL(aclrtSynchronizeStream(stream));
    ...
    *reinterpret_cast<uint32_t *>(outHost) = kMagicOutT28;
...
#endif
    INFO_LOG("RB-T28 decaps 4-launch host done");
    return 0;
}
