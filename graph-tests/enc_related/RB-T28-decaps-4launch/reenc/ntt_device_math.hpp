 ...
 * @brief RB-T15 设备侧：FIPS 203 Alg.9 正向 NTT（poly-batch，整 poly）。
 *
 * 流水线位置：MIX AIV0 在 SampleNTT 写完 Â 之后，对 y[4×256] 逐 poly 做 NTT → ŷ。
 * 数学契约：Alg.9 + kMlkemZetas；与 Host golden `mlkem_ntt` 对拍。
 *
 * 契约约束（KB §A2）：
 ...
 */
#ifndef RB_T15_NTT_DEVICE_MATH_HPP
#define RB_T15_NTT_DEVICE_MATH_HPP

#include "kernel_operator.h"
#include "reenc/tiling.h"
#include <cstdint>

namespace rb_t15_ntt {
                                  ...
                                  const AscendC::LocalTensor<int32_t> &zetas)
{
    ...
    for (int32_t length = 128; length >= 2; length /= 2) {
        for (int32_t start = 0; start < enc_tiling::kPolyN; start += 2 * length) {
...
{
    AscendC::DataCopy(dst, srcGm[static_cast<uint32_t>(polyIdx * enc_tiling::kPolyN)],
                      static_cast<uint32_t>(enc_tiling::kPolyN));
...
{
    AscendC::DataCopy(dstGm[static_cast<uint32_t>(polyIdx * enc_tiling::kPolyN)], src,
                      static_cast<uint32_t>(enc_tiling::kPolyN));
    AscendC::PipeBarrier<PIPE_ALL>();
}

/**
 * AIV0：对 y[kKem]（ws+OFF_Y = 设备 CBD 写出的 y‖e1‖e2 前 4 poly）正向 NTT → OFF_Y_HAT。
 *
 * @param ws 共享 workspace（含 Y / ZETAS / Y_HAT）
 *
 * 说明：本函数内自建 TPipe（SampleNTT 管已结束）；禁 limbsplit / Gather。
 */
__aicore__ inline void ComputeYHatNtt(GM_ADDR ws)
{
