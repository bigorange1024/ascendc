/**
 * @file tiling.h
 * @brief RB-T26-reenc：T22 设备 Encaps + liboqs ML-KEM-1024 交叉对拍 c/K。
 *
 * 设备路径同 T22：Host 仅预装 m/ek；Launch1 设备 (K‖r)←G(m‖H(ek))；
 * Launch2 μ←Decompress₁(m)→Decode₁₂→CBD→Â/ŷ→Mul/INTT→pack→c。
 * 本刀 golden_c/K 以 liboqs（或仓内 liboqs_kem_ref）为权威；缺库则 BLOCKED。
 *
 * Flag（永禁 5/7）：1/3 复用 + 4=GATE。F203_AHAT16_BLOCK_DIM=1。
 */
#ifndef RB_T26_ENC_TILING_H
#define RB_T26_ENC_TILING_H

#include <cstddef>
#include <cstdint>

/** Host→Device 运行时参数（固定 64B 槽，当前仅占位）。 */
struct EncTilingData {
    int32_t reserved0;
    int32_t reserved1;
};

namespace enc_tiling {
...
constexpr size_t kMsgBytes = 32; /**< Encaps 消息 m ∈ B^32 */
constexpr size_t kHashBytes = 32;      /**< H(ek)=SHA3-256 */
constexpr size_t kSharedKeyBytes = 32; /**< K = G[:32] */
constexpr size_t kGOutBytes = 64;      /**< G = SHA3-512 → K‖r */
 ...
 *   COINS / K / H → **设备** G 写出（禁 Host 预喂 coins/K）
...
/** 设备 Encaps 头产物：紧挨密文槽，避免挪动既有 LUT/μ 偏移。 */
...
constexpr uint32_t SLOT_AIV0_G_DONE = 27; /**< prep：设备 G/H 完成 */
...
constexpr uint32_t MAGIC_AIV0_G_DONE = 0x47303132u;   // "G012" 设备 G/H
constexpr uint32_t MAGIC_OUT_OK = 0x543F001Au;         // T26 Decaps
constexpr uint32_t MAGIC_PREP_MARK = 0x543F515Au;      // T26P
