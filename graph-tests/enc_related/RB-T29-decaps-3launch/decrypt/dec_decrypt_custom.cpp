/**
 * @file dec_decrypt_custom.cpp
 * @brief RB-T29 Launch1：单 MIX 串行三段(prep+NTT+INTT) —— NTT+su_dot 后 INTT+extract → m'。
 *
 * 背景：T26 将 NTT/INTT 拆成两 launch（反卡死 §5.1）；D08 已证明同核串行复用
 * flag {1,3} 各跑完整一轮可行。本刀在 **统一 dec workspace** 上融合两段，
 * Host Decaps 由 4 launch 压到 3（本核 + enc_prep + enc_compute）。
 *
 ...
 * @param tiling 占位
 */
extern "C" __global__ __aicore__ void dec_decrypt_custom(GM_ADDR out, GM_ADDR mOut, GM_ADDR dkIn,
                                                         GM_ADDR cIn, GM_ADDR ws, DecTilingData tiling)
{
    (void)tiling;
    ...
    // Segment0: AIV0 prep; AIC/AIV1 idle; SyncAll outside CrossCore Wait ring
    if (!isAic && subIdx == 0) {
