 ...
 * @brief RB-T28：w←INTT(ŵ)；m←ByteEncode₁(Compress₁(v−w))（NPU 安全写出）。
 *
 * 契约：Alg.10 InverseNTT；Compress₁=round(x·2/q) mod 2；BE₁ LSB-first 打包 32B。
 *
 * 背景：T26 用 GlobalTensor::SetValue 写 m —— 实机不可靠。结论：回迁 T25 修法，
 * m 经 LocalTensor + DataCopy 写出；禁 uint8 GM SetValue。
 */
#ifndef RB_T28_INTT_DEVICE_MATH_HPP
#define RB_T28_INTT_DEVICE_MATH_HPP
