/**
 * @file main.cpp
 * @brief RB-T29 Host：Alg.21 Decaps 3-launch（Decrypt 融合 + Encaps2）。
 *
 * 数据流：
 *   dk‖c → L1 dec_decrypt（prep+NTT+INTT 融合 MIX）→ m'
 *        → L2 enc_prep → L3 enc_compute → c' / K'
 *        → Host：c'≡c ? K' : J(z‖c) → K[32]
 *
 * 硬锁：blockDim=1；Decrypt 融合核内复用 flag 1/3；Reenc flag 1/3+4；永禁 5/7 / SoftSync。
...
#include "acl/acl.h"
#include "aclrtlaunch_dec_decrypt_custom.h"
...
#endif
extern "C" void dec_decrypt_custom(GM_ADDR out, GM_ADDR mOut, GM_ADDR dkIn, GM_ADDR cIn, GM_ADDR ws,
...
constexpr uint32_t kMagicOutT29 = 0x543F001Du;
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    if (!LoadDecWorkspace(decWs)) {
        ...
        INFO_LOG("Host: L1 dec_decrypt fused prep+NTT+INTT (MIX)");
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ICPU_RUN_KF(dec_decrypt_custom, blockDim, out, mPrime, const_cast<uint8_t *>(dkPke), cIn, decWs,
                *decTiling);
    ...
    INFO_LOG("Host: L2 enc_prep G(m'||H(ek)) (AIV)");
    ...
    INFO_LOG("Host: mid → L3 enc_compute Encrypt (MIX)");
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ...
    // 写 T28 魔数到 out
    *reinterpret_cast<uint32_t *>(out) = kMagicOutT29;
    ...
    INFO_LOG("Host: L1 dec_decrypt fused");
    ACLRT_LAUNCH_KERNEL(dec_decrypt_custom)(blockDim, stream, outDev, mDev, dkDev, cDev, decWsDev, decTiling);
    ...
    *reinterpret_cast<uint32_t *>(outHost) = kMagicOutT29;
...
#endif
    INFO_LOG("RB-T29 decaps 3-launch host done");
    return 0;
}
