 ...
 * @brief RB-T23 Launch2：握手 + μ←Decompress₁(m) + Decode₁₂→CBD(coins←设备G)→Â/ŷ→pack→c。
 *
...
#include "reenc/cbd_device.hpp"
#include "reenc/decode12_device.hpp"
#include "kernel_operator.h"
#include "reenc/light_cube.hpp"
#include "reenc/mu_embed_device.hpp"
#include "reenc/ntt_device_math.hpp"
#include "reenc/sample_ntt_device.hpp"
#include "reenc/tiling.h"
#include "reenc/uv_device_math.hpp"

namespace {

constexpr int32_t kDu = 11;
constexpr int32_t kDv = 5;
constexpr int32_t kC1PolyBytes = (enc_tiling::kPolyN * kDu) / 8; // 352
constexpr int32_t kC2PolyBytes = (enc_tiling::kPolyN * kDv) / 8; // 160
constexpr int32_t kC1Bytes = enc_tiling::kK * kC1PolyBytes;      // 1408
...
__aicore__ inline int32_t Compress5(int32_t u)
{
    ...
    if (x >= static_cast<uint32_t>(enc_tiling::kQ)) {
        x = static_cast<uint32_t>(enc_tiling::kQ) - 1u;
    }
 ...
 */
extern "C" __global__ __aicore__ void enc_compute_custom(GM_ADDR out, GM_ADDR cOut, GM_ADDR ws,
                                                     EncTilingData tiling)
{
    (void)tiling;
