/**
 * @file light_cube.hpp
 * @brief RB-T15 极轻 Cube：单趟 16×32×32 int8→int32 MMAD（无算法正确性要求）。
 *
 * 作用：NTT 握手段与 INTT 握手段各调用一次，证明两段均为有界真计算。
 * 输入/输出：GM 上 A[m,k] int8、B[k,n] int8 → C[m,n] int32（ND）。
 * 前置：仅在 AIC 路径调用；m=16、k=n=32（enc_tiling::kKk）。
 *
 * API 依据：查阅索引已有 DataCopy(Nd2Nz)/LoadData/Mmad/Fixpipe；本文件不引入新 API。
 */
#ifndef RB_T15_LIGHT_CUBE_HPP
#define RB_T15_LIGHT_CUBE_HPP
...
/**
 * 极轻 Cube 封装：CopyIn(Nd2Nz) → SplitA/B → Mmad → Fixpipe。
 * 与 u,v golden 无关；仅要求 SIM/CPU 不挂死。
 */
class LightCube {
public:
