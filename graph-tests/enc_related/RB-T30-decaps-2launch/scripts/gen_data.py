...
# -*- coding: utf-8 -*-
"""
RB-T30 gen_data：liboqs KeyGen+Encaps → dk/c/ek；golden_K ← liboqs Decaps。

契约：
  - 权威：scripts/liboqs_kem_ref（缺库 → BLOCKED）
  - Host 落盘 dk_kem[3168] + c[1568] + ζ/γ/mat；**不**预喂 K / m'
"""
...
for _cand in (
    _SCRIPT,
    _REPO / "graph-tests" / "enc_related" / "RB-T29-decaps-3launch" / "scripts",
...
def main() -> int:
    ...
    if liboqs_ref() is None:
        return _blocked(
            out,
            "缺 liboqs / scripts/liboqs_kem_ref；T30 权威 Decaps 不可用。"
    ...
    if src_kg != "liboqs":
        return _blocked(out, f"kem_keygen 回落为 {src_kg!r}；T30 禁止非 liboqs 权威")
    ...
    (inp / "ek.bin").write_bytes(ek)  # 诊断/对照；Host 从 dk 切片
    ...
    cube_rng = np.random.default_rng(20260910)
    ...
    print(
        f"[gen_data] dk={DK_KEM} c={C_LEN} ek={EK_LEN} golden_K via liboqs Decaps OK "
