#!/bin/bash
# RB-K01-kg-prep：KeyGen L1 prep — seed_d → Â + ŝ/ê
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=300 / sim=900（Â 16×SHAKE 偏长）
# KGR-P01 门禁：本刀 subagent 仅 CPU；禁止 -r npu
CURRENT_DIR=$(
...
echo "[SUCCESS] RB-K01-kg-prep (${RUN_MODE})"
