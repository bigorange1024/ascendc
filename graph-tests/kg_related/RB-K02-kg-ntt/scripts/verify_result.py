...
# -*- coding: utf-8 -*-
"""
RB-K02 verify：PASS_SYNC（握手+Cube）+ PASS_IO（NTT(ŝ)/NTT(ê) 对拍）。

硬条件：
  - out 魔数 K02:
  - TRACE Host PRE/POST；AIV 成对槽 GATE/NTT（勿硬绑 AIV0）
  - mat_c 非全 0
  - s_ntt / e_ntt 与 golden 逐元素一致（设备真算）
...
def main() -> int:
    ...
    pass_s = _cmp_bin(OUT / "s_ntt.bin", OUT / "golden_s_ntt.bin", "s_ntt")
    pass_e = _cmp_bin(OUT / "e_ntt.bin", OUT / "golden_e_ntt.bin", "e_ntt")
    pass_io = pass_s and pass_e
    ...
    if pass_io:
        print("[PASS_IO] NTT(s)+NTT(e) match golden (device MIX)")
    else:
        print("[FAIL_IO] NTT(s)/NTT(e) mismatch", file=sys.stderr)

    if pass_sync and pass_io:
        print("[SUCCESS] RB-K02 PASS_SYNC + PASS_IO (max=0)")
        return 0
    return 1
