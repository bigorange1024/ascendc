 ...
 * @brief RB-K03：MIX 设备 Â∘ŝ̂+ê̂ + ByteEncode₁₂ → ek_pke / dk_pke。
 *
 * KeyGen L2b 外形（KB §B2）：
 *   AIC：Wait(4)→Wait(1)→Cube→Set(3)
 *   AIV：Set(4)→Set(1)→Wait(3) → 点积+BE（仅 AIV0）
 *
 * 背景：K01 写出 Â/ρ；K02 写出 NTT(ŝ/ê)；本刀独立 gen_data 亦可造合法输入。
 * 结论：flag ∈ {1,3,4}；永禁 5/7 / SoftSync；主验收 ek/dk 对拍。
...
#include "dot_encode_device_math.hpp"
#include "tiling.h"
 ...
 * @param ekOut [out] 设备算出的 ek_pke[1568]；Host 禁预填最终值
                                                           ...
                                                           TilingData tiling)
{
    ...
    if (isAic) {
        ...
        if (subIdx == 0) {
            ...
            // out 魔数：标量指针写（非业务 ek/dk；业务已走 DataCopy）
            AscendC::PipeBarrier<PIPE_ALL>();
