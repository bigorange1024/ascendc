#!/bin/bash
# RB-K03-kg-dot-encode：KeyGen L2b 设备 MIX Â∘ŝ+ê + ByteEncode₁₂ → ek/dk
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=180 / sim=600（标量 4×4 MultiplyNTTs+BE）
...
_ORIG_ARGS=("$@")
# graph-tests/kg_related/<case> → 仓库根为 ../../..
...
echo "[SUCCESS] RB-K03-kg-dot-encode (${RUN_MODE})"
