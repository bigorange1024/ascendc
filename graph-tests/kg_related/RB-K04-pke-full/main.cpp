/**
 * @file main.cpp
 * @brief RB-K04 Host：三 launch PKE KeyGen — L1 prep → L2a NTT → L2b dot+encode → ek/dk。
 *
 ...
 *   D2H ek_pke[1568] / dk_pke[1536]
 *
 * 背景：禁融单核；禁抄 examples/pass-fix/frozen KeyGen；可链接 K01–K03 契约。
 * 结论：单 session 三核；每次 launch 后必 Host mid-sync；ρ/ζ/γ/mat 由 Host 预喂 L2 ws。
...
// 仅 include k02 tiling（k03 同名 namespace/TilingData 会撞；L2b 偏移用字面量对齐 k03_inc/tiling.h）
...
/**
 * 主流程：读 seed_d → 三 launch → 落盘 ek/dk + 两段 TRACE + 诊断中间量。
 */
int32_t main(int32_t argc, char *argv[])
{
    ...
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ICPU_RUN_KF(kg_dot_encode_custom, blockDim, outL2b, ekBuf, dkBuf, wsL2b, *mixTiling);
    {
        auto *tr = reinterpret_cast<uint32_t *>(wsL2b + kK03OffTrace);
        tr[11] = 0x484F5355u; // MAGIC_HOST_POST_SYNC
        PrintTraceL2b(tr);
    }
    INFO_LOG("Host: sync after Launch2b; D2H ek/dk");
    ...
    uint8_t *outL2bHost = nullptr, *ekHost = nullptr, *dkHost = nullptr, *wsL2bHost = nullptr;
    uint8_t *outL2bDev = nullptr, *ekDev = nullptr, *dkDev = nullptr, *wsL2bDev = nullptr;
    ...
    (blockDim, stream, outL2bDev, ekDev, dkDev, wsL2bDev, mixTiling);
    CHECK_ACL(aclrtSynchronizeStream(stream));
