#!/bin/bash
# RB-K04-pke-full：PKE KeyGen 全链三 launch：prep → NTT → dot+encode → ek/dk
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=300 / sim=900（三核串行；prep SHAKE 偏长）
# KGR-P04 门禁：本刀 subagent 仅 CPU；禁止 -r npu；npu: wait_npu
CURRENT_DIR=$(
...
echo "[SUCCESS] RB-K04-pke-full (${RUN_MODE})"
