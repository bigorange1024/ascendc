 ...
 * input 下 ek_pke.bin / dk_pke.bin / seed_d.bin 与写出 output 下 h.bin / z.bin / dk_kem.bin；
 * 不含 H/z/拼接算法逻辑，也不参与设备端编译；与 golden 的关系是纯粹 I/O 通道。
 */
#ifndef DATA_UTILS_H
#define DATA_UTILS_H
#include <iostream>
 ...
 * @return read result
 *
 * 中文说明：从 filePath 读取整个文件内容到 buffer（如 input/m.bin），
 ...
 * @return write result
 *
 * 中文说明：把 buffer 中的 size 字节原样写入 filePath（如 output/mu.bin），
...
// （打印 comp/poly 系数），其余分支为工具头通用能力，非本探针必需路径。
