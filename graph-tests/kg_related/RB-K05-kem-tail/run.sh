#!/bin/bash
# RB-K05-kem-tail：KEM KeyGen L3 — H(ek)+z → dk_kem（AIV-only）
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=120 / sim=300（SHA3-256(ek=1568)）
# KGR-K01 门禁：本刀 subagent 仅 CPU；禁止 -r npu（npu: wait_npu）
CURRENT_DIR=$(
...
echo "[SUCCESS] RB-K05-kem-tail (${RUN_MODE})"
