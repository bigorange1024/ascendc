 ...
 * @brief RB-K02 设备侧：FIPS 203 Alg.9 正向 NTT(ŝ) 与 NTT(ê)（poly-batch，整 poly）。
 *
 * 流水线位置：MIX AIV0 在握手 Wait(3) 后，对 ŝ[4×256]、ê[4×256] 逐 poly 做 NTT。
 * 数学契约：Alg.9 + kMlkemZetas（Appendix A）；与 Host golden `mlkem_ntt` 对拍。
 *
 * 实现形态：AIV 标量 + LocalTensor（UB）；非向量化/非 Cube Stage1–3 性能路径。
 * 契约约束（Tag5T / MLKEM-NTT notes）：
 *   - 每个 poly 完整 256 系数同核处理（禁 hi/lo limbsplit）
 *   - 本路径无 Gather；若后刀改走 S1–S3 仍禁 Gather（Gather 禁令仅 NTT S1–S3）
 * 写出：UB+DataCopy（X12）；禁 GlobalTensor::SetValue 写业务 GM。
 *
 * **禁止** 抄 KeyGen / Encrypt / Decrypt 整核或 frozen 设备源码。
 */
#ifndef RB_K02_NTT_DEVICE_MATH_HPP
#define RB_K02_NTT_DEVICE_MATH_HPP
...
/**
 * AIV0：对 ŝ[kKem]、ê[kKem] 逐 poly 正向 NTT，写出 ws 与独立 out。
 *
 * poly-batch：循环按 poly 索引；每次握完整 256 系数（含概念上的 hi+lo）；
 * 不按 limb 跨核切分；不调用 Gather。
 *
 * @param ws      共享 workspace（含 S / E / ZETAS / S_NTT / E_NTT）
 * @param sNttOut 独立 NTT(ŝ) 落盘缓冲（int32[4×256]）
 * @param eNttOut 独立 NTT(ê) 落盘缓冲（int32[4×256]）
 */
__aicore__ inline void ComputeSeNtt(GM_ADDR ws, GM_ADDR sNttOut, GM_ADDR eNttOut)
{
    ...
    AscendC::GlobalTensor<int32_t> gmSNttWs;
    AscendC::GlobalTensor<int32_t> gmENttWs;
    AscendC::GlobalTensor<int32_t> gmSNttOut;
    AscendC::GlobalTensor<int32_t> gmENttOut;
    gmS.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(ws + OFF_S));
    gmE.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(ws + OFF_E));
    gmZetas.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(ws + OFF_ZETAS));
    gmSNttWs.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(ws + OFF_S_NTT));
    gmENttWs.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(ws + OFF_E_NTT));
    gmSNttOut.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(sNttOut));
    gmENttOut.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(eNttOut));

    AscendC::TPipe pipe;
    // VECIN：UB 驻留（标量 Get/Set）；对齐 T12/D02 契约，勿用 VECCALC
    ...
    // ---------- ŝ[4]：逐 poly 完整 NTT（禁 limbsplit）----------
    for (int32_t p = 0; p < kKem; ++p) {
        CopyPolyIn(poly, gmS, p);
        ForwardNTT(poly, zetas);
        AscendC::PipeBarrier<PIPE_ALL>();
        CopyPolyOut(gmSNttWs, p, poly);
        CopyPolyOut(gmSNttOut, p, poly);
    }

    // ---------- ê[4]：同上 ----------
    for (int32_t p = 0; p < kKem; ++p) {
        CopyPolyIn(poly, gmE, p);
        ForwardNTT(poly, zetas);
        AscendC::PipeBarrier<PIPE_ALL>();
        CopyPolyOut(gmENttWs, p, poly);
        CopyPolyOut(gmENttOut, p, poly);
    }
