#!/bin/bash
# RB-K06-kem-full：KEM KeyGen 全链四 launch：prep → NTT → dot+encode → kem_tail → ek/dk_kem
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=300 / sim=900（四核串行；prep SHAKE 偏长）
# KGR-K02 门禁：本刀 subagent 仅 CPU；禁止 -r npu；npu: wait_npu
CURRENT_DIR=$(
...
echo "[SUCCESS] RB-K06-kem-full (${RUN_MODE})"
