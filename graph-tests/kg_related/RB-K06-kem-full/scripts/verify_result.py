...
# -*- coding: utf-8 -*-
"""
RB-K06 verify：四 launch 全链 — ek/dk_kem ≡ liboqs_kem_ref；L2a/L2b TRACE mid-sync 证据。

硬条件：
  - ek / dk_kem 与 golden（liboqs_kem_ref）逐字节一致
...
def main() -> int:
    ...
    pass_ek = _cmp_bytes(OUT / "ek.bin", OUT / "golden_ek.bin", "ek")
    pass_dk = _cmp_bytes(OUT / "dk_kem.bin", OUT / "golden_dk_kem.bin", "dk_kem")
    pass_io = pass_ek and pass_dk
    if pass_io:
        print(f"[OK] ek/dk_kem max=0 oracle={backend}")
    ...
    if pass_sync:
        print("[PASS_SYNC] 四 launch：L2a/L2b GATE+handshake + Cube ok（L1=prep，L3=kem_tail）")
    else:
        print("[FAIL_SYNC] handshake/Cube/mid-sync evidence", file=sys.stderr)
    if pass_io:
        print(f"[PASS_IO] ek+dk_kem match golden ({backend})")
    else:
        print("[FAIL_IO] ek/dk_kem mismatch", file=sys.stderr)

    if pass_sync and pass_io:
        print(f"[SUCCESS] RB-K06 PASS_SYNC + PASS_IO oracle={backend}")
        return 0
    return 1
