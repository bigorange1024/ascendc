...
{
    for (int32_t i = 0; i < tiling_k03::kPolyN / 2; ++i) {
...
{
    const uint32_t off =
        static_cast<uint32_t>((p * tiling_k03::kKem + j) * tiling_k03::kPolyN);
    AscendC::DataCopy(dst, aHatGm[off], static_cast<uint32_t>(tiling_k03::kPolyN));
...
__aicore__ inline void CopyPolyOut(AscendC::GlobalTensor<int32_t> &dstGm, int32_t polyIdx,
                                   const AscendC::LocalTensor<int32_t> &src)
{
    AscendC::DataCopy(dstGm[static_cast<uint32_t>(polyIdx * tiling_k03::kPolyN)], src,
                      static_cast<uint32_t>(tiling_k03::kPolyN));
    AscendC::PipeBarrier<PIPE_ALL>();
}
