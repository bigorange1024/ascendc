...
/**
 * @file light_cube.hpp
 * @brief RB-K03 极轻 Cube：单趟 16×32×32 int8→int32 MMAD（无算法正确性要求）。
 *
 * 作用：点积握手段调用一次，证明为有界真计算（非空转）。
 * 输入/输出：GM 上 A[m,k] int8、B[k,n] int8 → C[m,n] int32（ND）。
 * 前置：仅在 AIC 路径调用；m=16、k=n=32（tiling_k03::kKk）。
 *
 * API 依据：查阅索引已有 DataCopy(Nd2Nz)/LoadData/Mmad/Fixpipe；本文件不引入新 API。
 */
#ifndef RB_K03_LIGHT_CUBE_HPP
#define RB_K03_LIGHT_CUBE_HPP
 ...
 * 与 ek/dk golden 无关；仅要求 SIM/CPU 不挂死。
 */
class LightCube {
public:
