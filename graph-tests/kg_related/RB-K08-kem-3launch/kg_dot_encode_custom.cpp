/** RB-K06 拼装：本文件自 RB-K03/K04 接线，basename 锁定；算法未改；助手函数 static 防同库 ODR。 */
/**
 * @file kg_dot_encode_custom.cpp
 * @brief RB-K06 L2b：MIX 设备 Â∘ŝ̂+ê̂ + ByteEncode₁₂ → ek / dk_pke。
 *
 * KeyGen L2b 外形（KB §B2）：
 *   AIC：Wait(4)→Wait(1)→Cube→Set(3)
 *   AIV：Set(4)→Set(1)→Wait(3) → 点积+BE（仅 AIV0）
 *
 * 背景：K06 Host mid-sync 后消费 L1 Â/ρ 与 L2a NTT(ŝ/ê)；头文件走 k03_inc 隔离。
 * 结论：flag ∈ {1,3,4}；永禁 5/7 / SoftSync；与 L1/L2a/L3 同 binary 四 launch；禁与 L3 深 CrossCore 融合。
 */
