/** RB-K07 拼装：自 RB-K0x 契约头复制，仅改 include 路径 + 重命名 namespace/struct（避免与
 *  k02_inc/tiling.h 在同一融合 TU 内撞名）；算法未改。 */
/**
 * @file tiling.h
 * @brief RB-K03：KeyGen L2b — Â∘ŝ̂+ê̂ → ByteEncode₁₂ → ek_pke‖ρ / dk_pke。
 *
 * 命名注记：本文件原用 `namespace tiling` + `struct TilingData`，与 k02_inc/tiling.h 同名；
 * RB-K07 把两段揉进同一 MIX .cpp（同一 TU 同时 include 两侧 tiling.h），故重命名为
 * `namespace tiling_k03` + `struct TilingDataK03Unused`（未被融合核使用，Host/Device 统一走
 * k02_inc 的 `TilingData` 占位；此处保留字段布局仅为兼容旧引用）。
 *
...
struct TilingDataK03Unused {
    int32_t reserved0;
    int32_t reserved1;
};

namespace tiling_k03 {
