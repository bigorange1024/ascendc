 ...
 * @brief RB-K06 / KGR-K02 L3：KEM KeyGen 尾段（AIV-only）— H(ek)+z → dk_kem。
 *
 * 本文件在流水线中的位置：同 binary 四 launch 之 L3；承接 L2b 产出的 ek[1568]+dk_pke[1536]，
 * 完成 Alg.19 / Alg.16 增量尾段，写出完整 dk_kem[3168]（liboqs 布局）。
 * 契约与 RB-K05/KGR-K01 一致；本目录拼装进全链，禁抄 KeyGen/alg19 算子树/frozen。
 *
 * 布局契约（docs/notes/F203-KEM-Alg19…）：
 ...
 *
 * 工程约束：
 *   - basename：kg_kem_tail_custom（唯一；与 L1/L2a/L2b 互不撞名）
