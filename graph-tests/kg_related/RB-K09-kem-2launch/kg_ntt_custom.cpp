/** RB-K06 拼装：本文件自 RB-K02/K04 接线，basename 锁定；算法未改；助手函数 static 防同库 ODR。 */
/**
 * @file kg_ntt_custom.cpp
 * @brief RB-K06 L2a：MIX 设备 NTT(ŝ)+NTT(ê) — 握手(4/1/3) → AIV0 Alg.9×8 poly。
 *
 * KeyGen L2a 外形（KB §B2）：
 *   AIC：Wait(4)→Wait(1)→Cube→Set(3)
 *   AIV：Set(4)→Set(1)→Wait(3) → NTT(ŝ)、NTT(ê)（仅 AIV0）
 *
 * 背景：K06 Host mid-sync 后消费 L1 的 ŝ/ê；头文件走 k02_inc 隔离。
 * 结论：flag ∈ {1,3,4}；永禁 5/7 / SoftSync；与 L1/L2b/L3 同 binary 四 launch。
 */
