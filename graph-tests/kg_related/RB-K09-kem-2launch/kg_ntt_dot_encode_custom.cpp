 ...
 * @brief RB-K07：单 MIX 串行两段 —— NTT(ŝ/ê) 后 Â∘ŝ̂+ê̂+BE → ek/dk（Host 2 launch（K09：段2 收尾含 kem_tail））。
 *
 * L1 kg_prep → sync → 本核（NTT+dot/encode+kem_tail）→ sync。KEM 总 Host launch=2。
...
extern "C" __global__ __aicore__ void kg_ntt_dot_encode_custom(GM_ADDR out, GM_ADDR ekOut,
                                                               GM_ADDR dkOut, GM_ADDR seedDGm,
                                                               GM_ADDR hGm, GM_ADDR zGm,
                                                               GM_ADDR dkKemGm, GM_ADDR ws,
                                                               TilingData tiling)
{
    (void)tiling;
    ...
    if (isAic) {
        ...
        if (subIdx == 0) {
            ...
            *reinterpret_cast<__gm__ uint32_t *>(out) = tiling_k03::MAGIC_OUT_OK;
            AscendC::PipeBarrier<PIPE_ALL>();
            // 段2 收尾：同 AIV0 内嵌 kem_tail（省 Host Launch3）；无新 CrossCore
            kem_tail_device::KemTailRun(ekOut, dkOut, seedDGm, hGm, zGm, dkKemGm);
            AscendC::PipeBarrier<PIPE_ALL>();
        } else {
