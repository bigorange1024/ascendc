/**
 * @file main.cpp
 * @brief RB-K09 Host：两 launch KEM KeyGen（融合 MIX 内嵌 kem_tail） —— L1 prep → L2 融合 MIX(NTT+dot/encode) → ek/dk。
 *
 ...
 *   D2H ek[1568] / dk_kem[3168]（kem_tail 内嵌于 L2）
 *
...
extern "C" void kg_prep_custom(GM_ADDR seedDGm, GM_ADDR aHatGm, GM_ADDR sHatGm, GM_ADDR eGm,
                               ...
                               GM_ADDR tilingGm);
extern "C" void kg_ntt_dot_encode_custom(GM_ADDR out, GM_ADDR ekOut, GM_ADDR dkOut, GM_ADDR seedDGm,
                                         GM_ADDR hGm, GM_ADDR zGm, GM_ADDR dkKemGm, GM_ADDR ws,
                                         TilingData tiling);
#endif
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    // kem_tail 缓冲随融合 MIX 下发（无 Launch3）
    ...
    INFO_LOG("Host: Launch2 kg_ntt_dot_encode_custom (CPU MIX twin, fused+kem_tail)");
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    ICPU_RUN_KF(kg_ntt_dot_encode_custom, blockDim, outBuf, ekBuf, dkBuf, seedPadBuf, hBuf, zBuf, dkKemBuf, wsFused, *mixTiling);
    {
    ...
    // ---------- L2：融合 MIX（NTT + 点积/编码 + kem_tail，单 launch）----------
    ...
    INFO_LOG("Host: Launch2 kg_ntt_dot_encode_custom (fused MIX + kem_tail)");
    ACLRT_LAUNCH_KERNEL(kg_ntt_dot_encode_custom)
    (blockDim, stream, outDev, ekDev, dkDev, seedPadDev, hDev, zDev, dkKemDev, wsFusedDev, mixTiling);
    CHECK_ACL(aclrtSynchronizeStream(stream));
    ...
    if (!ok) {
        ...
        // kem_tail 已在 Launch2 完成；D2H h/z/dk_kem
