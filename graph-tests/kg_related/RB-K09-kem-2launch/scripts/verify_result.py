#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""RB-K08 verify：两 launch（融合 MIX（内嵌 kem_tail）） 验证 —— ek/dk ≡ liboqs（I/O 等价）+ 段1/段2 TRACE 因果证据。

背景：RB-K08 把 RB-K04 的 3 launch（prep → L2a NTT → L2b 点积/编码）压缩为 2 launch
...
def main():
    ...
    pass_io = _cmp_bytes(OUT / "ek.bin", OUT / "golden_ek.bin", "ek") and \
              _cmp_bytes(OUT / "dk_kem.bin", OUT / "golden_dk_kem.bin", "dk_kem")
    if pass_io:
    ...
    if pass_io:
        print(f"[PASS_IO] ek/dk_kem match ({backend})")
    else:
        print("[FAIL_IO]", file=sys.stderr)
    if pass_sync and pass_io:
        print(f"[SUCCESS] RB-K08 PASS_SYNC + PASS_IO oracle={backend}")
        return 0
    return 1
