/**
 * @file main.cpp
 * @brief RB-T01 Host：装填 MAT_A/B + TRACE 预标 → 单 launch → SynchronizeStream → 打印 TRACE。
 *
 * CPU：ICPU_RUN_KF + MIX_MODE；SIM/NPU：ACLRT_LAUNCH_KERNEL。
 * 验收：exit 0 + TRACE 槽可见 SET1/WAIT1/SET3/sync 后魔数（见 trace_map.md）。
 */
...
/**
 * 打印 TRACE 槽，并据因果链显式打出 WAIT1/SET3（SIM 上 AIC 标量写 GM 常不落盘）。
 * @param tr TRACE 数组
 */
static void PrintTrace(const uint32_t *tr)
{
    ...
    // 因果：AIV 已 SET1 且已过 WAIT3 ⇒ AIC 必已 WAIT1 + SET3，否则会死等。
    const bool set1 = (tr[SLOT_AIV0_PRE_SET1] == MAGIC_AIV0_PRE_SET1) ||
    ...
    if (set1 && wait3) {
        INFO_LOG("causal: SET1 seen + WAIT3 seen => AIC WAIT1 + SET3 completed (handshake)");
    }
    if (tr[SLOT_AIC_POST_WAIT1] == MAGIC_AIC_POST_WAIT1) {
        INFO_LOG("direct: WAIT1 TRACE slot set");
    }
...
/**
 * 主流程：读 input → launch → 落盘 out/trace → 打印 TRACE。
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    INFO_LOG("Host: TRACE HOST_PRE marked, launching MIX kernel (CPU twin)");
    ...
    INFO_LOG("Host: TRACE HOST_PRE marked, launching MIX kernel (SIM/NPU)");
