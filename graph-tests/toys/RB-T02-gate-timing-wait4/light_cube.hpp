/**
 * @file light_cube.hpp
 * @brief RB-T02 极轻 Cube：单趟 16×32×32 int8→int32 MMAD（无算法正确性要求）。
 *
 * 作用：在 AIC Wait(4) 唤醒后、进入 1/3 握手前占用 Cube，证明 GATE 后可插入计算。
 * 输入/输出：GM 上 A[m,k] int8、B[k,n] int8 → C[m,n] int32（ND）。
 * 前置：仅在 AIC 路径调用；m=16、k=n=32。
 *
 * API 依据：查阅索引已有 DataCopy(Nd2Nz)/LoadData/Mmad/Fixpipe 记录；本文件不引入新 API。
 */
#ifndef RB_T02_LIGHT_CUBE_HPP
#define RB_T02_LIGHT_CUBE_HPP
