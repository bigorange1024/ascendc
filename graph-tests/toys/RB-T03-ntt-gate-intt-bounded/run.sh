#!/bin/bash
# RB-T03-ntt-gate-intt-bounded：MIX NTT(1/3)+GATE(4)+INTT(1/3复用)+两段极轻 Cube+TRACE
#
# Usage（默认 = 全量验收路径）：
...
#
# 防挂死：KERNEL_COMPUTE_BUDGET_SEC 默认 cpu=60 / sim=180（仅 kernel 计算段）
...
echo "[SUCCESS] RB-T03-ntt-gate-intt-bounded (${RUN_MODE})"
