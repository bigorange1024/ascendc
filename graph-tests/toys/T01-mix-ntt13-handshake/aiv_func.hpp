#ifndef T01_MIX_NTT13_HANDSHAKE_AIV_FUNC_HPP
#define T01_MIX_NTT13_HANDSHAKE_AIV_FUNC_HPP

/**
 * @file aiv_func.hpp
 * @brief AIV 侧：桩「哈希」半片写 S0 + 完成标记写 out。
 *
 * 背景（T01 / D-EXP-T01）：只保留「写完左矩阵半片 → SET(1)」的握手意图；
 * 用 SetValue 常数填充代替真 SHAKE（Duplicate 不支持 int8）。
 * 禁止 SyncAll / SoftSync / GATE / INTT。
 */
