#ifndef T02_PROD_GATE_TIMING_TILING_H
#define T02_PROD_GATE_TIMING_TILING_H

/**
 * @file tiling.h
 * @brief T02：NTT 同构 1/3 + 生产 GATE 时序（flag 4/8）的维度与 workspace。
 *
 * 目标（D-EXP-T02）：在 T01 握手之上验证「AIC NTT 后立刻 WAIT(4)，AIV 体量后双 SET(4)」
 * 生产时序；禁止 INTT / 5/7 / Wait 中 SyncAll / SoftSync。
 ...
 * 槽序见 ToyTraceSlot / trace_map.md；须能区分 NTT 段与 GATE 段。
 */
