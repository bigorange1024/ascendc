#ifndef T03_FULL_FSM_NTT_GATE_INTT_AIV_FUNC_HPP
#define T03_FULL_FSM_NTT_GATE_INTT_AIV_FUNC_HPP

/**
 * @file aiv_func.hpp
 * @brief AIV 侧：桩哈希半片写 S0 + 轻体量 scratch + INTT 第二轮 1/3 + 完成标记写 out。
 *
 * 背景（T03 / D-EXP-T03）：NTT 1/3 → GATE 4/8 → INTT 复用 1/3（禁 5/7，KB X1）。
 ...
 *
 * 设计约束（T03 TASK）：勿巨型 MMAD；单次 SIM ≤5min。本类固定 4 轮 × 256 int8。
...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        // 完成标记：首元素 0xT03xx，其余填 subBlockID；非算法 oracle
        AscendC::Duplicate(t, static_cast<int32_t>(subBlockID_), kMarkElems);
        t.SetValue(0, static_cast<int32_t>(0x703000 + subBlockID_));
        outQ_.EnQue(t);
...
#endif /* T03_FULL_FSM_NTT_GATE_INTT_AIV_FUNC_HPP */
