/**
 * @file main.cpp
 * @brief T04 Host：装填 LUT=I₃₂、单 launch MIX、打印 TRACE 编号、落盘完成标记。
 *
 * 不对正确性对拍；SIM/NPU 路径 SynchronizeStream 返回 + TRACE 可见即视为核跑完。
 * Host 编号：101=launch 前，199=SynchronizeStream 返回后（KB §6 Host 100s）。
 * TRACE 须能区分 NTT / GATE / INTT 三段（见 trace_map.md）。
 */
