#ifndef T04_GATE_VOLUME_STRESS_TILING_H
#define T04_GATE_VOLUME_STRESS_TILING_H

/**
 * @file tiling.h
 * @brief T04：同 T03 全 FSM；GATE 段 AIV 体量加压（禁 5/7）。
 *
 * 目标（D-EXP-T04）：单 launch 闭环；INTT 第二轮仍用 flag 1/3，绝对禁止 5/7（KB X1）。
 * 矩阵仍取 Cube int8 最小粒度 16×32×32；WORK scratch 尺寸与 T03 相同，体量靠轮数↑。
 */
...
/** AIV 体量区：每 AIV 256 int8 scratch；T04 通过轮数加压（见 AivGateVolumeWorkload::kRounds）。 */
