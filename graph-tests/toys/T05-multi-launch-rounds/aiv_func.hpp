#ifndef T05_MULTI_LAUNCH_ROUNDS_AIV_FUNC_HPP
#define T05_MULTI_LAUNCH_ROUNDS_AIV_FUNC_HPP

/**
 * @file aiv_func.hpp
 * @brief AIV 侧：桩哈希半片写 S0 + 轻体量 scratch + INTT 第二轮 1/3 + 完成标记写 out。
 *
 * 背景（T05 / D-EXP-T05）：每 launch 轮 NTT 1/3 → GATE 4/8 → INTT 复用 1/3（禁 5/7，KB X1）。
 * GATE 段有界轻体量；完成标记在 INTT WAIT(3) 之后写入。
 */
 ...
 *
 * 设计约束（T05 TASK）：勿巨型 MMAD；沿用 T03 轻体量。本类固定 4 轮 × 256 int8。
...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        // 完成标记：首元素 0xT05xx，其余填 subBlockID；非算法 oracle
        AscendC::Duplicate(t, static_cast<int32_t>(subBlockID_), kMarkElems);
        t.SetValue(0, static_cast<int32_t>(0x705000 + subBlockID_));
        outQ_.EnQue(t);
...
#endif /* T05_MULTI_LAUNCH_ROUNDS_AIV_FUNC_HPP */
