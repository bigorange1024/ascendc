 ...
 * @brief T05：每 launch 轮 NTT 1/3 + 生产 GATE 4/8 + INTT 复用 1/3（设备 FSM 同 T03 轻量壳）。
 *
 * 图谱：D-EXP-T05 → Q-REPRO-ON-SIM / S3 多 launch 形态。
 *
 * Host 串行 launch ≥2 次；本核单次 launch 仍跑完整 NTT→GATE→INTT 闭环。
 ...
 * AscendC API：CrossCoreSetFlag/WaitFlag、DataCopy、Duplicate、Mmad 等复用 T01/T02 查阅记录。
 */
