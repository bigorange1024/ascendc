#!/bin/bash
# T05-multi-launch-rounds：Host 串行 launch 2 次；每轮 NTT 1/3 + 生产 GATE 4/8 + INTT 复用 1/3（禁 5/7）
#
# Usage（默认 = 生产全量；本玩具无调试分段开关）：
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4   # 验收（只认 SIM）
#   bash run.sh -r cpu -v Ascend910B4                # 编译壳冒烟
#
# 不对算法正确性；kernel 退出码 0 + 2 轮 TRACE 可见 + 无 stray dump 即过。
# KERNEL_COMPUTE_BUDGET_SEC 默认 180（防挂死，非性能定标；2 轮合计仍须 ≤25min）
CURRENT_DIR=$(
    cd $(dirname ${BASH_SOURCE:-$0})
    pwd
)
_ORIG_ARGS=("$@")
# graph-tests/toys/T01-… → 仓库根为三层上
...
echo "[SUCCESS] T05-multi-launch-rounds (${RUN_MODE}) kernel exit=0 launches=2 (no correctness cmp)"
