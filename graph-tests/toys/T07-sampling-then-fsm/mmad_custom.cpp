 ...
 * @brief T07：SAMPLE stub → NTT 1/3 + 生产 GATE 4/8（AIV 真 Vec MAC）+ INTT 复用 1/3（单 launch 全 FSM）。
 *
 * 图谱：D-EXP-T07 → Q-REPRO-ON-SIM / F-BRICKS-OK / F-BAN-INTT-57。
 *
 * 顺序（禁 SyncAll@AIC-Wait、禁 SoftSync、禁 INTT flag 5/7、禁 X14 空转）：
 *   SAMPLE：双 AIV 读 Host seed → 向量 mixing → 写 SAMPLE_OUT → TRACE(211/212/311/312)
 *   NTT：双 AIV SET(1) → AIC WAIT(1)+极轻 Cube → AIC SET(3) → 双 AIV WAIT(3)
 *   GATE（生产时序，同 T06）：
 *     AIC：SET(3) 后 TRACE(403) → WAIT(4)
                                                    ...
                                                    TilingData tiling)
{
    ...
    if (aic) {
        ...
        /* ---- SAMPLE 段（FSM 之前；TRACE 211/212/311/312）---- */
        {
            ToyTraceMark(trace, ws,
                         subBlockID == 0 ? TR_AIV0_SAMPLE_START : TR_AIV1_SAMPLE_START, aic,
                         subBlockID);
        }
        {
        ...
        {
            ToyTraceMark(trace, ws, subBlockID == 0 ? TR_AIV0_SAMPLE_DONE : TR_AIV1_SAMPLE_DONE,
                         aic, subBlockID);
        }
