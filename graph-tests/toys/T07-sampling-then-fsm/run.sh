#!/bin/bash
# T07-sampling-then-fsm：SAMPLE stub → NTT 1/3 + 生产 GATE 4/8（AIV 真 Vec MAC）+ INTT 复用 1/3（禁 5/7）
#
...
echo "[SUCCESS] T07-sampling-then-fsm (${RUN_MODE}) kernel exit=0 (no correctness cmp)"
