...
> **实机**：借入机独立网；**禁止**假设能 `git pull`；交付离线包或测试组清单，**禁止**零散请测。
...
### 1.6 实机交付形态（强制；2026-09-04 用户纠偏）

| 禁止 | 要求 |
|------|------|
| 想到一个假说就请用户上机测一次 | **SIM 上把能做的实验做完**（toy 决策树：过→下一刀 / 不过→另一刀） |
| 让用户「小包小包」零散加压 | 只交付 **一份 NPU 测试组**（多档并列），用户跑完后给**最简短报** |
| 用「请再测 Hostμ / 再测 TRACE」冒充进度 | 图谱未回答「如何写才不卡」前，**不上机** |

**硬自检**：若凭当前图谱积累，**不能**从头新写 Encrypt 且有信心 NPU 不卡死 → SIM 未穷尽，禁止请上机。
