#!/bin/bash
# toy-e02-softsync-then-set4：2-launch + SoftSyncArrive → SET(4) + 数字 TRACE ×≥3
#
# Usage（默认 = OMIT_SOFTSYNC=0，3 轮全绿）：
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# 可选对照（非默认；仍绿 → SoftSync 对本骨架非必要 / weaken）：
#   TOY_ROUNDS=3 OMIT_SOFTSYNC=1 SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# KERNEL_COMPUTE_BUDGET_SEC 默认 600（多轮防挂死，非性能定标）
# 本线验收：SIM only（勿当门禁跑 CPU）
# 禁止：OMIT_SET4 / 双 Cube / GATE alone 发现实验
CURRENT_DIR=$(
...
export OMIT_SOFTSYNC="${OMIT_SOFTSYNC:-0}"
if [ "${OMIT_SOFTSYNC}" != "0" ] && [ "${OMIT_SOFTSYNC}" != "1" ]; then
    echo "[ERROR] OMIT_SOFTSYNC must be 0 or 1, got: ${OMIT_SOFTSYNC}" >&2
    exit 1
fi
export TOY_ROUNDS="${TOY_ROUNDS:-3}"
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} CANN=${_ASCEND_INSTALL_PATH} OMIT_SOFTSYNC=${OMIT_SOFTSYNC} TOY_ROUNDS=${TOY_ROUNDS}"
...
echo "[SUCCESS] toy-e02-softsync-then-set4 (${RUN_MODE}) OMIT_SOFTSYNC=${OMIT_SOFTSYNC}"
