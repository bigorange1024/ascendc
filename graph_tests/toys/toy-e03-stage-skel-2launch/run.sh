#!/bin/bash
# toy-e03-stage-skel-2launch：Encrypt 形态骨架 — L1 采样 stub + L2 代数 stub + SET4 ×3
#
# Usage（默认 = 3 轮全绿；无 SoftSync；无 OMIT 故障注入）：
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# 调试（非默认）：
#   TOY_ROUNDS=5 SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# KERNEL_COMPUTE_BUDGET_SEC 默认 600（≥3 轮防挂死，非性能定标）
...
# Host TRACE tee → output/host_trace.log（verify 统计 111 / 阶段号段）
set +e
...
echo "[SUCCESS] toy-e03-stage-skel-2launch (${RUN_MODE})"
