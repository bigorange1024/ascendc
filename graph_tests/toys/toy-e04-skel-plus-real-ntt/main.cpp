/**
 * @file main.cpp
 * @brief E04 Host：数字 TRACE + ≥3 轮 2-launch（L1 stub → μ空 → L2 真 NTT+SET4）。
 *
 * 加载 ntt256 风格 input（src.bin + M4.bin）；输出 dst/out 为 int32[256] 供 golden 对拍。
 * 本 golden ≠ F203 Tag5T。
 */
 ...
 * L2 每轮写同一 out（同输入）；最后一帧对拍 golden。
 */
