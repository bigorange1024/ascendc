 ...
 * @brief E04：E03 形态骨架壳 + L2 真单 poly NTT（自包含 ntt256 积木）。
 *
 * 背景：图谱 D-exp-e04 — 在 E03 2-launch+SET(4) 壳上把假 NTT 换成真计算。
 * 结论：L1 仍为采样 TRACE stub；L2 跑 AIV Split→AIC Mmad×2→AIV Merge+Barrett，
 *       再用 CrossCore flag 4 做壳层 Wait/SET（与 NTT 内部 flag 1/2/3 不冲突）。
 * 未采用：抄 Encrypt；改 E01–E03；SoftSync；Tag5T RouteA。
 *
 * 语义声明：本 NTT golden ≠ F203 Tag5T（见 STATUS.md）。
 * TRACE 号段见 TRACE.md。
 */
#include "basic.hpp"
...
/** 壳层 SET(4)：L2 入口齐步（E03 合同；占用 4，避让 NTT 1/2/3）。 */
...
/**
 * L1 采样形态 stub：仅 TRACE，无真 SHAKE/CBD（同 E03）。
 * 仅 AIV0 打点；AIC/AIV1 barrier。
 */
...
/**
 * MIX kernel：phase=0 L1 stub；phase=1 L2 真 NTT + SET(4)。
 * @param out [out] NTT 结果 int32[256]
