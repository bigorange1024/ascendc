/**
 * @file main.cpp
 * @brief E05 Host：数字 TRACE + ≥3 轮 2-launch（L1 真 SHAKE → μ空 → L2 真 NTT+SET4）。
 *
 * L1 Sync 后立刻 D2H 前 32B 为 SHAKE y（随后 L2 覆盖 out 为 NTT）。
 * 加载 ntt256 风格 input（src.bin + M4.bin）；输出 dst/out 为 int32[256] 供 NTT golden。
 * 本 NTT golden ≠ F203 Tag5T；SHAKE golden = hashlib.shake_256(b"abc").digest(32)。
 */
