...
# coding=utf-8
"""
gen_data.py — E05：ntt256 风格输入 + SHAKE256 短向量 golden + tiling。

流水线：run.sh 编译后、kernel 前。
写 input/tiling.bin、src.bin、M4.bin 与 output/golden.bin、output/shake_golden.bin。
语义：NTT = merged_kyber / ntt_sim_kyber（≠ Tag5T）；
...
def main() -> None:
    ...
    # SHAKE256 短向量 oracle（与设备内嵌 / shake_l1_ub.hpp 同源）
    ...
    print(f"[gen_data] E05 ntt256 + shake256(abc→32B) under {in_dir} (NTT ≠ Tag5T)")
