...
# coding=utf-8
"""
verify_result.py — E05：Host/阶段 TRACE × rounds + SHAKE 短向量 + ntt256 golden。

读 output/host_trace.log、shake_y.bin、dst.bin / golden.bin。
NTT golden = ntt_sim_kyber，**≠ F203 Tag5T**。
SHAKE golden = hashlib.shake_256(b"abc").digest(32)。
...
# L1 真 SHAKE：200 入 → 210 开始 → 211 完成 → 212 UB PASS → 203 返回
...
def main() -> int:
    ...
    print("[verify] TRACE OK: L1 real-SHAKE → μ → L2 real-NTT + SET4")

    if skip_golden:
        print("[verify] TOY_SKIP_GOLDEN=1 — skip shake/ntt golden")
        print("[SUCCESS] E05 TRACE-only (no hang path)")
        return 0
    ...
    if diff.size != 0:
        ...
        return 9
    print("[SUCCESS] E05 TRACE × rounds + shake short-vector + ntt256 golden OK")
    return 0
