/**
 * @file main.cpp
 * @brief E06 Host：数字 TRACE + ≥3 轮 2-launch（L1 真 SHAKE → μ空 → L2 真 NTT+basemul+SET4）。
 *
 * L1 Sync 后立刻 D2H 前 32B 为 SHAKE y（随后 L2 覆盖 out 为 basemul ĥ）。
 * 加载 ntt256 风格 input（src.bin + M4.bin + g.bin→ws.G0）；输出 dst/out=MultiplyNTTs(NTT(src),ĝ)。
 * NTT ≠ Tag5T；basemul=Alg.11/12；SHAKE=hashlib.shake_256(b"abc").digest(32)。
 */
 ...
 * 每轮 L1 后保存 shake_y；最后一帧 NTT 对拍 golden。
 */
int32_t main(int32_t argc, char *argv[])
{
    ...
    size_t matMFileSize = tiling::n * tiling::n * 4; // 4×int8 肢拼成 M4.bin
