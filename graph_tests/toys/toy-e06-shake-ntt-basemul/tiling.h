#ifndef TOY_E06_SHAKE_NTT_BASEMUL_TILING_H
#define TOY_E06_SHAKE_NTT_BASEMUL_TILING_H

/**
 * @file tiling.h
 * @brief E06：E05 壳 + L1 真 SHAKE256 + L2 真 NTT + 真 basemul/MultiplyNTTs + SET(4)。
 *
 * NTT golden / 设备语义 = merged_kyber ntt256（≠ Tag5T）。
 * basemul = FIPS Alg.11/12 标量（γ=kMlkemGammas）；f̂=NTT(src)，ĝ=Host 玩具系数。
 * SHAKE 短向量 = hashlib.shake_256(b"abc").digest(32)。
 *
 * workspace：ntt256 布局 + 尾部 G0 = ĝ int32[n]。
 */
...
struct TilingData {
    int32_t phase;       /**< 0=L1 真 SHAKE256；1=L2 真 NTT+basemul + Wait/SET(4) */
...
constexpr size_t wssize = G0 + n * sizeof(int32_t);

/** 输出：basemul ĥ = MultiplyNTTs(NTT(src), ĝ) int32[n]。 */
