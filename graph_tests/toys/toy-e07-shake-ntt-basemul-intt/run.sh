#!/bin/bash
# toy-e07-shake-ntt-basemul-intt：E06 壳 + L1 真 SHAKE + L2 真 NTT + 真 basemul + 真 INTT + SET4 ×3
#
# Usage（默认 = 3 轮 SIM；真 SHAKE + 真 NTT + 真 MultiplyNTTs + 真 INTT + SET4）：
...
#
# KERNEL_COMPUTE_BUDGET_SEC 默认 900（3×真 SHAKE+NTT+basemul+INTT；防挂死，非性能定标）
# NTT/INTT ≠ Tag5T（ntt256 矩阵正/逆）；basemul=Alg.11/12；SHAKE=hashlib.shake_256(abc→32B)
CURRENT_DIR=$(
...
echo "[SUCCESS] toy-e07-shake-ntt-basemul-intt (${RUN_MODE})"
