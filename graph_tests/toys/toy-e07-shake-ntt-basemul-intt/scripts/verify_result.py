...
# coding=utf-8
"""
verify_result.py — E07：TRACE × rounds + SHAKE + INTT(MultiplyNTTs) golden。

读 output/host_trace.log、shake_y.bin、dst.bin / golden.bin。
golden = INTT_ntt256(Alg.11(NTT(src), ĝ))；NTT/INTT ≠ Tag5T。
...
# 530/531=真 basemul；540/541=真 INTT 开始；542/543=真 INTT 完成
L2_MUST = [
    "400", "401", "402",
...
def main() -> int:
    ...
    print("[verify] TRACE OK: L1 real-SHAKE → μ → L2 real-NTT + basemul + INTT + SET4")

    if skip_golden:
        print("[verify] TOY_SKIP_GOLDEN=1 — skip shake/INTT golden")
        print("[SUCCESS] E07 TRACE-only (no hang path)")
        return 0
    ...
    print(f"[verify] INTT(MultiplyNTTs) golden diffs={diff.size}/{golden.size}")
    if diff.size != 0:
        for i in diff[:20]:
            print(f"  idx={i} got={output[i]} exp={golden[i]}")
        print("[FAIL] INTT golden mismatch", file=sys.stderr)
        return 9
    print("[SUCCESS] E07 TRACE × rounds + shake + INTT∘basemul golden OK")
    return 0
