#!/bin/bash
# toy-e08-shake-cbd-ntt-chain：E07 壳 + L1 真 SHAKE + 真 CBD(η=2) + L2 真 NTT+basemul+INTT+SET4 ×3
#
# Usage（默认 = 3 轮 SIM；真 SHAKE + 真 CBD + 真 NTT + MultiplyNTTs + INTT + SET4）：
...
#
# KERNEL_COMPUTE_BUDGET_SEC 默认 900（3×真 SHAKE+CBD+NTT+basemul+INTT；防挂死，非性能定标）
# CBD=Alg.8 η=2；NTT/INTT ≠ Tag5T；basemul=Alg.11/12；SHAKE=hashlib.shake_256(abc→32B)
CURRENT_DIR=$(
...
echo "[SUCCESS] toy-e08-shake-cbd-ntt-chain (${RUN_MODE})"
