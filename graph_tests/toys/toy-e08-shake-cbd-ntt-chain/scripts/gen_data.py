...
# coding=utf-8
"""
gen_data.py — E08：prf→CBD(η=2)→NTT→basemul→INTT golden + SHAKE 短向量。

流水线：run.sh 编译后、kernel 前。
写 input/tiling.bin、src.bin（占位）、M4.bin、Minv4.bin、g.bin、prf.bin 与
output/golden.bin、golden_cbd.bin、golden_ntt.bin、golden_basemul.bin、shake_golden.bin。

语义：CBD = FIPS Alg.8 η=2（golden_se_sampling.sample_poly_cbd2）；
      NTT/INTT = ntt256 矩阵正/逆（≠ Tag5T）；
      basemul = FIPS Alg.11/12；
...
def main() -> None:
    ...
    # 先跑一次 gen_golden_data 以填充全局 M（与 E07 同副作用）
    ntt_sim_kyber.gen_golden_data(n=n, q=3329, g=17)

    # PRF → CBD(η=2) 单 poly（短链）
    ...
    # src 占位（设备 L1 会用 CBD 覆盖）；仍写文件满足 main ReadFile
    cbd_poly.tofile(os.path.join(in_dir, "src.bin"))

    # NTT(CBD) golden（ntt_sim_kyber 矩阵正变换；依赖上面对 M 的初始化）
    ...
    print(f"[gen_data] E08 SHAKE+CBD(η=2)+NTT+basemul+INTT under {in_dir} (SEED_D={SEED})")
