...
# coding=utf-8
"""
verify_result.py — E08：TRACE × rounds + SHAKE + CBD + INTT(MultiplyNTTs) golden。
...
L2_MUST = [
    "400", "401", "402",
    "500", "520", "530", "532", "540", "542", "502",
    "510", "521", "531", "533", "541", "543", "512",
]
...
def main() -> int:
    ...
    for r in range(rounds):
        ...
        for a, b in zip(L1_SEQ, L1_SEQ[1:]):
            if window.index(a) > window.index(b):
                ...
                return 8
        aiv0 = ["500", "520", "530", "532", "540", "542", "502"]
    ...
    print("[verify] TRACE OK: L1 real-SHAKE→real-CBD → μ → L2 real-NTT+basemul+INTT+SET4")

    if skip_golden:
        print("[verify] TOY_SKIP_GOLDEN=1 — skip shake/CBD/INTT golden")
        print("[SUCCESS] E08 TRACE-only (no hang path)")
        return 0
    ...
    print(f"[verify] INTT(MultiplyNTTs(NTT(CBD))) golden diffs={diff.size}/{golden.size}")
    if diff.size != 0:
        for i in diff[:20]:
            print(f"  idx={i} got={output[i]} exp={golden[i]}")
        print("[FAIL] INTT golden mismatch", file=sys.stderr)
        return 9
    print("[SUCCESS] E08 TRACE × rounds + shake + CBD + INTT∘basemul golden OK")
    return 0
