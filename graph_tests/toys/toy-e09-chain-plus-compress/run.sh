#!/bin/bash
# toy-e09-chain-plus-compress：E08 壳 + L2 INTT 后真 Compress_d(d=4) + SET4 ×3
#
# Usage（默认 = 3 轮 SIM；真 SHAKE+CBD+NTT+basemul+INTT+Compress+SET4）：
...
# Compress=FIPS §4.2.1 d=4；CBD=Alg.8 η=2；NTT/INTT ≠ Tag5T；basemul=Alg.11/12
CURRENT_DIR=$(
...
echo "[SUCCESS] toy-e09-chain-plus-compress (${RUN_MODE})"
