...
# coding=utf-8
"""
gen_data.py — E09：prf→CBD→NTT→basemul→INTT→Compress_d(d=4) golden + SHAKE 短向量。

流水线：run.sh 编译后、kernel 前。
写 input/tiling.bin、src.bin（占位）、M4.bin、Minv4.bin、g.bin、prf.bin 与
output/golden.bin（=Compress 域）、golden_intt.bin、golden_cbd.bin、…、shake_golden.bin。

语义：CBD = FIPS Alg.8 η=2；NTT/INTT = ntt256 矩阵正/逆（≠ Tag5T）；
      basemul = FIPS Alg.11/12；Compress = FIPS §4.2.1 d=4（vendor compress_d_ref）；
...
_REF_C = os.path.join(ROOT, "vendor/compress_d/compress_d_ref.c")
_REF_H_DIR = os.path.join(ROOT, "vendor/compress_d")
_REF_SO = os.path.join(ROOT, "scripts", "libcompress_d_ref.so")
...
def _build_ref_so() -> None:
    """现场编译 vendor compress_d_ref.c → scripts/libcompress_d_ref.so。"""
    cmd = [
        "gcc", "-shared", "-fPIC", "-O2",
        f"-I{_REF_H_DIR}",
        "-o", _REF_SO,
...
def _compress_ref(poly: np.ndarray, d: int) -> np.ndarray:
    ...
    if not os.path.isfile(_REF_SO):
        _build_ref_so()
    lib = ctypes.CDLL(_REF_SO)
...
def main() -> None:
    ...
    # 主 golden = Compress_d(INTT(...))；设备 dst 覆写为压缩域
    golden_comp = _compress_ref(golden_intt, COMPRESS_D)
    golden_comp.tofile(os.path.join(out_dir, "golden.bin"))
    ...
    print(
        f"[gen_data] E09 SHAKE+CBD+NTT+basemul+INTT+Compress_d={COMPRESS_D} "
        f"under {in_dir} (SEED_D={SEED})"
    )
