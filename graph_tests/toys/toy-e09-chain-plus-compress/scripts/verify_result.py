...
# coding=utf-8
"""
verify_result.py — E09：TRACE × rounds + SHAKE + CBD + Compress(INTT∘basemul∘NTT) golden。
...
L2_MUST = [
    "400", "401", "402",
    "500", "520", "530", "532", "540", "542", "550", "552", "502",
...
def main() -> int:
    ...
    for r in range(rounds):
        ...
        for a, b in zip(L1_SEQ, L1_SEQ[1:]):
            if window.index(a) > window.index(b):
                ...
                return 8
        aiv0 = ["500", "520", "530", "532", "540", "542", "550", "552", "502"]
    ...
    print("[verify] TRACE OK: L1 real-SHAKE→real-CBD → μ → L2 real-NTT+basemul+INTT+Compress+SET4")

    if skip_golden:
        print("[verify] TOY_SKIP_GOLDEN=1 — skip shake/CBD/Compress golden")
        print("[SUCCESS] E09 TRACE-only (no hang path)")
        return 0
    ...
    print(f"[verify] Compress_d(INTT(MultiplyNTTs(NTT(CBD)))) diffs={diff.size}/{golden.size}")
    if diff.size != 0:
        for i in diff[:20]:
            print(f"  idx={i} got={output[i]} exp={golden[i]}")
        print("[FAIL] Compress golden mismatch", file=sys.stderr)
        return 9
    print("[SUCCESS] E09 TRACE × rounds + shake + CBD + Compress∘INTT∘basemul golden OK")
    return 0
