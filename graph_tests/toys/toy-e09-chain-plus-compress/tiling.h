#ifndef TOY_E09_CHAIN_PLUS_COMPRESS_TILING_H
#define TOY_E09_CHAIN_PLUS_COMPRESS_TILING_H

/**
 * @file tiling.h
 * @brief E09：E08 壳 + L2 INTT 后真 Compress_d(d=4) + SET(4)。
 *
 * L1：SHAKE 短向量写 out 前缀；CBD 读 ws[P0] PRF，写 src[256]。
 * L2：NTT→basemul→INTT→Compress_d；输出 = Compress(INTT(MultiplyNTTs(NTT(CBD),ĝ)))。
 */
...
struct TilingData {
    int32_t phase;       /**< 0=L1 真 SHAKE+CBD；1=L2 真 NTT+basemul+INTT+Compress + Wait/SET(4) */
...
/** 输出：Compress_d(INTT(…)) int32[n]，值 ∈ [0, 2^d-1]（默认 d=4）。 */
