 ...
 * @brief E10：E09 壳 + L2 Compress 后 **真 ByteEncode_d(d=4)** + SET(4)。
 *
 * 背景：图谱 D-exp-e10 — 在 E09 真链 Compress 之后接入真 ByteEncode；保留上游真链。
 * 结论：L1=SHAKE→CBD；L2=NTT→MultiplyNTTs→INTT→Compress_d→ByteEncode_d(128B)+SET(4)。
 * 未采用：抄 Encrypt；改 E01–E09；复测 retracted；假 ByteEncode TRACE stub。
 *
 * 语义：Compress/ByteEncode=FIPS §4.2.1/Alg.5 d=4（vendor 自包含）；其余同 E09。
 * TRACE 号段见 TRACE.md。
 */
#include "basic.hpp"
...
/**
 * L2 AIV：真 NTT → basemul → INTT → **真 Compress_d** → **真 ByteEncode_d** → SET(4)。
 * @param src 已由 L1 CBD 覆写的噪声 poly
 * @param dst L2 工作区 int32[n]；ByteEncode 后 out 前缀为 uint8[128]
 */
__aicore__ inline void L2RealNttBasemulInttCompressByteEncodeAiv(int32_t subBlockID, GM_ADDR dst, GM_ADDR src,
                                                                 GM_ADDR ws, int32_t n)
{
    using namespace tiling;
    if (subBlockID == 0) {
        TraceDigit(500);
    ...
    // ---- 真 Compress_d（接在 INTT 后、ByteEncode 前）----
    AscendC::PipeBarrier<PIPE_ALL>();
    {
        const size_t halfOff = static_cast<size_t>(subBlockID) * (static_cast<size_t>(n) / 2) * sizeof(int32_t);
        const uint32_t halfLen = static_cast<uint32_t>(n / 2);
...
/**
 * MIX kernel：phase=0 L1 真 SHAKE+CBD；phase=1 L2 真 NTT+basemul+INTT+Compress+ByteEncode+SET(4)。
 */
extern "C" __global__ __aicore__ void mmad_custom(GM_ADDR out, GM_ADDR src, GM_ADDR ws, TilingData tiling)
{
    ...
    if (aic) {
        ...
        L2RealNttBasemulInttCompressByteEncodeAiv(subBlockID, out, src, ws, n);
    }
}
