#!/bin/bash
# toy-e10-chain-plus-byteencode：E09 壳 + L2 Compress 后真 ByteEncode_d(d=4) + SET4 ×3
#
# Usage（默认 = 3 轮 SIM；真 SHAKE+CBD+NTT+basemul+INTT+Compress+ByteEncode+SET4）：
...
#
# KERNEL_COMPUTE_BUDGET_SEC 默认 900（3×真链；防挂死，非性能定标）
# Compress/ByteEncode=FIPS §4.2.1/Alg.5 d=4（128B）；CBD=Alg.8 η=2；NTT/INTT ≠ Tag5T
CURRENT_DIR=$(
...
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} CANN=${_ASCEND_INSTALL_PATH} TOY_ROUNDS=${TOY_ROUNDS} F203_BYTE_ENCODE_D=${F203_BYTE_ENCODE_D}"
...
echo "[SUCCESS] toy-e10-chain-plus-byteencode (${RUN_MODE})"
