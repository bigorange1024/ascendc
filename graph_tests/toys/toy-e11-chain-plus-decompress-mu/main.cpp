/**
 * @file main.cpp
 * @brief E11 Host：数字 TRACE + ≥3 轮 2-launch（L1 真 SHAKE+CBD → μ H2D → L2 真 NTT+basemul+INTT+Decompress_1(μ)+Compress+ByteEncode+SET4）。
 *
 * L1 Sync 后：D2H SHAKE y（out 前缀 32B）+ D2H CBD 覆写后的 src。
 * μ：固定 input/mu.bin（32B，gen_data SEED_D 生成）→ ws[MU0] H2D；TRACE 105。
 * 输出 = ByteEncode_d(Compress_d(INTT+Decompress_1(μ)(MultiplyNTTs(NTT(CBD(prf)),ĝ))))（d=4→128B）。
 */
...
/** Host μ：TRACE 105；μ 已在 ws[MU0]（每轮 L2 前可读）。 */
static void HostMuReady()
{
    TraceDigit(105);
}
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    ok = WriteFile("./output/out.bin", out, tiling::kEncodeBytes);
    if (!ok) {
        return 14;
    }
    ok = WriteFile("./output/dst.bin", out, tiling::kEncodeBytes);
    if (!ok) {
        return 15;
    }
    ...
    CHECK_ACL(aclrtMemcpy(outHost, tiling::kEncodeBytes, outDevice, tiling::kEncodeBytes, ACL_MEMCPY_DEVICE_TO_HOST));
    ok = WriteFile("./output/out.bin", outHost, tiling::kEncodeBytes);
    if (!ok) {
        return 14;
    }
    ok = WriteFile("./output/dst.bin", outHost, tiling::kEncodeBytes);
    if (!ok) {
        return 15;
    }
