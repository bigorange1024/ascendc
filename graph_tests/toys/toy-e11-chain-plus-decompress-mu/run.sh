#!/bin/bash
# toy-e11-chain-plus-decompress-mu：E10 壳 + L2 INTT 后真 Decompress_1(μ) + Compress + ByteEncode + SET4 ×3
#
# Usage（默认 = 3 轮 SIM；真 SHAKE+CBD+NTT+basemul+INTT+Decompress_1(μ)+Compress+ByteEncode+SET4）：
...
# Decompress_1=消息嵌入 d=1（μ 32B）；Compress/ByteEncode d=4（128B）
CURRENT_DIR=$(
...
echo "[SUCCESS] toy-e11-chain-plus-decompress-mu (${RUN_MODE})"
