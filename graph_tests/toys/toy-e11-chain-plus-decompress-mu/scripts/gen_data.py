...
# coding=utf-8
"""
gen_data.py — E11：prf→CBD→NTT→basemul→INTT→+Decompress_1(μ)→Compress_d→ByteEncode_d(d=4) golden + SHAKE。

流水线：run.sh 编译后、kernel 前。
写 input/*（含 mu.bin 32B）与 output/golden.bin（=ByteEncode 128B）等。

语义：Decompress_1=消息嵌入（32B μ→256 系数 {0,(q+1)/2} 后 mod-q 加 INTT）；
      其余同 E10（CBD/NTT/basemul/INTT/Compress/ByteEncode d=4）。
μ：SEED_D+1 固定随机 32B（可复现）。
"""
import ctypes
import hashlib
import os
...
def main() -> None:
    ...
    golden_intt_plus_mu.tofile(os.path.join(out_dir, "golden_intt_plus_mu.bin"))
    ...
    print(
        f"[gen_data] E11 SHAKE+CBD+NTT+basemul+INTT+Decompress_1(μ)+Compress_d={COMPRESS_D}+"
        f"ByteEncode_d={ENCODE_D} ({ENCODE_BYTES[ENCODE_D]}B) mu=SEED_D+1 under {in_dir} (SEED_D={SEED})"
    )
