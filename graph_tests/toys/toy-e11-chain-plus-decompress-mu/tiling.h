#ifndef TOY_E11_CHAIN_PLUS_DECOMPRESS_MU_TILING_H
#define TOY_E11_CHAIN_PLUS_DECOMPRESS_MU_TILING_H

/**
 * @file tiling.h
 * @brief E11：E10 壳 + L2 INTT 后真 Decompress_1(μ) + Compress + ByteEncode + SET(4)。
 *
 * L1：SHAKE 短向量写 out 前缀；CBD 读 ws[P0] PRF，写 src[256]。
 * L2：NTT→basemul→INTT→+Decompress_1(μ)→Compress_d→ByteEncode_d；dst=128B。
 * μ：Host 写 input/mu.bin → ws[MU0]（32B）；L2 前 H2D。
 */
...
struct TilingData {
    int32_t phase;       /**< 0=L1 真 SHAKE+CBD；1=L2 真 NTT+basemul+INTT+Decompress_1(μ)+Compress+ByteEncode + SET(4) */
...
/** MU0：L2 Decompress_1 消息 μ 32B（Host H2D；32B 对齐）。 */
