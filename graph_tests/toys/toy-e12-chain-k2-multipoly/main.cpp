/**
 * @file main.cpp
 * @brief E12 Host：数字 TRACE + ≥3 轮 2-launch（L1 SHAKE+CBD×2 → μ H2D → L2 真链×2+SET4）。
 *
 * L1 Sync 后：D2H SHAKE y + D2H CBD 覆写后的 src（512 int32）。
 * μ：固定 input/mu.bin（32B，两 poly 共享）→ ws[MU0] H2D。
 * 输出 = 2×ByteEncode_d(128B)=256B 拼接。
 */
