 ...
 * @brief E12：E11 壳扩 k=2 — 两路 poly 各走真链；共享 2-launch + SET(4)。
 *
 * 背景：图谱 D-exp-e12 — k=2 多 poly 短链；L2 内 poly 串行复用 NTT/INTT CrossCore。
 * 结论：L1=SHAKE→CBD×2；L2=(NTT→basemul→INTT→Decompress_1(μ)→Compress→ByteEncode)×2 + SET(4)。
 * 未采用：抄 Encrypt；改 E01–E11；TRACE 翻倍 stub。
 */
#include "basic.hpp"
...
__aicore__ inline void L1RealShakeThenCbdK2(bool aic, int32_t subBlockID, GM_ADDR shakeYGm, GM_ADDR prfGm,
                                            GM_ADDR srcGm)
{
    AscendC::PipeBarrier<PIPE_ALL>();
    if (!aic && subBlockID == 0) {
        ...
        TraceDigit(220);
        CbdL1Toy::RunCbdEta2K2Polys(reinterpret_cast<__gm__ const uint8_t *>(prfGm),
                                    reinterpret_cast<__gm__ int32_t *>(srcGm));
        TraceDigit(222); // k=2 CBD 完成
        TraceDigit(221);
        TraceDigit(203);
    }
    AscendC::PipeBarrier<PIPE_ALL>();
...
/**
 * L2 AIC：单 poly 真 NTT Cube → 真 INTT Cube。
 * @param ws    workspace
 * @param n     poly 长度 256
 * @param polyIdx 0 或 1（仅 TRACE）
 */
...
/**
 * L2 AIV：单 poly 真链 → ByteEncode 写入 encOutGm（128B）。
 */
...
{
    using namespace tiling;
    const int32_t baseTrace = (polyIdx == 0U) ? 500 : 600;
...
/**
 * L2 AIC：k=2 串行两 poly；末 poly 后 Wait(4)。
 */
__aicore__ inline void L2RealNttBasemulInttAicK2(GM_ADDR ws, int32_t n)
{
...
/**
 * L2 AIV：k=2 串行两 poly；末 poly 后 SET(4)。
 */
__aicore__ inline void L2RealChainK2Aiv(int32_t subBlockID, GM_ADDR out, GM_ADDR src, GM_ADDR ws, GM_ADDR muGm,
                                        int32_t n)
{
    using namespace tiling;
    ...
    for (uint32_t p = 0; p < k; ++p) {
        ...
        L2RealChainOnePolyAiv(subBlockID, ws + workGm, src + srcGm, ws, ws + gGm, muGm, out + encGm, n, p);
        AscendC::PipeBarrier<PIPE_ALL>();
    }
...
extern "C" __global__ __aicore__ void mmad_custom(GM_ADDR out, GM_ADDR src, GM_ADDR ws, TilingData tiling)
{
    ...
    if (phase == tiling::kPhaseLaunch1) {
        L1RealShakeThenCbdK2(aic, subBlockID, out, ws + tiling::P0, src);
        return;
    }

    if (aic) {
        L2RealNttBasemulInttAicK2(ws, n);
    } else {
        L2RealChainK2Aiv(subBlockID, out, src, ws, ws + tiling::MU0, n);
    }
}
