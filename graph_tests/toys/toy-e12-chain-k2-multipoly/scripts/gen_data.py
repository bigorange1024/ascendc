...
# coding=utf-8
"""
gen_data.py — E12 k=2：2×(prf→CBD→NTT→basemul→INTT→Decompress_1(μ)→Compress→ByteEncode) golden。

输出：golden.bin = 2×128B 拼接；src=512 int32；prf=256B；g=512 int32。
μ：SEED_D+1 固定 32B，两 poly 共享。
"""
import ctypes
import hashlib
import os
...
PRF_BYTES = PRF_BYTES_PER_POLY * k
MU_BYTES = 32
...
ENCODE_BYTES = ENCODE_BYTES_PER_POLY[ENCODE_D] * k
...
def make_g_poly(seed_offset: int) -> np.ndarray:
    return np.array([(13 * i + 7 + seed_offset * 997) % Q for i in range(n)], dtype=np.int32)
...
def chain_one_poly(cbd_poly: np.ndarray, g_hat: np.ndarray, mu: np.ndarray, Minv: np.ndarray) -> np.ndarray:
...
def main() -> None:
    ...
    for p in range(k):
        ...
        cbd_polys.append(sample_poly_cbd2(bytes(row_prf)).astype(np.int32))
    cbd_all = np.concatenate(cbd_polys)
    cbd_all.tofile(os.path.join(out_dir, "golden_cbd.bin"))
    cbd_all.tofile(os.path.join(in_dir, "src.bin"))
    ...
    g_polys = [make_g_poly(p) for p in range(k)]
    g_all = np.concatenate(g_polys)
    g_all.tofile(os.path.join(in_dir, "g.bin"))
    ...
    enc_parts = []
    for p in range(k):
        enc = chain_one_poly(cbd_polys[p], g_polys[p], mu, Minv)
        enc_parts.append(enc)
        enc.tofile(os.path.join(out_dir, f"golden_encode_poly{p}.bin"))

    golden_enc = np.concatenate(enc_parts)
    ...
    print(
        f"[gen_data] E12 k=2 SHAKE+CBD×2+NTT+basemul+INTT+Decompress_1(μ)+Compress_d={COMPRESS_D}+"
        f"ByteEncode_d={ENCODE_D} ({ENCODE_BYTES}B) mu=shared SEED_D+1 under {in_dir} (SEED_D={SEED})"
    )
