...
# coding=utf-8
"""
verify_result.py — E12 k=2：TRACE × rounds + SHAKE + CBD×2 + 256B ByteEncode golden。
"""
import os
import sys
import numpy as np
...
ENCODE_OUT = ENCODE_PER_POLY * K

HOST_SEQ = ["100", "101", "105", "110", "111"]
L1_SEQ = ["200", "210", "211", "212", "220", "222", "221", "203"]
L2_POLY0_MUST = [
    "400", "401",
    "500", "520", "530", "532", "540", "542", "544", "546", "550", "552", "560", "562", "503",
    "510", "521", "531", "533", "541", "543", "545", "547", "551", "553", "513",
]
L2_POLY1_MUST = [
    "410", "411",
    "600", "620", "630", "632", "640", "642", "644", "646", "650", "652", "660", "662", "603",
    "610", "621", "631", "633", "641", "643", "645", "647", "651", "653", "613",
]
L2_FINAL_MUST = ["402", "502", "512"]
...
def main() -> int:
    ...
    if count("213") > 0:
        ...
        return 6
    if count("222") < rounds:
        print("[FAIL] L1 TRACE 222 missing (k=2 CBD)", file=sys.stderr)
        return 6

    for code in L2_POLY0_MUST + L2_POLY1_MUST + L2_FINAL_MUST:
        c = count(code)
    ...
    for r in range(rounds):
        ...
        need = ["200", "222", "400", "503", "600", "603", "402", "502"]
        for code in need:
            if code not in window:
                print(f"[FAIL] round {r} missing {code}", file=sys.stderr)
                return 8

    print("[verify] TRACE OK: L1 SHAKE→CBD×2 → μ → L2 k=2 real-chain+SET4")

    if skip_golden:
        print("[verify] TOY_SKIP_GOLDEN=1 — skip golden")
        print("[SUCCESS] E12 TRACE-only (no hang path)")
        return 0
    ...
    print(f"[verify] CBD×2 diffs={cdiff.size}/{cbd_g.size}")
    if cdiff.size != 0:
        return 11
    ...
    print(f"[verify] 2×ByteEncode golden diffs={diff.size}/{golden.size}B")
    if diff.size != 0:
        for i in diff[:20]:
            print(f"  idx={i} got={output[i]:02x} exp={golden[i]:02x}")
        return 9

    print("[SUCCESS] E12 k=2 TRACE × rounds + shake + CBD×2 + 256B golden OK")
    return 0
