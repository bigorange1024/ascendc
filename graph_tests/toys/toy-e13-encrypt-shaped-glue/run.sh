#!/bin/bash
# toy-e13-encrypt-shaped-glue：E12 积木 + Encrypt 形态粘合 — L1 采样 / L2 代数+压码 → c=c1||c2 ×3
#
# Usage（默认 = 3 轮 SIM；真 SHAKE+CBD(u×2+v×1)+u 路×2+v 路(μ)+SET4 → 384B）：
...
echo "[SUCCESS] toy-e13-encrypt-shaped-glue (${RUN_MODE})"
