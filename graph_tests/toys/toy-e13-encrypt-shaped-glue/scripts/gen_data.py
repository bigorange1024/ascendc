...
# coding=utf-8
"""
gen_data.py — E13 Encrypt 形态：L1 采样 + L2 代数+压码 → golden c=c1||c2(384B)。

u 路 c1：2×(CBD→NTT→basemul→INTT→Compress→ByteEncode)，**无** μ 嵌入。
v 路 c2：1×(e2 CBD→…→Decompress_1(μ)→Compress→ByteEncode)。
prf=384B（u0∥u1∥v）；g=768 int32（ĝ_u0∥ĝ_u1∥ĝ_v stub）。
"""
import ctypes
import hashlib
import os
...
def main() -> None:
    ...
    g_polys = [make_g_poly(p) for p in range(k + 1)]
    ...
    c1_parts = []
    for p in range(k):
        enc = chain_u_poly(u_polys[p], g_polys[p], Minv)
        c1_parts.append(enc)
        enc.tofile(os.path.join(out_dir, f"golden_c1_poly{p}.bin"))
    c1 = np.concatenate(c1_parts)

    c2 = chain_v_poly(e2_poly, g_polys[k], mu, Minv)
    ...
    print(
        f"[gen_data] E13 Encrypt glue: c1={C1_BYTES}B c2={C2_BYTES}B total={ENCODE_OUT}B "
        f"prf={PRF_BYTES}B g={g_all.size} coeffs mu=v-only SEED_D={SEED}"
    )
