 ...
 * @brief E14：E13 Encrypt 粘合 + 真 SampleNTT(Â) 替换 u 路 stub ĝ。
 *
 * 背景：D-exp-e14 — L2 前设备 SampleNTT(SEED_D,(0,0)/(0,1))→G0/G1；v 路 G2 仍 Host stub。
 * 结论：L1 同 E13；L2 先 SampleNTT(u×2) 再 NTT→basemul→INTT→压码→c1∥c2 + SET(4)。
 * 未采用：抄 Encrypt；全 2×2 矩阵（墙钟优先 u 路 2 poly）；改 E01–E13。
 */
#include "basic.hpp"
...
/** 仅 subBlock0：两次 SampleNTT → G0/G1（subBlock1 本 phase 空转）。 */
