#!/bin/bash
# toy-e14-glue-plus-samplentt：E13 Encrypt 粘合 + 真 SampleNTT(Â) u 路 ×3
#
# Usage（默认 = 3 轮 SIM；L1 SHAKE+CBD + L2 SampleNTT(0,0)/(0,1)+真链+SET4 → 384B）：
...
#
# KERNEL_COMPUTE_BUDGET_SEC 默认 1500（3×(u×2+v×1) 真链；防挂死，非性能定标）
...
echo "[SUCCESS] toy-e14-glue-plus-samplentt (${RUN_MODE})"
