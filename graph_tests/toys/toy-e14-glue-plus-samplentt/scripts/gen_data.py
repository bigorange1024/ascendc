...
# coding=utf-8
"""
gen_data.py — E14：E13 Encrypt 粘合 + 真 SampleNTT(Â) golden。

u 路 c1：SampleNTT(SEED_D,(0,0)/(0,1)) 替换 stub ĝ；v 路 G2 仍 stub。
"""
import ctypes
import hashlib
import os
...
def make_g_poly_stub(seed_offset: int) -> np.ndarray:
    """v 路 G2 仍用 E13 确定性 stub。"""
...
def main() -> None:
    ...
    # u 路：真 SampleNTT(0,0)/(0,1)；v 路：stub G2
    g_u0 = sample_ntt_a_hat(SEED, 0, 0)
    g_u1 = sample_ntt_a_hat(SEED, 0, 1)
    g_v = make_g_poly_stub(2)
    g_u0.tofile(os.path.join(out_dir, "golden_a_hat_u0.bin"))
    g_u1.tofile(os.path.join(out_dir, "golden_a_hat_u1.bin"))
    g_all = np.concatenate([g_u0, g_u1, g_v])
    ...
    print(
        f"[gen_data] E14 SampleNTT glue: c1={C1_BYTES}B c2={C2_BYTES}B total={ENCODE_OUT}B "
        f"u=SampleNTT(0,0)/(0,1) v=stub G2 SEED_D={SEED}"
    )
