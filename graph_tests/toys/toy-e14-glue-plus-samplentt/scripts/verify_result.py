...
# coding=utf-8
"""
verify_result.py — E14：TRACE × rounds + SHAKE + CBD(u) + SampleNTT TRACE + c golden(384B)。
"""
import os
import sys
import numpy as np
...
def main() -> int:
    ...
    for r in range(rounds):
        ...
        need = ["200", "222", "223", "300", "302", "303", "400", "503", "600", "603", "700", "744", "420", "402"]
        for code in need:
            if code not in window:
                print(f"[FAIL] round {r} missing {code}", file=sys.stderr)
                return 8

    print("[verify] TRACE OK: L1 → SampleNTT(u×2) → L2 c1||c2 + SET4")

    if skip_golden:
        print("[verify] TOY_SKIP_GOLDEN=1 — skip golden")
        print("[SUCCESS] E14 TRACE-only (no hang path)")
        return 0
    ...
    print("[SUCCESS] E14 SampleNTT glue TRACE × rounds + shake + CBD(u) + 384B c golden OK")
    return 0
