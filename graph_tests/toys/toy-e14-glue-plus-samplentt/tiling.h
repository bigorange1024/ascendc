#ifndef TOY_E14_GLUE_PLUS_SAMPLENTT_TILING_H
#define TOY_E14_GLUE_PLUS_SAMPLENTT_TILING_H
 ...
 * L2（代数+压码）：
 *   设备 SampleNTT(SEED_D,(j,i)) → ws[G0/G1]（u 路 Â 前两元，(0,0)/(0,1)）
 *   v 路仍用 Host g.bin 中 G2 stub
...
struct TilingData {
    int32_t phase;       /**< 0=L1 采样；1=L2 SampleNTT+代数+压码 + SET(4) */
...
/** G0/G1：L2 设备 SampleNTT 写 u 路 â[256]；(0,0)/(0,1)。G2：v 路 Host stub。 */
