 ...
 * @brief E15：E14 壳 + 完整 2×2 Â 真 SampleNTT（独立 launch phase）。
 *
 * 背景：D-exp-e15 — SampleNTT(SEED_D,(0,0)…(1,1))→G0…G3；v 路 basemul 用 G2=(1,0)。
 * 结论：L1 同 E14；独立 phase 跑 4×SampleNTT；L2 NTT→basemul→INTT→压码→c1∥c2 + SET(4)。
 * 未采用：SampleNTT 嵌 MIX 同 kernel；抄 Encrypt；改 E01–E14。
 */
#include "basic.hpp"
...
/** SampleNTT 完成：仅 AIV subBlock 间同步（phase=SampleNtt 时 AIC 空转）。 */
enum SampleNttState : uint16_t {
    SNTT_AIV_DONE = 8,
};
...
__aicore__ inline void SampleNttWait(SampleNttState st)
{
    AscendC::PipeBarrier<PIPE_ALL>();
    AscendC::CrossCoreWaitFlag<2, PIPE_MTE2>(st);
    AscendC::PipeBarrier<PIPE_ALL>();
...
__aicore__ inline void SampleNttSet(SampleNttState st)
{
    AscendC::PipeBarrier<PIPE_ALL>();
    AscendC::CrossCoreSetFlag<2, PIPE_MTE2>(st);
    AscendC::PipeBarrier<PIPE_ALL>();
...
/** 仅 subBlock0：四次 SampleNTT → G0…G3 完整 2×2（subBlock1 本 phase 空转）。 */
__aicore__ inline void L2RunSampleNttSubBlock0(GM_ADDR ws)
{
    using namespace tiling;
    TraceDigit(300);
    const __gm__ uint32_t *seedPtr = reinterpret_cast<const __gm__ uint32_t *>(ws + SD0);
    const uint32_t seed_d = seedPtr[0];

    F203Alg7::BuildAlg7SampleNttFromSeedD(seed_d, 0U, 0U, nullptr, nullptr, nullptr,
                                        reinterpret_cast<__gm__ int32_t *>(ws + G0));
    TraceDigit(302);
    F203Alg7::BuildAlg7SampleNttFromSeedD(seed_d, 0U, 1U, nullptr, nullptr, nullptr,
                                        reinterpret_cast<__gm__ int32_t *>(ws + G1));
    TraceDigit(303);
    F203Alg7::BuildAlg7SampleNttFromSeedD(seed_d, 1U, 0U, nullptr, nullptr, nullptr,
                                        reinterpret_cast<__gm__ int32_t *>(ws + G2));
    TraceDigit(304);
    F203Alg7::BuildAlg7SampleNttFromSeedD(seed_d, 1U, 1U, nullptr, nullptr, nullptr,
                                        reinterpret_cast<__gm__ int32_t *>(ws + G3));
    TraceDigit(305);
    AscendC::PipeBarrier<PIPE_ALL>();
}
...
/** L2 AIC：u×2 + v×1 串行 NTT/INTT；SampleNTT 已在独立 launch 完成。 */
...
extern "C" __global__ __aicore__ void mmad_custom(GM_ADDR out, GM_ADDR src, GM_ADDR ws, TilingData tiling)
{
    ...
    if (phase == tiling::kPhaseSampleNtt) {
        if (!aic && subBlockID == 0) {
            L2RunSampleNttSubBlock0(ws);
        }
        return;
    }
