#!/bin/bash
# toy-e15-samplentt-a-full-2x2：E14 壳 + 完整 2×2 Â 真 SampleNTT ×4
#
# Usage（默认 = 3 轮 SIM；L1 SHAKE+CBD + L2 SampleNTT(0,0)…(1,1)+真链+SET4 → 384B）：
...
#
# KERNEL_COMPUTE_BUDGET_SEC 默认 1800（3×(SampleNTT×4 + u×2+v×1) 真链；防挂死，非性能定标）
# c 布局：c1[0:256]=u0∥u1 ByteEncode_d4；c2[256:384]=v ByteEncode_d4（μ 仅 v 路）
CURRENT_DIR=$(
...
echo "SOC=${SOC_VERSION} RUN_MODE=${RUN_MODE} CANN=${_ASCEND_INSTALL_PATH} TOY_ROUNDS=${TOY_ROUNDS} SEED_D=${SEED_D} F203_ALG7_REJ_IMPL=${F203_ALG7_REJ_IMPL}"
...
echo "[SUCCESS] toy-e15-samplentt-a-full-2x2 (${RUN_MODE})"
