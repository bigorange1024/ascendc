# TRACE.md — toy-e18-l18-fsm-sep56

对照知识库 §6（三位十进制）。本实验仅打印数字，无长字符串。

**相对 E17 单因子**：伪 NTT 仍 1/3、GATE 仍 4/8；伪 INTT 改为独立 **5/6**（非复用 1/3）。
...
| 420 | AIC | 将 Wait(5) 伪 INTT（独立 5/6） |
| 421 | AIC | Wait(5) 返回 |
| 423 | AIC | 已 Set(6) 伪 INTT；AIC 全序完成 |
| 500 | AIV0 | 入口 / 将 Set(1) 伪 NTT |
| 501 | AIV0 | 已 Set(1) |
| 503 | AIV0 | Wait(3) 返回 |
| 504 | AIV0 | 已 Set(4) GATE |
| 508 | AIV0 | Wait(8) 返回 |
| 521 | AIV0 | 已 Set(5) 伪 INTT |
| 523 | AIV0 | Wait(6) 返回；写 magic |
| 510 | AIV1 | 入口 / 将 Set(1) 伪 NTT |
| 511 | AIV1 | 已 Set(1) |
| 513 | AIV1 | Wait(3) 返回 |
| 514 | AIV1 | 已 Set(4) GATE |
| 518 | AIV1 | Wait(8) 返回 |
| 531 | AIV1 | 已 Set(5) 伪 INTT |
| 533 | AIV1 | Wait(6) 返回 |
