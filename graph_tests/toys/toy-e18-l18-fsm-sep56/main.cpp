/**
 * @file main.cpp
 * @brief E18 Host：数字 TRACE + 同进程连续 TOY_ROUNDS（默认 8）单 launch MIX。
 *
 * TRACE 协议：仅 printf 三位数字；对照表见 TRACE.md。
 * 末码 111 = 本轮 Sync 返回（整轮成功关键点）。
 * 相对 E17：工程壳同构；设备侧伪 INTT CrossCore 改为独立 5/6。
 */
...
/**
 * 主流程：同进程 for N：100→单 launch→111。
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    for (int r = 0; r < rounds; ++r) {
        // ---- 单 launch：伪 NTT → GATE → 伪 INTT（独立 5/6）----
        TraceDigit(100);
