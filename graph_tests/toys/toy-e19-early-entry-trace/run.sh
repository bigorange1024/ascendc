#!/bin/bash
# toy-e19-early-entry-trace：MIX 入口写 fused-trace 槽 14/15，再极简 SET4 ×8
#
# Usage（默认 = TOY_ROUNDS=8，全绿）：
#   SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# 调试（非默认）：
#   TOY_ROUNDS=1 KERNEL_COMPUTE_BUDGET_SEC=120 SIM_DIRECT=1 bash run.sh -r sim -v Ascend910B4
#
# KERNEL_COMPUTE_BUDGET_SEC 默认 600（8 轮防挂死，非性能定标）
# 本线验收：SIM only（勿当门禁跑 CPU）；禁止并行 SIM；未上机
CURRENT_DIR=$(
...
# Host TRACE tee → output/host_trace.log（含 stderr [e19-trace]）
set +e
...
echo "[SUCCESS] toy-e19-early-entry-trace (${RUN_MODE})"
