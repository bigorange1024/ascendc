#!/usr/bin/env python3
"""
verify_result.py — E20：校验 Host TRACE 轮次计数 + magic。
...
def main() -> int:
    ...
    if c111 != rounds or c100 != rounds:
        ...
        print(f"REPORT: E20 HANG last={last}", flush=True)
        return 3
    if not os.path.isfile(OUT_BIN):
        print(f"[FAIL] missing {OUT_BIN}", file=sys.stderr)
        print("REPORT: E20 FAIL missing_out", flush=True)
        return 4
    ...
    if len(data) < 9 or data[:8] != MAGIC or data[8] != MARK:
        ...
        print("REPORT: E20 FAIL magic", flush=True)
        return 5
    print("[SUCCESS] E20 Host TRACE × rounds + magic OK")
    print("REPORT: E20 PASS last=111", flush=True)
    return 0
