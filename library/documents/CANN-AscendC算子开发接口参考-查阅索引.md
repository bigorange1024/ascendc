...
| 2026-09-10 | **RB-T30 Decaps 2-launch（dec+enc 双融合）** | 复用既有：`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`(仅 UB)/`PipeBarrier`/`CrossCoreSetFlag\|WaitFlag` modeId=0x2 / `SyncAll`（Wait 环外）/ Cube 族 `Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`；G/H 走 shared `Sha3OneShot`；CBD/SampleNTT 接活跃探针积木（非新增矢量 API） | 两核 basename `t30_dec_fused_custom`/`t30_enc_fused_custom`；L1 flag **1/3**；L2 **1/3+4**；BLOCK_DIM=1；X12 DataCopy；禁 SoftSync/5/7；禁抄 T28/T29/D08/D09 核；magic `0x54333032`；Host FO；权威 liboqs Decaps |
