...
| 2026-07-27 | **ML-KEM-512 W3 D19 KEM KeyGen device k2 探针** | `GetBlockIdx`/`GetSubBlockIdx`/`DataCopy`/`Duplicate`/`TPipe`/`TQue`/`TBuf`/`GlobalTensor`/`LocalTensor`/`GetValue`/`SetValue`/`PipeBarrier`/`Mmad`/`Fixpipe`/`ShiftRight`/`Muls`/`Add`/`Sub`/`Cast`/`CrossCore*` 均复用 D13 k2 与 D19 k3 既有记录；KEM tail `Sha3OneShot` 走 shared Keccak，非新增矢量 API；NTT S1–S3 禁 `Gather`；k2 D19 不新增 AscendC API | [`pass-fix-f203-alg19-kem-keygen-device-k2`](../../ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg19-kem-keygen-device-k2/)：2 launch；`ek_kem=800B`、`dk_kem=1632B = dk_pke(768)‖ek(800)‖H(ek)(32)‖z(32)`；复用 D13 k2 prep/compute，Alg.16 尾内嵌第二 launch；`z` 域为 `exp-mlkem-f203-kem-k2:SEED_Z=` |
