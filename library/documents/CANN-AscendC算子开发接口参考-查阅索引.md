...
| 2026-09-08 | **EN09-samplentt-device（D-EXP-EN09）Alg.7 SampleNTT** | `KERNEL_TASK_TYPE_*`/`DataCopy`/`TPipe`/`TQue`/`TBuf`/`GetValue`/`SetValue`/`PipeBarrier` 复用既有；shared `ShakeXofUb`/`KernelShakeGeneral`（非新增矢量 API）；**无新增 AscendC API** | L0 `enc_samplentt_real`：独立 AIV（X32）；SHAKE128(ρ‖j‖i) 固定 672B + 标量解交织/rej → Â[4,4,256]；喂 Matvec；禁 GATE 4/8 / 融 NTT |
| 2026-09-08 | **EN06-pack-compress-realbrick（D-EXP-EN06）Pack Compress+ByteEncode** | `KERNEL_TASK_TYPE_*`/`DataCopy`/`TPipe`/`TQue`/`TBuf`/`GetValue`/`SetValue`/`PipeBarrier` 复用既有；**无新增 AscendC API** | L5 `enc_pack_compress_real`：独立 AIV（X32）；Compress₁₁/₅ 统一整数舍入 + ByteEncode 标量逐组；c=1568B；禁 GATE 4/8 / 融 NTT |
| 2026-09-08 | **EN05-prep-sample-realbrick（D-EXP-EN05）Prep CBD η=2** | `KERNEL_TASK_TYPE_*`/`DataCopy`/`TPipe`/`TQue`/`TBuf`/`GetValue`/`SetValue`/`PipeBarrier` 复用既有；shared `ShakeXofUb`/`KernelShakeGeneral`（非新增矢量 API）；**无新增 AscendC API** | L1 `enc_prep_cbd_real`：独立 AIV（X32）；PRF=SHAKE256(σ‖N)+Alg.8 CBD η=2×4；布局 [4,256] 可喂 NTT；禁 GATE 4/8 / 融 NTT；Â Host 喂 |
| 2026-09-08 | **EN04-matvec-realbrick（D-EXP-EN04）NTT 域内积** | `KERNEL_TASK_TYPE_*`/`DataCopy`/`TPipe`/`TQue`/`TBuf`/`Duplicate`/`Add`/`GetValue`/`SetValue`/`PipeBarrier` 复用既有；`f203_mod_q::mod_q_final_vec`（shared）；**无新增 API** | L3 `enc_matvec_real`：独立 AIV（X32）；Alg.11/12 标量 paired basemul + P-inner-1 累加；禁 GATE 4/8 / 融 NTT |
| 2026-09-08 | **EN03-encrypt-host-skel（D-EXP-EN03）AIV 轻桩** | `KERNEL_TASK_TYPE_DEFAULT`/`KERNEL_TYPE_AIV_ONLY`/`KERNEL_TYPE_MIX_AIC_1_2` 复用既有；`DataCopy`/`TPipe`/`TQue`/`TBuf`/`Adds`/`Muls`/`ShiftRight`(int32)/`PipeBarrier<PIPE_V>` 复用既有；**无新增 API** | CPU：`AIV_ONLY` Prep/Matvec/Pack；SIM：桩核 `MIX_AIC_1_2` 占位且 AIC 立即 return（无 CrossCore）；NTT/INTT 独立 `mmad_custom` MIX；禁 GATE 4/8 / SoftSync / 融胖核 |

| 2026-09-07 | **ER02（就地修 ER01 SYNC-02）** | `TQue` **EnQue/DeQue**（VECIN 入 / VECOUT 出）；`PipeBarrier<PIPE_V>`（V→S）；`PipeBarrier<PIPE_ALL>`（S→V / 段边界）— 均复用既有 API；**未新增** `HardEvent::SetFlag/WaitFlag` | cannbot FIX-SYNC-02 模式 A：GM→UB 后 EnQue/DeQue 再算/标量读；`Duplicate` 后 `PIPE_V` 再 `SetValue`/`GetValue`；验收红线 0 + CPU/SIM 不挂 |
| 2026-09-07 | **ER01-encrypt-shaped-2launch-skel（D-EXP-ER01）** | CrossCoreSetFlag/WaitFlag≈p.15–16（复用 GT-3/T06）；`DataCopy`/`Duplicate`(int32)/`SetValue`/`TPipe`/`TQue`/`Mul`/`Add`/`Muls`/`Mmad`/`Fixpipe`/`Nd2Nz` 复用既有；**无新增 API** | MIX `KERNEL_TYPE_MIX_AIC_1_2`；Host 2 launch：phase PREP（AIV mixing，AIC 立即返回）→ COMPUTE（NTT **1/3** + 生产 GATE **4/8** 真 Vec MAC + INTT 复用 **1/3**）；禁 SyncAll@AIC-Wait；禁 SoftSync；禁 INTT flag 5/7；禁抄 Encrypt |
| 2026-09-03 | **DGT-20260903-3 pass-toy-decrypt-fsm-fused-skel1 Soft×2+GATE×2+NTT/INTT1/3** | CrossCoreSetFlag/WaitFlag≈p.15–16（复用 GT-2/GT-3/DGT-2）；`PipeBarrier`/`DataCopy`/`Duplicate`(int32)/`SetValue`/`TPipe`/`TQue`/`Mmad`/`Fixpipe`/`Nd2Nz` 复用既有（GT-4/DGT-1/2）；**无新增 API**；SoftSyncArrive/Clear 哨兵 `__gm__ int32` 标量写+忙等（生产同构，非矢量 API） | MIX `KERNEL_TYPE_MIX_AIC_1_2`；通道 `<2,PIPE_MTE2>`；AIV：SoftArrive(slot0)→SET(4)/WAIT(8)→Clear→SET(1)/WAIT(3)→SoftArrive(slot1)→SET(4)/WAIT(8)→Clear→SET(1)/WAIT(3)；AIC：WAIT(4)/SET(8)→WAIT(1)/轻MMAD/SET(3)×2；禁 SyncAll@AIC-Wait；禁 Wait(2)；禁 INTT flag 5/7；GT-4 TRACE ones→A1 DataCopy |
| 2026-09-03 | **DGT-20260903-2 fix-toy-decrypt-fsm-soft-gate1 SoftSync+GATE4/8** | CrossCoreSetFlag/WaitFlag≈p.15–16（复用 GT-2/GT-3）；`PipeBarrier`/`DataCopy`/`Duplicate`(int32)/`SetValue`/`TPipe`/`TQue`/`Mmad`/`Fixpipe`/`Nd2Nz` 复用既有（GT-1/GT-4/DGT-1）；**无新增 API**；SoftSync 哨兵 `__gm__ int32` 标量写+忙等（非矢量 API） | MIX `KERNEL_TYPE_MIX_AIC_1_2`；通道 `<2,PIPE_MTE2>` flag **仅 4/8**；AIV：SoftSyncArrive(slot0)→SET(4)→WAIT(8)→TRACE DataCopy；AIC：WAIT(4)→极轻 MMAD 16×32×32→SET(8)；禁 SyncAll@AIC-Wait；禁 NTT/INTT/第二轮 GATE；禁 CrossCore 代替 SoftSync |
| 2026-09-03 | **DGT-20260903-1 fix-toy-decrypt-fsm-softsync1 SoftSyncArrive** | `PipeBarrier<PIPE_ALL>` / `DataCopy` / `Duplicate`(int32) / `SetValue` / `TPipe`/`TQue` / `Mmad`/`Fixpipe`/`Nd2Nz` 复用既有（GT-1/GT-4）；**无新增 API**；哨兵用 `__gm__ int32` 标量写+忙等（生产 SoftSyncArrive 字面同构，非矢量 API） | MIX `KERNEL_TYPE_MIX_AIC_1_2`；AIV0 `s[0]=1`+PipeBarrier，AIV1 `while(s[0]==0)`；AIC 无 WaitFlag 仅极轻 MMAD；完成标记 GT-4 式 8×int32 DataCopy（禁 Duplicate(int8)）；禁 CrossCore/SyncAll 代替 SoftSync |
| 2026-09-03 | **stable-fips203-mlkem-pke-decrypt-k4 F203_DECRYPT_TRACE** | 无新增 API；写槽复用 Encrypt 标量 GM 模式 | env `F203_DECRYPT_TRACE=1`；17 槽 stride-8 标量 TRACE + softSync 轮询；禁 mark 内局部 TPipe（全链 SIM 对拍 FAIL）；AIC 槽可见性同 Encrypt 限制 |
| 2026-09-03 | **GT-20260903-7 pass-toy-encrypt-fsm-l18-skel1 两段 Host launch** | CrossCoreSetFlag/WaitFlag≈p.15–16（复用 GT-3）；`DataCopy`/`Mmad`/`Fixpipe`/`PipeBarrier`/`Duplicate`(int32)/`SetValue`/`TPipe`/`TQue` 复用 GT-4；无新增 API | `tiling.phase`∈{FULL=0,NTT_ONLY=1,GATE_INTT_ONLY=2}；NTT_ONLY 跑完 NTT return；GATE_INTT_ONLY 跳过 NTT 直进 GATE4/8+INTT1/3；禁 SyncAll@AIC-Wait；禁 INTT flag 5/7；Host `TOY_SPLIT_2LAUNCH=1` 两段 ACLRT_LAUNCH |
| 2026-09-03 | **GT-20260903-4 pass-toy-encrypt-fsm-l18-skel1 TRACE DataCopy** | `DataCopy` 2.3.1 / [07_0101](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/900/API/ascendcopapi/atlasascendc_api_07_0101.html)：GM↔Local 连续块，`count*sizeof(T)` 须 32B 对齐否则向下取整；A2 支持 int32；Cube 侧 L1(A1)↔GM 连续 `DataCopy`；Vector 侧 UB(VECOUT)↔GM。复用 `Duplicate`(int32)/`SetValue`/`PipeBarrier`/`TPipe`/`TQue`；无新增 API | 禁 `__gm__ int32` 标量直写 TRACE；每槽 8×int32=32B；AIV0：VECOUT `Duplicate`+`DataCopy`；AIC：`ws+TRACE_ONES` 全1模板 → A1 → `DataCopy` 到槽；握手仍 NTT 1/3→GATE 4/8→INTT 1/3 |
| 2026-09-03 | **GT-20260903-3 pass-toy-encrypt-fsm-l18-skel1** | CrossCoreSetFlag/WaitFlag≈p.15–16（复用）；`DataCopy`/`Mmad`/`Fixpipe`/`PipeBarrier`/`SetValue`/`Duplicate`(int32) 复用既有；无新增 API | MIX `KERNEL_TYPE_MIX_AIC_1_2`；同核串 NTT **1/3**→GATE **4/8**→INTT 复用 **1/3**（禁 5/7）；简易 TRACE GM 标量写槽；禁 SyncAll@AIC-Wait；桩 int8 用 `SetValue`（禁 Duplicate(int8)） |
| 2026-09-03 | **GT-20260903-2 pass-toy-encrypt-fsm-gate-intt1** | CrossCoreSetFlag/WaitFlag≈p.15–16（复用）；`DataCopy`/`Mmad`/`Fixpipe`/`PipeBarrier`/`SetValue`/`Duplicate`(int32) 复用既有；无新增 API | MIX `KERNEL_TYPE_MIX_AIC_1_2`；GATE flag **4/8** 后再 INTT 复用 **1/3**（禁 5/7）；禁 SyncAll@AIC-Wait；桩 int8 用 `SetValue`（禁 Duplicate(int8)）；极轻 `Mmad` 仅 INTT 段 |
| 2026-09-03 | **GT-20260903-1 fix-toy-encrypt-fsm-ntt1** | CrossCoreSetFlag/WaitFlag≈p.15–16；`DataCopy`/`Mmad`/`Fixpipe`/`PipeBarrier`/`Duplicate`(int32) 复用既有；**Duplicate 不支持 int8**（dav_c220 static_assert） | MIX `KERNEL_TYPE_MIX_AIC_1_2`；通道 `<2,PIPE_MTE2>` flag **1/3**；桩哈希 int8 用 `SetValue` 循环填常数（禁 SHAKE）；完成标记用 `Duplicate` int32；极轻 `Mmad` 16×32×32 |
