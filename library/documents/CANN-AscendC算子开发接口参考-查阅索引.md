...
| 2026-09-09 | **RB-K05 KEM KeyGen L3 kg_kem_tail** | `DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`(仅 UB)/`PipeBarrier`（既有 2.3.1/2.2）；`KERNEL_TYPE_AIV_ONLY`；H/z 走 shared `F203SeDeviceKeccak::Sha3OneShot`（非新增矢量 API） | 单 AIV launch；BLOCK_DIM=1；basename `kg_kem_tail_custom`；无 CrossCore/无 flag 5/7；`dk_kem=dk_pke‖ek‖H(ek)‖z`；z 域分离 `exp-mlkem-f203-kem-k4:SEED_Z=`；写出 **UB+DataCopy**（X12）；禁抄 KeyGen/fork Encaps 整核；host oracle 关闸；npu wait_npu |
| 2026-09-09 | **RB-K04 PKE KeyGen 三 launch 全链** | 复用既有：`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`(仅 UB)/`PipeBarrier`/`CrossCoreSetFlag\|WaitFlag` modeId=0x2 / Cube 族；`KERNEL_TYPE_AIV_ONLY`+`KERNEL_TYPE_MIX_AIC_1_2`；L1 SHAKE/CBD 积木（K01） | 三 launch+Host mid-sync：`kg_prep_custom`→`kg_ntt_custom`→`kg_dot_encode_custom`；basename 唯一；flag L2a/L2b **1/3+4**；BLOCK_DIM=1；X12 DataCopy；禁融单核/抄 KeyGen；权威 **liboqs_pke_ref** KeyGen（SEED_D=20260619）；npu wait_npu |
| 2026-09-09 | **RB-K03 KeyGen L2b Â∘ŝ̂+ê̂+ByteEncode₁₂** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 K02/D02）；`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`(仅 UB)/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；`KERNEL_TYPE_MIX_AIC_1_2` | 单 launch：flag **1/3 + 4=GATE**（永禁 5/7）；BLOCK_DIM=1；basename `kg_dot_encode_custom`；AIV0 标量 Alg.11 4×4 内积+Alg.5 BE₁₂→ek[1568]/dk[1536]；业务 **UB+DataCopy**（X12）；禁融 prep/NTT；禁抄 KeyGen/Encaps/Decrypt 整核；oracle=host FIPS（SampleNTT+mlkem_ntt+multiply_ntts+BE₁₂） |
| 2026-09-09 | **RB-K02 KeyGen L2a NTT(ŝ)+NTT(ê)** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 D02/T12）；`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`(仅 UB)/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；`KERNEL_TYPE_MIX_AIC_1_2` | 单 launch：flag **1/3 + 4=GATE**（永禁 5/7）；BLOCK_DIM=1；basename `kg_ntt_custom`；AIV0 标量 Alg.9×8 poly（ŝ[4]+ê[4]）；poly-batch 整 poly；禁 Gather/limbsplit；NTT 结果 **UB+DataCopy**（X12）；禁 SoftSync/Wait 环 SyncAll；禁抄 KeyGen/Encrypt/Decrypt 整核；oracle=`topology_math.mlkem_ntt` |
| 2026-09-09 | **RB-D07 Encaps↔Decaps 往返编排** | 复用既有：`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`(仅 UB)/`PipeBarrier`/`CrossCoreSetFlag|WaitFlag` modeId=0x2 / Cube 族；`KERNEL_TYPE_AIV_ONLY`+`KERNEL_TYPE_MIX_AIC_1_2`；G/FO 走 shared Keccak（非新增矢量 API） | 七核 basename 唯一：`enc_g`/`enc_prep`/`enc_mix`/`dec_prep`/`dec_ntt_dot`/`dec_intt_extract`/`decaps_fo`；多 launch+mid-sync；flag∈{1,3,4}；BLOCK_DIM=1；X12 DataCopy；禁两段 aclFinalize；禁抄 T25–T27/alg15|21/examples/frozen/l18；权威 liboqs Encaps/Decaps 往返 K |
| 2026-09-09 | **RB-D06-decaps-fo 设备 FO→K[32]** | `DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`(仅 UB)/`PipeBarrier`（既有 2.3.1/2.2）；`KERNEL_TYPE_AIV_ONLY`；J=SHAKE256 走 shared `F203SeDeviceKeccak::Shake256OneShot`（非新增矢量 API）；**未**用 `Compares`（标量逐字节 c≟c'） | 单 AIV launch；BLOCK_DIM=1；无 CrossCore/无 flag 5/7；输入 c/c'/K'/z（z=dk[3136:3168)）；合法选 K'、拒绝选 J(z‖c)；K **UB+DataCopy**（X12）；禁 GM SetValue；禁抄 T25–T27/alg21/decrypt|decaps/frozen；权威 liboqs Decaps 双路径 |
| 2026-09-09 | **RB-D04-decaps-G 设备 G(m'‖h)→(K'‖r')** | `DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`(仅 UB)/`PipeBarrier`（既有 2.3.1/2.2）；`KERNEL_TYPE_AIV_ONLY`；SHA3-512 走 shared `F203SeDeviceKeccak::Sha3OneShot`（非新增矢量 API） | 单 AIV launch；BLOCK_DIM=1；无 CrossCore/无 flag 5/7；h=dk_kem[3104:3136) 切片语义（禁默认可重算 H(ek)）；K'/r' **UB+DataCopy**（X12）；Host 不预喂 K'/r'；禁抄 T22–T27/alg15/21/decrypt|decaps/frozen |
| 2026-09-09 | **RB-D03 Decrypt L2b INTT+extract→m** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 D02/T01）；`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`(仅 UB)/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；`KERNEL_TYPE_MIX_AIC_1_2` | 单 launch：flag **1/3 + 4=GATE**（永禁 5/7）；BLOCK_DIM=1；AIV0 标量 Alg.10 INTT(ŵ)+Compress₁/ByteEncode₁→m[32]；ŵ/v 对齐 D02/D01；m **UB+DataCopy**（X12）；禁 NTT∥INTT 同核（X15）；禁 SoftSync/Wait 环 SyncAll；禁抄 alg15/T25/decrypt |
| 2026-09-09 | **RB-D02 Decrypt L2a NTT(u)+su_dot** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 T01/T12）；`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`(仅 UB)/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；`KERNEL_TYPE_MIX_AIC_1_2` | 单 launch：flag **1/3 + 4=GATE**（永禁 5/7）；BLOCK_DIM=1；AIV0 标量 Alg.9 NTT(u)+Alg.11 ΣMultiplyNTTs→ŵ；poly-batch 整 poly；禁 Gather/limbsplit；û/ŵ **UB+DataCopy**（X12）；禁 SoftSync/Wait 环 SyncAll；禁抄 alg15/T25/decrypt |
| 2026-09-08 | **RB-T27 设备 Encaps→Decaps 往返（含 X12 回炉）** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 T23/T26）；`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`(仅 UB)/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；`KERNEL_TYPE_AIV_ONLY`+`KERNEL_TYPE_MIX_AIC_1_2` | 七 launch；flag Decrypt **1/3**、Encaps/Reenc **1/3+4**；BLOCK_DIM=1；X12：禁 GM `SetValue`——prep 直读 H2D、m'/K/c/ek **UB+DataCopy**；NPU 首跑 FAIL 根因=decrypt 漏改 X12；禁 SoftSync/5/7；禁抄 alg15/20/21 |
| 2026-09-08 | **RB-T25 设备 Alg.15 Decrypt 多 launch** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 T12）；`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；shared `poly_byte_decode12_scalar_gm`；`KERNEL_TYPE_AIV_ONLY`+`KERNEL_TYPE_MIX_AIC_1_2` | 三 launch：prep AIV + NTT MIX + INTT MIX；flag **仅 1/3**（永禁 5/7）；BLOCK_DIM=1；禁 SoftSync/Wait 环 SyncAll；禁抄 alg15/decrypt/encrypt/encaps；golden=liboqs PKE Decrypt |
| 2026-09-08 | **RB-T18 设备 ByteDecode₁₂(ek)→t̂** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 T16）；`DataCopy`/`TPipe`/`TQue`/`SetValue`/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；`KERNEL_TYPE_MIX_AIC_1_2`；shared `poly_byte_decode12_scalar_gm` | 单 launch：flag **仅 1/3**（永禁 5/7）；Host 预喂 ek[1568]=BE₁₂‖ρ；AIV0 Decode₁₂→t̂[1024]；禁 Host 预喂最终 t̂；禁抄 alg14/encrypt/frozen；契约对齐 T05 |
| 2026-09-08 | **RB-T15 设备 Encrypt 半链 Â+ŷ→u,v→c** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 T11/T14）；`DataCopy`/`TPipe`/`TQue`/`TBuf`/`GetValue`/`SetValue`/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；积木内 `Mins`/`Gather`（Alg.7 rej，非 NTT S1–S3）；`KERNEL_TYPE_AIV_ONLY` + `KERNEL_TYPE_MIX_AIC_1_2` | 双 launch：prep AIV + compute MIX；flag **1/3 复用 + 4=GATE**（永禁 5/7）；AIV0 设备 SampleNTT→NTT(y)→Mul/INTT→pack；`F203_AHAT16_BLOCK_DIM=1`；禁 Host 预喂最终 Â/ŷ/u/v；禁抄 alg14/encrypt/frozen |
| 2026-09-08 | **RB-T14 设备 Â+ŷ 半链** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 T12/T13）；`DataCopy`/`TPipe`/`TQue`/`TBuf`/`GetValue`/`SetValue`/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；积木内 `Mins`/`Gather`（Alg.7 rej，非 NTT S1–S3）；`KERNEL_TYPE_MIX_AIC_1_2` | 单 launch：flag **仅 1/3**；AIV0 同核顺序 SampleNTT→NTT(y)；`F203_AHAT16_BLOCK_DIM=1`；禁 Host 预喂最终 Â/ŷ；禁抄 alg14/encrypt/frozen |
| 2026-09-08 | **RB-T14 设备 Â+ŷ 半链** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 T12/T13）；`DataCopy`/`TPipe`/`TQue`/`TBuf`/`GetValue`/`SetValue`/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；积木内 `Mins`/`Gather`（Alg.7 rej，非 NTT S1–S3）；`KERNEL_TYPE_MIX_AIC_1_2` | 单 launch：flag **仅 1/3**；AIV0 同核顺序 SampleNTT(ρ)→NTT(y)；`F203_AHAT16_BLOCK_DIM=1`；双对拍 Â/ŷ；禁 5/7；禁抄 alg14/encrypt/frozen |
| 2026-09-08 | **RB-T13 设备 Â←SampleNTT(ρ)** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 T12）；`DataCopy`/`TPipe`/`TQue`/`TBuf`/`GetValue`/`SetValue`/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；积木内 `Mins`/`Gather`（Alg.7 rej，非 NTT S1–S3）；`KERNEL_TYPE_MIX_AIC_1_2` | 单 launch：flag **仅 1/3**（永禁 5/7）；Host 预喂 ρ[32]；AIV0 接 lines3-7 `BuildAHat16ShardWithUb`；禁 Host 预喂最终 Â；禁抄 alg14/encrypt/frozen |
| 2026-09-08 | **RB-T12 设备 ŷ←NTT(y)** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 T03）；`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；`KERNEL_TYPE_MIX_AIC_1_2` | 单 launch：flag **仅 1/3**（永禁 5/7）；AIV0 标量 Alg.9 ForwardNTT 写 ŷ；poly-batch 整 poly；禁 Gather；禁 Host 预喂最终 ŷ；禁抄 alg14 |
| 2026-09-08 | **RB-T10 u,v 设备 MIX 真拓扑** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 T03）；`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；`KERNEL_TYPE_MIX_AIC_1_2` | 单 launch：flag **1/3 复用 + 4=GATE**（永禁 5/7）；AIV0 标量 MultiplyNTTs+INTT 写 u,v；AIC 极轻 Cube×2；禁 Host 预喂最终 u,v；禁抄 alg14 |
| 2026-09-08 | **RB-T09 Encrypt 外形双 launch** | CrossCoreSetFlag/WaitFlag modeId=0x2（复用 RB-T01/T03）；`DataCopy`/`TPipe`/`TQue`/`GetValue`/`SetValue`/`PipeBarrier`/`Nd2Nz`/`LoadData`/`Mmad`/`Fixpipe`（既有）；`KERNEL_TYPE_AIV_ONLY` + `KERNEL_TYPE_MIX_AIC_1_2` | 单库双核：Launch1 prep AIV 落 ρ/y_e1_e2；Launch2 MIX flag **1/3 复用 + 4=GATE**（永禁 5/7）+ 极轻 Cube×2 + AIV0 Compress/BE pack；同 session 禁双设备库 |
| 2026-09-08 | **RB-T05 ByteDecode₁₂ 薄壳** | `DataCopy`/`PipeBarrier`/`TPipe`/`TQue`/`SetValue`（既有）；复用 shared `poly_byte_decode12_scalar_gm`（GM 标量读字节） | AIV-only；k=4；ek BE₁₂ 1536B→t̂[1024] int32；无 CrossCore；禁抄 alg14 |
| 2026-09-08 | **RB-T04 μ_embed Decompress₁** | `DataCopy`/`PipeBarrier`/`TPipe`/`TQue`/`GetValue`/`SetValue`（2.3.1 / 2.2，既有记录）；`KERNEL_TYPE_AIV_ONLY` | AIV-only；m[32] uint8→μ[256] int32；ByteDecode₁ LSB-first + `(c·q+1)>>1`；无 CrossCore |
| 2026-09-08 | **RB-T02 MIX GATE Wait(4)+flag1/3** | 同 RB-T01：CrossCoreSetFlag/WaitFlag modeId=0x2；PipeBarrier；极轻 Cube API 见既有记录 | 新增 flagId **4**（GATE，AIV→AIC）；仍禁 5/7；AIC 首条 Wait(4)；Cube 在 Wait(1)→Set(3) 间 |
| 2026-09-08 | **RB-T01 MIX CrossCore flag1/3 握手** | API 列表 CrossCoreSetFlag/WaitFlag（≈p.15–16）；矩阵 `DataCopy`+`Nd2Nz`/`LoadData`/`LoadDataWithTranspose`/`Mmad`/`Fixpipe`（2.3.2，既有记录）；`PipeBarrier`/`GetSubBlockNum`/`GetSubBlockIdx`/`GlobalTensor::SetValue` | modeId=`0x2` 同核 AIC↔AIV；flagId **仅 1/3**（禁 5/7）；Wait 环内禁 SyncAll；极轻 Cube 16×32×32；910B 上 CrossCore pipe 模板参数不生效（见 cannbot api-crosscore-sync） |

| 2026-09-08 | **E20 toy 全 Mark 后 AIV-only 尾包 stub** | 复用 `CrossCoreSetFlag`/`WaitFlag`（flag 1/3 + 4/8）、`PipeBarrier`、`TPipe`/`TQue`/`DataCopy`/`SetValue`/`GetValue`；无新增设备 API；AIC INTT 末次 Set 后 return；双 AIV 非对称 GM↔UB 假 DataCopy | [`toy-e20-postmark-tail`](../../graph_tests/toys/toy-e20-postmark-tail/)：SIM×8；禁抄 Encrypt/tail_pack |
| 2026-09-08 | **E19b toy 入口 TRACE DataCopy RMW** | 复用 `TPipe`/`TQue`/`LocalTensor`/`GlobalTensor`/`DataCopy`/`SetValue`/`GetValue`/`PipeBarrier`；AIV 整表 16×int32 GM↔UB RMW 单槽置 1（64B 对齐）；AIC 仍标量；槽 15 偏移 60B 单元素/半块写 NPU D2H 不可见 | [`toy-e19-early-entry-trace`](../../graph_tests/toys/toy-e19-early-entry-trace/)：SIM×8 + NPU×12 槽 0+15 全绿；`-DTOY_E19_TRACE_SCALAR=1` 对照 Encaps FusedTraceMark |
...
| 2026-07-27 | **ML-KEM-512 W4 E13–E21ct incubating customspec×7** | `GetBlockIdx`/`GetSubBlockIdx`/`DataCopy`/`Duplicate`/`PipeBarrier`/`CrossCore*`/`SyncAll`/`Mmad`/`Fixpipe`/`ShiftRight`/`Muls`/`Add`/`Sub`/`Cast`/`Gather` 均复用 D13–D21ct k2 与 768 E13–E21ct 既有记录；KEM 头尾 `Sha3OneShot`/`Shake256OneShot` 走 shared Keccak，非新增矢量 API；NTT S1–S3 禁 `Gather`；W4 不新增 AscendC API | [`examples/incubating/ml-kem/ml-kem-512/`](../../examples/incubating/ml-kem/ml-kem-512/INDEX.md)：E13–E15 / E19–E21ct；I/O 800/768/1632/768；S-1 + polyvec4；行为基线 D13–D21ct k2 |
| 2026-07-27 | **ML-KEM-512 W3 D20 KEM Encaps device k2 探针** | `GetBlockIdx`/`GetSubBlockIdx`/`DataCopy`/`Duplicate`/`TPipe`/`TQue`/`TBuf`/`GlobalTensor`/`LocalTensor`/`GetValue`/`SetValue`/`PipeBarrier`/`Mmad`/`Fixpipe`/`ShiftRight`/`Muls`/`Add`/`Sub`/`Cast`/`CrossCore*` 均复用 D14 k2 与 D20 k3 既有记录；KEM head `Sha3OneShot` 走 shared Keccak，非新增矢量 API；NTT/INTT S1–S3 禁 `Gather`；k2 D20 不新增 AscendC API | [`pass-fix-f203-alg20-kem-encaps-device-k2`](../../ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg20-kem-encaps-device-k2/)：SIM 2 / CPU 5；`ek_kem=800B+m=32B→c=768B+K=32B`；FO 头 `K‖coins=G(m‖H(ek))` 在 prep 前段设备侧完成；S-1 true k2 `Â[4]`、`re[5]` |
| 2026-07-27 | **ML-KEM-512 W3 D21/D21ct KEM Decaps device k2 探针** | `GetBlockIdx`/`GetSubBlockIdx`/`DataCopy`/`Duplicate`/`TPipe`/`TQue`/`TBuf`/`GlobalTensor`/`LocalTensor`/`PipeBarrier`/`Mmad`/`Fixpipe`/`ShiftRight`/`Muls`/`Add`/`Sub`/`Cast`/`CrossCore*` 均复用 D14/D15 k2 与 D21 k3 既有记录；`SyncAll<isAIVOnly=true>` 复用 2026-07-13 记录；FO 中 `Sha3OneShot`/`Shake256OneShot` 走 shared Keccak 已登记设备原语；k2 D21/D21ct 不新增 AscendC API | [`pass-fix-f203-alg21-kem-decaps-device-k2`](../../ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg21-kem-decaps-device-k2/) / [`pass-fix-f203-alg21-kem-decaps-device-ct-k2`](../../ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg21-kem-decaps-device-ct-k2/)：`dk_kem=1632B+c=768B→K=32B`；D21 默认 `decaps_1session`，D21ct 默认 `decaps_2session`；accept/reject 双验 |
| 2026-07-27 | **ML-KEM-512 W2 D14 PKE Encrypt device k2 探针** | `GetBlockIdx`/`GetSubBlockIdx`/`DataCopy`/`Duplicate`/`TPipe`/`TQue`/`TBuf`/`GlobalTensor`/`LocalTensor`/`GetValue`/`SetValue`/`PipeBarrier`/`Mmad`/`Fixpipe`/`ShiftRight`/`Muls`/`Add`/`Sub`/`Cast`/`CrossCore*` 均复用既有记录；`SyncAll<isAIVOnly=true>` 复用 2026-07-13 记录；NTT/INTT S1–S3 禁 `Gather`；k2 D14 不新增 AscendC API | [`pass-fix-f203-alg14-pke-encrypt-device-k2`](../../ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg14-pke-encrypt-device-k2/)：2 launch；`ek_pke=800B+m+coins→c=768B`；Â[4]、`r‖e1‖e2=5`；AIV1 本核完成 `vOut+=e₂+μ` 后 pack c₂，避免跨 AIV 读写竞态 |
| 2026-07-27 | **ML-KEM-512 W2 D13 PKE KeyGen device k2 探针** | `GetBlockIdx`/`GetSubBlockIdx`/`DataCopy`/`Duplicate`/`TPipe`/`TQue`/`TBuf`/`GlobalTensor`/`LocalTensor`/`GetValue`/`SetValue`/`PipeBarrier`/`Mmad`/`Fixpipe`/`ShiftRight`/`Muls`/`Add`/`Sub`/`Cast`/`CrossCore*` 均复用既有记录；shared Keccak/SHAKE 仍走已登记设备原语；NTT S1–S3 禁 `Gather`；k2 D13 不新增 AscendC API | [`pass-fix-f203-alg13-device-keygen-k2`](../../ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg13-device-keygen-k2/)：2 launch；prep Â[4] 双 AIV 2+2 + `s‖e[4,256]` CBD-η3；compute MIX `blockDim=1`、polyvec4 NTT、InnerProduct 1+1；ek/dk=800/768 |
| 2026-07-27 | **ML-KEM-512 W2 D15 PKE Decrypt device k2 探针** | `GetBlockIdx`/`GetSubBlockIdx`/`DataCopy`/`Duplicate`/`TPipe`/`TQue`/`TBuf`/`GlobalTensor`/`LocalTensor`/`GetValue`/`SetValue`/`PipeBarrier`/`Mmad`/`Fixpipe`/`ShiftRight`/`Muls`/`Add`/`Sub`/`Cast`/`CrossCore*` 均复用既有记录；NTT S1–S3 禁 `Gather`；k2 D15 不新增 AscendC API | [`pass-fix-f203-alg15-pke-decrypt-device-k2`](../../ascendc-tests/ml-kem/ml-kem-512/pass-fix-f203-alg15-pke-decrypt-device-k2/)：1 fused MIX launch；`dk_pke=768B` + `c=768B` → `m=32B`；AIV0=poly0、AIV1=poly1；S-1/no limbsplit |
