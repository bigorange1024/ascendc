...
**最近刷新**：2026-07-31（T2-npu-env：1024 探针×7 + stable×7；`msprof_run.sh` 默认关闭；KEM golden 去 liboqs 硬依赖）
...
| **P1** | **T2-npu-env** | 实机 env 适配（`env.sh` 回写 CANN_HOME、`run.sh` REPO_ROOT、`ASCEND_DEVICE_ID` 缺省 **1**） | **已合入（2026-07-31）**：add_custom + toy + 1024 探针×7 + **stable×7**；**另**抽出 `scripts/msprof_run.sh`（`RUN_WITH_MSPROF=1` 才采集）；**零 thirdparty 可跑**（LUT 三级候选 + `library/shared/f203_kem_ref`）；借入机拉 main 后对各目录 `-r npu` / 可选 `RUN_WITH_MSPROF=1` |
...
| ID | 事项 | 关闭日 |
|----|------|--------|
| **T512-glue-c** | Encrypt `r` 误用 η2→η1=3 补缺；D14/E14/E20 + liboqs-512 Encaps `c`/`K` CPU+SIM max=0；W4+glue **完成**；未建 stable-512 | 2026-07-27 |
| **T512-W4-glue** | ML-KEM-512 W4：E13–E21ct customspec+CPU+SIM 全绿；AscendC RT PASS；liboqs 初验 K 齐 c 未齐（继任 **T512-glue-c**） | 2026-07-27 |
| **T512-W3-D21ct** | ML-KEM-512 W3：D21ct KEM Decaps CT device；`dk_kem(1632)+c(768)` → `K(32)`；CT 默认 `decaps_2session`；accept CPU/SIM max=0，SIM tick **570707**（D**163069**+E**407638**）；reject CPU/SIM `K=J(z‖c)` 且 `reject≠accept`，SIM tick **571369**（D**163025**+E**408344**）；根无 stray dump | 2026-07-27 |
| **T512-W3-D21** | ML-KEM-512 W3：D21 KEM Decaps device（delivery，非 `-ct`）；`dk_kem(1632)+c(768)` → `K(32)`；默认 `decaps_1session`；accept CPU/SIM max=0，SIM tick **571206**（D**163145**+E**408061**）；reject CPU/SIM `K=J(z‖c)`，SIM tick **570547**（D**163109**+E**407438**）；根无 stray dump | 2026-07-27 |
