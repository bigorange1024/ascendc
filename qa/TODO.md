...
**最近刷新**：2026-07-27（768 有条件完成；教材第8章/汇报简报；纪要更名；新增 T-WebViz）
...
主线 **ML-KEM 六算子 stable 已齐**（交付 Decaps **无 `-ct`**，2026-07-20 `#交付#` + **T19i**）；**`-ct`** 树为 `research/formal-lang-dag` CT 专题（第7章 / 五指标）。768 W0–W4 incubating 与 glue 已完成 **E13–E15 + E19–E21ct + registry + AscendC-only roundtrip**，本阶段仍 **禁 stable-768**。打开项：**教材细表 / 768 可选后续 / NPU / T23 / SHA3hp**。KEM **correctness 三探针已冻结**（见已关闭表）；**禁止**再当实现参考。

| 优先级 | ID | 事项 | 状态 |
|--------|-----|------|------|
| **P1** | **T-WebViz** | **方法论网页可视化**：参考领导用 DAG 网页展示研究/解决问题过程的思路，探索用网页更直观展示本方法论（依赖 DAG、门禁流、闭包演化等）；待下次任务拍板技术路线（静态 HTML / D3.js / 其它） | **待规划**（2026-07-27 领导汇报后提出） |
...
| **P0** | **T768-post** | ML-KEM-768 可选后续：liboqs-768 helper / device KAT、D14↔D15 PKE RT、stable-768（须 `#交付#`） | **待拍板 / 非门禁**；W4 glue 已关闭；仍禁 stable-768 |
