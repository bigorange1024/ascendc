...
**最近刷新**：2026-07-31（T2-npu-env：借入机体检已回填；代码仍待实施）
...
| **P1** | **T2-npu-env** | 实机 env 适配（`env.sh` 回写 CANN_HOME、`run.sh` REPO_ROOT、`ASCEND_DEVICE_ID` 缺省 **1**） | **P0 已合入（2026-07-31）**；借入机拉 main 后 `add_custom -r npu`；P1 verify-cann aarch64 仍可选 |
