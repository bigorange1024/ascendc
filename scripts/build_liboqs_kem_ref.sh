...
if [ ! -f "${LIBOQS_LIB}" ]; then
    if [ -f "${LIBOQS_BUILD}/lib/liboqs.a" ]; then
        ...
        echo "[build_liboqs_kem_ref] missing ${LIBOQS_BUILD}/lib/liboqs.{so,a}" >&2
        exit 1
    fi
fi
gcc -O2 -Wall -Wextra \
    -I"${LIBOQS_BUILD}/include" \
    "${SCRIPT_DIR}/liboqs_kem_ref.c" \
    "${LIBOQS_LINK[@]}" \
    -o "${OUT_BIN}"
echo "[build_liboqs_kem_ref] OK: ${OUT_BIN}"
